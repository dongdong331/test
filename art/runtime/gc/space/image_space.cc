...
static bool GenerateImage(const std::string& image_filename,
  ...
  if (Runtime::Current()->IsZygote()) {
    ...
    std::string content = "[" + StringPrintf("%d", getpid()) + "] GenerateImage remove dalvik-cache"
                          + ", image_isa=" + GetDalvikCache(GetInstructionSetString(image_isa))
...
static bool RelocateImage(const char* image_location,
  ...
  if (Runtime::Current()->IsZygote()) {
    ...
    std::string content = "[" + StringPrintf("%d", getpid()) + "] RelocateImage remove dalvik-cache"
...
static bool CheckSpace(const std::string& cache_filename, std::string* error_msg) {
  ...
  // Space self-adaption for Non-Root user processes.
  // 1. Imageless running is what we don't want.
  // 2. There must be some space available for Non-Root user processes,
  //    otherwise, process like android.process.acore may crash repeatly when bootup complete.
  if (TryReserveSpaces() > 0) {
    TryReleaseSpaces();
  }
...
std::unique_ptr<ImageSpace> ImageSpace::CreateBootImage(const char* image_location,
  ...
  std::string dalvik_cache;
  std::string content;
  std::string pidstr = StringPrintf("[%d] ", getpid());
  ...
  if (is_zygote && dalvik_cache_exists && !secondary_image) {
    ...
    if (!verified || !check_space) {
      ...
      content = pidstr + "0.b PruneDalvikCache"
                       + ", image_location=" + image_location
                       + ", image_isa=" + GetDalvikCache(GetInstructionSetString(image_isa))
                       + ", has_system=" + StringPrintf("%d", has_system)
                       + ", verified=" + StringPrintf("%d", verified)
                       + ", check_space=" + StringPrintf("%d", check_space)
                       + ", cache_filename=" + cache_filename
                       + ", system_filename=" + system_filename
                       + ", local_error_msg:" + local_error_msg;
      PrintLogToFile(content);
      PruneDalvikCache(image_isa);
  ...
  if (found_image && has_system && has_cache) {
    ...
    if (ChecksumsMatch(system_filename.c_str(), cache_filename.c_str(), &local_error_msg)) {
      ...
      if (relocated_space != nullptr) {
        ...
        content = pidstr + "1.a CheckSum Mismatch: local_error_msg=" + local_error_msg;
        PrintLogToFile(content);
    }
  ...
  if (found_image && has_system && relocate) {
    ...
    if (!dex2oat_enabled) {
      ...
      content = pidstr + "2.b PruneDalvikCache"
                       + ", image_location=" + image_location
                       + ", cache_filename=" + cache_filename;
      PrintLogToFile(content);
      PruneDalvikCache(image_isa);
      ...
      content = pidstr + "2.b RelocateImage-Allowed"
                       + ", image_location=" + image_location
                       + ", cache_filename=" + cache_filename
                       + ", patch_success=" + StringPrintf("%d", patch_success)
                       + ", local_error_msg=" + local_error_msg;
      PrintLogToFile(content);
      if (patch_success) {
    ...
    content = pidstr + "2.b RelocateImage Fail"
                     + ", image_location=" + image_location
                     + ", cache_filename=" + cache_filename
                     + ", local_error_msg=" + local_error_msg;
    PrintLogToFile(content);
  ...
  if (!has_system) {
    ...
    if (!dex2oat_enabled) {
      ...
      content = pidstr + "3 try GenerateImage cache_filename:" + cache_filename
              + ", compilation_success:" + StringPrintf("%d", compilation_success)
              + ", local_error_msg:" + local_error_msg;
      PrintLogToFile(content);
    ...
    content = pidstr + "3 fail"
                     + ", image_location=" + image_location
                     + ", image_isa=" + GetDalvikCache(GetInstructionSetString(image_isa))
                     + ", has_system=" + StringPrintf("%d", has_system)
                     + ", secondary_image=" + StringPrintf("%d", secondary_image)
                     + ", cache_filename=" + cache_filename
                     + ", local_error_msg=" + local_error_msg;
    PrintLogToFile(content);
  ...
  // image.
  content = pidstr + "4 fail: Prune the cache and return no image.";
  PrintLogToFile(content);
  PruneDalvikCache(image_isa);
