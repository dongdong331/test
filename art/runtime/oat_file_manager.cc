...
std::vector<std::unique_ptr<const DexFile>> OatFileManager::OpenDexFilesFromOat(
  ...
  if (source_oat_file != nullptr) {
    progressbits |= 0x010000;
    bool added_image_space = false;
    if (source_oat_file->IsExecutable()) {
      progressbits |= 0x020000;
      ...
      if (kEnableAppImage && (!runtime->IsJavaDebuggable() || source_oat_file->IsDebuggable())) {
        ...
        progressbits |= 0x040000;
      } else {
        image_space = nullptr;
      }
      if (image_space != nullptr) {
        progressbits |= 0x080000;
  ...
  if (dex_files.empty()) {
    progressbits |= 0x01000000;
    if (oat_file_assistant.HasOriginalDexFiles()) {
      progressbits |= 0x02000000;
      if (Runtime::Current()->IsDexFileFallbackEnabled()) {
        progressbits |= 0x04000000;
  ...
  if (VLOG_IS_ON(oat)) {
    if (source_oat_file == nullptr) {
      LOG(WARNING) << "source_oat_file is null"
                   ...
                   << ", progressbits:" << std::hex << progressbits;
    } else {
