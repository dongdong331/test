...
static int check_repart_and_alert(ZipArchiveHandle zip){
	static constexpr const char* REPART_BINARY_NAME = "META-INF/com/google/android/reparts";
	ZipString binary_name(REPART_BINARY_NAME);
	ZipEntry binary_entry;

	if (FindEntry(zip, binary_name, &binary_entry) != 0) {
		LOG(INFO) << "skip check, because no repart binary file " << REPART_BINARY_NAME;
		return INSTALL_SUCCESS;
	}else{
		LOG(INFO) << "found repart binary file " << REPART_BINARY_NAME;
		if(0 == ensure_path_mounted(SDCARD_ROOT)){
			ui->ShowText(true);
			if (!repart_yes_no(device)) {
				LOG(INFO) << "user choosed no!";
				return INSTALL_NONE;
			} else {
				LOG(INFO) << "user choosed yes!";
				return INSTALL_SUCCESS;
			}
		}else{
			LOG(ERROR) << "repart need sdcard!";
			return INSTALL_NEED_SDCARD;
		}
	}
}
...
static int really_install_package(const std::string& path, bool* wipe_cache, bool needs_mount,
  ...
  //Check to see if the partition changes and give a tips to user
  int result = check_repart_and_alert(zip);
    if (INSTALL_NONE == result) {
		CloseArchive(zip);
		return INSTALL_NONE;
	}else if (INSTALL_NEED_SDCARD == result){
		CloseArchive(zip);
		return INSTALL_NEED_SDCARD;
	}
