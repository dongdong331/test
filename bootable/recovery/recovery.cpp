...
const char *SDCARD_ROOT = "/storage/sdcard0";
      ...
      case Device::APPLY_SDCARD:
        {
          ...
          if (status != INSTALL_SUCCESS) {
            if (status == INSTALL_NEED_SDCARD) {
              ui->Print("Partition Has Been Changed, Can't Update From Internal Storage!\
                Please put update.zip into External Storage!\n");
            }
            ui->SetBackground(RecoveryUI::ERROR);
  ...
  if (update_package) {
      ...
      if (strncmp(update_package, "CACHE:", 6) == 0) {
          ...
          if(strstr(update_package, "/Android/data/com.adups.fota/files/adupsfota/update.zip") != NULL){
              char const *SDCARD_PATH = "/Android/data/com.adups.fota/files/adupsfota";
              ...
              char const *SDCARD_PATH = "/Android/data/com.adups.fota/files";
              strlcat(modified_path, SDCARD_PATH, len);
                      }
          #endif
  ...
  if (update_package != nullptr) {
    ...
    if (!is_battery_ok()) {
      ...
      if (status != INSTALL_SUCCESS) {
        ...
        } else if (status == INSTALL_NEED_SDCARD) {
          ui->Print("Partition Has Been Changed, Can't Update From Internal Storage!\
            Please put update.zip into External Storage!\n");
        }
...
bool repart_yes_no(Device* device) {
	const char* headers[] = { "Partition has been changed!",
							"Data will be lost and device may be",
							"not work if you continue this process.",
							"Are you sure to continue?",
							"THIS CAN NOT BE UNDONE!", NULL };
	const char* items[] = { "No", "No", "No","Yes", "No", "No", "No", NULL };
