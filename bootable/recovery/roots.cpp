  ...
  for (int i = 0; i < fstab->num_entries; ++i) {
	...
	if (strcmp(v->mount_point, "/storage/sdcard0") == 0) {
		...
		if (blkid_get_cache(&cache, NULL) < 0) {
			LOG(ERROR) << "get cache fail ";
		}
		blkid_dev dev = blkid_get_dev(cache, v->blk_device,BLKID_DEV_NORMAL);
		if(dev && dev->bid_type) {
			strcpy(v->fs_type, dev->bid_type);
...
int format_volume(const char* volume, const char* directory) {
  ...
  //using tem f2fs tools when partitions changed
  struct stat st;
  int using_tmp_f2fs =  ((stat("/sbin/sload.f2fs", &st) != 0) &&
                         (stat("/tmp/sload.f2fs",  &st) == 0));
  printf("format_volume() using_tmp_f2fs = %d \n", using_tmp_f2fs);
  // Has to be f2fs because we checked earlier.
  static constexpr int kSectorSize = 4096;
  //std::string cmd("/sbin/mkfs.f2fs");
  std::string cmd(using_tmp_f2fs ? "/tmp/mkfs.f2fs" :  "/sbin/mkfs.f2fs");
  // clang-format off
  ...
  if (result == 0 && directory != nullptr) {
    cmd = using_tmp_f2fs ? "/tmp/sload.f2fs" : "/sbin/sload.f2fs";
    // clang-format off
...
int setup_install_mounts() {
  ...
  for (int i = 0; i < fstab->num_entries; ++i) {
    ...
    } else if (strcmp(v->mount_point, SDCARD_ROOT) == 0){
