...
static int CreateStash(State* state, size_t maxblocks, const std::string& blockdev,
  ...
  int res = stat(STASH_DIRECTORY_BASE, &sb);
  if (res == -1 && errno != ENOENT) {
    ErrorAbort(state, kStashCreationFailure, "stat \"%s\" failed: %s", STASH_DIRECTORY_BASE,
               strerror(errno));
    return -1;
  } else if (res != 0) {
    fprintf(stderr, "creating stash base %s\n", STASH_DIRECTORY_BASE);
    res = mkdir(STASH_DIRECTORY_BASE, STASH_DIRECTORY_MODE);

    if (res != 0) {
      ErrorAbort(state, kStashCreationFailure, "mkdir \"%s\" failed: %s", STASH_DIRECTORY_BASE,
                 strerror(errno));
      return -1;
    }

    if (chown(STASH_DIRECTORY_BASE, AID_SYSTEM, AID_SYSTEM) != 0) {  // system user
      ErrorAbort(state, kStashCreationFailure, "chown \"%s\" failed: %s", STASH_DIRECTORY_BASE,
                 strerror(errno));
      return -1;
    }
  }
  ...
  std::string cache_temp_source = CacheLocation::location().cache_temp_source();
