...
VERSION_CHOICES=(native gms)

# check to see if the supplied version is valid
function check_version()
{
    local s
    for s in ${VERSION_CHOICES[@]}
    do
        if [ "$s" = "$1" ]
        then
            return 0
...
function chooseversion()
{
    ...
    local default_value=native
    local ANSWER

    export TARGET_BUILD_VERSION=
    while [ -z "$TARGET_BUILD_VERSION" ]
    do
        echo -n "Which would you like? [$default_value] "
        if [ -z "$1" ] ; then
            read ANSWER
        ...
        if [ -z "$ANSWER" ] ; then
            export TARGET_BUILD_VERSION=$default_value
        elif (echo -n $ANSWER | grep -q -e "^[0-9][0-9]*$") ; then
            if [ "$ANSWER" -le "${#VERSION_CHOICES[@]}" ] ; then
                export TARGET_BUILD_VERSION=${VERSION_CHOICES[$(($ANSWER-1))]}
            fi
        else
            if check_version $ANSWER
            then
                export TARGET_BUILD_VERSION=$ANSWER
            else
                echo "** Not a valid version: $ANSWER"
            fi
        fi
        if [ "$TARGET_BUILD_VERSION" = "gms" -a ! -d "vendor/partner_gms" ] ; then
             echo "** can not find gms build path"
        fi
        if [ -n "$1" ] ; then
            break
        fi
...
function add_lunch_combo()
{
    ...
    LUNCH_MENU_DESC=${LUNCH_MENU_DESC},"$desciption"
}
...
function print_lunch_menu()
{
    ...
    local choice
    local description
    IFS=',' read -r -a description <<< "${LUNCH_MENU_DESC}"
    for choice in ${LUNCH_MENU_CHOICES[@]}
    do
        echo "     $i. $choice	${description[$i]}"
        i=$(($i+1))
    done
...
{
    echo "secboot = $(get_build_var PRODUCT_SECURE_BOOT)"
    if [ "$(get_build_var PRODUCT_SECURE_BOOT)" != "NONE" ]; then
        if [ ! -e $OUT/PRODUCT_SECURE_BOOT_SPRD ]; then
            echo "touch PRODUCT_SECURE_BOOT_SPRD"
            touch $OUT/PRODUCT_SECURE_BOOT_SPRD
        fi
    fi
    ...
    if [ $is_packing -eq 0 ]; then
        ...
        if [ ! -e $(get_build_var HOST_OUT_EXECUTABLES)/make_vbmeta_gsi.sh ]; then
            echo "make_vbmeta_gsi.sh does not exist, will make it!"
            (\cd "$(gettop)/vendor/sprd/proprietories-source/packimage_scripts" && mma)
        fi
...
function lunch()
{
    ...
    if [ "$variant_and_version" != "$selection" ]; then
        ...
        if [ "$variant" != "$variant_and_version" ]; then
            style=${variant_and_version#*-}
        else
            style=
        fi
    fi

    if [ "$style" = "gms" -a  ! -d "vendor/partner_gms" ]
    then
        echo
        echo "** can not find gms build path"
        return 1
    ...
    export TARGET_BUILD_VERSION=$(get_build_var TARGET_BUILD_VERSION)
    if [ -n "$version" ]; then
...
function tapas()
{
    ...
    local style="$(echo $* | xargs -n 1 echo | \grep -E '^(native|gms)$' | xargs)"
    ...
    if [ $(echo $style | wc -w) -gt 1 ]; then
        echo "tapas: Error: Multiple build style supplied: $variant"
        return
    fi
    ...
    export TARGET_BUILD_VERSION=$style
