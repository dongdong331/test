...
# {sprd add begin system.img autosize
  test = glob_dict.get("system_image_autosize")
  print("system_image_autosize=%s" % test)
  if (glob_dict.get("system_image_autosize") == "true" and os.path.basename(out_file) == "system.img"):
    ret, auto_system_image_size = GetDirSize(in_dir)
    if (ret == True):
      system_available_str = glob_dict.get("system_available_size_m")
      if (system_available_str):
        auto_system_image_size += int(system_available_str) * 1024 * 1024
      else:
        auto_system_image_size += 200 * 1024 * 1024
      max_system_image_size = int(image_properties["partition_size"])
      #if (auto_system_image_size > max_system_image_size):
      #  auto_system_image_size = max_system_image_size
      print("--change system size from %d to %d" % (max_system_image_size, auto_system_image_size))
      image_properties["partition_size"] = str(auto_system_image_size)
# sprd add end }
