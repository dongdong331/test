...
Usage:  ota_from_target_files [flags] input_target_files output_ota_package
  ...
  --repart
        Partitions changed between AndroidO and AndroidP
"""
    ...
    if d is None:
      ...
      script.PatchCheck("%s:%s:%d:%s:%d:%s" %
                        (boot_type, boot_device,
                         ...
                         target_boot.size, target_boot.sha1), source_boot.sha1, target_boot.sha1)
...
""" % bcb_dev)
  ...
  if vendor_diff:
    vendor_diff.WriteScript(script, output_zip, progress=0.05)
  if product_diff:
    product_diff.WriteScript(script, output_zip, progress=0.05)
