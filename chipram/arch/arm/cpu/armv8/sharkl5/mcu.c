...
static uint32 mcu_clk_config(void)
{
	...
	REG32(REG_AON_SEC_APB_APCPU_ANANKE_CLK_CFG) |= BIT_AON_SEC_APB_CGM_CORE0_SEL(3) |
						BIT_AON_SEC_APB_CGM_CORE1_SEL(3) |
						BIT_AON_SEC_APB_CGM_CORE2_SEL(3);

	/* config core3 */
	set_mpll(mcu_clk_para.core3_freq.value, 1);
	REG32(REG_AON_SEC_APB_APCPU_PROMETHEUS_CLK_CFG) |= BIT_AON_SEC_APB_CGM_CORE3_SEL(3);
	...
	REG32(REG_AON_SEC_APB_APCPU_BUS_CLK_CFG) |= BIT_AON_SEC_APB_CGM_SCU_SEL(3);
...
static void misc_clk_config(void)
{
	...
	REG32(REG_AON_CLK_CORE_CGM_USB_CFG) = 0x3;
}
...
{
	REG32(REG_ANLG_PHY_GC_ANALOG_PLL_TOP_TWPLL_CTRL0) |= BIT_ANLG_PHY_GC_ANALOG_PLL_TOP_TWPLL_REF_SEL(1);
	REG32(REG_ANLG_PHY_GC_ANALOG_PLL_TOP_LPLL_CTRL0) |= BIT_ANLG_PHY_GC_ANALOG_PLL_TOP_LPLL_REF_SEL(1);
}
...
void pll_sel_cfg(void)
{
	...
	/* cpll select c2k */
	REG32(REG_PMU_APB_CPPLL_REL_CFG) |= BIT_PMU_APB_CPPLL_CDMA_SEL;
}
