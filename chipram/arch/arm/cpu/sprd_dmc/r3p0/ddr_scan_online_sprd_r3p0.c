...
#ifdef CONFIG_SOC_PIKE2
void dmc_lpddr2_ca_adj(void)
{
	DMC_R3P0_REG_INFO_PTR pdmc = (DMC_R3P0_REG_INFO_PTR)PUB_DMC_BASE;
	u32 regval, offset, ca_rfdl_cnt, half_mode_en;
	u32 ca_ui_dly, clk_mode, half_mode;

	ca_ui_dly = ca_ui_qrt_dly >> 2;
	clk_mode = get_clk_mode();
	half_mode = get_half_mode();
	ca_rfdl_cnt = pdmc->dmc_sts_dll_ac & 0x7f;
	/*get full mode dll cnt*/
	if ((clk_mode == 0) && (half_mode == 1))
	{
		ca_rfdl_cnt <<= 1;
	}
	else if ((clk_mode == 1) && (half_mode == 0))
	{
		ca_rfdl_cnt >>= 1;
	}
	/*clk_mode 0 half mode 0,clk_mode 1 half mode 1 no change*/
	if (ca_ui_dly > ca_rfdl_cnt)
	{
		offset = (ca_ui_dly - ca_rfdl_cnt)/2;
		regval = pdmc->dmc_clkwr_dll_ac;
		regval = u32_bits_set(regval, 30, 2, 2); // Plus offset compensation
		regval = u32_bits_set(regval, 16, 7, offset);
		pdmc->dmc_clkwr_dll_ac = regval;
