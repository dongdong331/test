...
# souce files overlay
cp -vr device/sprd/roc1/ud710_haps/source_code_overlay/* ./
...
sed -i 's/CONFIG_F2FS_CHECK_FS=y/# CONFIG_F2FS_CHECK_FS is not set/' kernel4.14/arch/arm64/configs/sprd_all_defconfig kernel4.14/arch/arm64/configs/sprd_roc1_defconfig
...
# disable gpsd
file=device/sprd/roc1/common/rootdir/root/init.common.rc;start=`cat -n $file | grep "[^#]service gpsd" | cut -f 1`;end=$((start + 3));sed -i "$start,$end{s/^/#/}" "$file"
