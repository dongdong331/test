...
# souce files overlay
cp -vr device/sprd/sharkl5/ums510_haps/source_code_overlay/* ./
...
sed -i 's/CONFIG_F2FS_CHECK_FS=y/# CONFIG_F2FS_CHECK_FS is not set/' kernel4.14/arch/arm64/configs/sprd_all_defconfig kernel4.14/arch/arm64/configs/sprd_sharkl5_defconfig

# disable logd exit from time changed
sed -i 's/[^\/]\(exit(0);\)/\/\/\1/' system/core/logd/LogListener.cpp

# disable gpsd
file=device/sprd/sharkl5/common/rootdir/root/init.common.rc;start=`cat -n $file | grep "[^#]service gpsd" | cut -f 1`;end=$((start + 3));sed -i "$start,$end{s/^/#/}" "$file"
...
# increase drm wait fence timeout
sed -i 's/msecs_to_jiffies(50));/msecs_to_jiffies(500));/' kernel4.14/drivers/gpu/drm/drm_atomic_helper.c
