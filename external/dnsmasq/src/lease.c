...
#define PC_FILE "/proc/sys/net/ipv4/ip_addr_pc"
...
int write_file(const char *path, const char *value) {
    ...
    if (fd < 0) {
        my_syslog(MS_DHCP | LOG_ERR, _("open file %s fail: %s\n"), path, strerror(errno));
        return -1;
    }

    len = strlen(value);
    if (write(fd, value, len) != len) {
        my_syslog(MS_DHCP | LOG_ERR, _("write %s to file %s fail: %s\n"), value, path, strerror(errno));
        close(fd);
        return -1;
    }

    my_syslog(MS_DHCP | LOG_ERR, _("write %s to file %s ok\n"), value, path);
    close(fd);
    return 0;
}
...
void lease_update_file(time_t now)
{
  ...
  if (file_dirty != 0 && daemon->lease_stream)
    ...
    //for ext_data geting the address of tethering pc
    if(lease == leases)
      write_file(PC_FILE, inet_ntoa(lease->addr));
