...
void SingleOpModel::BuildInterpreter(
  ...
  for (const auto& shape : input_shapes) {
    ...
    ASSERT_TRUE(interpreter_->ResizeInputTensor(input_idx, shape) == kTfLiteOk);
  }
  //CHECK(interpreter_->AllocateTensors() == kTfLiteOk)
  //    << "Cannot allocate tensors";
  ASSERT_TRUE(interpreter_->AllocateTensors() == kTfLiteOk);
}

void SingleOpModel::Invoke() {
  //CHECK(interpreter_->Invoke() == kTfLiteOk);
  ASSERT_TRUE(interpreter_->Invoke() == kTfLiteOk);
}
...
std::vector<string> SingleOpModel::ExtractVector(int index) {
  ...
  //ASSERT_TRUE(tensor_ptr != nullptr);
