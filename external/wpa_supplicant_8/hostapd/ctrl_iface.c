...
{
	hostapd_set_country(hapd, buf + 25);
	updateHostapdSupportChannel();
}

void hostapd_ctrl_iface_get_support_channel(
	struct hostapd_data *hapd, char *cmd, char *reply, size_t buflen)
{
	int hostap_support_channel[50];
	int channelCount = 0;
	int reply_len = 0;
	channelCount = get_softap_support_channel(hostap_support_channel);
	wpa_printf(MSG_DEBUG, "channelCount = %d",channelCount);
	reply_len = returnChannelToFw(hostap_support_channel,reply,channelCount);
	wpa_printf(MSG_DEBUG, "reply_len = %d",reply_len);
}
