...
static void hostapd_channel_list_update_timeout(void *eloop_ctx, void *timeout_ctx)
{
	wpa_printf(MSG_DEBUG, "Channel list update");
	hostapd_update_channel(iface_save);
}

void updateHostapdSupportChannel() {
	if (iface_save != NULL) {
		eloop_register_timeout(2, 0,
				hostapd_channel_list_update_timeout,iface_save, NULL);
	}
}

int get_softap_support_channel(int hostap_support_channel[]) {
	return get_support_channel(hostap_support_channel);
}
...
const char * hostapd_state_text(hostapd_iface_state s)
{
	switch (s) {
...
void hostapd_set_state(struct hostapd_iface *iface, hostapd_iface_state s)
{
