...
struct hotapd_support_channel
{
	int supportChannel[50];
	int count;
};
struct hotapd_support_channel *save_support_channel = NULL;
...
void update_hostapd_support_channel(int channel) {
	int i = 0;
	save_support_channel->supportChannel[save_support_channel->count] = channel;
	wpa_printf(MSG_DEBUG, "update_hostapd_support_channel add channel %d",save_support_channel->supportChannel[save_support_channel->count]);
}

int get_support_channel(int hostap_support_channel[]) {
	if (save_support_channel != NULL) {
		wpa_printf(MSG_DEBUG, "All support 5G channel count %d", save_support_channel->count);
		for (int i = 0; i < save_support_channel->count; i++) {
			hostap_support_channel[i] = save_support_channel->supportChannel[i];
			wpa_printf(MSG_DEBUG, "update_host %d", save_support_channel->supportChannel[i]);
		}
		return save_support_channel->count;
	}
	return 0;
}
...
int hostapd_update_channel(struct hostapd_iface *iface)
{
	wpa_printf(MSG_DEBUG, "hostapd_get_hw_features");
	...
	wpa_printf(MSG_DEBUG, "hostapd_get_hw_features --> num_modes = %d",num_modes);
	int reset = 1;
	if (reset) {
		free(save_support_channel);
		save_support_channel = NULL;
		reset = 0;
		save_support_channel = (struct hotapd_support_channel*)malloc(sizeof(struct hotapd_support_channel));
	}
	save_support_channel->count = 0;
		...
		for (j = 0; j < feature->num_channels; j++) {
			...
			if (reset) {
				if (save_support_channel != NULL) {
					free(save_support_channel);
					save_support_channel = NULL;
					wpa_printf(MSG_MSGDUMP, "free(save_support_channel)");
				}
				reset = 0;
			}
			wpa_printf(MSG_MSGDUMP, "Allowed Softap channel: mode=%d "
			...
			if (feature->mode == SOFTAP_5G_MODE_FLAG) {
				update_hostapd_support_channel(feature->channels[j].chan);
				save_support_channel->count++;
			}
		}
	}
	return 0;
				  ...
				  int channel, int primary)
{
	...
	for (i = 0; i < iface->current_mode->num_channels; i++) {
		...
		if (chan->chan != channel)
			continue;
		wpa_printf(MSG_DEBUG,"chan->flag = %d",chan->flag);
		if (!(chan->flag & HOSTAPD_CHAN_DISABLED))
			return 1;
