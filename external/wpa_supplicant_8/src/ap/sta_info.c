			   ...
			   int authorized)
{
	...
	if (authorized) {
		...
		sprintf(softapevent,AP_STA_CONNECTED"%s%s",buf,ip_addr);
		wpa_printf(MSG_ERROR,"hostadp softapevent = %s ",softapevent);
		if(hapd->event_cb != NULL){
		    hapd->event_cb(softapevent);
		}
		...
		if(hapd->event_cb != NULL) {
			sprintf(softapevent,AP_STA_DISCONNECTED"%s",buf);
			hapd->event_cb(softapevent);
		}
		if (hapd->msg_ctx_parent &&
		    hapd->msg_ctx_parent != hapd->msg_ctx)
