					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
	...
	if (os_strcasecmp(cmd, "EAPOL::heldPeriod") == 0) {
		...
		//SPRD:Bug #652821 Start to bgscan when enable auto roam switch,if dut is connected with AP BGN -->
		if (os_strncmp(cmd, "bgscan", 6) == 0 && wpa_s->wpa_state == WPA_COMPLETED) {
			char *temp_value = value + 1;
			if (os_strncmp(temp_value, "learn", 5) == 0) {
				wpa_supplicant_start_bgscan(wpa_s);
			} else {
				wpa_supplicant_stop_bgscan(wpa_s);
			}
		}
		//<--Start to bgscan when enable auto roam switch,if dut is connected with AP END
	}
...
#ifdef ANDROID
static int wpa_supplicant_driver_cmd(struct wpa_supplicant *wpa_s, u8 *cmd,
				     char *buf, size_t buflen)
{
	int ret;
	...
	if (ret == 0) {
		if (os_strncasecmp((const char *)cmd, "COUNTRY", 7) == 0) {
...
//<-- Support wifi-p2p do not coexist END
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		reply_len = wpa_supplicant_driver_cmd(wpa_s, (u8 *)(buf + 7), reply,
						      reply_size);
#endif /* ANDROID */
