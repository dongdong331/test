...
Boolean is_bss_connected(struct wpa_supplicant *wpa_s,
               struct wpa_bss* bss)
{
       ...
       if (!current_bss)
               return FALSE; /* current BSS not seen in scan results */
			  ...
			  int only_first_ssid)
{
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		struct wpa_bss *bss = wpa_s->last_scan_res[i];
                /* if((wpa_s->wpa_state == WPA_COMPLETED)&&is_bss_connected(wpa_s, bss)){
                    wpa_dbg(wpa_s, MSG_DEBUG, "Don't consider select already connected AP during roaming" MACSTR
                        " ssid='%s'", MAC2STR(bss->bssid),
                        wpa_ssid_txt(bss->ssid, bss->ssid_len));
                    continue;
                } */
...
int wpa_supplicant_event_assoc_wapi_network(struct wpa_supplicant *wpa_s, int freq){
	...
	if (wpa_s->current_ssid && (wpa_s->current_ssid->proto & WPA_PROTO_WAPI)) {
		...
		if(wpa_s->current_bss == NULL) return 0;
