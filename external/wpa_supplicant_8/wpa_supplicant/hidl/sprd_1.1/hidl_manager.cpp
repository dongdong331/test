...
int HidlManager::notifyOffloadRoam(struct wpa_supplicant *wpa_s, const char *reason){
    ...
    // callbacks.
    callWithEachStaIfaceCallbackEx(
        wpa_s->ifname,
                   std::bind(
...
void HidlManager::callWithEachStaIfaceCallback_1_1(
	...
	(android::sp<android::hardware::wifi::supplicant::V1_1::ISupplicantStaIfaceCallback>)>
	&method)
{
 ...
 */
void HidlManager::callWithEachStaIfaceCallbackEx(
    const std::string &ifname,
    const std::function<Return<void>(android::sp<ISupplicantStaIfaceCallbackEx>)>
	&method)
{
