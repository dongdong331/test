...
using vendor::sprd::hardware::wifi::supplicant::V1_1::ISupplicantStaNetwork;
...
constexpr uint32_t kAllowedKeyMgmtMask =
    (static_cast<uint32_t>(ISupplicantStaNetwork::KeyMgmtMask::NONE) |
     ...
     static_cast<uint32_t>(ISupplicantStaNetwork::KeyMgmtMaskEx::PSK_SHA256) |
     static_cast<uint32_t>(ISupplicantStaNetwork::KeyMgmtMaskEx::EAP_SHA256));
...
Return<void> StaNetwork::registerCallback_sprd_1_1(
    const sp<::vendor::sprd::hardware::wifi::supplicant::V1_1::ISupplicantStaNetworkCallbackEx>
    & callback, registerCallback_sprd_1_1_cb _hidl_cb)
{
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_NETWORK_INVALID,
	    &StaNetwork::registerCallbackInternal_sprd_1_1, _hidl_cb, callback);
}
...
SupplicantStatus StaNetwork::registerCallbackInternal_sprd_1_1(
    const sp<ISupplicantStaNetworkCallbackEx> &callback)
{
	HidlManager *hidl_manager = HidlManager::getInstance();
	if (!hidl_manager ||
	    hidl_manager->addStaNetworkCallbackHidlObjectEx(
		ifname_, network_id_, callback)) {
		return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
	}
	return {SupplicantStatusCode::SUCCESS, ""};
...
SupplicantStatus StaNetwork::setNetworkParamInternal(const std::string& key, const std::string& value)
{
    ...
    if (!key.compare("psk_key_type")) {
        ...
        if (setStringFieldAndResetState(
            value.c_str(), &(wpa_ssid->wapi_as_cert), "wapi_as_cert")) {
            return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
        }
    } else if (!key.compare("wapi_user_cert")) {
        if (setStringFieldAndResetState(
            value.c_str(), &(wpa_ssid->wapi_user_cert), "wapi_user_cert")) {
            return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
        }
    } else
#endif
...
std::pair<SupplicantStatus, std::string>
StaNetwork::getNetworkParamInternal(const std::string& key)
{
    struct wpa_ssid *wpa_ssid = retrieveNetworkPtr();
