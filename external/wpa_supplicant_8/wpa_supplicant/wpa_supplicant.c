	...
	if (strstr(iface->ifname, "p2p0")) {
		...
		if (wpa_s != NULL) {
		    ...
		    wpas_p2p_add_p2pdev_interface(
		        global->ifaces, global->params.conf_p2p_dev);
		    wpa_s = wpa_supplicant_get_iface(global, "p2p-dev-wlan0");
		    return wpa_s;
		}
		return wpa_s;
...
int wpas_start_assoc_wapi_network_cb(struct wpa_supplicant *wpa_s,
	...
	if (ssid->proto & WPA_PROTO_WAPI) { /* associating to a WAPI network */
		...
		char *wapi_cert_name = "/data/vendor/wifi/wapi_merge.cer";
		...
		if (ssid->key_mgmt & WPA_KEY_MGMT_WAPI_CERT) {
			if ( (ssid->wapi_as_cert) && (ssid->wapi_user_cert) ) {
				...
				if (wapi_merge_cert_files(ssid->wapi_as_cert, ssid->wapi_user_cert, wapi_cert_name)) {
					...
					wpas_notify_wapi_disconnect(wpa_s, "WAPI: validate certificate failed");
					wpas_connect_work_done(wpa_s);
					return 0;
				}
		...
		if (ssid->key_mgmt & WPA_KEY_MGMT_WAPI_PSK) {
			if(ssid->passphrase == NULL) return 0;
