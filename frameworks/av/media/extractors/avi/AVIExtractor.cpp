...
struct AVIExtractor::AVISource : public MediaTrack{
...
struct AVIExtractor::MP3Splitter : public RefBase {
    ...
    void clear();
    void append(MediaBufferBase *buffer);
...
status_t AVIExtractor::AVISource::start(MetaDataBase */*params*/) {
    CHECK(!mBufferGroup);

    mBufferGroup = new MediaBufferGroup;

    if (8 == mTrack.mBitsPerSample) {
       mBufferGroup->add_buffer(MediaBufferBase::Create(mTrack.mMaxSampleSize<<1));
       mBufferGroup->add_buffer(MediaBufferBase::Create(mTrack.mMaxSampleSize<<1));
    } else {
       mBufferGroup->add_buffer(MediaBufferBase::Create(mTrack.mMaxSampleSize));
       mBufferGroup->add_buffer(MediaBufferBase::Create(mTrack.mMaxSampleSize));
    }
    mSampleIndex = 0;
...
status_t AVIExtractor::AVISource::getFormat(MetaDataBase &meta) {
...
void AVIExtractor::MP3Splitter::append(MediaBufferBase *buffer) {
...
status_t AVIExtractor::MP3Splitter::read(MediaBufferBase **out) {
    *out = NULL;
...
AVIExtractor::AVIExtractor(DataSourceBase *source)
    : mDataSource(source) {
...
MediaTrack *AVIExtractor::getTrack(size_t index) {
    return index < mTracks.size() ? new AVISource(this, index) : NULL;
}

status_t AVIExtractor::getTrackMetaData(
        MetaDataBase &meta,
        size_t index, uint32_t /* flags */) {
    if(index < mTracks.size()) {
        meta = mTracks.editItemAt(index).mMeta;
        return OK;
    }else {
...
status_t AVIExtractor::getMetaData(MetaDataBase &meta) {
     meta.clear();
    ...
    if (isVideo) {
        ...
        switch(format){
            ...
            case WAVE_FORMAT_IMAADPCM:
                if(4 == bitsPerSample){
                    track->mMeta.setCString(kKeyMIMEType, "audio/ima-adpcm");
                    track->mMeta.setInt32('blka', blockAlign);
                }else{
...
status_t AVIExtractor::getSampleIndexAtTime(
        size_t trackIndex,
        int64_t timeUs, MediaTrack::ReadOptions::SeekMode mode,
    ...
    if (mode == MediaTrack::ReadOptions::SEEK_CLOSEST) {
