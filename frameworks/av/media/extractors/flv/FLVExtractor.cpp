...
struct FLVExtractor::FLVSource : public MediaTrack {
public:
...
status_t FLVExtractor::FLVSource::start(MetaDataBase *params) {
    if (params) {
    ...
    mBufferGroup->add_buffer(MediaBufferBase::Create(mTrack.mMaxTagSize));
    mBufferGroup->add_buffer(MediaBufferBase::Create(mTrack.mMaxTagSize));
    mTagIndex = 0;
...
status_t FLVExtractor::FLVSource::getFormat(MetaDataBase &meta) {
    meta = mTrack.mMeta;
    return OK;
}
...
FLVExtractor::FLVExtractor(DataSourceBase *dataSource)
    ...
    for(size_t i = 0; i < mTracks.size(); i++) {
        MetaDataBase mdata = mTracks.editItemAt(i).mMeta;
        const char *mime;
...
MediaTrack *FLVExtractor::getTrack(size_t index) {
...
status_t FLVExtractor::getTrackMetaData(
        MetaDataBase &meta,
...
status_t FLVExtractor::getMetaData(MetaDataBase &meta) {
    meta.clear();
