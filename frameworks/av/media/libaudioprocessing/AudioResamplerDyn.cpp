        ...
        AudioBufferProvider* provider)
{
    ...
    while (outputIndex < outputSampleCount) {
        ...
        while (mBuffer.frameCount == 0 && inFrameCount > 0) {
            ...
            buffer_status=provider->getNextBuffer(&mBuffer);
            if (mBuffer.raw == NULL) {
                // We are either at the end of playback or in an underrun situation.
                // Reset buffer to prevent pop noise at the next buffer.
                if(NOT_ENOUGH_DATA==buffer_status){
                    ALOGD("getNextBuffer NOT_ENOUGH_DATA with record,continue");
                }else{
                    mInBuffer.reset();
                }
                goto resample_exit;
