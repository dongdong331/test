...
void NuPlayer::GenericSource::prepareAsync() {
    Mutex::Autolock _l(mLock);
    ALOGI("prepareAsync: (looper: %d)", (mLooper != NULL));
...
void NuPlayer::GenericSource::onPrepareAsync() {
    ALOGI("onPrepareAsync: mDataSource: %d", (mDataSource != NULL));
    ...
    ALOGI("onPrepareAsync: Done");
}

void NuPlayer::GenericSource::finishPrepareAsync() {
    ALOGI("finishPrepareAsync");
...
status_t NuPlayer::GenericSource::dequeueAccessUnit(
    ...
    if (isSeeking()) {  //when seeking, not provide data to decoder
        return -EWOULDBLOCK;
    }
...
status_t NuPlayer::GenericSource::seekTo(int64_t seekTimeUs, MediaPlayerSeekMode mode) {
    ...
    if (mCachedSource != NULL) {  //http streaming
        ALOGI("seeking start,mSeekingCount:%d", mSeekingCount);
        {
...
void NuPlayer::GenericSource::onSeek(const sp<AMessage>& msg) {
    ...
    if (mCachedSource != NULL) {   // http Streaming do not reply
        Mutex::Autolock _l(mSeekingLock);
        if (mSeekingCount > 1) {
            ALOGI("seek timeUs:%lld, miss, mSeekingCount:%d", (long long)seekTimeUs, mSeekingCount);
            mSeekingCount--;
            mSeekingLock.unlock();
            notifySeekDone(OK);
            mSeekingLock.lock();
    ...
    ALOGI("seek timeUs:%lld", (long long)seekTimeUs);
    ...
    if (mCachedSource != NULL) {   // http Streaming do not reply
        ALOGI("seek finish, mSeekingCount:%d", mSeekingCount);
        {
            Mutex::Autolock _l(mSeekingLock);
            mSeekingCount--;
        }
