...
void NuPlayer::setDataSourceAsync(
    ...
    if (IsHTTPLiveURL(url)) {
        ...
        if (!strncasecmp(url, "http://", 7) || !strncasecmp(url, "https://", 8)) {
            mDataSourceType = DATA_SOURCE_TYPE_GENERIC_HTTP;
...
void NuPlayer::finishResume() {
    if (mResumePending) {
        mResumePending = false;
        if ((mDataSourceType != DATA_SOURCE_TYPE_GENERIC_HTTP)
             && (mDataSourceType != DATA_SOURCE_TYPE_RTSP)) {
            notifyDriverSeekComplete();
        } else {
            ALOGI("http/rtsp streaming not notify seek complete when decoders resumed");
        }
    }
}
    ...
    switch (what) {
        ...
        case Source::kWhatSeekDone:
        {
            if ((mDataSourceType == DATA_SOURCE_TYPE_GENERIC_HTTP)
                ...
                ALOGI("%s streaming notify NuPlayerDriver seek complete",
                      mDataSourceType == DATA_SOURCE_TYPE_GENERIC_HTTP ? "http":"rtsp");
            }
            break;
        }
        default:
