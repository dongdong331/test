...
#include "include/openmax/OMX_AudioSprdExt.h"
...
static bool findVideoBitrateControlInfo(const sp<AMessage> &msg,
    ...
    *mode = getVideoBitrateMode(msg);
    ...
    status_t err1 = msg->findInt32("bitrate", bitrate);
    status_t err2 = msg->findInt32("quality", quality);
    ALOGI("findVideoBitrateControlInfo isCQ:%d, bitrate:%d, quality:%d, err1:%d, err2:%d", isCQ, *bitrate, *quality, err1, err2);
    return err1;
...
ACodec::~ACodec() {
    if(mSSense != NULL)
    {
        ...
        ALOGW("report data to the sprdssense service,---stop");
    }
}
...
static const struct VideoCodingMapEntry {
    ...
    { "video/mjpg", OMX_VIDEO_CodingMJPEG },
};
...
status_t ACodec::setupVideoDecoder(
    ...
    // Start video
    if (mSSense != NULL) {
        mSSense->reportData(DATA_TYPE_APP_VIDEO, mUid, DATA_SUBTYPE_APP_VIDEO_START, width, height);
        ALOGW("report data to the sprdssense service,---start");
    }
...
status_t ACodec::setupAVCEncoderParameters(const sp<AMessage> &msg) {
    ...
    err = configureBitrate(bitrateMode, bitrate);
    if (err != OK) {
        ALOGW("[%s] configureBitrate failed w/ err %d", mComponentName.c_str(), err);
        // allow failure
        err = OK;
    }
    return err;
...
void ACodec::LoadedToIdleState::stateEntered() {
    ...
    if ((err = allocateBuffers()) != OK) {
        ...
        ALOGI("%p sendCommand OMX_CommandStateSet OMX_StateLoaded", this);
...
status_t ACodec::setParameters(const sp<AMessage> &params) {
    ...
    int32_t encoder;
    if (!params->findInt32("encoder", &encoder)) {
        encoder = false;
    }
    ...
    if (!encoder && params->findString("scene-mode", &mode)) {
        OMX_INDEXTYPE index;
        status_t err =
            mOMXNode->getExtensionIndex(
                    "OMX.sprd.index.DecSceneMode",
...
void ACodec::ExecutingToIdleState::changeStateIfWeOwnAllBuffers() {
    if (mComponentNowIdle && mCodec->allYourBuffersAreBelongToUs()) {
        status_t err = mCodec->mOMXNode->sendCommand(
                OMX_CommandStateSet, OMX_StateLoaded);
        ALOGI("%p err = %d ExecutingToIdleState sendCommand OMX_CommandStateSet OMX_StateLoaded", this, err);
        if (err == OK) {
