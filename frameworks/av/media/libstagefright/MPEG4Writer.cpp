...
private:
    ...
    int64_t mStartTimelimitstampUs;
...
status_t MPEG4Writer::reset(bool stopSource) {
    ...
    for (List<Track *>::iterator it = mTracks.begin();
        ...
        ALOGI("track duration:%lld,%lld",(long long)durationUs,(long long)maxDurationUs);
...
MPEG4Writer::Track::Track(
    ...
    : mOwner(owner),
      ...
      mReachedEOS(false),
      mStartTimestampUs(-1),
      mStartTimelimitstampUs(2592000000000),
      mRotation(0),
      mDimgRefs("dimg"),
      mCdscRefs("cdsc"),
      mImageItemId(0),
...
////////////////////////////////////////////////////////////////////////////////
        ...
        if (!mIsHeic) {
            if (mStszTableEntries->count() == 0) {
                ...
                if((timestampUs>mStartTimelimitstampUs)||((timestampUs<0)&&(mIsAudio))){
...
int64_t MPEG4Writer::Track::getDurationUs() const {
    ALOGI("mTrackDurationUs:%lld",(long long)mTrackDurationUs);
...
int64_t MPEG4Writer::Track::getStartTimeOffsetTimeUs() const {
    ...
    int64_t moovStartTimeUs = mOwner->getStartTimestampUs();
    ALOGI("getStartTimeOffsetTimeUs:moovStartTimeUs%lld,mStartTimestampUs:%lld",(long long)moovStartTimeUs,(long long)mStartTimestampUs);
    if (mStartTimestampUs != -1 && mStartTimestampUs != moovStartTimeUs) {
        CHECK_GT(mStartTimestampUs, moovStartTimeUs);
        trackStartTimeOffsetUs = mStartTimestampUs - moovStartTimeUs;
        ALOGI("mStartTimestampUs:%lld",(long long)mStartTimestampUs);
    }
    ALOGI("trackStartTimeOffsetUs:%lld",(long long)trackStartTimeOffsetUs);
