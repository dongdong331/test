...
void MediaCodec::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        ...
        case kWhatInit:
        {
            ...
            char proc_name[PROCESS_NAME_MAX_SIZE] = {0};
            getProcessName(getpid(),proc_name,sizeof(proc_name));
            ALOGI("app porc_name:%s", proc_name);

            if((!strcmp(proc_name,"com.google.android.apps.tachyon.readyapp"))&& (!strcmp(name.c_str(),"OMX.sprd.vp8.encoder"))) {
                format->setString("componentName", "OMX.google.vp8.encoder");
            } else {
