...
public:
    VolumeCurvesForStream() : mIndexMin(0), mIndexMax(1), mCanBeMuted(true)
    {
        ...
        mIndexCur.add(AUDIO_DEVICE_OUT_DEFAULT_FOR_VOLUME, 1);
#else
    ...
    void setCanBeMuted(bool canBeMuted) { mCanBeMuted = canBeMuted; }
...
public:
    ...
    /*sprd add for camera*/
    virtual void setCanBeMuted(audio_stream_type_t stream, bool canBeMuted) {return editValueAt(stream).setCanBeMuted(canBeMuted);}
