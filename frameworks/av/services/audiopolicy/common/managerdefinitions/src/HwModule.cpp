...
#ifdef AUDIO_WHALE
    audio_devices_t device_type =device & ~AUDIO_DEVICE_BIT_IN;
#endif
...
#ifdef AUDIO_WHALE
        if((device_type&AUDIO_DEVICE_OUT_ALL_USB)||
            (device_type&AUDIO_DEVICE_IN_ALL_USB)){
            sp<DeviceDescriptor> devDesc = declaredDevices.getDeviceFromTagName(String8(device_name));
            if(!devDesc){
                sp<DeviceDescriptor> devDesc = new DeviceDescriptor(device);
                devDesc->setName(String8(device_name));
                devDesc->mAddress = address;
                ALOGI("getDeviceDescriptor new :%s",device_name);
                return devDesc;
            }else{
                ALOGI("getDeviceDescriptor return :%s",device_name);
                return devDesc;
            }
        }
#endif
