...
audio_devices_t Engine::ManagerInterfaceImpl::getDeviceForStrategy(routing_strategy strategy) const
{
    ...
    if (strategy == STRATEGY_SONIFICATION_RESPECTFUL &&
            ...
            if(outputs.isStreamActive(AUDIO_STREAM_MUSIC, SONIFICATION_RESPECTFUL_AFTER_MUSIC_DELAY)) {
#ifdef SPRD_CUSTOM_AUDIO_POLICY
            //modify for FM
                if(outputs.isStreamActive(AUDIO_STREAM_FM)) {
                    return mPolicyEngine->getPropertyForKey<audio_devices_t, routing_strategy>(STRATEGY_FM);
                }
#endif
    ...
    //add for bug#894798
    if(is_state_in_call(getPhoneState()) && ((strategy==STRATEGY_MEDIA)||(strategy==STRATEGY_ENFORCED_AUDIBLE))){
        return mPolicyEngine->getPropertyForKey<audio_devices_t, routing_strategy>(STRATEGY_PHONE);
    }
...
#ifdef SPRD_CUSTOM_AUDIO_POLICY
    //modify for FM
    if((!is_state_in_call(getPhoneState()))&&((strategy==STRATEGY_MEDIA)||(strategy==STRATEGY_DTMF))){
        if(outputs.isStreamActive(AUDIO_STREAM_FM)) {
            return mPolicyEngine->getPropertyForKey<audio_devices_t, routing_strategy>(STRATEGY_FM);
        }
    }
    ...
    if ((strategy == STRATEGY_MEDIA) && (!is_state_in_call(getPhoneState()))) {
        DeviceVector availableOutputDevices = mPolicyEngine->mApmObserver->getAvailableOutputDevices();
        uint32_t availableOutputDevicesType =
            availableOutputDevices.types() & ~AUDIO_DEVICE_NONE;
