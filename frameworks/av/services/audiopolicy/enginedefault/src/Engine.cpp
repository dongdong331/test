...
#ifdef SPRD_CUSTOM_AUDIO_POLICY
            if (strategy == STRATEGY_ENFORCED_AUDIBLE && outputs.isStreamActive(AUDIO_STREAM_FM) && device2 &&
                mForceUse[AUDIO_POLICY_FORCE_FOR_FM] != AUDIO_POLICY_FORCE_SPEAKER) {
                if (availableOutputDevicesType & AUDIO_DEVICE_OUT_WIRED_HEADPHONE) {
                    device2 = device2 | (availableOutputDevicesType & AUDIO_DEVICE_OUT_WIRED_HEADPHONE);
                }
                if (availableOutputDevicesType & AUDIO_DEVICE_OUT_WIRED_HEADSET) {
                    device2 = device2 | (availableOutputDevicesType & AUDIO_DEVICE_OUT_WIRED_HEADSET);
                }
            }
#endif
        }
