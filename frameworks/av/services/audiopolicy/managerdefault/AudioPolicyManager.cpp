...
#define AUDIO_POLICY_A2DP_OFFLOAD_XML_CONFIG_FILE_NAME \
        "audio_policy_a2dp_offload_configuration.xml"
                                                         ...
                                                         const char *device_name)
{
    ...
    if ((device ==  AUDIO_DEVICE_OUT_USB_DEVICE)
        ...
        if(isUsboffloadSupported==0){
            if(device==AUDIO_DEVICE_OUT_USB_DEVICE){
                devDesc=mHwModules.getDeviceDescriptor(device, device_address, "USB Device Out");
            }

            if(device==AUDIO_DEVICE_OUT_USB_HEADSET){
                devDesc=mHwModules.getDeviceDescriptor(device, device_address, "USB Headset Out");
            }

            if(device==AUDIO_DEVICE_IN_USB_DEVICE){
                devDesc=mHwModules.getDeviceDescriptor(device, device_address, "USB Device In");
            }

            if(device==AUDIO_DEVICE_IN_USB_HEADSET){
                devDesc=mHwModules.getDeviceDescriptor(device, device_address, "USB Headset In");
            }
            ALOGI("defaultusb:0x%x",devDesc->type());
        }else{
            if(device==AUDIO_DEVICE_OUT_USB_DEVICE){
                devDesc=mHwModules.getDeviceDescriptor(device, device_address, "SPRD USB Device Out");
            }

            if(device==AUDIO_DEVICE_OUT_USB_HEADSET){
                devDesc=mHwModules.getDeviceDescriptor(device, device_address, "SPRD USB Headset Out");
            }

            if(device==AUDIO_DEVICE_IN_USB_DEVICE){
                devDesc=mHwModules.getDeviceDescriptor(device, device_address, "SPRD USB Device In");
            }

            if(device==AUDIO_DEVICE_IN_USB_HEADSET){
                devDesc=mHwModules.getDeviceDescriptor(device, device_address, "SPRD USB Headset In");
            }
            ALOGI("sprdusb:0x%x",devDesc->type());
        }
    }else{
        devDesc=mHwModules.getDeviceDescriptor(device, device_address, device_name);
    }
#else
    sp<DeviceDescriptor> devDesc =
    ...
    if (audio_is_output_device(device)) {
        ...
        case AUDIO_POLICY_DEVICE_STATE_AVAILABLE: {
            ...
            if(true==isDeviceForFm) {
                String8 fm_addr=String8("sprd_fm_devices");
                ALOGI("setDeviceConnectionStateInt Fm addr:%s",
                devDesc->mAddress.string());
                if (checkOutputsForDevice(devDesc, state, outputs, fm_addr) != NO_ERROR) {
                    mAvailableOutputDevices.remove(devDesc);
...
void AudioPolicyManager::setPhoneState(audio_mode_t state)
{
    ...
    ALOGI("setPhoneState() ringtone is active = %d Music is active = %d oldState = %d state = %d",
        isStreamActive(AUDIO_STREAM_RING, 0), isStreamActive(AUDIO_STREAM_MUSIC, 0), oldState, state);
    // check for device and output changes triggered by new phone state
    if((isStreamActive(AUDIO_STREAM_RING, 0) || isStreamActive(AUDIO_STREAM_MUSIC, 0))
        && ((oldState == AUDIO_MODE_RINGTONE) && (state == AUDIO_MODE_NORMAL))) {
        checkA2dpSuspend(true);
    } else {
        checkA2dpSuspend();
    }
...
//              getDeviceForStrategy(STRATEGY_MEDIA, true /*fromCache*/));
    ...
    if (state == AUDIO_MODE_RINGTONE &&
        ...
        // Always set mLimitRingtoneVolume is false
                                         ...
                                         audio_policy_forced_cfg_t config)
{
    ...
    if(AUDIO_POLICY_FORCE_FOR_FM == usage) {
        ...
        ALOGV("FM device un connected, setDeviceConnectionState() setParameters handle_fm");
...
audio_io_handle_t AudioPolicyManager::getOutput(audio_stream_type_t stream)
{
    ...
    ALOGI("getOutput() stream %d selected device %08x, output %d", stream, device, output);
    return output;
}
                                              ...
                                              audio_port_handle_t *portId)
{
    ...
    *selectedDeviceId = outputDevices.size() > 0 ? outputDevices.itemAt(0)->getId()
    ...
    ALOGI("  getOutputForAttr() returns output %d selectedDeviceId %d", *output, *selectedDeviceId);
                                             ...
                                             uint32_t *delayMs)
{
    ...
    *delayMs = 0;
    ...
    if (outputDesc->mRefCount[stream] == 1 || device != AUDIO_DEVICE_NONE) {
        ...
        if (isInCall()) {
            // when the apk set the InCall mode in order to using earpiece, don't mute the notification
            // also don't unumte in stopOutput function
            if (!(isStreamActive(AUDIO_STREAM_MUSIC) && stream == AUDIO_STREAM_NOTIFICATION
                                            ...
                                            bool forceDeviceUpdate)
{
    ...
    if (isInCall()) {
        if (!(isStreamActive(AUDIO_STREAM_MUSIC) && stream == AUDIO_STREAM_NOTIFICATION
                && outputDesc->device() == AUDIO_DEVICE_OUT_EARPIECE)) {
        ...
        if (outputDesc->mRefCount[stream] == 0 || forceDeviceUpdate) {
            ...
            for (size_t i = 0; i < mOutputs.size(); i++) {
                ...
                if (desc != outputDesc &&
                    ...
                    bool force = false;
                    if (desc->device() != newDevice2) {
                        force = true;
                    } else if (newDevice2 && ((newDevice & newDevice2) == newDevice)) {
                        force = true;
                    }
        ...
        for (int curStream = 0; curStream < AUDIO_STREAM_FOR_POLICY_CNT; curStream++) {
            ...
            if (!(desc->isStreamActive((audio_stream_type_t)curStream) ||
                ...
                if((isInCall()==false)
                    ...
                    ALOGI("force set bt sco voip volume");
                }else{
                    continue;
                }
            }
...
bool AudioPolicyManager::isStreamActive(audio_stream_type_t stream, uint32_t inPastMs) const
{
    ...
    if(AUDIO_STREAM_FM==stream){
         active = mOutputs.isStreamActive((audio_stream_type_t)AUDIO_STREAM_FM, inPastMs);
        return active;
    }
#endif
...
#ifdef AUDIO_WHALE
                    if(mAvailableOutputDevices.types() & AUDIO_DEVICE_OUT_ALL_USB){
                        audio_devices_t support_devices=desc->supportedDevices();
                        ALOGV("checkOutputsForDevice(): adding opened output %d support_devices:0x%x"
                            , mOutputs.keyAt(i),support_devices);
                        if(isUsboffloadSupported==0){
                            if((support_devices&AUDIO_DEVICE_OUT_SPEAKER)
                                &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADSET)
                                &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADPHONE)){
                                ALOGI("line:%d primary output do not support usb devices",__LINE__);
                                continue;
                            }

                            if(0==(support_devices&
                                (AUDIO_DEVICE_OUT_SPEAKER|AUDIO_DEVICE_OUT_WIRED_HEADSET|AUDIO_DEVICE_OUT_WIRED_HEADPHONE))){
                                ALOGI("line:%d use default usbhal output",__LINE__);
                                outputs.add(mOutputs.keyAt(i));
                            }
                        }else{
                            if((support_devices&AUDIO_DEVICE_OUT_SPEAKER)
                                &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADSET)
                                &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADPHONE)){
                                ALOGI("line:%d primary output support usb devices",__LINE__);
                                outputs.add(mOutputs.keyAt(i));
                            }
                    ...
                    if (!device_distinguishes_on_address(device) ||
                        ...
                        if((mFlags&AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD)
                            ...
                            if(mAvailableOutputDevices.types() & AUDIO_DEVICE_OUT_ALL_USB){
                                ...
                                audio_devices_t support_devices=supportedDevices.types();
                                ALOGI("usb out profile support device:%x",support_devices);
                                if((support_devices&AUDIO_DEVICE_OUT_SPEAKER)
                                    &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADSET)
                                    &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADPHONE)){
                                    if(isUsboffloadSupported==0){
                                        ALOGI("primary output do not support usb devices");
                                    }else{
                                        ALOGI("primary output support usb devices");
...
#ifdef AUDIO_WHALE
            audio_devices_t support_devices = openOutputs.valueAt(i)->supportedDevices();
            if(device&AUDIO_DEVICE_OUT_ALL_USB){
                if((device==AUDIO_DEVICE_OUT_USB_HEADSET)||(device==AUDIO_DEVICE_OUT_USB_DEVICE)){
                    ...
                    if(isUsboffloadSupported==0){
                        if((support_devices&AUDIO_DEVICE_OUT_SPEAKER)
                            &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADSET)
                            &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADPHONE)
                            &&((device==(AUDIO_DEVICE_OUT_USB_HEADSET|AUDIO_DEVICE_OUT_SPEAKER))
                               ||(device==(AUDIO_DEVICE_OUT_USB_DEVICE|AUDIO_DEVICE_OUT_SPEAKER)))){
                            ALOGI("getOutputsForDevice primary output do not support usb devices:0x%x",
                                support_devices);
                            continue;
                        }else{
                            ALOGI("getOutputsForDevice add usb output devices:0x%x",support_devices);
                        ...
                        if((support_devices&AUDIO_DEVICE_OUT_SPEAKER)
                            &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADSET)
                            &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADPHONE)){
                            ALOGI("getOutputsForDevice primary output support usb devices:0x%x",
                                support_devices);
                            outputs.add(openOutputs.keyAt(i));
                        }else{
                ...
                if(((device==(AUDIO_DEVICE_OUT_USB_HEADSET|AUDIO_DEVICE_OUT_SPEAKER))
                    ||(device==(AUDIO_DEVICE_OUT_USB_DEVICE|AUDIO_DEVICE_OUT_SPEAKER)))
                    &&(openOutputs.valueAt(i)->isDuplicated())){

                    if(isUsboffloadSupported==0){
                        if(openOutputs.valueAt(i)->isDuplicated()){
                            ALOGI("getOutputsForDevice use duplicate output for speaker and usb");
                            outputs.add(openOutputs.keyAt(i));
                        }
                    }else{
                        if((support_devices&AUDIO_DEVICE_OUT_SPEAKER)
                            &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADSET)
                            &&(support_devices&AUDIO_DEVICE_OUT_WIRED_HEADPHONE)){
                            if(false==openOutputs.valueAt(i)->isDuplicated()){
                                ALOGI("getOutputsForDevice primary output for speaker and usb");
                                outputs.add(openOutputs.keyAt(i));
                            }
                        }else{
...
void AudioPolicyManager::checkA2dpSuspend(bool doDelay)
{
    ...
    if (mA2dpSuspended) {
        if (!isScoConnected ||
            ...
            if (doDelay) {
                ALOGI("checkA2dpSuspend() there are doDely 400ms");
                usleep(400 * 1000);
            }
    ...
    for (size_t i = 0; i < NUM_STRATEGIES; i++) {
        ...
        if (doMute) {
            for (size_t j = 0; j < mOutputs.size(); j++) {
                ...
                ALOGVV("checkDeviceMuteStrategies() %s strategy %zu (curDevice %04x) ,device %d",
                      mute ? "muting" : "unmuting", i, curDevice, device);
                if(!mute && (device == AUDIO_DEVICE_OUT_WIRED_HEADSET || device == (AUDIO_DEVICE_OUT_WIRED_HEADSET | AUDIO_DEVICE_OUT_SPEAKER))
                         && (curDevice == AUDIO_DEVICE_OUT_WIRED_HEADSET)){
                    setStrategyMute((routing_strategy)i, mute, desc, mute ? 0 : delayMs * 2);
                } else {
                                                  ...
                                                  audio_input_flags_t flags)
{
    ...
    for (const auto& hwModule : mHwModules) {
        for (const auto& profile : hwModule->getInputProfiles()) {
            ...
            if((device==AUDIO_DEVICE_IN_USB_HEADSET)
                &&(support_devices&AUDIO_DEVICE_IN_USB_HEADSET)){

                ALOGI("getInputProfile usb input profile support device:%x",support_devices);
                if(isUsboffloadSupported==0){//use usb hal
                    if((support_devices&AUDIO_DEVICE_IN_BUILTIN_MIC)
                        &&(support_devices&AUDIO_DEVICE_IN_BACK_MIC)){
                                                   ...
                                                   bool force)
{
    ...
    if ((stream == AUDIO_STREAM_VOICE_CALL && forceUseForComm == AUDIO_POLICY_FORCE_BT_SCO) ||
        ...
        if((isInCall()==false)&&
            (device&AUDIO_DEVICE_OUT_ALL_SCO)&&
            (stream == AUDIO_STREAM_BLUETOOTH_SCO && forceUseForComm != AUDIO_POLICY_FORCE_BT_SCO)){
            ALOGI("checkAndSetVolume()  force set stream %d volume with force use = %d for comm",
                                           ...
                                           audio_devices_t device)
{
    ...
    if (on) {
        ...
        if (--outputDesc->mMuteCount[stream] == 0) {
            ...
            if((AUDIO_STREAM_ENFORCED_AUDIBLE==stream)
...
#ifdef SPRD_CUSTOM_AUDIO_POLICY
    if(STRATEGY_FM==strategy){
        if ((getStrategy(AUDIO_STREAM_FM) == strategy) &&
            outputDesc->isStreamActive(AUDIO_STREAM_FM, inPastMs, sysTime)) {
            return true;
        }else{
            return false;
        }
