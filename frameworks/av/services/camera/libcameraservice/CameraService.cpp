...
//SPRD: add systemui special for facelock feature
...
Status CameraService::validateClientPermissionsLocked(const String8& cameraId,
    ...
    //SPRD: fix for facelock feature
    if (callingPid != getpid() && (mAllowedUsers.find(clientUserId) == mAllowedUsers.end()) && !isSpecialModeForFaceLock(clientName8)) {
