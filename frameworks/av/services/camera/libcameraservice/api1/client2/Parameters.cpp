...
status_t Parameters::initialize(const CameraMetadata *info, int deviceVersion) {
    ...
    camera_metadata_ro_entry_t maxPreviewSize =
        staticInfo(ANDROID_SPRD_MAX_PREVIEW_SIZE, 2);
    if (maxPreviewSize.count) {
        ALOGV("maxPreviewSize w=%d, h=%d", maxPreviewSize.data.i32[0],
            maxPreviewSize.data.i32[1]);
        if (maxPreviewSize.data.i32[0] > 0 &&
                maxPreviewSize.data.i32[1] > 0 &&
                maxPreviewSize.data.i32[0] <= MAX_PREVIEW_SIZE.width &&
                maxPreviewSize.data.i32[1] <= MAX_PREVIEW_SIZE.height) {
            MAX_PREVIEW_SIZE.width = maxPreviewSize.data.i32[0];
            MAX_PREVIEW_SIZE.height = maxPreviewSize.data.i32[1];
        }
    }
#else
...
status_t Parameters::set(const String8& paramString) {
    ...
    if (validatedParams.flashMode != flashMode) {
        ...
        if (!strcmp(FRONT_CAMERA_FLASH_TYPE, "lcd") && validatedParams.flashMode) {
            isFlashAvailable = ANDROID_FLASH_INFO_AVAILABLE_TRUE;
        }
