...
public class Activity extends ContextThemeWrapper
    ...
    protected void onResume() {
        ...
        if (LONG_SCREENSHOT_ENABLED) {
            if (!mLongScreenshotReceiverRegistered) {
                registerReceiver(longscreenshotReceiver, new IntentFilter("action.longscreenshot"));
                mLongScreenshotReceiverRegistered = true;
                if (DEBUG_LONG_SCREENSHOT) Log.d(LongScreenshotUtil.TAG, "register longscreenshotReceiver");
            }
        }
    }

    private boolean mLongScreenshotReceiverRegistered = false;
    private static final boolean DEBUG_LONG_SCREENSHOT = true;
    private  boolean LONG_SCREENSHOT_ENABLED = SystemProperties.getBoolean("ro.longscreenshot.enable", true);

    private BroadcastReceiver longscreenshotReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context ctx, Intent intent) {
            //Log.d(LongScreenshotUtil.TAG, "onReceive longscreenshot action");
            String pacakgeName = intent.getStringExtra("package");
            String activityName = intent.getStringExtra("activity");
            String screenshotFilePath = intent.getStringExtra("filepath");
            Log.d(LongScreenshotUtil.TAG, "onReceive longscreenshot action: pacakgeName="+getComponentName().getPackageName() + " activityName=" + getComponentName().getClassName());
            if (getComponentName().getPackageName().equals(pacakgeName) && getComponentName().getClassName().equals(activityName)) {
                //takeScreenshotContext = ctx;
                Log.d(LongScreenshotUtil.TAG, "receive longscreenshot broadcast: screenshotFilePath = " + screenshotFilePath);
                //Toast.makeText(ctx, "reveice longscreenshot broadcast", Toast.LENGTH_LONG).show();
                if (longScreenshotUtil == null) {
                    longScreenshotUtil = new LongScreenshotUtil(ctx, screenshotFilePath);
                }
                longScreenshotUtil.screenshotFilePath = screenshotFilePath;
                if (!longScreenshotUtil.screenshotServiceBinded){
                    longScreenshotUtil.bindScreenshotService();
                } else {
                    //longScreenshotUtil.takeLongscreenshot();
                    //already bind, just update support state
                    longScreenshotUtil.updateSupportState();
                }
            }
        }
    };

    private static LongScreenshotUtil longScreenshotUtil;

     class LongScreenshotUtil {
        ...
        private static final int LONG_BITMAP_HEIGHT_LIMIT = 8 * 1024;
        private Context lContext;
        private String screenshotFilePath = null;
        private boolean screenshotServiceBinded = false;
        private int screenBottom;
        private boolean shouldCheckOverlay = true;
        private String[] noCheckOverlayPackages = {
        ...
        private Messenger screenshotMessenger = null;
        private ServiceConnection conn = new ServiceConnection() {
            @Override
            public void onServiceConnected(ComponentName name, IBinder service) {
                synchronized (mScreenshotLock) {
                    screenshotServiceBinded = true;
                    screenshotMessenger = new Messenger(service);
                    Log.d(TAG, "connected to screenshot service");
                    resetData();
            ...
            public void onServiceDisconnected(ComponentName name) {
                screenshotServiceBinded = false;
                screenshotMessenger = null;
                setRequestedOrientation(screenOrientation);
            }
        };

        private void bindScreenshotService() {
        ...
        private void unbindScreenshotService() {
            if (!screenshotServiceBinded) {
        ...
        private boolean stopFromService = false;
        private Handler receiveFromServiceHandler = new Handler(){
            @Override
            public void handleMessage(Message msg) {
                if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "receiveFromService: what=" + msg.what);
                if (msg.what == CONTINUE_LONG_SCREENSHOT) {
                    if (scrollableView != null && screenshotServiceBinded) {
                        continueScroll();
                    }
                } else if (msg.what == STOP_LONG_SCREENSHOT) {
                    if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "stop by service");
                    stopFromService = true;
                    stopScrolling();
                } else if (msg.what == START_LONG_SCREENSHOT) {
                    if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "start by service");
                    takeLongscreenshot();
                } else if (msg.what == CANCEL_LONG_SCREENSHOT) {
                    if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "cancel by service");
        ...
        private Messenger receiveMessenger = new Messenger(receiveFromServiceHandler);
        ...
        private boolean scrollCompleted = false;
        ...
        private void stopScrolling() {
            if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "stopScrolling");

            //scrollComplete("stopScrolling");
            if (scrollController != null) scrollController.setScrollComplete();
            //sendMessageToService(COMPLETE_LONG_SCREENSHOT);
            unbindScreenshotService();
        }
        ...
        private void startScroll() {
            ...
            Bitmap first = captureScreen(lContext, getWindow().getDecorView());
            if(first == null) {
        ...
        //msg from scrollController
        private static final int CAPTURE_SCREEN = 10;
        private static final int SCROLL_COMPLETE = 11;
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case CAPTURE_SCREEN:
                        ...
                        Bitmap b = Bitmap.createBitmap(temp, 0, temp.getHeight() - scrollDistance, temp.getWidth(), scrollDistance);
                        shotOnce(b);
                        break;
        ...
        //merge bitmaps to long-bitmap
        private List<Bitmap> bitmaps = new ArrayList<Bitmap>();
        //secondBitmapHeight is only useful when scrollableView's bottom don't equal to screen's bottom
        private int secondBitmapHeight = 0;

        //params for saving bitmap
        private boolean isSavingBitmap = false;
        //private Bitmap currentSavingBitmap = null;
        ...
        private LongScreenshotUtil(Context context, String filePath) {
            lContext = context;
            screenshotFilePath = filePath;
            setShouldCheckOverlay();
        }

        private void setShouldCheckOverlay() {
            String packageName = getComponentName().getPackageName();
            for (int i=0; i<noCheckOverlayPackages.length; i++) {
                if(packageName.equals(noCheckOverlayPackages[i])) {
                    shouldCheckOverlay = false;
                    break;
                }
            }
        }
        ...
        public void takeLongscreenshot() {
            //resetData();
            takeLongscreenshot(lContext, getWindow().getDecorView());
        }

        private void resetData() {
            if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "resetData");
            bitmaps.clear();
            secondBitmapHeight = 0;
            cropPosition = 0;
            overlayCropPosition = 0;
            scrollableView = null;
            scrollType = 0;
            stopFromService = false;
            scrollController = null;
            isSavingBitmap = false;
            supportLongScreenshot = true;
            scrollCompleted = false;
            isScreenshotError = false;
            totalHeight = 0;
        ...
        private void sendMessageToService(Message msg) {
            if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "sendMessageToService what = " + msg.what);
            msg.replyTo = receiveMessenger;
            try {
                screenshotMessenger.send(msg);
            } catch (RemoteException e) {
                e.printStackTrace();
                if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "screenshotMessenger send message fail !  what=" + msg.what);
            }
        }

        private ViewGroup getScrollableView(ViewGroup viewGroup) {
            int childCount = viewGroup.getChildCount();
            if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "getScrollView1:viewGroup = " + viewGroup.getClass().getName() + " childCount=" + childCount);
        ...
        private int getOverlayViewTop() {
            int top = 0;
            if (overlayViews.size() > 0) {
                for (int i=0; i<overlayViews.size(); i++) {
                    Rect rect = new Rect();
                    overlayViews.get(i).getGlobalVisibleRect(rect);
                    if (i == 0) {
                        top = rect.top;
                        continue;
                    }
            ...
            if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "getOverlayViewTop: top = " + top);
            return top;
        }

        private int cropPosition = 0;
        private int overlayCropPosition = 0;
        private Rect scrollViewRect = new Rect();
        private List<View> overlayViews = new ArrayList<>();
        private void checkOverlay(ViewGroup viewGroup) {
            int n = viewGroup.getChildCount();
            if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "checkOverlay: viewGroup=" + viewGroup.getClass().getName() + " ChildCount=" +n);
            Rect rect = new Rect();
            for (int i=0; i<n; i++) {
                View childView = viewGroup.getChildAt(i);
                if (childView.getVisibility() != View.VISIBLE || childView == scrollableView || !childView.getLocalVisibleRect(new Rect())) continue;
                childView.getGlobalVisibleRect(rect);
                if ((scrollViewRect.contains(rect) || Rect.intersects(scrollViewRect, rect))
                        && scrollViewRect.height() > rect.height() * 2
                        && (rect.top > (scrollViewRect.top + scrollViewRect.bottom)/2)
                        && (scrollViewRect.left <= rect.left
                        ||scrollViewRect.right >= rect.right
                        ||scrollViewRect.bottom > rect.bottom
                        || scrollViewRect.top < rect.top)
                        && cropPosition > rect.top) {
                    overlayViews.add(childView);
                    Log.d(TAG, "get overlayView : view=" + childView.getClass().getName() + " rect=" + rect);
                } else if (childView instanceof ViewGroup) {
                    checkOverlay((ViewGroup)childView);
                }
            }
        }
        ...
        private boolean supportLongScreenshot = true;
        ...
        private boolean isSupportLongScreenshot() {
            synchronized (mScreenshotLock) {
                ...
                if (scrollableView != null) {
                    scrollableView.getGlobalVisibleRect(scrollViewRect);
                    cropPosition = scrollViewRect.bottom;
                    if (scrollType != RECYCLEVIEW) cropPosition -= scrollableView.getPaddingBottom();
                    Log.d(TAG, "getGlobalVisibleRect : scrollViewRect=" + scrollViewRect + " scrollableView=" + scrollableView.getClass().getName()
                            + " cropPosition = " + cropPosition);
                    int width = lContext.getResources().getDisplayMetrics().widthPixels;
                    int height = lContext.getResources().getDisplayMetrics().heightPixels;
                    if (scrollViewRect.width() < width / 2 || scrollViewRect.height() < height / 2) {
                        result = false;
                    } else if (scrollableView.toString().contains("com.google.android.calendar.timely.TimelineRecyclerView")){
                        //fix for bug 940613: for google calendar speacial
                        List<View> list = getAllChildViews(getWindow().getDecorView());
                        for(View view: list) {
                            if(view != null && view.toString().contains("android.support.design.widget.FloatingActionButton")) {
        ...
        private void takeLongscreenshot(final Context context, final View decor) {
            synchronized (mScreenshotLock) {
//                scrollableView = getScrollableView((ViewGroup)decor);
//                if (scrollableView != null) {
//                    scrollableView.getGlobalVisibleRect(scrollViewRect);
//                    Log.d(TAG, "getGlobalVisibleRect : scrollViewRect=" + scrollViewRect + " scrollableView=" + scrollableView.getClass().getName());
//                    int width = context.getResources().getDisplayMetrics().widthPixels;
//                    int height = context.getResources().getDisplayMetrics().heightPixels;
//                    if (scrollViewRect.width() < width/2 || scrollViewRect.height() < height/2) supportLongScreenshot = false;
//                } else {
//                    supportLongScreenshot = false;
//                }
                if (!supportLongScreenshot) {
                    Toast.makeText(context, "do not support long screenshot", Toast.LENGTH_SHORT).show();
                    sendMessageToService(NOT_SUPPORT_LONG_SCREENSHOT);
                    unbindScreenshotService();
                    return;
                }

                if (shouldCheckOverlay) {
                    checkOverlay((ViewGroup)decor);
                    if (overlayViews.size() > 0) {
                        for (int i=0; i<overlayViews.size(); i++) {
                            View view = overlayViews.get(i);
                ...
                scrollController.setScrollHandler(scrollHandler);
                scrollController.setOnceScrollDistance(scrollViewRect.height() * 3 / 4);
                startScroll();
            }
        }
        ...
        private void disableOverlayViews() {
            if (shouldCheckOverlay) {
                for (int i = 0; overlayViews != null && i < overlayViews.size(); i++) {
                    View v = overlayViews.get(i);
        ...
        private Bitmap captureScreen(Context context, View view) {
            ...
            Bitmap b = Bitmap.createBitmap(b1, 0, 0, width, cropPosition > b1.getHeight() ? b1.getHeight() : cropPosition);
            secondBitmapHeight = screenHeight - cropPosition;
            if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "secondBitmapHeight = " + secondBitmapHeight);
        ...
        public static final int SELF_DEFINED_LISTVIEW = 5;
        public static final int SELF_WEBVIEW = 6;
        private int scrollType = 0;

        private ScrollController getScrollController() {
            if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "getScrollController:scrollState = " + scrollType);
            ScrollController sc = null;
            switch (scrollType) {
                case LISTVIEW:
                    sc = new ListViewController(scrollableView);
                    break;
                case RECYCLEVIEW:
                    sc = new RecyclerViewController(scrollableView, recyclerViewClass, isSelfRecyclerView);
                    break;
                case WEBVIEW:
                    sc = new WebViewController(scrollableView);
                    break;
                case SCROLLVIEW:
                    sc = new ScrollViewController(scrollableView);
                    break;
                case SELF_DEFINED_LISTVIEW:
                    sc = new ListViewController(scrollableView, true);
                    break;
                case SELF_WEBVIEW:
                    int index = -1;
                    int maxHeight = 0;
                    for (int i=0; i<scrollableView.getChildCount(); i++) {
                        View child = scrollableView.getChildAt(i);
                        Log.d(TAG, "child's height = " + child.getMeasuredHeight());
                        int height = child.getHeight();
                        if (height > maxHeight
                                && child instanceof ViewGroup
                                && child.getVisibility() == View.VISIBLE
                                && child.getLocalVisibleRect(new Rect())) {
                            maxHeight = height;
                            index = i;
                        }
                    }
                    sc = new WebViewController(scrollableView, (ViewGroup)(scrollableView.getChildAt(0)));
                    break;
                default:
                    break;
            }
        ...
        private Class recyclerViewClass = null;
        ...
        private int checkWhetherScrollableView(ViewGroup viewGroup) {
            ...
            for (;;) {
                ...
                if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "className = " + classNamepath + " index = " + classNamepath.lastIndexOf("."));
        ...
        abstract class ScrollController {
            ...
            protected void continueScroll() {
                ...
                if (this.canScrollDown()) {
                    ...
                    if (lastItemView != null) lastItemPreBottom = lastItemView.getBottom();
                        if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "continueScroll: lastItemPreBottom = " + lastItemPreBottom);
            ...
            protected int fitScrollDistance(int scrollDistance) {
                if (canScrollDown() /*|| scrollDistance > onceScrollDistance*/) {
                    ...
                    if (lastItemView != null) {
                        if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "continueScroll: lastItemPreBottom = " + lastItemView.getBottom());
            ...
            protected class CheckScrollDistanceRunnable implements Runnable {
                ...
                public void run() {
                    ...
                    if (lastItemView != null) {
                        if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "run: lastItemPreBottom = " + lastItemView.getBottom());
        ...
        class ListViewController extends ScrollController {
            ...
            //use to compute last scroll distance
            private int lastItemPreBottom = 0;
            private View lastItemView = null;
            private boolean selfDefinedListView = false;
            private Method mGetDividerHeight, mSmoothScrollBy, mGetFirstVisiblePosition, mGetLastVisiblePosition, mGetCount;

            Runnable scrollRunnable = new Runnable() {
                @Override
                public void run() {
                    int index = -1;
                    if (selfDefinedListView) {
                        try {
                            index = (Integer)mGetFirstVisiblePosition.invoke(view);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }

                    } else {
                        index = sListView.getFirstVisiblePosition();
                    }

                    int position = view.getChildAt(0).getTop();
                    int scrollDistance = prePosition - position + (index - preIndex) * itemHeight;
                    if (preIndex == 0 && firstItemHeight == 0) {
                        scrollDistance = prePosition - position + (index - preIndex - 1) * itemHeight;
                    }
                    Log.d(TAG, "scrollRunnable scrollDistance = " + scrollDistance);
                    scrollDistance = fitScrollDistance(scrollDistance);
                    Log.d(TAG, "scrollDistance = " + scrollDistance + " after fitted");
                    //last scroll
                    if (!canScrollDown() && lastItemView != null) scrollDistance = lastItemPreBottom - lastItemView.getBottom();
                    Log.d(TAG, "scrollDistance = " + scrollDistance + " after fitted");
                    sendScrollMessage(scrollDistance);
                }
            };
            ...
            public ListViewController(ViewGroup view, boolean selfDefinedListView) {
                ...
                try {
                    mGetDividerHeight = view.getClass().getMethod("getDividerHeight", null);
                    mSmoothScrollBy =  view.getClass().getMethod("smoothScrollBy", int.class, int.class);
                    mGetLastVisiblePosition = view.getClass().getMethod("getLastVisiblePosition", null);
                    mGetFirstVisiblePosition = view.getClass().getMethod("getLastVisiblePosition", null);
                    mGetCount = view.getClass().getMethod("getCount", null);
                    ...
                    itemHeight = itemViewHeight + (Integer)mGetDividerHeight.invoke(view);
                } catch (Exception e) {
                    e.printStackTrace();
                }

                //getChildAt(0).getHeight() maybe zero
                firstItemHeight = view.getChildAt(0).getHeight();
            }

            public ListViewController(ViewGroup view) {
                super(view);
                sListView = (AbsListView)view;
                //getChildAt(0).getHeight() maybe zero
                int index = 0;
                int itemViewHeight = 0;
                if (sListView instanceof ListView) {
                    ListView listView = (ListView)sListView;
                    do {
                        itemViewHeight = view.getChildAt(index).getHeight();
                        index += 1;
                    } while (itemViewHeight == 0);
                    itemHeight = itemViewHeight + listView.getDividerHeight();
                } else {
                    do {
                        itemViewHeight = view.getChildAt(index).getHeight();
                        index += 1;
                    } while (itemViewHeight == 0);
                    itemHeight = itemViewHeight;
                }
                firstItemHeight = view.getChildAt(0).getHeight();
            }
            ...
            protected void continueScroll() {
                ...
                if (selfDefinedListView) {
                    try {
                        preIndex = (Integer)mGetFirstVisiblePosition.invoke(view);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }

                } else {
                    preIndex = sListView.getFirstVisiblePosition();
                }
                //if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "listView.getCount() = " + sListView.getCount());
                prePosition = view.getChildAt(0).getTop();
                //itemHeight = sListView.getChildAt(1).getHeight() + sListView.getDividerHeight();
                lastItemView = view.getChildAt(view.getChildCount() - 1);
                lastItemPreBottom = lastItemView.getBottom();
                if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "preIndex = " + preIndex + " itemHeight = " + itemHeight);
                if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "startShotListView:prePosition = " + prePosition);
                //scroll 500 by 50ms
                if (selfDefinedListView) {
                    try {
                        mSmoothScrollBy.invoke(view, onceScrollDistance, SCROLL_DURATION);
                        //view.scrollBy(0, onceScrollDistance);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    ...
                    sListView.smoothScrollBy(onceScrollDistance, SCROLL_DURATION);
                }

                view.postDelayed(scrollRunnable, 60);
            }
            ...
            @Override
            protected void clearScrollDistanceListener() {
                //sListView.setOnScrollListener(null);
            }

            @Override
            boolean canScrollDown() {
                boolean result = true;
                if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "viewGroup is ListView");
                int count = 0;
                int lastVisiblePosition = 0;
                if (selfDefinedListView) {
                    try {
                        count = (Integer)mGetCount.invoke(view);
                        lastVisiblePosition = (Integer)mGetLastVisiblePosition.invoke(view);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }

                } else {
                    count = sListView.getCount();
                    lastVisiblePosition = sListView.getLastVisiblePosition();
                }
                if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "count = " + count + " getChildCount = " + view.getChildCount() + " scrollY = " + view.getScrollY());
                if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "LastVisiblePosition = " + lastVisiblePosition);
                //whether listview scroll to bottom or not
                ...
                if (lastVisiblePosition >= count - 1 && count > 0) {
                    int lastItemBottom = view.getChildAt(view.getChildCount() - 1).getBottom();
            ...
            protected int countMaxScrollDistance() {
                //int count = sListView.getCount();
                //maxScrollDistance = count * itemHeight;
                //leftScrollDistance = maxScrollDistance;
        ...
        class ScrollViewController extends ScrollController {
            ...
            boolean canScrollDown() {
                ...
                //whether scrollView scroll to bottom or not
                if (child.getMeasuredHeight() <= view.getScrollY() + view.getHeight()) {
                    result = false;
                    if (DEBUG_LONG_SCREENSHOT) Log.d(TAG, "ScrollView has reached bottom");
                }
            ...
            protected int countMaxScrollDistance() {
                View child = view.getChildAt(0);
                maxScrollDistance = child.getMeasuredHeight() - view.getHeight();
                leftScrollDistance = maxScrollDistance;
                return maxScrollDistance;
            }

            protected int fitScrollDistance(int scrollDistance) {
                int lastScrollDistance = countMaxScrollDistance() % onceScrollDistance;
                if (canScrollDown()) {
                    return onceScrollDistance;
                } else {
        ...
        class WebViewController extends ScrollController {
            private boolean isSelfWebView = false;
            private ViewGroup selfWebView = null;
            private ViewGroup childView = null;
            private int preLeftScrollFistance = 0;
            ...
            protected void continueScroll() {
                preLeftScrollFistance = leftScrollDistance;
                Log.d(TAG, "continueScroll: preLeftScrollFistance=" + preLeftScrollFistance);
                super.continueScroll();
            }

            protected int fitScrollDistance(int scrollDistance) {
                int result = 0;
                if (maxScrollDistance > onceScrollDistance) {
                    result = onceScrollDistance;
                } else {
                    result = maxScrollDistance % onceScrollDistance;
                }
                maxScrollDistance -= result;
                Log.d(TAG, "fitScrollDistance: result = " +  result);
                return result;
//                if (canScrollDown()) {
//                    return onceScrollDistance;
//                } else {
//                    return leftScrollDistance % onceScrollDistance;
//                }
            }
            ...
            boolean canScrollDown() {
                boolean result = true;
                if (maxScrollDistance <= 0) result = false;
//                if (isSelfWebView) {
//                    leftScrollDistance = countMaxScrollDistance() - childView.getScrollY();
//                    if (leftScrollDistance > 0 && leftScrollDistance == preLeftScrollFistance) leftScrollDistance -= onceScrollDistance * scrolltimes;
//                    Log.d(TAG, "leftScrollDistance = " + leftScrollDistance + " currentScrollDistance = " + childView.getScrollY());
...
//                }
            ...
            protected int countMaxScrollDistance() {
                ...
                if (isSelfWebView) {
                    try {
                        Method m1 = selfWebView.getClass().getMethod("getContentHeight", null);
                        Method m2 = selfWebView.getClass().getMethod("getScale", null);
                        m1.setAccessible(true);
                        m2.setAccessible(true);
                        int contentHeight = (Integer)m1.invoke(selfWebView);
                        float scale = (Float)m2.invoke(selfWebView);
                        int currentScrollDistance = selfWebView.getScrollY() != 0 ? selfWebView.getScrollY() : childView.getScrollY();
                        result = (int)(contentHeight * scale - selfWebView.getHeight() - currentScrollDistance);

                        Log.d(TAG, "leftScrollDistance = " + leftScrollDistance + " currentScrollDistance = " + currentScrollDistance);
                    } catch(Exception e) {
                        e.printStackTrace();
                    }
                } else {
                    WebView webView = (WebView) view;
                    result = (int)(webView.getContentHeight()*webView.getScale() - webView.getHeight()-webView.getScrollY());
                }
    ...
    protected void onPause() {
        ...
        if (LONG_SCREENSHOT_ENABLED) {
            if(longScreenshotUtil != null) {
                longScreenshotUtil.unbindScreenshotService();
            }
            if (mLongScreenshotReceiverRegistered) {
                unregisterReceiver(longscreenshotReceiver);
                mLongScreenshotReceiverRegistered = false;
            }
        }
    }
    ...
    protected void onStop() {
        ...
        if (longScreenshotUtil != null && !longScreenshotUtil.screenshotServiceBinded) {
            longScreenshotUtil = null;
        }
    }
