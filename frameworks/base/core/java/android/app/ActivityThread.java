...
public final class ActivityThread extends ClientTransactionHandler {
    ...
    private static final int LOG_AM_APP_PUB_PROVIDERS = 30101;
    ...
    private Activity performLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        try {
            ...
            CheckTime.checkTime(startTime, "instantiate activity");
        ...
        try {
            ...
            if (activity != null) {
                ...
                if (r.overrideConfig != null) {
                    // add systrace tag for bug 661753
                    Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "updateFrom");
                ...
                // add systrace tag for bug 661753
                Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "activity attach");
                ...
                if (theme != 0) {
                    // add systrace tag for bug 661753
                    Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "activity setTheme");
                ...
                CheckTime.checkTime(startTime, "activity onCreate", 800);
                // add systrace tag for bug 661753
                Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
                if (!activity.mCalled) {
    ...
    private ContextImpl createBaseContextForActivity(ActivityClientRecord r) {
        ...
        // add systrace tag for bug 661753
        Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "createActivityContext");
    ...
    private void handleReceiver(ReceiverData data) {
        ...
        try {
            ...
            CheckTime.checkTime(startTime, "instantiate receiver");
        } catch (Exception e) {
    ...
    private void handleCreateService(CreateServiceData data) {
        ...
        try {
            ...
            CheckTime.checkTime(startTime, "instantiate service");
        } catch (Exception e) {
    ...
    private void handleStopService(IBinder token) {
        ...
        try {
            // UNISOC: bug832698, ensure serviceDoneExecuting be called
            // after stop service code execution is done.
            int serviceDoneExecutingType = SERVICE_DONE_EXECUTING_ANON;
            if (s != null) {
                ...
                QueuedWork.waitToFinish();
                serviceDoneExecutingType = SERVICE_DONE_EXECUTING_STOP;
            } else {
            ...
            try {
                ActivityManager.getService().serviceDoneExecuting(
                        token, serviceDoneExecutingType, 0, 0);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        } catch (Exception e) {
    ...
    public void handleStopActivity(IBinder token, boolean show, int configChanges,
        ...
        final ActivityClientRecord r = mActivities.get(token);
        // UNISOC: bug741847,the activity of token may be null.
        if (r == null) {
    ...
    public void handleWindowVisibility(IBinder token, boolean show) {
        ...
        if (!show && !r.stopped) {
            ...
            // we are back active so skip it.
            unscheduleGcIdler();
            // UNISOC: bug950947, keep onStart and onStop in pair.
            if (!r.activity.mFinished) {
    ...
    private void handleRelaunchActivityLocally(IBinder token) {
        ...
        final ActivityRelaunchItem activityRelaunchItem = ActivityRelaunchItem.obtain(
                ...
                mergedConfiguration, /*shouldPreserveWindow(mergedConfiguration) */
                shouldPreserveWindowLocally(r));
    ...
    // Workaround for bug 960170 & bug 906365, just mark preserveWindow for Launcher
    private static boolean shouldPreserveWindowLocally(ActivityClientRecord r) {
        if (r == null || r.activity == null) return false;
        Intent intent = r.activity.getIntent();
        if (intent != null) {
    ...
    // SPRD: Should assume wether preserve window if relaunch locally
    /* maybe use preserveWindow = true directly
    private static boolean shouldPreserveWindow(MergedConfiguration mergedConfiguration) {
        if (mergedConfiguration == null) return true;

        if (mergedConfiguration.getOverrideConfiguration() == null) return true;
        return mergedConfiguration.getGlobalConfiguration().diff(
                mergedConfiguration.getOverrideConfiguration()) & (ActivityInfo.CONFIG_SCREEN_SIZE
    ...
    */
    ...
    private Configuration performConfigurationChangedForActivity(ActivityClientRecord r,
        ...
        // add systrace tag for bug 661753
        Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "performConfigurationChanged");
    ...
    private Configuration performActivityConfigurationChanged(Activity activity,
        ...
        // add systrace tag for bug 661753
        Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "updateResourcesForActivity");
        ...
        if (shouldChangeConfig) {
            activity.mCalled = false;
            // add systrace tag for bug 661753
            Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "onConfigurationChanged");
    ...
    public void handleConfigurationChanged(Configuration config) {
        PerformanceManagerInternal.getDefault().scheduleBoostRRForAppName(true, mInitialApplication.getPackageName(), ProcessInfo.TYPE_CONFIG_CHANGE);
        ...
        Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
        PerformanceManagerInternal.getDefault().scheduleBoostRRForAppName(false, mInitialApplication.getPackageName(), ProcessInfo.TYPE_CONFIG_CHANGE);
    }
    ...
    private void installContentProviders(
        ...
        try {
            EventLog.writeEvent(LOG_AM_APP_PUB_PROVIDERS, results.size());
    ...
    public final IContentProvider acquireProvider(
        ...
        // add systrace tag for bug 661753
        Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "acquireExistingProvider");
        ...
        // SPRD: None multiprocess Provider should get valid binder of holder.provider,
        // but failed when remote killed, try to recover in this condition. Workarounds.
        if (holder != null && holder.provider == null
                && (holder.info != null && !holder.info.multiprocess)
                && !("settings".equals(auth)) // Do not recover SettingsProvider
                && !(holder.info.packageName.equals(c.getPackageName()))) {

            // Try to release old reference
            if (holder.connection != null && !holder.noReleaseNeeded) {
                try {
                    ActivityManager.getService().removeContentProvider(holder.connection, stable);
                } catch (Exception e) {
                    Slog.w(TAG, "Before recovering from getContentProvider, remove ref but failed",
                            e);
                }
            }

            // And try to get a new holder.
            try {
                Slog.w(TAG, "Found " + auth + " for provider(" + holder.info
                        + ") but can not get remote obj from holder, try to recovery");
                holder = ActivityManager.getService().getContentProvider(
                        getApplicationThread(), auth, userId, stable);
            } catch (RemoteException ex) {
                throw ex.rethrowFromSystemServer();
        ...
        // add systrace tag for bug 661753
        Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "installProvider");
    ...
    private ContentProviderHolder installProvider(Context context,
        ...
        if (holder == null || holder.provider == null) {
            ...
            try {
                ...
                CheckTime.checkTime(startTime, "instantiate provider");
        ...
        synchronized (mProviderMap) {
            ...
            if (localProvider != null) {
                ...
                if (prc != null) {
                    ...
                    // add systrace tag for bug 661753
                    Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "installProviderAuthoritiesLocked");
    ...
    private void attach(boolean system, long startSeq) {
        ...
        if (!system) {
            ...
            try {
                ...
                CheckTime.checkTime(startTime, "attachApplication", 400);
            } catch (RemoteException ex) {
                throw ex.rethrowFromSystemServer();
            } finally {
                // add systrace tag for bug 661753
            ...
            try {
                ...
                // add systrace tag for bug 661753
                Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "mInitialApplication onCreate");
