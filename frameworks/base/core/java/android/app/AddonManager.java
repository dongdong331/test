...
public class AddonManager /* TODO extends IAddonManager */ {
    /**
     * This interface contains the callbacks which called by AddonManager when
     * the plugin context is created.
     */
    public interface InitialCallback {
        /**
         * Will be called when plugin context is created.
         * @param context plugin context
         * @param clazz plugin feature class
         * @return
         */
        Class onCreateAddon(Context context, Class clazz);
    }
    ...
    public AddonManager(Context context) {
        ...
        if (sAddonManagerService == null) {
            sAddonManagerService =
                    IAddonManager.Stub.asInterface(ServiceManager.getService("addon"));
        }
    ...
    /**
     * Get the addon packages for the application, this is the strategy method for
     * retrieve addon packages, if the {@link AddonManagerService} is exists, we will
     * first to get the addon packages from it, otherwise will scan the all installed
     * packages to get the addon packages for this application in its process.
     *
     * @param context The application context or the context set by the application
     * @return The addon feature list for the application
     */
    private static List<PackageInfo> manageFeatureListS(Context context) {
        ...
        if (sAddonManagerService != null) {
            try {
                addonPackageList = sAddonManagerService.getAppAddonFeatureList(pkgName);
                printCostTime(true, startTime, addonPackageList, pkgName);
            } catch (Exception e) {
                Log.d(LOGTAG, "manageFeatureListS occur error:" + e);
            }
        } else {
            addonPackageList = manageFeatureList(context);
            printCostTime(false, startTime, addonPackageList, pkgName);
        }
        return addonPackageList;
    }

    private static void printCostTime(boolean fromAddonService,
            long startTime, List list, String pkgName) {
        if (DEBUG) {
            Log.i(LOGTAG, "get addon packages" + (fromAddonService ? " from AddonManagerService"
                    : " by scan all installed packages") + " for app:" + pkgName + "-->"
                    + list + " cost " + (CheckTime.getTime() - startTime) + "ms");
        }
    }
                    ...
                    for (String key : metaKeys) {
                        if (key.startsWith("targetPackages")) {
                            if (metaData.getString(key, "").contains(
                                    currentPackageName)) {
                                if (DEBUG) {
                                    Log.d(LOGTAG, "-> manageFeatureList: The Plugin of <"
    ...
    public Object getAddon(String featureClassName, Class defClazz) {
        if (DEBUG) {
            ...
            if (defClazz != null) {
                Log.i(LOGTAG, "-> getAddon,The classloader of defClazz is "
                        + defClazz.getClassLoader());
            }
        }
        ...
        if (NO_PLUGIN.equals(featureClassName)) {
                ...
                for (PackageInfo pkg : mManagedList) {
                    ...
                    if (hasDefinedFeatureClass) {
                        loader = ApplicationLoaders.getDefault().getClassLoader(
                                pkg.applicationInfo.sourceDir, Build.VERSION.SDK_INT, false,
                                null, null, mContext.getClassLoader(), null);
                        break;
                    }
                }
            }
            ...
            if (DEBUG) {
                Log.d(LOGTAG, "->getAddon, createAddon app: " + applicationInfo
        ...
        if (featureObject == null) {
            try {
                if (DEBUG) {
                    Log.d(LOGTAG, "->getAddon: Create feature <" + featureClassName
                            + "> object Failed," + " AddonManager will return <"
                            + defClazz + "> object.");
                }
        ...
        if (clazz != null) {
            try {
                ...
                if (featureObject instanceof InitialCallback) {
                    ...
                    if (DEBUG) {
                        Log.d(LOGTAG, "->createAddon, wantedClass <" + wantedClass
