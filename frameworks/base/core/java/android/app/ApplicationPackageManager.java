...
public class ApplicationPackageManager extends PackageManager {
    ...
    public List<PackageInfo> getInstalledPackagesAsUser(int flags, int userId) {
        try {
            ...
            //return parceledList.getList();
            List<PackageInfo> list = parceledList.getList();
            Iterator<PackageInfo> iterator = list.iterator();
            while(iterator.hasNext()){
               PackageInfo packageInfo = iterator.next();
               if("com.sprd.opm".equals(packageInfo.packageName)){
                       Log.e(TAG, "remove sprd.opm from getInstalledPackagesAsUser");
                       iterator.remove();
               }
            }
            return list;
    ...
    public List<ApplicationInfo> getInstalledApplicationsAsUser(int flags, int userId) {
        try {
            ...
            //return parceledList.getList();
            List<ApplicationInfo> list = parceledList.getList();
            Iterator<ApplicationInfo> iterator = list.iterator();
            while(iterator.hasNext()){
               ApplicationInfo applicationInfo = iterator.next();
               if("com.sprd.opm".equals(applicationInfo.packageName)){
                       Log.e(TAG, "remove sprd.opm from getInstalledApplications");
                       iterator.remove();
               }
            }
            return list;
    ...
    public List<ResolveInfo> queryIntentActivitiesAsUser(Intent intent,
	    ...
	    List<ResolveInfo> list = parceledList.getList();
            ...
            while(iterator.hasNext()){
               ResolveInfo resolve = iterator.next();
               if(resolve.activityInfo!=null && "com.sprd.opm".equals(resolve.activityInfo.packageName)){
                       Log.e(TAG, "remove sprd.opm from queryIntentActivitiesAsUser");
                       iterator.remove();
               }
            }
            return list;
    ...
    protected @Nullable VolumeInfo getPackageCurrentVolume(ApplicationInfo app,
        ...
        if (app.isInternal()) {
             ...
             * Add support for install apk to external sdcard @{
             * @orig return storage.getPrimaryPhysicalVolume();
             */
            StorageVolume sdcardVolume = storage.getStorageVolume(EnvironmentEx.getExternalStoragePath());
    ...
    private boolean isPackageCandidateVolume(
        ...
        if (vol.isPrimaryPhysical()) {
            // SPRD: ASEC is not supported and logic code is removed by Google on AndroidP @{
            return false;
