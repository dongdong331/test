...
public class DownloadManager {
    ...
    /**
     * Value of {@link #COLUMN_STATUS} before network confirming.
     * @hide
     */
    public final static int STATUS_TO_BE_PAUSED_FOR_CONFIRM_NETWORK = 1 << 5;

    /**
     * Value of {@link #COLUMN_STATUS} when network is being confirmed.
     * @hide
     */
    public final static int STATUS_WAITING_FOR_CONFIRM_NETWORK = 1 << 6;

    /**
     * Value of {@link #COLUMN_STATUS} after network confirming.
     * @hide
     */
    public final static int STATUS_CONFIRMED = 1 << 7;
    ...
    /**
     * request scan by DownloadScanner instead of sending ACTION_MEDIA_SCANNER_SCAN_FILE
     * @hide
     */
    public static final String TO_SCAN_BY_DOWNLOAD = "to_scan_by_download";
    ...
    /** @hide */
	public static final Object mStatusLock = new Object();
    /**
    ...
    public static class Query {
        ...
        private Integer mControlFlags = null;
        ...
        /**
         * Include only downloads with control.
         * @hide
         * @param flags of the CONTROL_*
         * @return this object
         */
        public Query setFilterByControl(int flags) {
            mControlFlags = flags;
            return this;
        }
        ...
        Cursor runQuery(ContentResolver resolver, String[] projection, Uri baseUri) {
            ...
            if (mStatusFlags != null) {
                ...
                if ((mStatusFlags & STATUS_TO_BE_PAUSED_FOR_CONFIRM_NETWORK) != 0) {
                ...
                if ((mStatusFlags & STATUS_WAITING_FOR_CONFIRM_NETWORK) != 0) {
                    parts.add(statusClause("=", Downloads.Impl.STATUS_WAITING_FOR_CONFIRM_NETWORK));
                }
                if ((mStatusFlags & STATUS_CONFIRMED) != 0) {
                    parts.add(statusClause("=", Downloads.Impl.STATUS_CONFIRMED));
                }
            ...
            if (mControlFlags != null) {
                selectionParts.add(Downloads.Impl.COLUMN_CONTROL + "='" + mControlFlags + "'");
            }
    ...
    /**
     * Returns the status of the given downloaded file id.
     *
     * @param id the id of the downloaded file.
     * @return the status of the given downloaded file id.
     * @hide
     */
    public int getStatusForDownloadedFile(long id) {
        Query query = new Query().setFilterById(id);
        Cursor cursor = null;
        try {
            cursor = query(query);
            ...
            while (cursor.moveToFirst()) {
                return cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_STATUS));
            }
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    public boolean rename(Context context, long id, String displayName) {
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                data = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_LOCAL_FILENAME));
            }
        } finally {
            if (cursor != null) {
                cursor.close();
        ...
        if (data != null) {
            parent = new File(data).getParentFile();
        } else {
            parent = Environment.getExternalStoragePublicDirectory(
                Environment.DIRECTORY_DOWNLOADS);
        }

        File before = new File(parent, oldDisplayName);
        if (!before.exists()) {
            Log.d(TAG, "before file :" + oldDisplayName
                      + " not exist, use _data as file name when rename");
            before = new File(data);
        }
        ...
        context.getContentResolver().delete(externalUri,
                "_data LIKE ?1 AND lower(_data)=lower(?2)",
                new String[] { before.getAbsolutePath(), before.getAbsolutePath() });
        ...
        values.put(TO_SCAN_BY_DOWNLOAD, 1);
        long[] ids = {id};
    ...
    /**
     * Get the config of download alert.If set, alert dialog pops up when download starting in GPRS.
     * @hide
     */
    ...
    private static class CursorTranslator extends CursorWrapper {
        ...
        private int translateStatus(int status) {
            switch (status) {
                ...
                case Downloads.Impl.STATUS_WAITING_FOR_NETWORK:
                case Downloads.Impl.STATUS_QUEUED_FOR_WIFI:
                case Downloads.Impl.STATUS_WAITING_FOR_CONFIRM_NETWORK:
                case Downloads.Impl.STATUS_CONFIRMED:
