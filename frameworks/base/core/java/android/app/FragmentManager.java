...
final class FragmentManagerImpl extends FragmentManager implements LayoutInflater.Factory2 {
    ...
    void moveToState(int newState, boolean always) {
        ...
        if (mActive != null) {
            ...
            // UNISOC: bug709255, avoid IndexOutOfBoundException occur.
            ArrayList<Fragment> tmpAddedFragments = new ArrayList<>();
            tmpAddedFragments.addAll(mAdded);
            final int numAdded = tmpAddedFragments.size();
            for (int i = 0; i < numAdded; i++) {
                Fragment f = tmpAddedFragments.get(i);
                if (!f.mAdded) continue;
    ...
    /**
     * Remove the fragment from the saved nonconfig fragments data
     * which is collected in the {@link saveNonConfig} when
     * {@link saveAllState} is called.
     *
     * @param fragment The fragment to be removed.
     */
    private void removeFragementFromSavedNonConfig(Fragment fragment) {
        if (mSavedNonConfig != null && fragment != null) {
            List<Fragment> fragments = mSavedNonConfig.getFragments();
            if (fragments != null && fragments.remove(fragment)) {
                Log.i(TAG, fragment + " is been removed from saved nonconfig"
                        + " data:" + fragments);
            }
        }
    }
    ...
    void makeInactive(Fragment f) {
        ...
        // SPRD: bug768800, remove inactive fragment from saved nonconfig data,
        // avoid restoreAllState fail because of it.
        removeFragementFromSavedNonConfig(f);
        f.initState();
    }
    ...
    private void addAddedFragments(ArraySet<Fragment> added) {
        ...
        mTmpAddedFragments.addAll(mAdded);
        final int numAdded = mTmpAddedFragments.size();
        for (int i = 0; i < numAdded; i++) {
            Fragment fragment = mTmpAddedFragments.get(i);
