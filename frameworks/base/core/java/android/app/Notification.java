...
public class Notification implements Parcelable
{
    ...
    // work around for app small icon not compatible for platform grayscale icon rules
    private static final boolean CHECK_SDK_VERSION_FOR_GRAYSCALE_ICON = false;
    ...
    public void writeToParcel(Parcel parcel, int flags) {
        ...
        if (collectPendingIntents) {
            PendingIntent.setOnMarshaledListener(
                ...
                if (parcel == out) {
                    //fixed bug912190 The allPendingIntents variables of type is ArraySet.
                    //ArraySet is not thread-safe.
                    synchronized (this){
    ...
    public static class Builder {
        ...
        private void processSmallIconColor(Icon smallIcon, RemoteViews contentView,
                boolean ambient) {
            boolean colorable = CHECK_SDK_VERSION_FOR_GRAYSCALE_ICON ?
                    (!isLegacy()|| getColorUtil().isGrayscaleIcon(mContext, smallIcon))
                    : getColorUtil().isGrayscaleIcon(mContext, smallIcon);
            int color;
            if (ambient) {
