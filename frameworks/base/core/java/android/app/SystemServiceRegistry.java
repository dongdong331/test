...
import android.app.sprdpower.PowerGuruFactory;
...
final class SystemServiceRegistry {
    ...
    static {
        ...
        /* SPRD: Initialize extended service. @{ */
        registerService("phone_ex", TelephonyManagerEx.class,
                new CachedServiceFetcher<TelephonyManagerEx>() {
            @Override
            public TelephonyManagerEx createService(ContextImpl ctx) {
                return new TelephonyManagerEx(ctx.getOuterContext());
            }});
        /* @} */
        ...
        registerService(Context.FACE_SERVICE, FaceManager.class,
            ...
            public FaceManager createService(ContextImpl ctx) throws ServiceNotFoundException {
                IBinder binder = ServiceManager.getServiceOrThrow(Context.FACE_SERVICE);
        ...
        //SPRD: Bug 692795 add for heartbeat powerguru service --> BEG
        if (AbsPowerGuru.isEnabled()) {
            registerService(Context.POWERGURU_SERVICE, AbsPowerGuru.class,
                    new CachedServiceFetcher<AbsPowerGuru>() {
                @Override
                public AbsPowerGuru createService(ContextImpl ctx) {
                    IBinder b = ServiceManager.getService(Context.POWERGURU_SERVICE);
                    IPowerGuru service = IPowerGuru.Stub.asInterface(b);
                    return PowerGuruFactory.getInstance().createExtraPowerGuru(service, ctx);
                }});
        }
        //SPRD: Bug 692795 add for heartbeat powerguru service --> END
