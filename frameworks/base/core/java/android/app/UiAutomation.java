...
public final class UiAutomation {
    ...
    public void waitForIdle(long idleTimeoutMillis, long globalTimeoutMillis)
        ...
        try {
            ...
            boolean getRotation = false;
            for (StackTraceElement se : steArray) {
                if ("android.autofillservice.cts.SessionLifecycleTest".equals(se.getClassName())
                        && "testDatasetAuthResponseWhileAutofilledAppIsLifecycled".equals(se.getMethodName())) {
                    target = true;
                } else if ("android.support.test.uiautomator.UiDevice".equals(se.getClassName())
                        && "getDisplayRotation".equals(se.getMethodName())) {
                    getRotation = true;
                }
            }
            if (getRotation && target) {
                SystemClock.sleep(2000);
                Log.w(LOG_TAG, "waitForIdle workaroud for cts, should be removed if google's bug is fixed");
            }
        } catch (Exception e) {
            //we catch all exception here becuase we don't want that the process has benn interupted by the workaround.
            Log.e(LOG_TAG, "Error while doing workaround!", e);
        }
        synchronized (mLock) {
            throwIfNotConnectedLocked();
