...
public class FingerprintManager implements BiometricFingerprintConstants {
    ...
    public void enroll(byte [] token, CancellationSignal cancel, int flags,
        ...
        //add by unisoc for Bug 908662 begin
        LockPatternUtils lockPatternUtils = new LockPatternUtils(mContext);
        final int security = lockPatternUtils.getActivePasswordQuality(userId);
        if(DevicePolicyManager.PASSWORD_QUALITY_UNSPECIFIED == security){
            callback.onEnrollmentError(FINGERPRINT_ERROR_CANCELED,
                    getErrorString(FINGERPRINT_ERROR_CANCELED, 0 /* vendorCode */));
            return;
        }
        //add by unisoc for Bug 908662 end
