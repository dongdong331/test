...
class TunerCallbackAdapter extends ITunerCallback.Stub {
    ...
    void setProgramListObserver(@Nullable ProgramList programList,
        ...
        synchronized (mLock) {
            ...
            Log.w(TAG,"ProgramList update, onComplete....");
            ...
            programList.addOnCompleteListener(() -> {
                synchronized (mLock) {
                    Log.w(TAG,"onComplete,before list set....");
                    if (mProgramList != programList) return;
                    mLastCompleteList = programList.toList();
                    Log.w(TAG,"onComplete,after list set:"+mLastCompleteList);
    ...
    public void onProgramListUpdated(ProgramList.Chunk chunk) {
        Log.d(TAG,"onProgramListUpdate start.");
        synchronized (mLock) {
            Log.d(TAG,"onProgramListUpdate, mlock.");
            if (mProgramList == null) return;
            Log.d(TAG,"onProgramListUpdate, call ProgramList apply");
