...
public class ViewRootImpl extends AbsViewRootImpl implements ViewParent,
    ...
    private void enableHardwareAcceleration(WindowManager.LayoutParams attrs) {
        ...
        if (SystemProperties.getBoolean(PROPERTY_DISABLE_HWACC, false)) {
            // SPRD: Disable HW Acceleration by adb command
            Log.d(TAG, "disable hardware acceleration by DEBUG!!!, "
                  + (mWindowAttributes != null ? mWindowAttributes.getTitle() : null));
            return;
        }
    ...
    void setWindowStopped(boolean stopped) {
        if (mStopped != stopped) {
            ...
            if (!mStopped) {
                //SPRD: Get new surface if it was released from setWindowStopped to prevent view cannot be drawn.
                mNewSurfaceNeeded = true;
                scheduleTraversals();
            } else {
                if (renderer != null) {
    ...
    final class WindowInputEventReceiver extends InputEventReceiver {
        ...
        public void onInputEvent(InputEvent event, int displayId) {
            if (!dropIfNeeded(event)) { // SPRD: add for dynamic navigationbar
