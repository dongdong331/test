...
class ResolverComparator implements Comparator<ResolvedComponentInfo> {
    ...
    public void getContentAnnotations(Intent intent) {
        //SPRD:bug952762 handle BadParcelableException when unmarshall Intent.
        ArrayList<String> annotations;
        try {
            annotations = intent.getStringArrayListExtra(
                    Intent.EXTRA_CONTENT_ANNOTATIONS);
        } catch (BadParcelableException e) {
            Log.e(TAG, "Failed to getContentAnnotations: " + intent, e);
            return;
        }
        if (annotations != null) {
