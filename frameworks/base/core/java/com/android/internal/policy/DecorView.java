...
public class DecorView extends FrameLayout implements RootViewSurfaceTaker, WindowCallbacks {
    ...
    private static boolean sDebugDispatchInput = false;

    /**
     * Prop for debug input event.
     *
     * @hide
     */
    private static final String PROPERTY_PROFILE_DISPATCHINPUT = "persist.sys.dispatch_input";
    ...
    // UNISOC: Bug711794,add decor view attach flag.
    ...
    DecorView(Context context, int featureId, PhoneWindow window,
        ...
        sDebugDispatchInput = SystemProperties.getBoolean(PROPERTY_PROFILE_DISPATCHINPUT, false);
    }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (isDown && (event.getRepeatCount() == 0)) {
         ...
         if (sDebugDispatchInput) {
               Log.d(TAG, "No window pannel handle, so deliver to activity, mWindow.isDestroyed() " + mWindow.isDestroyed()
                   + ", mWindow.getCallback(): " + mWindow.getCallback());
           }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (sDebugDispatchInput) {
            Log.d(TAG, "dispatchTouchEvent, deliver to activity, mWindow.isDestroyed() " + mWindow.isDestroyed()
                + ", mWindow.getCallback(): " + mWindow.getCallback());
        }
        final Window.Callback cb = mWindow.getCallback();
        ...
        if (!fixedWidth && widthMode == AT_MOST) {
            ...
            if (tv.type != TypedValue.TYPE_NULL) {
                ...
                if (tv.type == TypedValue.TYPE_DIMENSION) {
                    ...
                    /*SPRD:Modify for bug 903696 in 20180724*/
                    updateAvailableWidth();
