...
jobject doDecode(JNIEnv* env, std::unique_ptr<SkStreamRewindable> stream, jobject padding, jobject options);
...
static jobject nativeDecodeDrmStream(JNIEnv* env, jobject clazz, jobject client, jobject handle,
    ...
    if (stream) {
        bitmap = doDecode(env, std::unique_ptr<SkStreamRewindable>(stream), NULL, opts);
