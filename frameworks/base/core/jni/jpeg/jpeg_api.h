...
typedef void (*jpg_evt_cb_ptr)(enum jpg_jpeg_evt evt, void* data, void* privdata);

jpg_int sprd_jpeg_init(JPEG_CODEC_CALLER_T*  oem_handle,jpg_evt_cb_ptr jpg_evt_cb);
jpg_int sprd_jpeg_deinit(JPEG_CODEC_CALLER_T*  oem_handle);
jpg_int sprd_jpg_encode(JPEG_CODEC_CALLER_T* oem_handle,
                      struct yuvbuf_frm *src, struct yuvbuf_frm *dst, struct jpg_op_mean *mean, jpeg_enc_cb_param *out_enc_param);
...
jpg_int sprd_jpg_dec_get_resolution(unsigned char* jpg_src, int jpg_size,  unsigned int *wdith, unsigned int *height, unsigned int *yuv_buffer_size);
jpg_int sprd_jpg_set_resolution(void* jpg_buf,int jpg_size,int width,int height);
