...
public class SoterKeyStoreKeyPairRSAGeneratorSpi extends KeyPairGeneratorSpi {
    ...
    throws InvalidAlgorithmParameterException {
        ...
        try {
            ...
            try {
                    ...
                    for (int keymasterPadding : mKeymasterEncryptionPaddings) {
                        if (!isKeymasterPaddingSchemeIndCpaCompatibleWithAsymmetricCrypto(
                            ...
                            throw new InvalidAlgorithmParameterException(
                                    ...
                                    + ". See " + KeyGenParameterSpec.class.getName() + " documentation.");
                        }
                    }
                }
                ...
                KeyGenParameterSpec newSpec = new KeyGenParameterSpec.Builder(mSpec)
                    .setUserAuthenticationValidWhileOnBody(false)
                    .setInvalidatedByBiometricEnrollment(true)
                    .build();
                KeymasterUtils.addUserAuthArgs(new KeymasterArguments(), newSpec);
    ...
    public KeyPair generateKeyPair() {
        ...
        KeyGenParameterSpec newSpec = new KeyGenParameterSpec.Builder(mSpec)
            ...
            .setInvalidatedByBiometricEnrollment(true)
            .build();
        KeymasterUtils.addUserAuthArgs(args, newSpec);
    ...
    private void addAlgorithmSpecificParameters(KeymasterArguments keymasterArgs) {
        ...
        if (isAutoSignedWithAttkWhenGetPublicKey) {
            ...
            // if ASKey we need key name as file id; here just use tag(USER_PRIVATE_KEY)+user id + keyalias
            mAutoSignedKeyNameWhenGetPublicKey = Credentials.USER_PRIVATE_KEY+ UserHandle.getUserId(android.os.Process.myUid()) + mEntryAlias ;
            Log.d("Soter", "use mEntryAlias as key name, pass it to args");
            if(!SoterUtil.isNullOrNil(mAutoSignedKeyNameWhenGetPublicKey)) {
                keymasterArgs.addBytes(KeymasterDefs.KM_TAG_SOTER_AUTO_SIGNED_COMMON_KEY_WHEN_GET_PUBLIC_KEY, mAutoSignedKeyNameWhenGetPublicKey.getBytes());
            }
        }
        ...
        if (isAutoSignedWithCommonkWhenGetPublicKey) {
            ...
            if(!SoterUtil.isNullOrNil(mAutoSignedKeyNameWhenGetPublicKey)) {
                ...
                keymasterArgs.addBytes(KeymasterDefs.KM_TAG_SOTER_AUTO_SIGNED_COMMON_KEY_WHEN_GET_PUBLIC_KEY, (Credentials.USER_PRIVATE_KEY+ UserHandle.getUserId(android.os.Process.myUid()) + mAutoSignedKeyNameWhenGetPublicKey).getBytes());
            }
        }
