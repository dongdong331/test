...
bool CanvasContext::makeCurrent() {
    ...
    if (nwin) {
        int dataspace = -1;
        int res = nwin->query(nwin, NATIVE_WINDOW_DEFAULT_DATASPACE, &dataspace);
        if (res == NO_INIT) {
            ALOGE("makeCurrent %p, but the nativewindow is not inited", nwin);
            setSurface(nullptr);
            return false;
        }
    }
...
void CanvasContext::draw() {
    ...
    if (frame.mSurface == EGL_NO_SURFACE){
        ALOGE("CanvasContext::draw, frame not available in pipeline getFrame");
        return;
    }
    ...
    if (nwin) {
        ...
        int res = nwin->query(nwin, NATIVE_WINDOW_DEFAULT_DATASPACE, &dataspace);
        if (res == NO_INIT) {
            ALOGE("CanvasContext::draw %p, but the nativewindow is not inited", nwin);
