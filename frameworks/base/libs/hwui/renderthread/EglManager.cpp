...
static int queryNativeWindowStatus(EGLNativeWindowType window)
{
    ANativeWindow *nwin = (ANativeWindow *)window;
    int dataspace = -1;
    int res = -1;

    if (nwin == NULL)
    {
        ALOGD("queryNativeWindowStatus win is null");
        return -1;
    }
    res = nwin->query(nwin, NATIVE_WINDOW_DEFAULT_DATASPACE, &dataspace);
    ALOGI("queryNativeWindowStatus %p query=%d dataspace=%d", nwin, res, dataspace);
    if (res == NO_INIT) {
        ALOGD("queryNativeWindowStatus win:%p, but the nativewindow is not inited", nwin);
        return -1;
    }
...
EGLSurface EglManager::createSurface(EGLNativeWindowType window, bool wideColorGamut) {
    ...
    if (queryNativeWindowStatus(window) != 0)
    {
        ALOGE("The surface:%p has been abandoned, err:%s", (void*)window, eglErrorString());
    ...
    if (mSwapBehavior != SwapBehavior::Preserved) {
        ret = eglSurfaceAttrib(mEglDisplay, surface, EGL_SWAP_BEHAVIOR, EGL_BUFFER_DESTROYED);
        if (ret == EGL_FALSE)
        {
            if (queryNativeWindowStatus(window) != 0)
            {
                ALOGE("The surface:%p has been abandoned, eglSurfaceAttrib err:%s", (void*)window,
                      eglErrorString());
                return EGL_NO_SURFACE;
            }

            LOG_ALWAYS_FATAL("Failed to set swap behavior to destroyed for window %p, eglErr = %s",
...
Frame EglManager::beginFrame(EGLSurface surface) {
    ...
    Frame frame;
    EGLint error = 0;
    makeCurrent(surface, &error);
    if (error != 0) {
        frame.mSurface = EGL_NO_SURFACE;
        return frame;
    }
...
void EglManager::damageFrame(const Frame& frame, const SkRect& dirty) {
    ...
    if (EglExtensions.setDamage && mSwapBehavior == SwapBehavior::BufferAge) {
        ...
        if(queryBufferAge(frame.mSurface) >=0){
