...
public class MtpDatabase implements AutoCloseable {
    ...
    //MediaProvider can't deal with /mnt/media_rw,use /storage/ instead.
    private String path2storage(String path){
        if(!TextUtils.isEmpty(path)){
            String storagePath = path.replace("/mnt/media_rw/","/storage/");
            return storagePath;
        }
        return path;
    }

    private Path path2storage(Path path){
        String tmp = path.toString();
        tmp = path2storage(tmp);
        Path storagePath = Paths.get(tmp);
        return storagePath;
    }
        ...
        if (succeeded) {
            ...
            try {
                if (obj.getParent().isRoot()) {
                    ...
                    Path parentPath = path2storage(obj.getParent().getPath());
                    int parentId = findInMedia(parentPath);
                    if (parentId != -1) {
                        values.put(Files.FileColumns.PARENT, parentId);
                    } else {
                        Log.d(TAG, "endSendObject: The parent isn't in MediaProvider. Don't add the new file.");
                        return;
                    }
                }
    ...
    private int renameFile(int handle, String newName) {
        ...
        oldPath = path2storage(oldPath);
        newPath = path2storage(newPath);
    ...
    private void endMoveObject(int oldParent, int newParent, int oldStorage, int newStorage,
        ...
        path = path2storage(path);
        oldPath = path2storage(oldPath);
        Log.d(TAG, "endMoveObject oldPath: " + oldPath + "path: " + path);
        values.put(Files.FileColumns.DATA, path.toString());
        if (obj.getParent().isRoot()) {
            values.put(Files.FileColumns.PARENT, 0);
        } else {
            int parentId = findInMedia(path.getParent());
            Log.d(TAG,"endMoveObject findInMedia :"+parentId);
            if (parentId != -1) {
                values.put(Files.FileColumns.PARENT, parentId);
            } else {
                Log.d(TAG,"The new parent isn't in MediaProvider, so delete the object instead");
    ...
    private void endCopyObject(int handle, boolean success) {
        ...
        int format = obj.getFormat();
        // Get parent info from MediaProvider, since the id is different from MTP's
        Log.d(TAG, "endCopyObject path: " + path);
        ContentValues values = new ContentValues();
        values.put(Files.FileColumns.DATA, path);
        values.put(Files.FileColumns.FORMAT, format);
        values.put(Files.FileColumns.SIZE, obj.getSize());
        ...
        try {
            if (obj.getParent().isRoot()) {
                ...
                if (parentId != -1) {
                    ...
                    Log.d(TAG, "endCopyObject: The parent isn't in MediaProvider. Don't add the new file.");
                    return;
                }
            }
            if (obj.isDir()) {
    ...
    private void deleteFromMedia(Path path, boolean isDir) {
        path = path2storage(path);
        Log.d(TAG, "deleteFromMedia path: " + path);
        try {
