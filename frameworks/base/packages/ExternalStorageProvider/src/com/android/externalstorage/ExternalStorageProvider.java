...
public class ExternalStorageProvider extends FileSystemProvider {
    ...
    public Cursor queryChildDocuments(
        ...
        final File parent = getFileForDocId(parentDocumentId, true);
    ...
    public String getDocumentType(String documentId) throws FileNotFoundException {
        final File file = getFileForDocId(documentId, true);
        String mimeType = file.isDirectory() ? null : checkMediaMimeType(getContext(), file.getAbsolutePath());
        if(mimeType == null ) {
    ...
    public String renameDocument(String docId, String displayName) throws FileNotFoundException {
        ...
        final File beforeVisibleFile = getFileForDocId(docId, true,false);
    ...
    @Override
    protected File getFileForDocIdMaybeCreate(String docId, boolean visible) throws FileNotFoundException {
        RootInfo root = getRootFromDocId(docId);
        return buildFile(root, docId, visible, true, true);
    }
    ...
    private File getFileForDocId(String docId, boolean visible, boolean mustExist)
            throws FileNotFoundException {
        RootInfo root = getRootFromDocId(docId);
        return buildFile(root, docId, visible, mustExist, false);
    }

    private Pair<RootInfo, File> resolveDocId(String docId, boolean visible)
            throws FileNotFoundException {
        RootInfo root = getRootFromDocId(docId);
        return Pair.create(root, buildFile(root, docId, visible, true, false));
    }
    ...
    private File buildFile(RootInfo root, String docId, boolean visible, boolean mustExist, boolean createParentIfNotExist)
            throws FileNotFoundException {
        final int splitIndex = docId.indexOf(':', 1);
        final String path = docId.substring(splitIndex + 1);
        Log.d(TAG, "buildFile  mustExist="+mustExist + "  createParentIfNotExist="+createParentIfNotExist);
        ...
        if (mustExist && !target.exists()) {
            if (createParentIfNotExist) {
                target.mkdirs();
                Log.d(TAG, "buildFile mkdirs file="+target+", exists="+target.exists());
            } else {
    ...
    public void ejectRoot(String rootId) {
        ...
        if (root != null) {
            String volumeId = root.volumeId;
            if (volumeId.startsWith(VolumeInfo.ID_EMULATED_INTERNAL)){
                volumeId = root.volumeId.replace(VolumeInfo.ID_EMULATED_INTERNAL,VolumeInfo.ID_PRIVATE_INTERNAL);
            }
            Log.d(TAG, "eject root id " + volumeId);
            try {
                mStorageManager.unmount(volumeId);
    ...
    public Bundle call(String method, String arg, Bundle extras) {
        if (DocumentsContract.METHOD_CREATE_DOCUMENT.equals(method)) {
            Log.d(TAG, "call  android:createDocument");
            final Uri doUri = extras.getParcelable(DocumentsContract.EXTRA_URI);
            final String docId = DocumentsContract.getDocumentId(doUri);
            try {
                final File file = getFileForDocIdMaybeCreate(docId, false);
            } catch (FileNotFoundException e) {
                Log.w(TAG, "docId '" + docId + "' not found");
            }
        }
