...
public class FusionEngine implements LocationListener {
    ...
    public void setRequest(ProviderRequestUnbundled request, WorkSource source) {
        if (DEBUG) {
            Log.d(TAG, "setRequest request " + request.toString() + " , source " + source.toString());
        } else {
    ...
    private void enableProvider(String name, long minTime) {
        if (DEBUG) {
            Log.d(TAG, "enableProvider name " + name + " , minTime " + minTime);
        } else {
            Log.i(TAG, "enableProvider");
        }
    ...
    private void disableProvider(String name) {
        if (DEBUG) {
            Log.d(TAG, "disableProvider name " + name);
        } else {
            Log.i(TAG, "disableProvider");
        }
        ProviderStats stats = mStats.get(name);
        if (stats == null) return;
    ...
    private static boolean isBetterThan(Location locationA, Location locationB) {
        if (DEBUG) {
            Log.d(TAG, "isBetterThan locationA "
                    + (locationA != null ? locationA.toString() : "null") + " locationB "
                    + (locationB != null ? locationB.toString() : "null"));
        }
      if (locationA == null) {
        return false;
      }
