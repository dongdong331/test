...
public class ApplicationsState {
            ...
            try {
                ...
                ParceledListSlice<ApplicationInfo> list =
                        mIpm.getInstalledApplications(
                                ...
                                user.id);
                List<ApplicationInfo> alist = list.getList();
                Iterator<ApplicationInfo> iterator = alist.iterator();
                while(iterator.hasNext()){
                    ApplicationInfo applicationInfo = iterator.next();
                    if("com.sprd.opm".equals(applicationInfo.packageName)){
                        Log.e(TAG, "remove sprd.opm for cmcc");
                        iterator.remove();
                    }
                }
                mApplications.addAll(alist);
    ...
    /*SPRD: 862319 Application list should remove the updated system applications.@{*/
    public static final AppFilter SPRD_FILTER_THIRD_PARTY = new AppFilter() {
        @Override
        public void init() {
        }
        ...
        @Override
        public boolean filterApp(AppEntry entry) {
            return (entry.info.flags & (ApplicationInfo.FLAG_UPDATED_SYSTEM_APP
