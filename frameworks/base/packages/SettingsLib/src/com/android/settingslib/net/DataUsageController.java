...
public class DataUsageController {
    ...
    public DataUsageInfo getDataUsageInfo(NetworkTemplate template) {
        ...
        Log.d(TAG," getDataUsageInfo template = " + template);
        try {
            final NetworkStatsHistory history = session.getHistoryForNetwork(template, FIELDS);
            final long now = System.currentTimeMillis();
            final long start, end;
            //Modified by UNISOC, if it's the last one, use the last 4 weeks as the period
            if (policy != null && NetworkPolicyManager.cycleIterator(policy).hasNext()) {
    ...
    private NetworkPolicy findNetworkPolicy(NetworkTemplate template) {
        ...
        Log.d(TAG," findNetworkPolicy policies.length = " + N);
        for (int i = 0; i < N; i++) {
            final NetworkPolicy policy = policies[i];
            if (policy != null && template.equals(policy.template)) {
                Log.d(TAG," findNetworkPolicy i = " + i + " return policy = " + policy);
                return policy;
            }
        }
        return null;
