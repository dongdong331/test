...
class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private void loadSecureSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            loadIntegerSetting(stmt, Settings.Secure.CAMERA_DOUBLE_TAP_POWER_GESTURE_DISABLED,
                    R.integer.def_camera_double_tap_power_gesture_disabled);

            loadBooleanSetting(stmt, Settings.Secure.DOZE_PULSE_ON_PICK_UP,
                    R.bool.def_doze_pulse_on_pick_up_enabled);
    ...
    private void loadStringSetting(SQLiteStatement stmt, String key, int resid) {
        if (Settings.Secure.LOCATION_PROVIDERS_ALLOWED.equals(key)) {
            // If china mobile version gps default turn off so remove gps configuration
            boolean gpsOff = SystemProperties.get("ro.carrier").equals("cmcc");
            loadSetting(stmt, key, gpsOff ? "" : mContext.getResources().getString(resid));
            return;
        }
