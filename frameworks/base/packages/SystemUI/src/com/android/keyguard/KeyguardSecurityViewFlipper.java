...
public class KeyguardSecurityViewFlipper extends ViewFlipper implements KeyguardSecurityView {
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        boolean subsidyLock = Resources.getSystem().getBoolean(com.android.internal.R.bool.config_subsidyLock);
        final View view = getChildAt(getDisplayedChild());
        if (subsidyLock && view !=  null) {
        ...
        if (subsidyLock && view !=  null) {
            ...
            width = Math.max(width,
                    Math.min(view.getMeasuredWidth(), widthSize - wPadding));
            height = Math.max(height,
                    Math.min(view.getMeasuredHeight(), heightSize - hPadding));
        } else {
            for (int i = 0; i < count; i++) {
                final View child = getChildAt(i);
                final LayoutParams lp = (LayoutParams) child.getLayoutParams();
