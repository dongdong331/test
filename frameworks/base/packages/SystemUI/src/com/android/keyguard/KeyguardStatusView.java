...
public class KeyguardStatusView extends GridLayout implements
    ...
    private void refreshTime() {
        refreshFormat();
        Log.d(TAG,"refreshTime mClockView.getText()="+mClockView.getText());
        mClockView.refresh();
    }

    private void refreshFormat() {
        Patterns.update(mContext);
        Log.d(TAG,"refeshFormat mClockView.getText()="+mClockView.getText());
         /* SPRD: 692446 clock add am/pm @{ */
        if(KeyguardSupportAmPm.getInstance(mContext).isEnabled()) {
            int textSize = (int) getResources().getDimension(R.dimen.widget_label_font_size);
            KeyguardSupportAmPm.getInstance(mContext).setFormat12Hour(textSize, mClockView);
