...
import static android.hardware.face.FaceManager.FACE_ERROR_LOCKOUT;
import static android.hardware.face.FaceManager.FACE_ERROR_NOFACE;
import static android.hardware.face.FaceManager.FACE_ERROR_CAMERA_UNAVAILABLE;
...
import com.sprd.sensor.LightSensorManager;
...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    private final static int MODEM_RESET_MSG = 338;
    private static final int MSG_SIMLOCK_UPDATE = 339;
    private static final int MSG_UPDATE_FACELOCK_STATE = 340;
    protected static final int MSG_HANDLE_CHECK_LIGHT = 666;
    ...
    /** FaceLock state: Not listening to face lock. */
    private static final int FACELOCK_STATE_STOPPED = 0;

    /** FaceLock state: Listening. */
    private static final int FACELOCK_STATE_RUNNING = 1;

    /**
     * FaceLock state: Cancelling and waiting for the confirmation from FaceManager to
     * send us the confirmation that cancellation has happened.
     */
    private static final int FACELOCK_STATE_CANCELLING = 2;

    /**
     * FaceLock state: During cancelling we got another request to start listening, so when we
     * receive the cancellation done signal, we should start listening again.
     */
    private static final int FACELOCK_STATE_CANCELLING_RESTARTING = 3;
    ...
    /* SPRD: add for face lock begin @{ */
    private int mFaceLockRunningState = FACELOCK_STATE_STOPPED;
    private FaceManager mFaceManager;
    private CancellationSignal mFaceLockCancelSignal;
    private static final int WIDTH = 960;
    private static final int HEIGHT = 720;
    private static final String FACEID_FUNCTION = "faceid_function";
    private boolean mGoingToSleepDisableFaceLock;
    private boolean mFaceLockout = false;
    private boolean mFingerWakeAndUnlocking;
    /* @} */
    ...
    private static final int LIGHT_BRIGHTNESS_LIMIT = 5;
    private static final String FACEID_BRIGHTNESS = "faceid_brightness";
    private static final boolean FACEID_BRIGHTNESS_FEATURE_ENABLED = "userdebug".equals(Build.TYPE) || "eng".equals(Build.TYPE);
    ...
    private final static String MODEM_RESET_KEY = "persist.vendor.sys.modemreset";
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SIM_STATE_CHANGE:
                    // UNISOC: add for bug 907015
                    refreshSimStateForUnlock(msg.arg1, msg.arg2, false);
                ...
                case MODEM_RESET_MSG:
                    for (int i = 0; i < mCallbacks.size(); i++) {
                        ...
                        if (cb != null) {
                            Log.i(TAG, "modem reset time out,so updateTelephony and refreshImsIcons");
                            cb.onModemAssert(false);
                        }
                    }
                        break;
                    /* @} */
                case MSG_UPDATE_FACELOCK_STATE:
                    updateFaceLockListeningState();
                    break;
                    /* UNISOC: Bug 926152 Refresh carrier info when sim is under simlock status.@{*/
                case MSG_SIMLOCK_UPDATE:
                    handleSimlockUpdate();
                    /* @} */
                    break;
    ...
    private SparseBooleanArray mUserFaceLockAuthenticated = new SparseBooleanArray();
    ...
    public boolean getUserTrustIsManaged(int userId) {
        return mUserTrustIsManaged.get(userId) && !isTrustDisabled(userId) || isFaceLockRunning();
    }
            ...
            if (Intent.ACTION_TIME_TICK.equals(action)
                ...
                if (DEBUG_SIM_STATES) {
                    Log.v(TAG, "action " + action
                        + " state: " + stateExtra + " slotId: " + args.slotId + " subid: " + args.subId);
                }
    ...
    private FaceManager.AuthenticationCallback mFaceLockAuthenticationCallback
            = new FaceManager.AuthenticationCallback() {

        @Override
        public void onAuthenticationFailed() {
            handleFaceLockAuthFailed();
        };

        @Override
        public void onAuthenticationSucceeded(FaceManager.AuthenticationResult result) {
            handleFaceLockAuthenticated();
        }

        @Override
        public void onAuthenticationHelp(int helpMsgId, CharSequence helpString) {
            //handleFaceLockHelp(helpMsgId, helpString != null ? helpString.toString() : null);
        }

        @Override
        public void onAuthenticationError(int errMsgId, CharSequence errString) {
            handleFaceLockError(errMsgId, errString != null ? errString.toString() : null);
        }
    };

    private void onFaceLockAuthenticated() {
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onFaceLockAuthenticated();
            }
        }
    }

    private void handleFaceLockAuthFailed() {
        stopScreenBrightnessIfNeeded();
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onFaceLockAuthFailed();
            }
        }
        handleFingerprintHelp(-1, mContext.getString(R.string.fingerprint_not_recognized));
    }

    private void handleFaceLockAuthenticated() {
        Log.d(TAG, "FaceLock handleFaceLockAuthenticated");
        stopScreenBrightnessIfNeeded();
        onFaceLockAuthenticated();
        setFaceLockRunningState(FACELOCK_STATE_STOPPED);
    }
    ...
    private void handleFaceLockHelp(int msgId, String helpString) {
        Log.d(TAG, "FaceLock handleFaceLockHelp");
        stopScreenBrightnessIfNeeded();
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onFaceLockHelp(msgId, helpString);
            }
        }
    }

    private void handleFaceLockError(int msgId, String errString) {
        Log.d(TAG, "FaceLock handleFaceLockError msgId:" + msgId + " errString:" + errString);
        stopScreenBrightnessIfNeeded();
        if (mFaceLockRunningState == FACELOCK_STATE_CANCELLING_RESTARTING) {
            setFaceLockRunningState(FACELOCK_STATE_STOPPED);
            startListeningForFaceLock();
        } else {
            setFaceLockRunningState(FACELOCK_STATE_STOPPED);
        }
        if(msgId == FACE_ERROR_NOFACE) {
            errString = mContext.getString(R.string.facelock_noface);
        } else {
            errString = mContext.getString(R.string.fingerprint_not_recognized);
        }
        if(msgId == FACE_ERROR_LOCKOUT) {
            mFaceLockout = true;
        }
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                if(msgId == FACE_ERROR_LOCKOUT || msgId == FACE_ERROR_CAMERA_UNAVAILABLE) {
                    cb.onFaceLockStrongAuthRequired();
                } else {
                    cb.onFaceLockError(msgId, errString);
                }
            }
        }
    }

    //UNISOC: fix for bug 980769
    public void stopFaceLockIfNeeded() {
        stopListeningForFaceLock();
    }

    public boolean isFaceLockRunning() {
        return mFaceLockRunningState == FACELOCK_STATE_RUNNING;
    }
    ...
    private static class SimData {
        ...
        static SimData fromIntent(Intent intent) {
            ...
            if (IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(stateExtra)) {
                ...
                final String lockedReason = intent
                        .getStringExtra(IccCardConstants.INTENT_KEY_LOCKED_REASON);
                Log.d(TAG, "fromIntent: lockedReason = " + lockedReason);
                if (IccCardConstants.INTENT_VALUE_LOCKED_ON_PIN.equals(lockedReason)) {
                    ...
                    /* @{ */
                    if(SimLockUtil.isSimlockLocked(lockedReason)){
                        state = SimLockUtil.getStateByReason(lockedReason);
                        Log.d(TAG, "Sim Lock state： " + state);
                    } else {
                        state = IccCardConstants.State.UNKNOWN;
                    }
                    /*
        ...
        public String toString() {
            return "SimData{state=" + simState + ",slotId=" + slotId
                    + ",subId=" + subId + ",isUnlock=" + isUnlock + "}";
        }
    }
    ...
    protected void handleStartedWakingUp() {
        ...
        Log.d(TAG, "FaceLock handleStartedWakingUp");
        mGoingToSleepDisableFaceLock = false;
        updateFingerprintListeningState();
        if(maybeCameraActivity()) {
            mHandler.removeMessages(MSG_UPDATE_FACELOCK_STATE);
            mHandler.sendEmptyMessageDelayed(MSG_UPDATE_FACELOCK_STATE, 300);
        } else {
            updateFaceLockListeningState();
        }
    ...
    private void stopScreenBrightnessIfNeeded() {
    ...
    private void resetDefaultScreenBrightness() {
        Log.d(TAG, "resetDefaultScreenBrightness:" + mDefaultBrightness);
        if(mDefaultBrightness == Settings.System.getIntForUser(mContext.getContentResolver(),
    ...
    private void setMaxScreenBrightness() {
        Log.d(TAG, "setMaxScreenBrightness");
        Settings.System.putIntForUser(mContext.getContentResolver(),
                Settings.System.SCREEN_BRIGHTNESS, 255, ActivityManager.getCurrentUser());
    }

    private boolean isFaceBrightnessEnabled() {
        return FACEID_BRIGHTNESS_FEATURE_ENABLED
                && Settings.System.getIntForUser(mContext.getContentResolver(),
                        FACEID_BRIGHTNESS, 0, ActivityManager.getCurrentUser()) == 1;
    }

    private void handleBrightnessLightScreen() {
        Log.d(TAG, "handleShowLightScreen");
        if(isFaceBrightnessEnabled()) {
            LightSensorManager.getInstance().start(mContext);
            handleCheckBrightnessLightScreen();
        }
    }

    private void handleCheckBrightnessLightScreen() {
        Log.d(TAG, "handleCheckBrightnessLightScreen");
    ...
    private boolean maybeCameraActivity() {
        String activity = getTopActivityClassName();
        return activity != null && activity.contains("Camera");
    }
    ...
    protected void handleStartedGoingToSleep(int arg1) {
        ...
        mGoingToSleep = true;
        mGoingToSleepDisableFaceLock = true;
        updateFingerprintListeningState();
        updateFaceLockListeningState();
    }
    ...
    protected KeyguardUpdateMonitor(Context context) {
        ...
        // UNISOC: modify for bug693456
        filter.addAction(ACTION_MODEM_CHANGE);
    ...
    private void updateFaceLockListeningState() {
        boolean shouldListenForFaceLock = shouldListenForFaceLock();
        Log.d(TAG, "updateFaceLockListeningState shouldListenForFaceLock:" + shouldListenForFaceLock
                + " running?" + (mFaceLockRunningState == FACELOCK_STATE_RUNNING) + " mFaceLockRunningState:" + mFaceLockRunningState);
        if (mFaceLockRunningState == FACELOCK_STATE_RUNNING && !shouldListenForFaceLock) {
            stopListeningForFaceLock();
            stopScreenBrightnessIfNeeded();
        } else if (mFaceLockRunningState != FACELOCK_STATE_RUNNING
                && shouldListenForFaceLock) {
            startListeningForFaceLock();
        }
    }

    //UNISOC: add for bug 951463
    public void onCameraGestureTriggered() {
        Log.d(TAG, "onCameraGestureTriggered mFaceLockRunningState:" + mFaceLockRunningState);
        if (mHandler != null && mHandler.hasMessages(MSG_UPDATE_FACELOCK_STATE)) {
            mHandler.removeMessages(MSG_UPDATE_FACELOCK_STATE);
        }
        if(mFaceLockRunningState == FACELOCK_STATE_RUNNING) {
            stopListeningForFaceLock();
        }
        setFaceLockRunningState(FACELOCK_STATE_CANCELLING);
    }

    //UNISOC: add for bug 945497
    public boolean shouldStrongAuthForCurrentUser() {
        int currentUser = ActivityManager.getCurrentUser();
        boolean trust = mTrustManager.isTrustUsuallyManaged(currentUser);
        int strongAuth = mStrongAuthTracker.getStrongAuthForUser(currentUser);
        if((strongAuth & LockPatternUtils.StrongAuthTracker.STRONG_AUTH_REQUIRED_AFTER_LOCKOUT) != 0) {
            return true;
        }
        return false;
    }

    private boolean shouldListenForFaceLock() {
        return (mKeyguardIsVisible || !mDeviceInteractive) && !mBouncer
                 && !mGoingToSleepDisableFaceLock && !mSwitchingUser && mBootCompleted && !mFaceLockout;
    }

    private boolean isFaceLockDisabled(int userId) {
        if(mFaceManager == null) return true;
        int enrolledFaces = mFaceManager.getEnrolledFaces(userId).size();
        Log.d(TAG, "mFaceManager.getEnrolledFaces(userId).size():" + enrolledFaces);

        boolean settingsEnable = Settings.System.getIntForUser(mContext.getContentResolver(),
                FACEID_FUNCTION, 0, ActivityManager.getCurrentUser()) == 1;
        final DevicePolicyManager dpm =
                (DevicePolicyManager) mContext.getSystemService(Context.DEVICE_POLICY_SERVICE);
        return mFaceManager != null && !mFaceManager.isHardwareDetected()
                || isSimPinSecure() || !mLockPatternUtils.isSecure(userId)
                || enrolledFaces <= 0 || !settingsEnable || !mStrongAuthTracker.hasUserAuthenticatedSinceBoot()
                || (dpm != null && (dpm.getKeyguardDisabledFeatures(null, userId)
                        & DevicePolicyManager.KEYGUARD_DISABLE_FACE) != 0)
                || mFingerWakeAndUnlocking || getUserCanSkipBouncer(userId);
    }

    private void startListeningForFaceLock() {
        if (mFaceLockRunningState == FACELOCK_STATE_CANCELLING) {
            return;
        }
        if (DEBUG) Log.v(TAG, "startListeningForFaceLock()");
        int userId = ActivityManager.getCurrentUser();
        if (isUnlockWithFaceLockPossible(userId)) {
            if (DEBUG) Log.v(TAG, "startListeningForFaceLock() start authenticate");
            if (mFaceLockCancelSignal != null) {
                mFaceLockCancelSignal.cancel();
            }
            mFaceLockCancelSignal = new CancellationSignal();
            mFaceManager.authenticate(null, mFaceLockCancelSignal, 0, mFaceLockAuthenticationCallback, WIDTH, HEIGHT, null, userId);
            handleBrightnessLightScreen();
            setFaceLockRunningState(FACELOCK_STATE_RUNNING);
        }
    }

    private void stopListeningForFaceLock() {
        if (DEBUG) Log.v(TAG, "stopListeningForFaceLock()");
        if (mFaceLockRunningState == FACELOCK_STATE_RUNNING) {
            mFaceLockCancelSignal.cancel();
            mFaceLockCancelSignal = null;
            setFaceLockRunningState(FACELOCK_STATE_STOPPED);
        }
        if (mFaceLockRunningState == FACELOCK_STATE_CANCELLING_RESTARTING) {
            setFaceLockRunningState(FACELOCK_STATE_CANCELLING);
    ...
    public boolean isUnlockWithFaceLockPossible(int userId) {
        return !isFaceLockDisabled(userId);
    }

    private void setFaceLockRunningState(int facelockRunningState) {
        boolean wasRunning = mFaceLockRunningState == FACELOCK_STATE_RUNNING;
        boolean isRunning = facelockRunningState == FACELOCK_STATE_RUNNING;
        mFaceLockRunningState = facelockRunningState;
        int userId = ActivityManager.getCurrentUser();
        if(isRunning) {
            mHandler.sendMessage(mHandler.obtainMessage(MSG_FACE_UNLOCK_STATE_CHANGED, 1, userId));
        } else {
            mHandler.sendMessage(mHandler.obtainMessage(MSG_FACE_UNLOCK_STATE_CHANGED, 0, userId));
        }
    }
    ...
    void handleSimStateChange(int subId, int slotId, State state) {
        ...
        /* UNISOC: add for bug900017 @{ */
        if (state == State.ABSENT) {
            if (mSimDatas != null) {
                for (SimData data : mSimDatas.values()) {
                    Log.d(TAG, "data " + data.toString());
                    if (data.slotId == slotId && (data.simState == State.PIN_REQUIRED
                            || data.simState == State.PUK_REQUIRED)) {
                        subId = data.subId;
                    }
                }
            }
    ...
    public void onKeyguardVisibilityChanged(boolean showing) {
        ...
        updateFingerprintListeningState();
        if (showing && mFaceLockRunningState == FACELOCK_STATE_CANCELLING) {
            setFaceLockRunningState(FACELOCK_STATE_STOPPED);
        }
    }
    ...
    private void sendUpdates(KeyguardUpdateMonitorCallback callback) {
        ...
        if (Resources.getSystem().getBoolean(com.android.internal.R.bool.config_subsidyLock)) {
            callback.onSubsidyDeviceLock(KeyguardSubsidyLockController.getInstance(mContext).getSubsidyLockMode());
            callback.onSubsidyEnterCode();
        }
    ...
    public void reportSimUnlocked(int subId) {
        ...
        // UNISOC: add for bug 907015
        refreshSimStateForUnlock(subId, slotId, true);
        handleSimStateChange(subId, slotId, State.READY);
    }

    /**
     * for now only SIM Ready or Absent will invokes this function
     * but when Absent we shouldn't call refreshSimStateForUnlock
     * UNISOC: add for bug 802572
     */
    public void reportSimUnlocked(int subId ,State state) {
        Log.d(TAG, "reportSimUnlocked(subId=" + subId + ",state=" + state + ")");
        if (state == null) {
            reportSimUnlocked(subId);
            return;
        }
        int slotId = SubscriptionManager.getSlotIndex(subId);
        handleSimStateChange(subId, slotId, State.READY);
    ...
    public boolean isSimPinOrPuk() {
        for (SubscriptionInfo info : getSubscriptionInfo(false /* forceReload */)) {
            IccCardConstants.State simState = getSimState(info.getSubscriptionId());
            if (simState == IccCardConstants.State.PIN_REQUIRED ||simState == IccCardConstants.State.PUK_REQUIRED ) return true;
        }
        return false;
    ...
    public boolean isSecure() {
        return mLockPatternUtils != null && mLockPatternUtils.isSecure(ActivityManager.getCurrentUser());
    }
    ...
    /* UNISOC: add for bug 907015 @{ */
    private void refreshSimStateForUnlock(int subId, int slotId, boolean mUnlock){
        if (!SubscriptionManager.isValidSubscriptionId(subId)) {
            Log.w(TAG, "invalid subId in handleSimStateChange()");
            return;
        }
        SimData data = mSimDatas.get(subId);
        if (data != null) {
            data.isUnlock = mUnlock;
            Log.d(TAG, "refreshSimStateForUnlock after " + data.toString());
        }
    }
    /* @} */
    ...
    public static boolean isSimPinSecure(IccCardConstants.State state) {
        ...
        return (simState == IccCardConstants.State.PIN_REQUIRED
                ...
                // Unisoc: Support SimLock
                || KeyguardSimLockMonitor.isSimLockShowing(state));
    }
    ...
    public void setFingerprintWakeAndUnlocking(boolean isFingerWakeAndUnlocking) {
        mFingerWakeAndUnlocking = isFingerWakeAndUnlocking;
    }
    ...
    private void handleSimlockUpdate() {
        Log.d(TAG, "handleSimlockUpdate");
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onRefreshCarrierInfo();
