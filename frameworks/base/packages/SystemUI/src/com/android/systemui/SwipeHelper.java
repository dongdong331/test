        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (!handleUpEvent(ev, mCurrView, velocity, getTranslation(mCurrView))) {
                    ...
                    if (Recents.ENABLE_TASK_LOCK) {
                        if (isDismissGesture(ev) && (getTranslation(mCurrView) < 0) && !mCallback.isLockedIcon(mCurrView)) {// slide to left
                            // flingadingy
                            dismissChild(mCurrView, velocity,
                                    !swipedFastEnough() /* useAccelerateInterpolator */);
                        } else {
                            if ((swipedFastEnough() || swipedFarEnough()) && (getTranslation(mCurrView) > 0)) {// slide to right
                                mCallback.onLockIcon(mCurrView);
                            }
                            // snappity
