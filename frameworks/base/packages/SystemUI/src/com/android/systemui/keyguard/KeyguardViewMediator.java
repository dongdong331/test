...
import com.android.keyguard.KeyguardSimLockMonitor;
...
public class KeyguardViewMediator extends SystemUI {
    ...
    private static final String SHOW_TRAFFIC_TIPS = "show_traffic_tips";
    ...
    /**
     * Key for isFaceLockEnable from settings.
     */
    public static final String FACEID_FUNCTION = "faceid_function";
    ...
    //Unisoc: SimLock Support
    private KeyguardSimLockMonitor mSimLockMonitor;
    ...
    private KeyguardSubsidyLockController mSubsidyController;
    private boolean mSubsidySecure = false;
    KeyguardUpdateMonitorCallback mUpdateCallback = new KeyguardUpdateMonitorCallback() {
        ...
        /* @} */
        @Override
        public void onFaceLockAuthenticated() {
            Log.d(TAG, "FaceLock onFaceLockAuthenticated");
            if (mUpdateMonitor != null && mUpdateMonitor.shouldStrongAuthForCurrentUser()) {
                mStatusBarKeyguardViewManager.makeExpandedInvisible();
                mStatusBarKeyguardViewManager.showPromptReason(KeyguardSecurityView.PROMPT_REASON_TIMEOUT);
            } else if (isSpecialCameraActivity()) {
                if (DEBUG) Log.d(TAG, "keyguardDoned delay for special..");
                EventLog.writeEvent(70000, 2);
                Message msg = mHandler.obtainMessage(KEYGUARD_DONE, 1);
        ...
        @Override
        public void onFaceLockStrongAuthRequired() {
            Log.d(TAG, "FaceLock onFaceLockStrongAuthRequired");
            if (mUpdateMonitor != null) {
                mStatusBarKeyguardViewManager.makeExpandedInvisible();
                if (mUpdateMonitor.isFaceLockout()) {
                    mStatusBarKeyguardViewManager.showPromptReason(KeyguardSecurityView.PROMPT_REASON_AFTER_FACE_LOCKOUT);
                } else {
                    mStatusBarKeyguardViewManager.showPromptReason(KeyguardSecurityView.PROMPT_REASON_AFTER_FACE_CAMERA_ERROR);
                }
            }
        }
        /* For SubsidyLock feature @{ */
        ...
        public void onSubsidyDeviceLock(int mode) {
            Log.d(TAG, "onSubsidyDeviceLock");
            int size = mKeyguardStateCallbacks.size();
            boolean oldSubsidySecure = mSubsidySecure;
            mSubsidySecure = mSubsidyController.isSubsidyLock();
            if (oldSubsidySecure != mSubsidySecure) {
                Log.d(TAG, "subsidySecure state changed to " + mSubsidySecure);
                for (int i = size - 1; i >= 0; i--) {
                    try {
                        mKeyguardStateCallbacks.get(i).onSubsidySecureStateChanged(mSubsidySecure);
                    } catch (RemoteException e) {
                        Slog.w(TAG, "Failed to call onSubsidySecureStateChanged", e);
                        if (e instanceof DeadObjectException) {
                            mKeyguardStateCallbacks.remove(i);
                        }
                    }
            ...
            synchronized (this) {
                if (!mShowing) {
                    if (DEBUG_SIM_STATES) Log.d(TAG,
                            "SubSidyLocked and keygaurd isn't "
                            + "showing; need to show keyguard so user can enter unlock");
                    doKeyguardLocked(null);
                } else {
                    resetStateLocked();
        ...
        public void onSubsidyEnterCode() {
            ...
            synchronized (this) {
                if (!mShowing) {
                    if (DEBUG_SIM_STATES) Log.d(TAG,
                            "SubSidyEnterCode and keygaurd isn't "
                            + "showing; need to show keyguard so user can enter unlock");
                    doKeyguardLocked(null);
                } else {
                    resetStateLocked();
    ...
    //for gms message camera preview activity
    private boolean isSpecialCameraActivity() {
        String activity = getTopActivityClassName();
        return activity != null && activity.contains("Camera") && activity.contains("messaging");
    }
    ...
    private void setupLocked() {
        ...
        //Unisoc: Support SimLock
        mSimLockMonitor = KeyguardSimLockMonitor.getInstance(mContext);
        //For SubsidyLock feature
        mSubsidyController = KeyguardSubsidyLockController.getInstance(mContext);
    ...
    /* UNISOC: carrier datausage dialog warning feature for 644278 @{ */
    protected void showPackageDialogIfNeeded(final Context context) {
        ...
        String stringTV = mContext.getString(R.string.dialog_messages_full);
        tv.setText(Html.fromHtml(stringTV));

        tv.setMovementMethod(LinkMovementMethod.getInstance());

        Dialog myAlertDialog = new AlertDialog.Builder(context)
                .setView(tv)
                .setCancelable(false)
                .setPositiveButton(R.string.dialog_messages_nomore,
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                Settings.Global.putInt(context.getContentResolver(),
                                        SHOW_TRAFFIC_TIPS, 0);
                                Log.d(TAG,
                                        "showNeedPackageDialog, positive button value:"
                        ...
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                Settings.Global.putInt(
                                        ...
                                        SHOW_TRAFFIC_TIPS, 1);
                                Log.d(TAG,
                                        "showNeedPackageDialog, negative button value:"
                                                + Settings.Global.getInt(
                                                context.getContentResolver(),
                                                SHOW_TRAFFIC_TIPS, 1));
                            }
                        }).create();
        myAlertDialog.getWindow().setType(
                WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
        myAlertDialog.getWindow().addFlags(
    ...
    private SpannableString getClickableSpan() {
        View.OnClickListener l = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Uri uri = Uri.parse(mContext.getString(R.string.charge_website_url));
        ...
        CharSequence cs = mContext.getString(R.string.dialog_messages_full);
        String str1 = mContext.getString(R.string.dialog_messages_part1);
        String strLink = mContext.getString(R.string.dialog_messages_link);
        SpannableString spanableInfo = new SpannableString(cs);
        int str1Len = str1.length();
        int linkLen = strLink.length();
        int end = str1Len + linkLen;
        int start = str1Len;
        if (start > end) {
            start = end;
        }
        spanableInfo.setSpan(new Clickable(l), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        spanableInfo.setSpan(new ForegroundColorSpan(Color.RED), start, end,
    ...
    private void doKeyguardLocked(Bundle options) {
        ...
        //SubsidyLock Keyguard unable to disable by other app
        if (!mExternallyEnabled  && !mSubsidyController.isSubsidyLock()) {
        ...
        if (!mustNotUnlockCurrentUser()
            ...
            final boolean lockedOrMissing = mUpdateMonitor.isSimPinSecure()
                    || ((absent || disabled) && requireSim) || mSubsidyController.isSubsidyLock();
    ...
    public boolean isSecure(int userId) {
        return mLockPatternUtils.isSecure(userId)
                ...
                || (mSystemReady ? KeyguardSubsidyLockController.getInstance(mContext).isSubsidyLock(): false);
    }
    ...
    private void handleKeyguardDone() {
        ...
        if(mUpdateMonitor != null) {
            mUpdateMonitor.stopFaceLockIfNeeded();
        }
        final int currentUser = KeyguardUpdateMonitor.getCurrentUser();
    ...
    private void handleStartKeyguardExitAnimation(long startTime, long fadeoutDuration) {
        ...
        synchronized (KeyguardViewMediator.this) {
            ...
            boolean isSimPinOrPuk = mUpdateMonitor.isSimPinOrPuk();
            Log.d(TAG, "isSimPinOrPuk =" + isSimPinOrPuk);
            if(isSimPinOrPuk){
                return;
            }
    ...
    public void onFingerWakeAndUnlocking() {
        Trace.beginSection("KeyguardViewMediator#onWakeAndUnlocking");
        mWakeAndUnlocking = true;
        KeyguardUpdateMonitor.getInstance(mContext).setFingerprintWakeAndUnlocking(true);
        keyguardDone();
        Trace.endSection();
    }
    ...
    public void onWakeAndUnlocking() {
        Trace.beginSection("KeyguardViewMediator#onWakeAndUnlocking");
        mWakeAndUnlocking = true;
        KeyguardUpdateMonitor.getInstance(mContext).setFingerprintWakeAndUnlocking(false);
        keyguardDone();
        Trace.endSection();
    }
    ...
    public void addStateMonitorCallback(IKeyguardStateCallback callback) {
        synchronized (this) {
            ...
            try {
                ...
                //For SubsidyLock feature
                callback.onSubsidySecureStateChanged(mSubsidyController.isSubsidyLock());
            } catch (RemoteException e) {
