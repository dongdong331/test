...
public class PipTouchHandler {
    ...
    private class PipMenuListener implements PipMenuActivityController.Listener {
        ...
        public void onPipDismiss() {
            //UNISOC: fix for bug 1003789
            Pair<ComponentName, Integer> topActivityInfo = PipUtils.getTopPinnedActivity(mContext, mActivityManager);
            if(topActivityInfo.first != null) {
                MetricsLoggerWrapper.logPictureInPictureDismissByTap(mContext, topActivityInfo);
            }
    ...
    void setMinimizedState(boolean isMinimized, boolean fromController) {
        ...
        if (mIsMinimized != isMinimized) {
            ...
            Pair<ComponentName, Integer> topActivityInfo = PipUtils.getTopPinnedActivity(mContext, mActivityManager);
            if(topActivityInfo.first != null) {
                MetricsLoggerWrapper.logPictureInPictureMinimize(mContext, isMinimized, topActivityInfo);
            }
        }
        ...
        public boolean onUp(PipTouchState touchState) {
            ...
            if (ENABLE_DISMISS_DRAG_TO_EDGE) {
                ...
                if (mMotionHelper.shouldDismissPip() || isFlingToBot) {
                    //UNISOC: fix for bug 920770
                    Pair<ComponentName, Integer> topActivityInfo = PipUtils.getTopPinnedActivity(mContext, mActivityManager);
                    if(topActivityInfo.first != null) {
                        MetricsLoggerWrapper.logPictureInPictureDismissByDrag(mContext, topActivityInfo);
                    }
