...
public class PowerNotificationWarnings implements PowerUI.WarningsUI {
    ...
    private SystemUIDialog mCriticalBatteryDialog;
    ...
    @Override
    public void showCriticalBatteryWarningDialog() {
        Slog.d(TAG, "show mCriticalBatteryDialog");
        if (mCriticalBatteryDialog != null) return;
        final SystemUIDialog d = new SystemUIDialog(mContext);
        d.setIconAttribute(android.R.attr.alertDialogIcon);
        d.setTitle(R.string.critical_battery_title);
        d.setMessage(R.string.critical_battery_dialog_message);
        d.setPositiveButton(com.android.internal.R.string.ok, null);
        d.setShowForAllUsers(true);
        d.setOnDismissListener(dialog -> mCriticalBatteryDialog = null);
        d.show();
        mCriticalBatteryDialog = d;
    }

    @Override
    public void dismissCriticalBatteryWarningDialog() {
        if (mCriticalBatteryDialog != null) {
            Slog.d(TAG, "dismiss mCriticalBatteryDialog");
            mCriticalBatteryDialog.dismiss();
            mCriticalBatteryDialog = null;
        }
    }
    ...
    private void setSaverMode(boolean mode, boolean needFirstTimeWarning) {
        ...
        if (PowerManagerEx.isPowerControllerEnabled()) {
            (new PowerManagerEx(mContext)).forcePowerSaveMode(mode);
            return;
        }
