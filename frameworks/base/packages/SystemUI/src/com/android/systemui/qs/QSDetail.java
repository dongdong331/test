...
public class QSDetail extends LinearLayout {
    ...
    public void handleShowingDetail(final DetailAdapter adapter, int x, int y,
        ...
        if (adapter != null) {
            ...
            /* SPRD bug 895499 click battery icon did not show more setting  in super power  @{ */
            if(StatusBar.SUPPORT_SUPER_POWER_SAVE && mQsPanel !=null
                && adapter.getTitle().equals(mContext.getString(
                    R.string.battery_detail_switch_title))){
                int mode = mQsPanel.getPowerSaveModeInternal();
                if (mode == PowerManagerEx.MODE_ULTRASAVING) {
                    mDetailSettingsButton.setVisibility(View.INVISIBLE);
                }
            }
            /* @} */
        } else {
            ...
            //UNISOC: fix for bug 969180
            if(IS_LOWRAM_DEVICE && mDetailAdapter != null && mDetailAdapter.getTitle().equals(mContext.getString(
                    R.string.battery_detail_switch_title))) {
        ...
        if(IS_LOWRAM_DEVICE && mDetailAdapter != null && mDetailAdapter.getTitle().equals(mContext.getString(
            ...
            if(visibleDiff) {
                ...
                if ((mStatusBar != null) && (mStatusBar.isUsingDarkTheme())) {
    ...
    /* SPRD: Bug 956912 response MoreSetting button should visable @{ */
    public void ChangeButtonVisible(boolean visable) {
        Log.d(TAG,"enter ChangeButtonVisible visable=" +visable);
        if (StatusBar.SUPPORT_SUPER_POWER_SAVE && mQsPanel !=null
            &&mDetailAdapter != null && mDetailAdapter.getTitle().equals(mContext.getString(
            R.string.battery_detail_switch_title))) {
            //int mode = mQsPanel.getPowerSaveModeInternal();
            //if (mode == PowerManagerEx.MODE_ULTRASAVING)
            if (!visable) {
                Log.d(TAG,"set MOreSetting btn invisble");
                mDetailSettingsButton.setVisibility(View.INVISIBLE);
            }else{
                Log.d(TAG,"set MoreSetting btn visible");
                mDetailSettingsButton.setVisibility(View.VISIBLE);
            }
        }
    }
    /* @} */
