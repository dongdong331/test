...
public class QSFragment extends Fragment implements QS, CommandQueue.Callbacks {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        mIntent = getContext().registerReceiver(mIntentReceiver, filter);
    ...
    /* SPRD: Bug 955459 battery saver issue @{ */
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive (Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
                ...
                boolean mIsChargeExitLow = false;
                QSTileHost mHost = mQSPanel.getHost();
                if (null != intent) {
                    mplugged = intent.getIntExtra(BatteryManager.EXTRA_PLUGGED, -1) == 0;
                }
                if (oldplugged == mplugged){
                    return;
                }
                if (DEBUG) Log.d(TAG,"onReceive ACTION_BATTERY_CHANGED usb status has changed new mplugged="+mplugged);
                if(mplugged){
                    Settings.Global.putInt(getContext().getContentResolver(),
                            "usb_plugin", 0);
                }else{
                ...
                try {
                    ...
                    if (DEBUG) Log.d(TAG, "mIsChargeExitLow= " + mIsChargeExitLow);
                } catch (RemoteException e) {
                   // Not much we can do here
                    if (DEBUG) Log.e(TAG,"getSmartSavingModeWhenCharging error");
                }

                if (mIsChargeExitLow) {
                     // to update QS power save tile
                    if( mHost != null ) {
                        for (QSTile tile : mHost.getTiles()) {
                            if (tile.getTileSpec().equals("battery")) {
                               if (DEBUG) Log.d(TAG,"update powersave tile");
                               tile.refreshState();
                               if ((tile instanceof SuperBatteryTile) && (mQSPanel.getPowerSaveModeInternal() == PowerManagerEx.MODE_SMART)) {
                                  ((SuperBatteryTile) tile).refreshDetailAdapter();
                               }
                               break;
                            }
                        }
                    ...
                    if (mQSPanel.isShowingCustomize()) {
                        if (DEBUG) Log.d(TAG,"to updateTiles");
                        mQSCustomizer.updateTiles();
                    }
                }
            } else if (StatusBar.SUPPORT_SUPER_POWER_SAVE  && action.equals(PowerManagerEx.ACTION_POWEREX_SAVE_MODE_CHANGED)) {
               if (mQSDetail.isShowingDetail()) {
                   if (DEBUG) Log.d(TAG,"to update QSDetail MoreSetting button");
                   if (mQSPanel.getPowerSaveModeInternal() != PowerManagerEx.MODE_ULTRASAVING) {
                        mQSDetail.ChangeButtonVisible(true);
                    } else {
                        mQSDetail.ChangeButtonVisible(false);
                    }
                }
            }
        }
