...
public class QSPanel extends LinearLayout implements Tunable, Callback, BrightnessMirrorListener {
    ...
    public void onTuningChanged(String key, String newValue) {
        if (QS_SHOW_BRIGHTNESS.equals(key)) {
            /*SPRD bug 885650:Super power feature*/
            if(StatusBar.SUPPORT_SUPER_POWER_SAVE){
                Log.d(VIEW_LOG_TAG, "onTuningChanged newValue="+newValue+",mMode="+mMode);
                mBrightnessView.setVisibility((newValue == null || Integer.parseInt(newValue) != 0) && mMode != PowerManagerEx.MODE_ULTRASAVING ? VISIBLE : GONE);
            }else{
    ...
    private void updateVisiable(int mode){
        Log.d(VIEW_LOG_TAG, "updateVisiable mode="+mode);
        View edit = findViewById(android.R.id.edit);
        if(edit != null){
            edit.setVisibility(mode == PowerManagerEx.MODE_ULTRASAVING ? INVISIBLE : VISIBLE);
        }
        onTuningChanged(QS_SHOW_BRIGHTNESS, mode == PowerManagerEx.MODE_ULTRASAVING ? "0" : "1");
    }
    /*@}*/
}
