...
public class TaskView extends FixedSizeFrameLayout implements Task.TaskCallbacks,
    ...
    public void onLockIcon() {
        SharedPreferences sharedPreference = PreferenceManager.getDefaultSharedPreferences(getContext().getApplicationContext());
        Editor editor = sharedPreference.edit();
        if (mTask.isTaskLocked == false) {
            mTask.isTaskLocked = true;
            editor.putInt(mTask.key.toStringKey(), mTask.key.id);
            editor.apply();
        } else {
            mTask.isTaskLocked = false;
            editor.remove(mTask.key.toStringKey());
            editor.apply();
        }
        mHeaderView.toggleDismissAndLockButton(mTask);
    }

    public boolean isLockedIcon() {
        return mTask.isTaskLocked;
    }
    /* @} */
}
