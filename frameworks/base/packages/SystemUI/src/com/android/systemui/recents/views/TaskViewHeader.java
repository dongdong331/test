...
public class TaskViewHeader extends FrameLayout
    ...
    public TaskViewHeader(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        /* SPRD: new feature of lock recent apps. @{ */
        mLightLockDrawable = context.getDrawable(R.drawable.recents_lock_light);
        mDarkLockDrawable = context.getDrawable(R.drawable.recents_lock_dark);
        /* @} */
    ...
    public void onTaskViewSizeChanged(int width, int height) {
        ...
        /* SPRD: new feature of lock recent apps. @{ */
        if (mTask != null) {
            toggleDismissAndLockButton(mTask);
        }
        /* @} */
        mDismissButton.setVisibility(showDismissIcon ? View.VISIBLE : View.INVISIBLE);
        mDismissButton.setAlpha(showDismissIcon ? 1.0f : .0f);
    ...
    /* SPRD: new feature of lock recent apps. @{ */
    public void toggleDismissAndLockButton(Task task) {
        if (task.isTaskLocked) {
            mDismissButton.setImageDrawable(task.useLightOnPrimaryColor ?
            mLightLockDrawable : mDarkLockDrawable);
        } else {
            mDismissButton.setImageDrawable(task.useLightOnPrimaryColor ?
            mLightDismissDrawable : mDarkDismissDrawable);
        }
        ((RippleDrawable) mDismissButton.getBackground()).setAlpha(task.isTaskLocked ? 0 : 255);
        mDismissButton.setClickable(task.isTaskLocked ? false : true);
    }
    /* @} */
    ...
    void startNoUserInteractionAnimation() {
        ...
        /* SPRD: new feature of lock recent apps. @{ */
        if(Recents.ENABLE_TASK_LOCK && mTask != null && mTask.isTaskLocked) {
            // do nothing
        } else {
