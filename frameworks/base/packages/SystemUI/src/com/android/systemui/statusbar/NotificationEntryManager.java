...
public class NotificationEntryManager implements Dumpable, NotificationInflater.InflationCallback,
    ...
    public void onAsyncInflationFinished(NotificationData.Entry entry) {
        ...
        if (isNew && !entry.row.isRemoved()) {
            //UNISOC: add for bug 961162
            updateScheduleRRForIncamingCall(entry);
            addEntry(entry);
    ...
    //UNISOC: add for bug 961162
    private void updateScheduleRRForIncamingCall(NotificationData.Entry entry) {
        if(entry != null && entry.notification != null && entry.notification.getNotification() != null) {
            String channel = entry.notification.getNotification().getChannel();
            if(channel != null && channel.equals("phone_incoming_call")) {
