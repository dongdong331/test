...
public class StatusBarMobileView extends FrameLayout implements DarkReceiver,
    ...
    private ImageView mMobile, mMobileType, mMobileRoaming, mMobileVolte, mMobileId;
    ...
    private void init() {
        ...
        mMobileId = findViewById(R.id.mobile_id);
        ...
        mMobileVolte = findViewById(R.id.mobile_volte_dual);
        initDotView();
    }
    ...
    private void initViewState() {
        ...
        if (KEEP_AOSP) {
            ...
            /* UNISOC: add for FEATURE bug886899 @{ */
            if (mState.mobileCardId > 0) {
                mMobileId.setImageResource(mState.mobileCardId);
            }
            /* @} */
        }
        if (mState.typeId > 0) {
        ...
        if (KEEP_AOSP) {
            ...
            if (mState.dataConnected) {
                mIn.setImageResource(getMobileDataInOutId(mState));
        ...
        if (mState.mMobileVolteId > 0) {
            mMobileVolte.setImageResource(mState.mMobileVolteId);
    ...
    private boolean updateState(MobileIconState state) {
        ...
        if (mState.visible != state.visible) {
            Log.d(TAG, "state.visible = " + state.visible + " mSubId = " + state.subId);
            mMobileGroup.clearAnimation();
        ...
        if (KEEP_AOSP) {
            ...
            if (mState.dataConnected != state.dataConnected || mState.activityOut != state.activityOut
                    || mState.activityIn != state.activityIn) {
                // UNISOC: bug 918354
                if (state.dataConnected) {
                    mIn.setImageResource(getMobileDataInOutId(state));
                    mIn.setVisibility(View.VISIBLE);
                } else {
                    mIn.setVisibility(View.GONE);
                }
            ...
            /* UNISOC: add for FEATURE bug886899 @{ */
            if (state.mobileCardId > 0) {
                Log.d(TAG, "mobileCardId = " + state.mobileCardId + " mSubId = " + state.subId);
                mMobileId.setImageResource(state.mobileCardId);
                mMobileId.setVisibility(View.VISIBLE);
            } else {
                Log.d(TAG, " mobileCardId gone mSubId = " + state.subId);
                mMobileId.setVisibility(View.GONE);
            }
            /* @} */
        }
        /* @} */
        if (state.mMobileVolteId > 0) {
            Log.d(TAG, "mMobileVolteId = " + state.mMobileVolteId + " mSubId = " + state.subId);
            mMobileVolte.setImageResource(state.mMobileVolteId);
            mMobileVolte.setVisibility(View.VISIBLE);
        } else {
            Log.d(TAG, " mMobileVolte gone mSubId = " + state.subId);
            mMobileVolte.setVisibility(View.GONE);
        }
        needsLayout |= state.roaming != mState.roaming
                || state.activityIn != mState.activityIn
                || state.activityOut != mState.activityOut
                || state.dataConnected != mState.dataConnected;
    ...
    public void onDarkChanged(Rect area, float darkIntensity, int tint) {
        ...
        mMobileId.setImageTintList(color);
        mMobileRoaming.setImageTintList(color);
        mMobileVolte.setImageTintList(color);
    ...
    public void setStaticDrawableColor(int color) {
        ...
        mMobileId.setImageTintList(list);
        mMobileRoaming.setImageTintList(list);
        mMobileVolte.setImageTintList(list);
    ...
    public void setVisibleState(int state) {
        Log.d(TAG, "mVisibleState = " + mVisibleState + " state = " + state);
    ...
    /* UNISOC: add feature statusbar signal cluster view. @{ */
    private int getMobileDataInOutId(MobileIconState state) {
        int mobileDataInOutId = 0;
        if (state.dataConnected) {
            if (state.activityIn && state.activityOut) {
                mobileDataInOutId = R.drawable.stat_sys_data_inout_ex;
            } else if (state.activityIn) {
                mobileDataInOutId = R.drawable.stat_sys_data_in_ex;
            } else if (state.activityOut) {
                mobileDataInOutId = R.drawable.stat_sys_data_out_ex;
            } else {
                mobileDataInOutId = R.drawable.stat_sys_data_default_ex;
            }
        }
