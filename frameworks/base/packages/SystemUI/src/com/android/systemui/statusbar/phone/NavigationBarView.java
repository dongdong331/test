...
public class NavigationBarView extends FrameLayout implements PluginListener<NavGesture> {
    ...
    private StatusBarManager mStatusBarManager = (StatusBarManager) mContext.getSystemService(android.app.Service.STATUS_BAR_SERVICE);
    /* @} */

    /* UNISOC: Modify for bug920072 {@ */
    ...
    private void updateIcons(Context ctx, Configuration oldConfig, Configuration newConfig) {
        ...
        if (oldConfig.densityDpi != newConfig.densityDpi
            ...
            /* SPRD: Bug 886422 new feature of dynamic navigationbar @{ */
            if(mSupportDynamicBar){
                /* UNISOC: Modify for bug939814 {@ */
                mHideIcon = getDrawable(lightContext, darkContext, R.drawable.ic_sysbar_hide);
                mPullDownIcon = getDrawable(lightContext, darkContext, R.drawable.ic_sysbar_pull_down);
                mPullUpIcon = getDrawable(lightContext, darkContext, R.drawable.ic_sysbar_pull_up);
                /* @} */
            }
            /* @} */
        ...
        if (mSupportDynamicBar && newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
            ...
            if (rotation == Surface.ROTATION_90) {
                ...
                mHideIcon = getDrawable(lightContext, darkContext, R.drawable.ic_sysbar_hide_land);
            } else if (rotation == Surface.ROTATION_270) {
                Log.d(TAG, "rotation=Surface.ROTATION_270");
                mHideIcon = getDrawable(lightContext, darkContext, R.drawable.ic_sysbar_hide_land_270);
            }
            updataNavigationBar();
        } else {
            mHideIcon = getDrawable(lightContext, darkContext, R.drawable.ic_sysbar_hide);
    ...
    private void orientBackButton(KeyButtonDrawable drawable) {
        ...
        /*UNISOC: Modified for bug960602 @{*/
        drawable.setRotation(useAltBack
                ? -90 : (TextUtils.getLayoutDirectionFromLocale(Locale.getDefault()) == View.LAYOUT_DIRECTION_RTL) ? 180 : 0);
        /*}@*/
    }

    private void orientHomeButton(KeyButtonDrawable drawable) {
        /*UNISOC: Modify for bug 940614 @{*/
        final boolean quickStepEnabled = mOverviewProxyService.shouldShowSwipeUpUI();
        if (quickStepEnabled) {
    ...
    public void updateNavButtonIcons() {
        ...
        if(mSupportDynamicBar) {
            getPullSwitchButton().setVisibility(mKeyguardManager.inKeyguardRestrictedInputMode() ? View.INVISIBLE : View.VISIBLE);///* UNISOC: Bug 950095 modify */
            /*UNISOC: Modify for bug 950532 @{*/
            if ((Settings.System.getInt(mContext.getContentResolver(), "navigationbar_config", 0) & 0x10) != 0) {
                getHideSwitchButton().setVisibility(mKeyguardManager.inKeyguardRestrictedInputMode() ? View.INVISIBLE : View.VISIBLE);
            }
            /*}@*/
        }
    }
    ...
    private void updateSlippery() {
        //UNISOC: fix for bug 993898
        setSlippery(!isQuickStepSwipeUpEnabled() || (mPanelView != null && mPanelView.isFullyExpanded()));
    }
    ...
    public void setAccessibilityButtonState(final boolean visible, final boolean longClickable) {
        ...
        /* UNISOC: Modify for bug920072 {@ */
        if (mStatusBar.SUPPORT_SUPER_POWER_SAVE && mBattery != null) {
            if (mBattery.getPowerSaveModeInternal() == PowerManagerEx.MODE_ULTRASAVING) {
                getAccessibilityButton().setVisibility(View.INVISIBLE);
            }
        }
        /* @} */
    }
    ...
    */
    ...
    private final OnClickListener mHideBarClickListener = new OnClickListener() {
        @Override
        public void onClick(View view) {
            boolean hide = Settings.System.putInt(mContext.getContentResolver(), "show_navigationbar", 0);
            android.util.Log.d(TAG,"hide navigationbar");
        }
    };
    ...
    public void updataNavigationBar(){
        ...
        if((lastNavLayoutSettings & 0x10) != 0){
    ...
    public void updateSwitchButton(int lastNavLayoutSettings){
        if((lastNavLayoutSettings & 0x0F) == NavigationBarInflaterView.NAVIGATION_RIGHT_NOTI
                || (lastNavLayoutSettings & 0x0F) == NavigationBarInflaterView.NAVIGATION_LEFT_NOTI ){
            getPullSwitchButton().setVisibility(View.VISIBLE);
            if(mStatusBar != null && !mStatusBar.isPresenterFullyCollapsed()){
    ...
    public boolean isInPowerSaveMode() {
        if (mBattery != null) {
            return mBattery.getPowerSaveModeInternal() == PowerManagerEx.MODE_ULTRASAVING;
        }
        return false;
    }
}
