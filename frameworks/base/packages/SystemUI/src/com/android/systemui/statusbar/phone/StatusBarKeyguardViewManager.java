...
public class StatusBarKeyguardViewManager implements RemoteInputController.Callback {
    ...
    private final KeyguardUpdateMonitorCallback mUpdateMonitorCallback =
        ...
        public void onEmergencyCallAction() {
            ...
            if (mOccluded) {
                ...
                mStatusBar.animateCollapseKeyguardQuickSettings();
            }
        }
    };
    ...
    protected void showBouncerOrKeyguard(boolean hideBouncerWhenShowing) {
        ...
        if (ActivityManager.getCurrentUser() == UserHandle.USER_OWNER
                && Settings.Secure.getInt(mContext.getContentResolver(), Settings.Secure.ASSISTANT_ON, 0) != 0
                && SystemUIFloatKey.getInstance(mContext).isFloatViewVisible()
                && !isNavBarVisible()) {
            mContext.sendBroadcastAsUser(new Intent(SystemUIFloatKey.ACTION_FLOATKEY_HIDE), UserHandle.CURRENT);
        }
        /* @} */
        updateStates();
    }
    ...
    public void showPromptReason(int reason) {
        mBouncer.showPromptReason(reason);
    }
    ...
    public boolean onBackPressed(boolean hideImmediately) {
        if (mBouncer.isShowing()) {
            /* For SubsidyLock feature @{ */
            if(KeyguardSubsidyLockController.getInstance(mContext).isSubsidyEnterCode()){
                Log.d(TAG, "subsidy lock etner code");
                if (mSp.getLong(UNLOCK_DEADLINE, 0) != 0) {
                    Log.d(TAG, "failed max time");
                    return true;
                }
                KeyguardSubsidyLockController.getInstance(mContext).resetSubSidyEnterCode();
                mBouncer.show(true);
            } else if (KeyguardSubsidyLockController.getInstance(mContext).getSubsidyLockMode() == KeyguardSubsidyLockController.SUBSIDY_LOCK_SCREEN_MODE_LOCK) {
                Log.d(TAG, "subsidy lock reset");
                KeyguardSubsidyLockController.getInstance(mContext).handleSubsidyDeviceLock(KeyguardSubsidyLockController.SUBSIDY_LOCK_SCREEN_MODE_LOCK);
                mBouncer.show(true);
            }
            /* @} */
