...
public class StatusBarWindowManager implements RemoteInputController.Callback, Dumpable {
    ...
    private void applyKeyguardFlags(State state) {
        ...
        if (keyguardOrAod && !state.backdropShowing && !scrimsOccludingWallpaper) {
            final boolean isEnableLockWallPaper = SystemProperties.getBoolean("ro.lockwallpaper.enable", true);
            if ((StatusBar.SUPPORT_SUPER_POWER_SAVE)&&(!isEnableLockWallPaper)){
                 mPowerManagerEx = IPowerManagerEx.Stub.asInterface(ServiceManager.getService("power_ex"));
                 if ((mStatusBar != null) && (mStatusBar.getCurPowerSaveMode() == PowerManagerEx.MODE_ULTRASAVING)) {
                      Log.d(TAG,"lockwallpaper disabled and not set FLAG_SHOW_WALLPAPER");
    ...
    private void applyUserActivityTimeout(State state) {
        if (state.isKeyguardShowingAndNotOccluded()
                ...
                && !state.qsExpanded) {
            //UNISOC: fix for face lock timeout
            mLpChanged.userActivityTimeout = isFaceLockEnable() ? KeyguardViewMediator.AWAKE_INTERVAL_DEFAULT_MS * 2
    ...
    private boolean isFaceLockEnable() {
        ...
        return faceManager != null
                ...
                && faceManager.getEnrolledFaces().size() > 0
                && Settings.System.getInt(mContext.getContentResolver(),
                        KeyguardViewMediator.FACEID_FUNCTION, 0) == 1;
    }
