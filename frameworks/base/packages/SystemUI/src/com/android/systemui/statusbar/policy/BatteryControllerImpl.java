...
public class BatteryControllerImpl extends BroadcastReceiver implements BatteryController {
    ...
    private void setPowerSave(boolean powerSave) {
        ...
        /*SPRD bug 885650:Don't update statusbar in SUPPORT_SUPER_POWER_SAVE*/
        if(StatusBar.SUPPORT_SUPER_POWER_SAVE){
            int mode = getPowerSaveModeInternal();
            Log.d(TAG, "setPowerSave mode="+mode);
            mPowerSave = powerSave && mode != PowerManagerEx.MODE_LOWPOWER
    ...
    private void initSuperPowerSave(){
        mPowerManagerEx =
                IPowerManagerEx.Stub.asInterface(ServiceManager.getService("power_ex"));
    }

    @Override
    public boolean setPowerSaveMode(int mode) {
        Log.d(TAG, "setSprdPowerSaveMode mode="+mode);
        if(mPowerManagerEx != null){
            try {
                return mPowerManagerEx.setPowerSaveMode(mode);
            } catch (RemoteException e) {
                // TODO: handle exception
                e.printStackTrace();
            }
        }else{
            Log.d(TAG, "setSprdPowerSaveMode mPowerManagerEx is null!");
        }
        return false;
    }
