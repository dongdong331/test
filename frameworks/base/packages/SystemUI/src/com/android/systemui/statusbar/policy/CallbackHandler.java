...
public class CallbackHandler extends Handler implements EmergencyListener, SignalCallback {
    ...
    /* UNISOC: Bug 697836 impl the new interface for volte. @{ */
    @Override
    public void setMobileVolteIndicators(final boolean show, final int subId,
        ...
        post(new Runnable() {
            ...
            public void run() {
                for (SignalCallback signalCluster : mSignalCallbacks) {
                     signalCluster.setMobileVolteIndicators(show, subId, resId);
                }
            }
        });
    }
    ...
    /* UNISOC: Bug 692599 impl the new interface for vowifi. @{ */
      @Override
    public void setMobileVoWifiIndicators(final IconState vowifiIcon) {
        post(new Runnable() {
            @Override
            public void run() {
                for (SignalCallback signalCluster : mSignalCallbacks) {
                    signalCluster.setMobileVoWifiIndicators(vowifiIcon);
                }
            }
        });
    }
    ...
    @Override
    public void setMobileHdVoiceIndicators(final IconState hdVoiceIcon) {
        post(new Runnable() {
            @Override
            public void run() {
                for (SignalCallback signalCluster : mSignalCallbacks) {
                    signalCluster.setMobileHdVoiceIndicators(hdVoiceIcon);
                }
            }
        });
    }
    ...
    @Override
    public void setMobileDataConnectedIndicators(final boolean show, final int subId) {
        post(new Runnable() {
            @Override
            public void run() {
                for (SignalCallback signalCluster : mSignalCallbacks) {
                    signalCluster.setMobileDataConnectedIndicators(show, subId);
                }
            }
        });
    }
