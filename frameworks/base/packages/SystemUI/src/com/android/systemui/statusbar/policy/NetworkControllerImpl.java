...
public class NetworkControllerImpl extends BroadcastReceiver
     ...
     * restore google by set the value true
     */
    public static final boolean KEEP_AOSP = false;
    ...
    private boolean mIsVoLteEnable = false;

    private boolean mVowifiConnected = false;
    private boolean mVowifiNotificationShown = false;
    private NotificationManager mNotificationManager;
    private boolean mShowEmergencyCallLimitedNotification = false;
    private static final int NOTIFICATION_FOR_WIFICALL_EMERGENCY = 1000;
    private boolean mHdVoiceConnected = false;
    ...
    NetworkControllerImpl(Context context, ConnectivityManager connectivityManager,
        ...
        mShowEmergencyCallLimitedNotification = mContext.getResources().getBoolean(
                R.bool.config_show_emergency_call_limited_notification_vowifi);
    }

    /* UNISOC: Bug 697836 add for volte icon. @{ */
    private synchronized void tryRegisterImsListener() {
        mIImsServiceEx = ImsManagerEx.getIImsServiceEx();
        if (mIImsServiceEx != null) {
            //UNISOC: bug 916375
            Log.d(TAG, "mIImsServiceEx != null mIsImsListenerRegistered = "
                    + mIsImsListenerRegistered);
            try {
            ...
            Log.d(TAG, "mIImsServiceEx == null");
        }
    }

    /* UNISOC: bug 916375 @{ */
    private synchronized void tryUnRegisterImsListener() {
        Log.d(TAG, "tryUnRegisterImsListener() mIsImsListenerRegistered = "
                + mIsImsListenerRegistered);
        mIImsServiceEx = ImsManagerEx.getIImsServiceEx();
        try {
            if (mIImsServiceEx != null) {
    ...
    private final IImsRegisterListener.Stub mImsUtListenerExBinder =
            new IImsRegisterListener.Stub() {
                    ...
                    public void imsRegisterStateChange(boolean isRegistered)
                        ...
                        Log.d(TAG, "--imsRegistered type: " + mImsType + " isRegistered = " +isRegistered);
                        for (int i = 0; i < mMobileSignalControllers.size(); i++) {
                            MobileSignalController mobileSignalController = mMobileSignalControllers.valueAt(i);
                            mobileSignalController.refreshImsState(isRegistered ,mCallbackHandler);
                        }
                        refreshVoWiFiIndicators(mVoWifiRegistered, mCallbackHandler);
                        if (mShowEmergencyCallLimitedNotification) {
                            updateNotification();
                        }
                    }
    };
    ...
    /* UNISOC: Bug 826505 add For VoWiFi icon refresh. @{ */
    private void refreshVoWiFiIndicators(boolean show, SignalCallback callback) {
        if (DEBUG) {
            Log.d(TAG, "refreshVoWiFiIndicators show = " + show);
        }
        mVowifiConnected = show;
        int vowifiRes = getVoWifiIcon();
        if (callback != null ) {
            callback.setMobileVoWifiIndicators(getVowifiIconState());
        }
    }
    /* @} */

    public IconState getVowifiIconState() {
        return new IconState(mVowifiConnected,
                getVoWifiIcon(), R.string.accessibility_vowifi_on, mContext);
    }

    public int getVoWifiIcon() {
        int wifiLev = getConnectedWifiLevel();
        Log.d(TAG, "wifiLev = " + wifiLev);
        switch (wifiLev) {
            case 0:
            case 1:
                return TelephonyIcons.ICON_STAT_SYS_VOWIFI_POOR;
            case 2:
                return TelephonyIcons.ICON_STAT_SYS_VOWIFI_FAIR;
            case 3:
            case 4:
                return TelephonyIcons.ICON_STAT_SYS_VOWIFI_GOOD;
            default:
                return 0;
        }
    }
    ...
    /*
     * UNISOC: Bug 673215 showing User notifications on
     * "Emergency Calls only available in Cellular Coverage"
     * at VoWiFi Service activation and in Lock Screen. @{
     */
    protected void updateNotification() {
        // TODO Auto-generated method stub
        int id = NOTIFICATION_FOR_WIFICALL_EMERGENCY;
        Log.d(TAG, "updateNotification mVowifiNotificationShown= " + mVowifiNotificationShown
                + " mIsWifiCallingActive = " + mVowifiConnected);
        if (!mVowifiNotificationShown && mVowifiConnected) {
            Notification.Builder builder = new Notification.Builder(mContext,
                    NotificationChannels.ALERTS)
                    // UNISOC: bug 691804
                    .setSmallIcon(R.drawable.wifi_call_notification)
                    .setWhen(0)
                    .setOngoing(true)
                    .setTicker(mContext.getString(R.string.emergency_only_notification_wificall))
            ...
            // UNISOC: add for bug846628
            builder.setStyle(new Notification.BigTextStyle().bigText(
                    mContext.getString(R.string.emergency_only_notification_wificall)));

            overrideNotificationAppName(mContext, builder, R.string.wifi_call_notification_app_name);
            Notification notification = builder.build();
            mVowifiNotificationShown = true;
            mNotificationManager.notifyAsUser(null, id, notification, UserHandle.ALL);
        } else if (!mVowifiConnected && mVowifiNotificationShown) {
    ...
    public static void overrideNotificationAppName(Context context, Notification.Builder n,
        ...
        extras.putString(Notification.EXTRA_SUBSTITUTE_APP_NAME, context.getString(stringId));
        n.addExtras(extras);
    }
    /* @} */

    public IconState getHdVoiceIconState() {
        return new IconState(mHdVoiceConnected,
                TelephonyIcons.ICON_STAT_SYS_HD_VOICE, R.string.accessibility_hd_voice_on, mContext);
    }
    ...
    private void registerListeners() {
        ...
        filter.addAction(TelephonyIntentsEx.ACTION_HIGH_DEF_AUDIO_SUPPORT);
        mContext.registerReceiver(this, filter, null, mReceiverHandler);
        mListening = true;
        Log.d(TAG, "tryRegisterImsListener");
    ...
    public void addCallback(SignalCallback cb) {
        ...
        cb.setMobileVoWifiIndicators(getVowifiIconState());
        cb.setMobileHdVoiceIndicators(getHdVoiceIconState());
        mWifiSignalController.notifyListeners(cb);
        mEthernetSignalController.notifyListeners(cb);
        for (int i = 0; i < mMobileSignalControllers.size(); i++) {
            MobileSignalController mobileSignalController = mMobileSignalControllers.valueAt(i);
            mobileSignalController.notifyListeners(cb);
            // UNISOC: Bug 697836 refresh volte icon when addcallback.
            mobileSignalController.refreshImsState(mIsVoLteEnable, cb);
        }
        ...
        switch (action) {
            ...
            case TelephonyIntents.ACTION_SIM_STATE_CHANGED:
                ...
                if (state.equals(IccCardConstants.INTENT_VALUE_ICC_LOADED)) {
                    Log.d(TAG,"ACTION_SIM_STATE_CHANGED LOADED");
                    tryRegisterImsListener();
                }
            ...
            case TelephonyIntents.ACTION_SERVICE_STATE_CHANGED:
                ...
                Log.d(TAG, "tryRegisterImsListener on receive servicestate change");
                tryRegisterImsListener();
                break;
            ...
            case ImsManager.ACTION_IMS_SERVICE_DOWN:
                /* UNISOC: bug 916375 @{ */
                Log.d(TAG, "on receive ims service change -- mIsImsListenerRegistered = "
                        + mIsImsListenerRegistered);
                if (mIsImsListenerRegistered) {
                    tryUnRegisterImsListener();
                }
                ...
                /* @} */
                break;
            case TelephonyIntentsEx.ACTION_HIGH_DEF_AUDIO_SUPPORT:
                boolean isHdVoiceSupport = intent.getBooleanExtra("isHdVoiceSupport", false);
                Log.d(TAG, "HIGH_DEF_AUDIO_SUPPORT = " + isHdVoiceSupport);
                mHdVoiceConnected = isHdVoiceSupport;
                mCallbackHandler.setMobileHdVoiceIndicators(getHdVoiceIconState());
                break;
            case WifiManager.RSSI_CHANGED_ACTION:
                mWifiSignalController.handleBroadcast(intent);
                Log.d(TAG, "WifiManager.RSSI_CHANGED_ACTION mVoWifiRegistered = " + mVoWifiRegistered);
                refreshVoWiFiIndicators(mVoWifiRegistered, mCallbackHandler);
                if (mShowEmergencyCallLimitedNotification) {
                    updateNotification();
                }
    ...
    private void notifyListeners() {
        mCallbackHandler.setIsAirplaneMode(new IconState(mAirplaneMode,
                TelephonyIcons.FLIGHT_MODE_ICON, R.string.accessibility_airplane_mode, mContext));
        mCallbackHandler.setMobileVoWifiIndicators(getVowifiIconState());
        mCallbackHandler.setMobileHdVoiceIndicators(getHdVoiceIconState());
        mCallbackHandler.setNoSims(mHasNoSubs, mSimDetected);
    }
    ...
    static class Config {
        ...
        // UNISOC: FEATURE_ALWAYS_SHOW_RAT_ICON - bug886582.
        boolean alwaysShowRAT = false;
        // UNISOC: bug 886918 always hide volte icon feature.
        boolean alwaysShowVolteIcon = true;
        ...
        static Config readConfig(Context context) {
            ...
            // UNISOC: FEATURE_ALWAYS_SHOW_RAT_ICON - bug886582.
            config.alwaysShowRAT = res.getBoolean(R.bool.config_alwaysShowRAT);
            config.alwaysShowVolteIcon = res.getBoolean(R.bool.config_show_volte_icon);
            ...
            /* UNISOC: FEATURE_ALWAYS_SHOW_RAT_ICON - bug886582 @{ */
            if (!KEEP_AOSP) {
                if (config.alwaysShowDataRatIcon || config.alwaysShowRAT) {
