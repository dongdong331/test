...
public class NotificationChildrenContainer extends ViewGroup {
   ...
   /* add for Bug#918565 */
    private boolean isReachMax = false;  //reach max call   
    private static final int MAX_NUMBER_OF_MISSED_CALL = 10;  
    private boolean isfirstimeToshow = true;
    ...
    public void addNotification(ExpandableNotificationRow row, int childIndex) {
        int newIndex = (childIndex < 0 || childIndex > mChildren.size()) ? mChildren.size() : childIndex;
    ...
    public void updateGroupOverflow() {
        ...
        /* add for Bug#918565 */
        if(childCount>0){
           ExpandableNotificationRow notifyrow = mChildren.get(0);
           NotificationData.Entry entry = notifyrow.getEntry();
           if ((entry != null) && (entry.channel != null)) {
               channelId = notifyrow.getEntry().channel.getId();
           }
           Log.d(TAG,"updateGroupOverflow childCount="+childCount+" maxAllowedVisibleChildren="+maxAllowedVisibleChildren);
           if ((channelId != null) && channelId.equals("phone_missed_call")){
               if(childCount==MAX_NUMBER_OF_MISSED_CALL){
                   Log.d(TAG,"updateGroupOverflow arrives max");
                   isReachMax = true;
               }else if(childCount==1){
                   Log.d(TAG,"updateGroupOverflow return to zero");
                   isReachMax = false;
               }
          }
        }
        ...
        if (childCount > maxAllowedVisibleChildren) {
            ...
            Log.d(TAG,"updateGroupOverflow chanelid="+ channelId);
            /* add for Bug#918565 */
            if((isReachMax) && (childCount>0) && (channelId != null) && (channelId.equals("phone_missed_call"))){
               if(!isfirstimeToshow){ 
                    Log.d(TAG,"updateGroupOverflow is not first time and not show");     
                    number = MAX_NUMBER_OF_MISSED_CALL-maxAllowedVisibleChildren;
               }else{
                    Log.d(TAG,"updateGroupOverflow set isfirstimeToshow false");
                    isfirstimeToshow = false;
               }
             }
            /* add for Bug#918565 */
            Log.d(TAG,"updateGroupOverflow to show");
            ...
            Log.d(TAG,"updateGroupOverflow removeOverflowNumber");
