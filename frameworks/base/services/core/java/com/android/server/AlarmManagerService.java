...
import static android.app.AlarmManagerWrapper.POWER_ON_WAKEUP;
import static android.app.AlarmManagerWrapper.POWER_OFF_ALARM;
...
class AlarmManagerService extends SystemService {
    ...
    static final boolean DEBUG_SPRD_ALARM = true;
    static final boolean DEBUG_VALIDATE = localLOGV || false;
    static final boolean DEBUG_ALARM_CLOCK = localLOGV || false;
    static final boolean DEBUG_LISTENER_CALLBACK = localLOGV || DEBUG_SPRD_ALARM;
    static final boolean DEBUG_WAKELOCK = localLOGV || false;
    static final boolean DEBUG_BG_LIMIT = localLOGV || DEBUG_SPRD_ALARM;
    static final boolean DEBUG_STANDBY = localLOGV || DEBUG_SPRD_ALARM;
    ...
    final class Batch {
        ...
        // SPRD: Regular PowerOnOff Feature. start>
        boolean removePowerOffOrPowerWakeupAlarm(final PendingIntent operation , final IAlarmListener listener){
             if (operation == null && listener == null) {
                 if (localLOGV) {
                     Slog.w(TAG, "requested removePowerOffOrPowerWakeupAlarm() of null operation",
            ...
            for (int i = 0; i < alarms.size(); ) {
                Alarm alarm = alarms.get(i);
                if ((alarm.type == POWER_OFF_WAKEUP || alarm.type == POWER_ON_WAKEUP) &&
        ...
        Alarm findExtraAlarm(final int type){
            checkExtraAlarm(type);
            final int N = alarms.size();
            for (int i = 0; i < N; i++) {
                Alarm a = alarms.get(i);
                if (a.type == type) {
        ...
        boolean hasExtraAlarm(final int type){
            return findExtraAlarm(type) != null;
        }
        // <end
    ...
    static long convertToElapsed(long when, int type) {
        final boolean isRtc = (type == RTC || type == RTC_WAKEUP
            || type == POWER_OFF_WAKEUP || type == POWER_ON_WAKEUP || type == POWER_OFF_ALARM);
        if (isRtc) {
    ...
    public void onStart() {
        ...
        mAlignHelper = AlarmAlignHelperFactory.getInstance(getContext());
        mAlignHelper.setAlarmManager(this);
    ...
    void setImpl(int type, long triggerAtTime, long windowLength, long interval,
        ...
        synchronized (mLock) {
            if (DEBUG_BATCH || DEBUG_SPRD_ALARM) {
    ...
    private boolean adjustDeliveryTimeBasedOnStandbyBucketLocked(Alarm alarm) {
        ...
        if (DEBUG_SPRD_ALARM) {
            if (oldWhenElapsed != alarm.whenElapsed || oldMaxWhenElapsed != alarm.maxWhenElapsed) {
                Slog.d(TAG, "adjustDeliveryTimeBasedOnStandbyBucketLocked oldWhenElapsed = " + oldWhenElapsed + " oldMaxWhenElapsed = " + oldMaxWhenElapsed + " changing to " + alarm.whenElapsed +" " + alarm.maxWhenElapsed);
            }
        }
    ...
    private void setImplLocked(Alarm a, boolean rebatching, boolean doValidate) {
        ...
        if (mAlignHelper.isEnabled()) {
        final long alignTime = mAlignHelper.checkAndAdjustAlarm(a.type, a.when,
                a.whenElapsed, a.maxWhenElapsed, a.operation, a.flags, a.tiggerTimeAdjusted);
        if(alignTime != a.whenElapsed && alignTime != 0) {
            a.whenElapsed = a.expectedWhenElapsed = alignTime;
            if(a.maxWhenElapsed > (a.whenElapsed + 5))
                a.maxWhenElapsed = a.expectedMaxWhenElapsed = a.whenElapsed + 5;
    ...
    private final IBinder mService = new IAlarmManager.Stub() {
         ...
         */
        @Override
        public void removeExtraAlarm(PendingIntent operation, IAlarmListener listener) {
            getContext().enforceCallingOrSelfPermission(
                    "android.permission.SET_TIME",
                    "removeExtraAlarm");
            if (operation == null && listener == null) {
            ...
            synchronized (mLock) {
                removeExtraAlarmLocked(operation, listener);
            }
        }
        // <end
     ...
     */
    private Alarm findFirstExtraAlarmLocked(final int type){
        checkExtraAlarm(type);
        final int N = mAlarmBatches.size();
        for (int i = 0; i < N; i++) {
            Batch b = mAlarmBatches.get(i);
            Alarm a = b.findExtraAlarm(type);
            if (a != null) {
                return a;
            }
        }
    ...
    void rescheduleKernelAlarmsLocked() {
        ...
        if (mAlarmBatches.size() > 0) {
            ...
            // SPRD: Regular PowerOnOff Feature. start>
            final Alarm firstPoweroffWakeup = findFirstExtraAlarmLocked(POWER_OFF_WAKEUP);
            final Alarm firstPoweronWakeup = findFirstExtraAlarmLocked(POWER_ON_WAKEUP);
            final Alarm firstPoweroffAlarm = findFirstExtraAlarmLocked(POWER_OFF_ALARM);

            if (firstWakeup != null
               && !firstWakeup.hasExtraAlarm(POWER_ON_WAKEUP) 
               && !firstWakeup.hasExtraAlarm(POWER_OFF_ALARM)) {
        ...
        while (mAlarmBatches.size() > 0) {
            ...
            if (batch.start > nowELAPSED) {
                ...
                if (DEBUG_SPRD_ALARM) {
                    ...
                    if (firstBatch == firstWakeup) {
                        Slog.d(TAG, "triggerAlarmsLocked firstBatch = firstWakeup : " + firstBatch + " mNextWakeup = " + mNextWakeup);
                    } else {
                        Slog.d(TAG, "triggerAlarmsLocked firstBatch : " + firstBatch + " firstWakeup : " + firstWakeup + " mNextWakeup = " + mNextWakeup);
                    }
                }
                break;
            }
    ...
    static class Alarm {
        ...
        public static String makeTag(PendingIntent pi, String tag, int type) {
            final String alarmString = type == ELAPSED_REALTIME_WAKEUP || type == RTC_WAKEUP
                 || type == RTC_WAKEUP || type == POWER_OFF_WAKEUP || type == POWER_OFF_ALARM
                    ? "*walarm*:" : "*alarm*:";
    ...
    private class AlarmThread extends Thread
    {
        ...
        public void run()
        {
            ...
            while (true)
            {
                ...
                // SPRD: Add Debug log. start>
                if (DEBUG_SPRD_ALARM) {
                    Slog.d(TAG, "waitForAlarm result:" + result + "currentTime=" + nowRTC + ", elapsedRealtime=" + nowELAPSED);
                }
                // <end
                synchronized (mLock) {
    ...
    class UninstallReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            synchronized (mLock) {
                ...
                String pkgList[] = null;
                Slog.d(TAG, "the broacast action is: " + action);
    ...
    class DeliveryTracker extends IAlarmCompleteListener.Stub implements PendingIntent.OnFinished {
        ...
        public void deliverLocked(Alarm alarm, long nowELAPSED, boolean allowWhileIdle) {
            ...
            if (alarm.type == ELAPSED_REALTIME_WAKEUP
                ...
                // NOTE: Bug #627645 low power Feature BEG -->
                // note wake up alarm of app is sending
                mAlignHelper.noteWakeupAlarm(alarm.uid, alarm.packageName);
               // <-- NOTE: Bug #627645 low power Feature END
            }
        }
    }
     ...
     * SPRD: Regular PowerOnOff Feature. start>
     */
    private boolean lookforExtraAlarmLocked(final int type){
        checkExtraAlarm(type);
        for (int i = 0; i < mAlarmBatches.size(); i++) {
            Batch b = mAlarmBatches.get(i);
            if (b.hasExtraAlarm(type)) {
                return true;
            }
        }
        return false;
    ...
    private void removeExtraAlarmLocked(PendingIntent operation, IAlarmListener listener) {
        final boolean hasPowerOnWakeup = lookforExtraAlarmLocked(POWER_ON_WAKEUP);
        final boolean hasPowerOffAlarm = lookforExtraAlarmLocked(POWER_OFF_ALARM);
        ...
        for (int i = mAlarmBatches.size() - 1; i >= 0; i--) {
            Batch b = mAlarmBatches.get(i);
            didRemove |= b.removePowerOffOrPowerWakeupAlarm(operation,listener);
            if (b.size() == 0) {
                mAlarmBatches.remove(i);
            }
        }
        ...
        if (didRemove) {
            if (DEBUG_BATCH) {
                Slog.v(TAG, "removeExtraAlarmLocked (operation) changed bounds; rebatching");
            }
            if (mNextWakeFromIdle != null && mNextWakeFromIdle.matches(operation, listener)) {
                mNextWakeFromIdle = null;
            }
            rebatchAllAlarmsLocked(true);
            updateNextAlarmClockLocked();
        ...
        if (hasPowerOnWakeup && !lookforExtraAlarmLocked(POWER_ON_WAKEUP)) {
            clear(mNativeData, POWER_ON_WAKEUP);
            mNextPoweronWakeup = 0;
        }

        if(hasPowerOffAlarm && !lookforExtraAlarmLocked(POWER_OFF_ALARM)){
            clear(mNativeData, POWER_OFF_ALARM);
            mNextPoweroffAlarm = 0;
        }
    }

    private void checkExtraAlarm(final int type){
        if (type < POWER_OFF_WAKEUP || type > POWER_OFF_ALARM) {
            throw new IllegalArgumentException("It's not a extra alarm type. type=" + type);
        }
    }
    // <end
}
