...
public class GestureLauncherService extends SystemService {
    ...
    private static final long CAMERA_POWER_DOUBLE_TAP_DELAY_TIME = 300;
    ...
    private boolean isDelayCamera = false;
    private int mUserId;
    ...
    public boolean interceptPowerKeyDown(KeyEvent event, boolean interactive,
        ...
        synchronized (this) {
            ...
            if (mCameraDoubleTapPowerEnabled
                ...
                mPowerButtonConsecutiveTaps++;
                    ...
                    && powerTapInterval > CAMERA_POWER_DOUBLE_TAP_MAX_TIME_MS
                    && isScreenOn && mPowerButtonConsecutiveTaps == 1) {
                isDelayCamera = true;
        ...
        if (launched) {
            Slog.i(TAG, "Power button double tap gesture detected, launching camera. Interval="
                    + powerTapInterval + "ms");
            mCameraLaunchRunnable.setCameraLaunchType(StatusBarManager.CAMERA_LAUNCH_SOURCE_POWER_DOUBLE_TAP);
            if(!isDelayCamera){
    ...
    boolean handleCameraGesture(boolean useWakelock, int source) {
        ...
        if (source == StatusBarManager.CAMERA_LAUNCH_SOURCE_POWER_DOUBLE_TAP) {
            mCameraLaunchRunnable.setCameraLaunchType(source);
            mHandler.postDelayed(mCameraLaunchRunnable, 100);
        } else {
    ...
    private class CameraLaunchRunnable implements Runnable {
        private int mSource;

        public void setCameraLaunchType(int source) {
            mSource = source;
        }
        ...
        public void run() {
            StatusBarManagerInternal service = LocalServices.getService(
                            StatusBarManagerInternal.class);
            service.onCameraLaunchGestureDetected(mSource);
        }
    }

    private final CameraLaunchRunnable mCameraLaunchRunnable = new CameraLaunchRunnable();
    ...
    private final class GestureEventListener implements SensorEventListener {
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if (event.sensor == mCameraLaunchSensor) {
                ...
                // SPRD: only launch camera when TAP_REAR, otherwise do nothing
                if (event.values[1] == TAP_REAR && handleCameraGesture(true /* useWakelock */,
