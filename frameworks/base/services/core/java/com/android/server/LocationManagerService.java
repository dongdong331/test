...
import com.android.server.power.sprdpower.PowerControllerHelper;
...
public class LocationManagerService extends ILocationManager.Stub {
    ...
    public static final boolean D = isDebug();
    ...
    private static final long SLOW_DISPATCH_THRESHOLD_MS = 1000;
    private static final long SLOW_DELIVERY_THRESHOLD_MS = 3000;
    ...
    // location is not available, no gnss provider and network provider
    private static final boolean LOCATION_DISABLED = SystemProperties.get("ro.location").equals("disabled");
    // location is available, no gnss provider but may has network provider
    private static final boolean GNSS_DISABLED = SystemProperties.get("ro.location.gnss").equals("disabled");
    // has gnss provider (device support gnss and is not no wcn version)
    private static final boolean SUPPORT_GNSS = !GNSS_DISABLED && !WCN_DISABLED;
    ...
    public LocationManagerService(Context context) {
        ...
        // NOTE: Bug #627645 low power Feature BEG-->
        PowerControllerHelper.getInstance(mContext).setLocationManagerService(this);
        // <-- NOTE: Bug #627645 low power Feature END
    ...
    public void systemRunning() {
        synchronized (mLock) {
            ...
            mLocationThread = new HandlerThread("locationHandler");
            mLocationThread.start();
            mLocationThread.getLooper().setSlowLogThresholdMs(SLOW_DISPATCH_THRESHOLD_MS,SLOW_DELIVERY_THRESHOLD_MS);
            mLocationHandler = new LocationWorkerHandler(mLocationThread.getLooper());
            ...
            AppOpsManager.OnOpChangedListener callback
                ...
                public void onOpChanged(int op, String packageName) {
                    synchronized (mLock) {
                        if (D) Log.d(TAG, "OnOpChangedListener");
            ...
            PackageManager.OnPermissionsChangedListener permissionListener
                ...
                public void onPermissionsChanged(final int uid) {
                    synchronized (mLock) {
                        if (D) Log.d(TAG, "onPermissionsChanged");
            ...
            for (Entry<String, ArrayList<UpdateRecord>> entry
                ...
                for (UpdateRecord record : entry.getValue()) {
                    if (record.mReceiver.mIdentity.mUid == uid
                            ...
                            Log.i(TAG, "request from uid " + uid + " is now "
    ...
    private void loadProvidersLocked() {
        ...
        if(D) {
            Log.d(TAG, "LOCATION_DISABLED: " + LOCATION_DISABLED
                    + ", SUPPORT_GNSS: " + SUPPORT_GNSS);
        }

        if (!LOCATION_DISABLED) {
            if (GnssLocationProvider.isSupported() && SUPPORT_GNSS) {
                ...
                Log.i(TAG, "gnss location provider not support !");
            }
            ...
            */
            ...
            Slog.w(TAG, "location disabled, gnss and network location provider not support !");
        }
    ...
    private final class Receiver implements IBinder.DeathRecipient, PendingIntent.OnFinished {
        ...
        public void binderDied() {
            Log.i(TAG, "Location listener died");
    ...
    private boolean hasGnssPermissions(String packageName) {
        ...
        Log.i(TAG, "hasGnssPermissions, PID: " + pid +
                ", UID: " + uid +
    ...
    public boolean addGnssBatchingCallback(IBatchedLocationCallback callback, String packageName) {
        Log.i(TAG,"addGnssBatchingCallback");
    ...
    public void removeGnssBatchingCallback() {
        Log.i(TAG,"removeGnssBatchingCallback");
    ...
    public void reportLocationBatch(List<Location> locations) {
        ...
        if (D) {
            Log.d(TAG,"reportLocationBatch locations.size " + locations.size());
        } else {
            Log.i(TAG,"reportLocationBatch");
        }
    ...
    boolean checkLocationAccess(int pid, int uid, String packageName, int allowedResolutionLevel) {
        ...
        // NOTE: Bug #627645 low power Feature BEG-->
        if (PowerControllerHelper.getInstance(mContext).inGpsConstraintAppList(packageName, uid)) {
            Slog.d(TAG, "App: " + packageName + "uid:" + uid + " is constraint gps, ignore its location request!!");
            return false;
        }
        // <-- NOTE: Bug #627645 low power Feature END
    ...
    private void updateProvidersLocked() {
        Log.i(TAG, "updateProvidersLocked");
        ...
        for (int i = mProviders.size() - 1; i >= 0; i--) {
            ...
            if (D){
                Log.d(TAG, "updateProvidersLocked provider " + name + " isEnabled " + isEnabled + " , shouldBeEnabled" + shouldBeEnabled);
            }
    ...
    private void applyRequirementsLocked(String provider) {
        ...
        Log.i(TAG, "applyRequirementsLocked to " + provider + " prvider.");
        ...
        Log.i(TAG, "provider request: " + provider + " " + providerRequest);
    ...
    private class UpdateRecord {
        ...
        UpdateRecord(String provider, LocationRequest request, Receiver receiver) {
            ...
            // NOTE: Bug #627645 low power Feature BEG-->
            PowerControllerHelper.getInstance(mContext).notifyLocationRequestListeners(
                                            mReceiver.mIdentity.mPackageName, mReceiver.mIdentity.mUid, 1);
            // <-- NOTE: Bug #627645 low power Feature END
        }
        ...
        void disposeLocked(boolean removeReceiver) {
            ...
            // NOTE: Bug #627645 low power Feature BEG-->
            PowerControllerHelper.getInstance(mContext).notifyLocationRequestListeners(
                                            mReceiver.mIdentity.mPackageName, mReceiver.mIdentity.mUid, 0);
            // <-- NOTE: Bug #627645 low power Feature END
    ...
    public void removeGeofence(Geofence geofence, PendingIntent intent, String packageName) {
        ...
        if (D) {
            Log.d(TAG, "removeGeofence: " + geofence + " , packageName " + packageName);
        } else {
    ...
    public void setLocationEnabledForUser(boolean enabled, int userId) {
        Log.i(TAG, "setLocationEnabledForUser enabled " + enabled + " , userId " + userId);
    ...
    public boolean setProviderEnabledForUser(String provider, boolean enabled, int userId) {
        if (D) {
            ...
            Log.d(TAG, "setProviderEnabledForUser provider " + provider + " , enabled " + enabled + " , userId " + userId);
        } else {
            Log.i(TAG, "setProviderEnabledForUser");
        }
        mContext.enforceCallingPermission(
                android.Manifest.permission.WRITE_SECURE_SETTINGS,
                "Requires WRITE_SECURE_SETTINGS permission");
    ...
    private void handleLocationChangedLocked(Location location, boolean passive) {
        Log.i(TAG, "incoming location: " + location);
    ...
    public String getFromLocation(double latitude, double longitude, int maxResults,
        ...
        if (D) {
            Log.d(TAG, "getFromLocation latitude " + latitude + " , longitude " + longitude + " , maxResults " + maxResults);
        } else {
    ...
    public String getFromLocationName(String locationName,
            ...
            GeocoderParams params, List<Address> addrs) {
        if (D) {
            Log.d(TAG, "getFromLocationName locationName " + locationName + " , lowerLeftLatitude " + lowerLeftLatitude + " , lowerLeftLongitude " + lowerLeftLongitude
                    + " , upperRightLatitude " + upperRightLatitude + " , upperRightLongitude " + upperRightLongitude + " , maxResults " + maxResults);
        } else {
