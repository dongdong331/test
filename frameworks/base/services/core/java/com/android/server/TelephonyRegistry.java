...
class TelephonyRegistry extends ITelephonyRegistry.Stub {
            ...
            if (validatePhoneId(phoneId)) {
                ...
                for (Record r : mRecords) {
                    ...
                    if (r.matchPhoneStateListenerEvent(PhoneStateListener.LISTEN_SERVICE_STATE) &&
                            idMatch(r.subId,r.phoneId, subId, phoneId)) {
                        try {
                            if (DBG) {
    ...
    /* UNISOC: Modified AOSP ServiceState notify wrong For Bug900323 @{ */
    boolean idMatch(int rSubId, int rPhoneId, int subId, int phoneId) {
        if (subId < 0) {
            // Invalid case, we need compare phoneId with default one.
