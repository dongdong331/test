...
final class ActivityRecord extends ConfigurationContainer implements AppWindowContainerListener {
    ...
    int callerPid;          // SPRD: add for protectArea feature
    ...
    boolean inReusedState;  // whether this record is in resued state, if so do not remove it while
                            // clean dead process data
    ...
    ActivityRecord(ActivityManagerService _service, ProcessRecord _caller, int _launchedFromPid,
        ...
        callerPid = _caller != null ? _caller.pid : -1;
    ...
    void reparent(TaskRecord newTask, int position, String reason) {
        ...
        // UNISOC: bug938670,Fix NPE caused by why WindowContainerController has not been
        // created before reparent task.
        if (mWindowContainerController == null) {
    ...
    void applyOptionsLocked() {
        if (pendingOptions != null
            ...
            android.app.ActivityOptionsHelper.setAnimationHelper(pendingOptions);//Bug 661753 add for animation
            ...
            // Clean up helper
            android.app.ActivityOptionsHelper.setAnimationHelper(null);//Bug 661753 add for animation
            if (task == null) {
    ...
    void setVisibility(boolean visible) {
        // UNISOC: bug813872, Fix the NPE to avoid Android restart.
        if (mWindowContainerController != null) {
    ...
    public void startFreezingScreenLocked(ProcessRecord app, int configChanges) {
        if (mayFreezeScreenLocked(app) && mWindowContainerController != null) {
    ...
    public void stopFreezingScreenLocked(boolean force) {
        if (force || frozenBeforeDestroy) {
            ...
            // SPRD:bug 805977, Fix the NPE when call stopFreezingScreenLocked.
            if (mWindowContainerController != null) {
                mWindowContainerController.stopFreezingScreen(force);
            } else {
                Slog.w(TAG, "stopFreezingScreenLocked: force:" + force
                    + " the mWindowContainerController of " + this
                    + " is null");
            }
        }
    }
    ...
    public void onWindowsDrawn(long timestamp) {
        ...
        synchronized (service) {
            mStackSupervisor.getActivityMetricsLogger().notifyWindowsDrawn(getWindowingMode(),
                    timestamp);
            if (displayStartTime != 0) {
                b.putLong(ProcessInfo.KEY_LAUNCH_TIME, timestamp - displayStartTime);
                reportLaunchTimeLocked(timestamp);
            }
            if (task != null) {
                b.putBoolean(ProcessInfo.KEY_FULLSCREEN, task.mFullscreen);
            } else {
                b.putBoolean(ProcessInfo.KEY_FULLSCREEN, false);
            }
            mStackSupervisor.mService.notifyActivityStateChange(this.intent, ProcessInfo.ACTIVITY_STATE_LAUNCHDONE, b);
    ...
    boolean ensureActivityConfiguration(int globalChanges, boolean preserveWindow,
        ...
        final ActivityStack stack = getStack();
        if (stack != null && stack.mConfigWillChange) {
    ...
    void relaunchActivityLocked(boolean andResume, boolean preserveWindow) {
        ...
        // SPRD: bug766534, skip relaunch activity which attached on the
        // guest process that is being switched out.
        if (app != null && !app.isBelongToCurrentUser(service.mUserController)) {
            Slog.i(TAG, "Skipping to schedule relaunch for " + this
                    + ", because it maybe leads the " + app
                    + " to crash for its user is locked.");
            return;
        }
