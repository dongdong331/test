...
class ActivityStack<T extends StackWindowController> extends ConfigurationContainer
    ...
    private static final int STOP_TIMEOUT = ActivityManager.isLowRamDeviceStatic() ? 15 * 1000 : 11 * 1000;

    // How long we wait until giving up on an activity telling us it has
    // finished destroying itself.
    private static final int DESTROY_TIMEOUT = ActivityManager.isLowRamDeviceStatic() ? 15 * 1000 : 11 * 1000;
            ...
            if (mPausingActivity == r) {
                ...
                if (r.isState(PAUSING)) {
                    ...
                    if (r.finishing) {
                        ...
                        // UNISOC: bug721481, Ensure the pause workflow resume key dispatching
                        // when pause failed.
                        Slog.i(TAG, "Pause failed but not finishing, we need to resume"
                                + " key dispatching for r:" + r);
    ...
    final void ensureActivitiesVisibleLocked(ActivityRecord starting, int configChanges,
        ...
        try {
            ...
            // UNISOC: Use temporary task history list to iterate, because some tasks will be
            // removed in some cases, and cause index out of bound.
            ArrayList<TaskRecord> tmpTaskHistory = new ArrayList<TaskRecord>(mTaskHistory);
            for (int taskNdx = tmpTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
                final TaskRecord task = tmpTaskHistory.get(taskNdx);
                final ArrayList<ActivityRecord> activities = task.mActivities;
                for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
                    final ActivityRecord r = activities.get(activityNdx);
                    if (r.finishing) {
    ...
    boolean checkKeyguardVisibility(ActivityRecord r, boolean shouldBeVisible, boolean isTop) {
        ...
        // UNISOC: bug810932, reduce the redundant canShowWhenLocked and hasDismissKeyguardWindows
        // function calling times.
        final boolean showWhenLocked = shouldBeVisible && (isTop || keyguardLocked)
                && r.canShowWhenLocked();
        final boolean dismissKeyguard = shouldBeVisible && r.hasDismissKeyguardWindows();
        if (shouldBeVisible) {
    ...
    final boolean finishActivityLocked(ActivityRecord r, int resultCode, Intent resultData,
        ...
        try {
            ...
            mService.notifyActivityStateChange(r.intent, ProcessInfo.ACTIVITY_STATE_FINISH, null);
            final ArrayList<ActivityRecord> activities = task.mActivities;
    ...
    void removeTimeoutsForActivityLocked(ActivityRecord r) {
        ...
        // SPRD: bug 868203, in order to avoid NPE in start activity flow, we need to keep
        // the finishing reused activity in stack when we found its process was dead during
        // the start activity flow.
        // We need to ensure that the finishing activity record will be removed after
        // its process was dead, So we keep this destory timeout message for it to delay
        // the remove from stack action.
        if (!(r.finishing && r.inReusedState)) {
    ...
    final boolean destroyActivityLocked(ActivityRecord r, boolean removeFromApp, String reason) {
        ...
        if (hadApp) {
            ...
            // UNISOC: bug 955600, Reused activity should not be removed from task.
            if (r.finishing && !r.inReusedState) {
        ...
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            ...
            while (!mTmpActivities.isEmpty()) {
                ...
                if (r.app == app) {
                    ...
                    // SPRD: bug832129, do not remove the reused activity record.
                    if (((!r.haveState && !r.stateNotNeeded) || r.finishing)
                            && !r.inReusedState) {
                        // Don't currently have state for the activity, or
                        // it is finishing -- always remove it.
                        if (!IS_USER_BUILD || DEBUG_ADD_REMOVE) {
                            Slog.i(TAG, "remove from history because of state is not"
                                    + " needed anymore or have no state: r:" + r);
                        }
                        remove = true;
                        ...
                        if (!r.inReusedState) {
                            remove = true;
                            if (!IS_USER_BUILD || DEBUG_ADD_REMOVE) {
                                Slog.i(TAG, "remove from history because of it have been"
                                        + " launched too many times since it was able to"
                                        + " run: r:" + r);
                            }
                        } else {
                            remove = false;
                            r.inReusedState = false;
                        }
                    } else {
                    ...
                    if (remove) {
                        ...
                        if (!r.finishing) {
                            ...
                            if (r.getState() == RESUMED) {
                                ...
                                mService.notifyActivityStateChange(r.intent, ProcessInfo.ACTIVITY_STATE_PAUSE, null);
                            }
                        }
                    } else {
    ...
    final boolean moveTaskToBackLocked(int taskId) {
        ...
        // UNISOC: bug959813, moveTaskToBackLocked resume top activity failed make ANR happened.
        boolean resumed = mStackSupervisor.resumeFocusedStackTopActivityLocked();
        if (!resumed) {
            Slog.i(TAG, "moveTaskToBackLocked resume top activity failed, try again.");
            mStackSupervisor.resumeFocusedStackTopActivityLocked();
        }
        return true;
    }
    ...
    void handleAppCrashLocked(ProcessRecord app) {
        ArrayList<TaskRecord> tmpTaskHistory = new ArrayList<>(mTaskHistory);
        for (int taskNdx = tmpTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            // SPRD: bug714863, use tmp data to workaround for OOB error.
            final ArrayList<ActivityRecord> tmpActivities = new ArrayList<>(
                    tmpTaskHistory.get(taskNdx).mActivities);
            for (int activityNdx = tmpActivities.size() - 1; activityNdx >= 0; --activityNdx) {
                final ActivityRecord r = tmpActivities.get(activityNdx);
                if (r.app == app) {
                    Slog.w(TAG, "  Force finishing activity "
                            + r.intent.getComponent().flattenToShortString());
    ...
    void removeTask(TaskRecord task, String reason, int mode) {
        ...
        if (removed) {
            EventLog.writeEvent(EventLogTags.AM_REMOVE_TASK, task.taskId, getStackId(), reason);
        }
