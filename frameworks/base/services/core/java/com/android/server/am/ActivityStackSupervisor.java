...
public class ActivityStackSupervisor extends ConfigurationContainer implements DisplayListener,
    ...
    // Unisoc, The dynamic resolution/density modify supports.
    static final boolean WITH_DYNAMIC_RESOLUTION = SystemProperties.getBoolean(
            "ro.sprd.superresolution", false);
    ...
    boolean attachApplicationLocked(ProcessRecord app) throws RemoteException {
        ...
        // SPRD:bug782972, ensure the top running activity of focused stack is resumed.
        if (mService.mBooted && mFocusedStack.mResumedActivity == null) {
    ...
    final boolean realStartActivityLocked(ActivityRecord r, ProcessRecord app,
        ...
        // UNISOC: bug741916, while user is switching, should skip activity start
        // for the stopped user. Same as resume workflow, we will just leave it
        // as is because someone should be bringing another user's activities to
        // the top of the stack.
        if (r != null && !mService.mUserController.hasStartedUserState(r.userId)) {
            Slog.w(TAG, "realStartActivityLocked: Skipping start of activity " + r
                    + ": user " + r.userId + " is stopped");
            return false;
        }
    ...
    boolean ensureVisibilityAndConfig(ActivityRecord starting, int displayId,
        ...
        // UNISOC: bug907847, Invalid stack id make system_server crash.
        if (displayId == INVALID_STACK_ID) {
            Slog.w(TAG, "ensureVisibilityAndConfig, display id is invalid, starting:"
                    + starting + ", Stack:" + this);
            return true;
        }
    ...
    void sendPowerHintForLaunchStartIfNeeded(boolean forceSend, ActivityRecord targetActivity) {
        ...
        if (sendHint && mService.mLocalPowerManager != null) {
            if (mService.enhancedPowerHintForLaunchEnabled()) {
                mService.sendPowerHintForLaunch(1);
            } else {
    ...
    void sendPowerHintForLaunchEndIfNeeded() {
        ...
        if (mPowerHintSent && mService.mLocalPowerManager != null) {
            if (mService.enhancedPowerHintForLaunchEnabled()) {
                mService.sendPowerHintForLaunch(0);
            } else {
    ...
    final ActivityRecord activityIdleInternalLocked(final IBinder token, boolean fromTimeout,
        ...
        for (int i = 0; i < NF; i++) {
            ...
            final ActivityStack stack = r.getStack();
            if (stack != null) {
                if (!stack.isHomeOrRecentsStack() || allResumedActivitiesVisible()) {
    ...
    boolean resumeFocusedStackTopActivityLocked(
        ...
        if (r == null || !r.isState(RESUMED)) {
            ...
            // UNISOC: bug 1018979, top resumed activity been stopped make anr happen.
    ...
    void notifyAppTransitionDone() {
        ...
        for (int i = mResizingTasksDuringAnimation.size() - 1; i >= 0; i--) {
            ...
            // UNISOC: bug985831, Check window mode before calling setTaskDockedResizing.
            if (task != null && !task.getStack().inSplitScreenWindowingMode()) {
                Slog.w(TAG, "notifyAppTransitionDone " + task + " is not in Split " +
                        "Screen Windowing Mode ");
                continue;
            }
    ...
    void cleanUpRemovedTaskLocked(TaskRecord tr, boolean killProcess, boolean removeFromRecents) {
        ...
        for (int i = 0; i < pmap.size(); i++) {
            ...
            for (int j = 0; j < uids.size(); j++) {
                ...
                if (proc.info != null && mService.keepImpAppAliveForce(proc.info.packageName)) {
                    return;
                }
                //modify for performance Bug #685274 end

                // UNISOC: Bug1010081, Do not kill the top activity host process.
                if (isProcessStartedForTopActivity(proc)) {
                    Slog.i(TAG, "cleanUpRemovedTaskLocked, " + proc + " is the top" +
                            " activity host process, do not kill.");
                    return;
                }
    ...
    void moveActivityToPinnedStackLocked(ActivityRecord r, Rect sourceHintBounds, float aspectRatio,
        ...
        mWindowManager.deferSurfaceLayout();
        Rect destBounds = null;
        PinnedActivityStack stack = null;
    ...
    void applySleepTokensLocked(boolean applyToStacks) {
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            ...
            // UNISOC: bug 936883, Fix the OOB of ActivityDisplay#getChildAt function.
            ArrayList<ActivityStack> stacks = display.getStacksCopy();
    ...
    private boolean putStacksToSleepLocked(boolean allowDelay, boolean shuttingDown) {
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            final ActivityDisplay display = mActivityDisplays.valueAt(displayNdx);
            // UNISOC: bug 950719,use copy data of stacks in display to avoid OOB error.
            final ArrayList<ActivityStack> stacks = display.getStacksCopy();
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; --stackNdx) {
                final ActivityStack stack = stacks.get(stackNdx);
                if (allowDelay) {
    ...
    boolean reportResumedActivityLocked(ActivityRecord r) {
        ...
        if (isFocusedStack(stack)) {
            ...
            mService.notifyActivityStateChange(r.intent, ProcessInfo.ACTIVITY_STATE_RESUME, null);
        }
    ...
    void ensureActivitiesVisibleLocked(ActivityRecord starting, int configChanges,
        ...
        try {
            ...
            for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
                final ActivityDisplay display = mActivityDisplays.valueAt(displayNdx);
                // UNISOC: bug904057, fix the OOB error in ensureActivitiesVisibleLocked.
                ArrayList<ActivityStack> stacks = display.getStacksCopy();
                for (int stackNdx = stacks.size() - 1; stackNdx >= 0; --stackNdx) {
                    final ActivityStack stack = stacks.get(stackNdx);
    ...
    private StackInfo getStackInfo(ActivityStack stack) {
        ...
        final ActivityDisplay display = mActivityDisplays.get(displayId);
        boolean isUserAMonkey = mService.isUserAMonkey();
    ...
    void handleNonResizableTaskIfNeeded(TaskRecord task, int preferredWindowingMode,
        ...
        final boolean inSplitScreenMode = actualStack != null && actualStack.getDisplay() != null
    ...
    boolean isProcessStartedForTopActivity(ProcessRecord app) {
        if (app == null) return false;

        if (!mService.mBooted) return false;

        if (mFocusedStack.mResumedActivity != null) return false;

        if (!mService.mUserController.hasStartedUserState(app.userId)) return false;

        final ActivityRecord r = mFocusedStack.topRunningActivityLocked();
        if (r != null && !r.isState(RESUMED) && r.app == null
                && app.uid == r.info.applicationInfo.uid
                && app.processName.equals(r.processName)) {
            return true;
        }
