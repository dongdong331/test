...
import static com.android.server.am.ActivityManagerService.DUMP_TRACE_MIN_SPACE_LIMIT;
import static com.android.server.am.ActivityManagerService.MY_PID;
import static com.android.server.am.ActivityManagerService.SYSTEM_DEBUGGABLE;

// [SDBG]: add debuggable check @{
...
class AppErrors {
    ...
    /**
    *The kernel CPU usage is high print ANR app kernel stack.app list.
    */
    private static final String[] APP_ANR_PRINT_KERNEL_STACKS = new String[] {
        "com.android.browser",
    ...
    final void appNotResponding(ProcessRecord app, ActivityRecord activity,
        ...
        if (controller != null) {
            try {
                ...
                int res = controller.appEarlyNotResponding(
        ...
        synchronized (mService) {
            ...
            //SPRD:bug960214,isInterestingToUserLocked() should be scheduled in locked.
            isInterestingToUser = app.isInterestingToUserLocked();
            ...
            if (!isSilentANR) {
                ...
                // SPRD: Adding launching provider stack trace
                for (ContentProviderRecord cpr : mService.mLaunchingProviders) {
                    int pid = -1;
                    if (cpr.proc != null) pid = cpr.proc.pid;

                    if (pid > 0 && !firstPids.contains(pid)) firstPids.add(pid);

                    if (cpr.launchingApp != null) {
                        pid = cpr.launchingApp.pid;
                    } else {
                        pid = -1;
                    }
        ...
        boolean preventDumpStackTrace = ActivityManagerService.shouldPreventDumpStackTraces(
                DUMP_TRACE_MIN_SPACE_LIMIT);
        // For background ANRs, don't pass the ProcessCpuTracker to
        // avoid spending 1/2 second collecting stats to rank lastPids.
        if (!preventDumpStackTrace) {
        tracesFile = ActivityManagerService.dumpStackTraces(
                true, firstPids,
        ...
        // [SDBG]: trigger sysrq @{
        if (Build.IS_DEBUGGABLE && isCpuUsageHighInKernel()) {
            for(int i = 0; i < APP_ANR_PRINT_KERNEL_STACKS.length; i++)
            {
                if(APP_ANR_PRINT_KERNEL_STACKS[i].equals(app.processName)){
                    writeToSysRqTrigger("l");
                    writeToSysRqTrigger("w");
                    break;
                }
            }
        }
        ...
        if (tracesFile == null && !preventDumpStackTrace) {
        ...
        if (controller != null) {
            try {
                ...
                int res = controller.appNotResponding(
    ...
    // [SDBG]: trigger sysrq @{
    public final void writeToSysRqTrigger(String str) {
        char[] buffer = str.toCharArray();
        FileWriter fw = null;

        try {
            fw = new FileWriter("/proc/sysrq-trigger", false);
            if (fw != null){
                fw.write(buffer);
            }
        } catch (IOException e) {
            Log.d(TAG, "write sysrqtrigger failed");
            e.printStackTrace();
        } finally {
            try {
                if(fw != null)
    ...
    private  boolean isCpuUsageHighInKernel(){
        int user,system,iowait,irq,softIrq,idle,total;
        synchronized (mService.mProcessCpuTracker) {
            user = mService.mProcessCpuTracker.getLastUserTime();
            system = mService.mProcessCpuTracker.getLastSystemTime();
            iowait = mService.mProcessCpuTracker.getLastIoWaitTime();
            irq = mService.mProcessCpuTracker.getLastIrqTime();
            softIrq = mService.mProcessCpuTracker.getLastSoftIrqTime();
            idle = mService.mProcessCpuTracker.getLastIdleTime();
        }

        total = user + system + iowait + irq + softIrq + idle;
        total = total == 0 ? 1 : total;
        long user_usage = 100 * user / total;
        long system_usage = 100 * system / total;
        StringBuffer buf=new StringBuffer();
        buf.append("isCpuUsageHighInKernel,");
        buf.append(",user=");
        buf.append(user);
        buf.append(",system=");
        buf.append(system);
        ...
        buf.append(",total=");
        buf.append(total);
        buf.append(",user_usage=");
        buf.append(user_usage);
        buf.append(",system_usage=");
        buf.append(system_usage);
        Slog.d(TAG,buf.toString());
        if (user_usage < system_usage && system_usage >=40 ){
            Slog.d(TAG,"isCpuUsageHighInKernel,true");
            return true;
        }
        return false;
    }
