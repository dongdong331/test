...
import com.android.internal.app.BroadcastTimeoutListener;
...
public final class BroadcastQueue {
    ...
    private static final boolean DEBUG_BROADCAST_SPRD = true;
    private static final boolean DEBUG_BROADCAST_SPRD_TORTUOUS = false;
    ...
    public void enqueueParallelBroadcastLocked(BroadcastRecord r) {
        ...
        //UNISOC: add for broadcast debug.
        if (DEBUG_BROADCAST_SPRD) {
            Slog.d(TAG, "Add broadcast <" + r + "> into ["
                    + "parallel | " + mQueueName + "], pending size "
                    + (mParallelBroadcasts.size() - 1));
        }
    }
    ...
    public void enqueueOrderedBroadcastLocked(BroadcastRecord r) {
        ...
        if (DEBUG_BROADCAST_SPRD) {
            Slog.d(TAG, "Add broadcast <" + r + "> into ["
                    + "ordered | " + mQueueName + "],"
                    + " pending size " + (mOrderedBroadcasts.size() - 1)
                    + "\nHeader is "
                    + mOrderedBroadcasts.get(0) + " now");
        }
    }
    ...
    private final void processCurBroadcastLocked(BroadcastRecord r,
        ...
        // SPRD: bug 776733, skip broadcast receiver of the guest process
        // while the guest is being switched out.
        if (!app.isBelongToCurrentUser(mService.mUserController)) {
            Slog.i(TAG, "Skipping deliver " + r + " to " + app
                    + " , if not process may be crash by handle"
                    + " it in the locked user state.");
            skipReceiverLocked(r);
            return;
        }
    ...
    final void processNextBroadcastLocked(boolean fromMsg, boolean skipOomAdj) {
        ...
        int pNum = mParallelBroadcasts.size();
        String parallelStr = mParallelBroadcasts.toString();
        ...
        if ((DEBUG_BROADCAST_LIGHT || DEBUG_BROADCAST_SPRD) && pNum != 0) {
            Slog.d(TAG_BROADCAST, "Done with parallel broadcast [" + mQueueName + "] "
                    + parallelStr);
        }
        ...
        do {
            ...
            if (r.receivers == null || r.nextReceiver >= numReceivers
                ...
                long dur = rightNow - r.dispatchTime;
                String receiversStr;
                if(DEBUG_BROADCAST_SPRD_TORTUOUS) {
                    receiversStr = String.valueOf(r.receivers);
                } else {
                    receiversStr = String.valueOf(numReceivers);
                }
                if (DEBUG_BROADCAST_LIGHT || DEBUG_BROADCAST_SPRD) {
                    Slog.d(TAG_BROADCAST,
                            "Finished with ordered broadcast " + r + " receivers:("
                                    + receiversStr + ") take " + dur + "ms in ["
                                    + mQueueName + "], remains "
        ...
        if (brOptions != null &&
            ...
            if (DEBUG_BROADCAST_SPRD) {
                Slog.d(TAG,"SdkVersion is not ok : (" + component +") skip");
            }
            skip = true;
        }
        ...
        if (!skip) {
            ...
            if (!isAvailable) {
                if (DEBUG_BROADCAST || DEBUG_BROADCAST_SPRD) Slog.d(TAG_BROADCAST,
    ...
    final void broadcastTimeoutLocked(boolean fromMsg) {
        ...
        // UNISOC: Only debug receiver registered in system_server
        if (r.receiver instanceof BroadcastTimeoutListener) {
            BroadcastTimeoutListener timeoutListener = (BroadcastTimeoutListener) r.receiver;
            timeoutListener.onBroadcastTimeout(r.intent);
        }
        r.receiverTime = now;
        if (!debugging) {
            r.anrCount++;
