...
public final class ProcessList {
    ...
    private void updateOomLevels(int displayWidth, int displayHeight, boolean write) {
        ...
        if (calcLmkMinfree(write)) {
            // if use custom minfree&adj, we also use custom extra_free_kbytes.
            // so we dont set extra_free_kbytes here.
            Slog.i(TAG, "lmk use custom minfree!");
            return;
        }
        // SPRD: optimazation for low memory @}
    ...
    // SPRD: optimazation for low memory @{
    private boolean calcLmkMinfree(boolean write) {
        boolean autocalc = SystemProperties.getBoolean("sys.lmk.autocalc", true);
        int[] tmpOomAdj = new int[mOomAdj.length];
        int[] tmpOomMinFree = new int[mOomAdj.length];

        if (!autocalc) {
            String adj = SystemProperties.get("sys.lmk.adj", "");
            String minfree = SystemProperties.get("sys.lmk.minfree", "");

            if (!TextUtils.isEmpty(adj) && !TextUtils.isEmpty(minfree)) {
                try {
                    String[] adjs = adj.split(",");
                    String[] minfrees = minfree.split(",");
                    int length = Math.min(adjs.length, mOomAdj.length);

                    for (int i = 0; i < length; ++i) {
                        tmpOomAdj[i] = Integer.parseInt(adjs[i]);
                        if (tmpOomAdj[i] >= UNKNOWN_ADJ || tmpOomAdj[i] < FOREGROUND_APP_ADJ) {
                            Slog.w(TAG, "lmk custom adj has error value(" + tmpOomAdj[i] + ").");
                            return false;
                        }
                        // Log.i(TAG, "adj: " + mOomAdj[i]);
                        tmpOomMinFree[i] = Integer.parseInt(minfrees[i]) * PAGE_SIZE / 1024;
                    }
                    for (int i = 0; i < mOomAdj.length ; ++i) {
                        if (i < length) {
                            mOomAdj[i] = tmpOomAdj[i];
                            mOomMinFree[i] = tmpOomMinFree[i];
                        } else {
                            mOomAdj[i] = tmpOomAdj[length - 1];
                            mOomMinFree[i] = tmpOomMinFree[length - 1];
                        }
                    }
                    ...
                    Slog.w(TAG, "calc lmk minfree fail.");
                }
            }
        }
        return false;
