 ...
 */
final class ProcessRecord extends BaseProcessRecord {
    ...
    boolean persisForegroundActivities; // Running any activities that are foreground and persistent?
    boolean persisRepForegroundActivities; // Last reported foreground and persistent activities.
    ...
    void dump(PrintWriter pw, String prefix) {
        ...
        if (hasClientActivities || foregroundActivities || repForegroundActivities
                || persisForegroundActivities || persisRepForegroundActivities) {
            pw.print(prefix); pw.print("hasClientActivities="); pw.print(hasClientActivities);
                    ...
                    pw.print(" persisForegroundActivities="); pw.print(persisForegroundActivities);
                    pw.print(" (rep="); pw.print(persisRepForegroundActivities); pw.println(")");
        }
    ...
    /**
     * Check whether the process instance is belong to the current user,
     * if not we think that it is not ok to schedule component which is
     * attached on it to execute.If we do schedule component execute in
     * this case the process may crash by it.In general, use this check
     * to avoid the application crash in the progress of switch user.
     *
     * And now this check is just limit the guest process schedule component
     * execute when current user is not the guest.
     *
     * @param userController {@link UserController}
     * @return if the process may be cash by schedule component execute will
     *         return false, otherwise will return true.
     */
    boolean isBelongToCurrentUser(UserController userController) {
        UserInfo userInfo = userController.getUserInfo(userId);
        if (userInfo != null && userInfo.isGuest()
                && userController.getCurrentUserIdLU() != userId) {
            return false;
        }
        return true;
    }
