...
public class AudioService extends IAudioService.Stub
    ...
    // SPRD bug 604729 when alarm ring, press volume button, alarm will not off or pause
    private static final String ALARM_VOLUME_BEHAVIOR = "com.sprd.deskclock.volume_behavior";
    private boolean mIsAlarmFired = false;
    private boolean mIsSoundRecorderPlayInEARPIECE = false;
   ...
   protected static final int DEFAULT_VOL_STREAM_NO_PLAYBACK = AudioSystem.STREAM_MUSIC;
    ...
    public static final String CONNECT_INTENT_KEY_DEVICE = "device";
    ...
    public AudioService(Context context) {
        ...
        /* SPRD bug 604729 when alarm ring, press volume button, alarm will not off or pause @{ */
        //TODO it may isn't the optimal solution.
    ...
    public void onSystemReady() {
        ...
        // indicate to audio HAL that system boot complete,HAL can quit headset check thread.bug 934688
        Log.w(TAG, "Send boot_complete=true to audio HAL.");
        AudioSystem.setParameters("boot_complete=true");
    }
    ...
    private void readPersistedSettings() {
        ...
        if (!isValidRingerMode(ringerMode)) {
            ...
            /** SPRD:bug474612 support outdoor mode. @{ */
    ...
    private boolean volumeAdjustmentAllowedByDnd(int streamTypeAlias, int flags) {
        //sprd seprate ringtone and notification stream.
        //cannot adjust STREAM_NOTIFICATION volume in vibrate/silent mode.
        if ((mRingerMode == AudioManager.RINGER_MODE_VIBRATE
                || mRingerMode == AudioManager.RINGER_MODE_SILENT)
                && streamTypeAlias == AudioSystem.STREAM_NOTIFICATION) {
            Log.w(TAG, "volumeAdjustmentAllowedByDnd cannot adjust STREAM_NOTIFICATION volume in vibrate/silent mode.");
            return false;
        }
    ...
    protected void sendVolumeUpdate(int streamType, int oldIndex, int index, int flags) {
        ...
        try {//fix bug 863426.set system properties fail cause system reboot.
            /* SPRD: use property to tell the bootanimation @{ */
            ...
            Log.e(TAG, "SystemProperties.set RuntimeException", e);
        }
    ...
    private void setRingerMode(int ringerMode, String caller, boolean external) {
        ...
        Log.i(TAG, "setRingerMode  ringerMode= " + ringerMode + " external= "+external+"  caller= "+caller);
        if ((ringerMode == AudioManager.RINGER_MODE_VIBRATE) && !mHasVibrator) {
            ringerMode = AudioManager.RINGER_MODE_SILENT;
        }
        final long identity = Binder.clearCallingIdentity();
    ...
    private void setRingerModeExt(int ringerMode) {
        synchronized(mSettingsLock) {
            Log.i(TAG, "setRingerModeExt  ringerMode= " + ringerMode + " mRingerModeExternal= "+mRingerModeExternal);
    ...
    private void muteRingerModeStreams() {
        ...
        for (int streamType = numStreamTypes - 1; streamType >= 0; streamType--) {
            ...
            //sprd seprate ringtone and notification stream.
            //mute/unmute STREAM_NOTIFICATION always.
            if (streamType != AudioSystem.STREAM_NOTIFICATION && isMuted == shouldMute) continue;
            if (!shouldMute) {
                // unmute
    ...
    private void setRingerModeInt(int ringerMode, boolean persist) {
        ...
        synchronized(mSettingsLock) {
            Log.i(TAG, "setRingerModeInt  ringerMode= " + ringerMode + " mRingerMode= "+mRingerMode+" persist= "+persist);
    ...
    public boolean isSpeakerphoneOn() {
        Log.d(TAG, "isSpeakerphoneOn  mForcedUseForCommExt= "+mForcedUseForCommExt+
			" mForcedUseForComm= "+mForcedUseForComm);
    ...
    public void setBluetoothScoOnInt(boolean on, String eventSource) {
        if (DEBUG_SCO) {
            Log.d(TAG, "setBluetoothScoOnInt()  on= "+on + ", eventSource= " + eventSource);
        }
        if (on) {
    ...
    public boolean isBluetoothScoOn() {
        if (DEBUG_SCO) {
            Log.d(TAG, "isBluetoothScoOn  mForcedUseForComm= "+mForcedUseForComm);
        }
    ...
    private class ScoClient implements IBinder.DeathRecipient {
        ...
        ScoClient(IBinder cb) {
            ...
            if (DEBUG_SCO) {
                Log.d(TAG, "ScoClient()  mCreatorPid= "+mCreatorPid);
            }
            mStartcount = 0;
        }
        ...
        public void incCount(int scoAudioMode) {
            synchronized(mScoClients) {
                ...
                mStartcount++;
                if (DEBUG_SCO) {
                    Log.d(TAG, "incCount()  mStartcount= "+mStartcount);
                }
            }
        }
        ...
        public void decCount() {
            synchronized(mScoClients) {
                if (mStartcount == 0) {
                    ...
                    if (DEBUG_SCO) {
                        Log.d(TAG, "decCount()  mStartcount= "+mStartcount);
                    }
        ...
        public void clearCount(boolean stopSco) {
            synchronized(mScoClients) {
                ...
                mStartcount = 0;
                if (DEBUG_SCO) {
                    Log.d(TAG, "clearCount()  stopSco= "+stopSco);
                }
                if (stopSco) {
                    requestScoState(BluetoothHeadset.STATE_AUDIO_DISCONNECTED, 0);
                }
        ...
        public int getCount() {
            if (DEBUG_SCO) {
                Log.d(TAG, "getCount()  mStartcount= "+mStartcount);
            }
            return mStartcount;
        }
        ...
        public int getPid() {
            if (DEBUG_SCO) {
                Log.d(TAG, "getPid()  mCreatorPid= "+mCreatorPid);
            }
            return mCreatorPid;
        }
    ...
    public void clearAllScoClients(int exceptPid, boolean stopSco) {
        if (DEBUG_SCO) {
            Log.d(TAG, "clearAllScoClients() exceptPid: "+exceptPid+"  stopSco= "+stopSco);
        }
        synchronized(mScoClients) {
            ...
            synchronized (mA2dpAvrcpLock) {
                ...
                if (toRemove != null) {
                    ...
                    if(AudioSystem.isStreamActive(AudioSystem.STREAM_MUSIC, 0) ){
                        Log.e(TAG, "disconnectA2dp,music active,makeA2dpDeviceUnavailableLater delay "+delay);
                        for (int i = 0; i < toRemove.size(); i++) {
                            makeA2dpDeviceUnavailableLater(toRemove.valueAt(i), delay);
                        }
                    }else {
                        Log.e(TAG, "disconnectA2dp,music not active,makeA2dpDeviceUnavailable right now.");
                        for (int i = 0; i < toRemove.size(); i++) {
        ...
        synchronized (mSafeMediaVolumeState) {
            ...
            if ((mMcc != mcc) || ((mMcc == 0) && force)) {
                ...
                if (safeMediaVolumeEnabled && !safeMediaVolumeBypass) {
                    ...
                    // it to "active".
                    if (mSafeMediaVolumeState != SAFE_MEDIA_VOLUME_INACTIVE) {
                        /** SPRD: bug734790, SafetyWarningDialog doesn't show after phone is reboot. @{ */
                        /*
                        if (mMusicActiveMs == 0) {
    ...
    private int checkForRingerModeChange(int oldIndex, int direction, int step, boolean isMuted,
        ...
        case RINGER_MODE_OUTDOOR:
            ...
            /* SPRD:bug618373 Cann't adjust volume with volume up key. @{ */
            if (oldIndex == MAX_STREAM_VOLUME[AudioSystem.STREAM_RING] * 10) {
    ...
    private void broadcastRingerMode(String action, int ringerMode) {
        ...
        broadcast.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT
                | Intent.FLAG_RECEIVER_REPLACE_PENDING | Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
        ...
        synchronized (mConnectedDevices) {
            if (profile == BluetoothProfile.A2DP && !suppressNoisyIntent) {
                ...
                delay = checkSendBecomingNoisyIntent(AudioSystem.DEVICE_OUT_BLUETOOTH_A2DP,
                        intState, musicDevice);
                if (intState == 0 && !AudioSystem.isStreamActive(AudioSystem.STREAM_MUSIC, 0)) {
                    delay = 0;
                }
            } else {
                delay = 0;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_REPORT_NEW_ROUTES: {
                    Log.d(TAG, "case MSG_REPORT_NEW_ROUTES");
    ...
    private boolean handleDeviceConnection(boolean connect, int device, String address,
        ...
        synchronized (mConnectedDevices) {
			...
			AudioSystem.getDeviceConnectionState(device,address) != AudioSystem.DEVICE_STATE_AVAILABLE) {
                    Slog.e(TAG, "not connecting device 0x" + Integer.toHexString(device) +
                            " due to command error " + res +" and not connected in lower layer");
                    return false;
                }
    ...
    private void sendDeviceConnectionIntent(int device, int state, String address,
        ...
        intent.putExtra(CONNECT_INTENT_KEY_DEVICE, device);
    ...
    private class AudioServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_DOCK_EVENT)) {
                ...
                //Some music app keep playing state during reboot,but don't feed audio data to hardware.It will generate some noisy sound.
                //So,we mute music stream while system shutdown.
                Log.e(TAG, "mute music stream while system shutdown.");
    ...
    public void setBluetoothA2dpOnInt(boolean on, String eventSource) {
        synchronized (mBluetoothA2dpEnabledLock) {
            mBluetoothA2dpEnabled = on;
            Log.d(TAG, "setBluetoothA2dpOnInt  on=" +on+" eventSource="+eventSource);
    ...
    private void setForceUseInt_SyncDevices(int usage, int config, String eventSource) {
        Log.d(TAG, "setForceUseInt_SyncDevices  usage= "+usage+"  config= "+config+"  eventSource="+eventSource);
    ...
    private int safeMediaVolumeIndex(int device) {
       Log.d(TAG, "safeMediaVolumeIndex  device= "+device);
       if ((device & mSafeMediaVolumeDevices) == 0) {
    ...
    public void notifyVolumeControllerVisible(final IVolumeController controller, boolean visible) {
        ...
        if (!visible && mIsSoundRecorderPlayInEARPIECE) {
            Intent i = new Intent("com.sprd.volumebar.dismiss");
            i.addFlags(Intent.FLAG_RECEIVER_FOREGROUND|Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
            mContext.sendBroadcast(i);
            Log.d(TAG, "soundRecorder play in earpiece send broadcast ---> com.sprd.volumebar.dismiss" );
        }
    ...
    public boolean getSafeMediaVolumeEnabled(String caller){
            synchronized (mSafeMediaVolumeState) {
                Log.e(TAG, "getSafeMediaVolumeEnabled  mSafeMediaVolumeState= "+mSafeMediaVolumeState);
                return (mSafeMediaVolumeState == SAFE_MEDIA_VOLUME_ACTIVE);
            }
    }

    private class ForceUseDeathHandler implements IBinder.DeathRecipient {
        IBinder mCb;
        int mPid;  // setter
        int mForceUsage = -1;
        boolean mIsSpeakerOn = false;

        ForceUseDeathHandler(IBinder cb, int pid, int usage, boolean on) {
            mCb = cb;
            mPid = pid;
            mForceUsage = usage;
            mIsSpeakerOn = on;
        ...
        public void binderDied() {
            synchronized (mForceUseLock) {
                Log.w(TAG, "ForceUseSpeaker client died, pid " + mPid);

                if (mIsSpeakerOn) {
                    Log.w(TAG, "client died: app " + mPid + " setForceUse " + mForceUsage + " to Speaker, reset to NONE");
                    //AudioSystem.setForceUse(mForceUsage, AudioSystem.FORCE_NONE);
                    sendMsg(mAudioHandler, MSG_SET_FORCE_USE, SENDMSG_QUEUE,
                            mForceUsage, AudioSystem.FORCE_NONE, null, 0);
                }

                unlink();
                if (mForceUsage == AudioSystem.FOR_FM) {
                    mFmForceUseDeathHandlers = null;
                } else if (mForceUsage == AudioSystem.FOR_MEDIA) {
                    mMediaForceUseDeathHandlers = null;
                }
            }
        }
        ...
        public void link() {
            try {
                mCb.linkToDeath(this, 0);
                Log.d(TAG, "ForceUseDeathHandler() linked to " + mCb);
            } catch (RemoteException exp) {
                Log.w(TAG, "ForceUseDeathHandler() can not link to " + mCb + " binder death");
                mCb = null;
            }
        }
        ...
        public void unlink() {
            if (mCb != null) {
                mCb.unlinkToDeath(this, 0);
                Log.d(TAG, "ForceUseDeathHandler unlink from " + mCb);
                mCb = null;
            }
        }
    };
    /** only latest one and setSpeakerOn(usage) is saved into mFmForceUseDeathHandlers.
      if set false, mFmForceUseDeathHandlers will be null */
    private ForceUseDeathHandler mFmForceUseDeathHandlers = null;
    private ForceUseDeathHandler mMediaForceUseDeathHandlers = null;
    private final Object mForceUseLock = new Object();

    /**
     * SPRD add: app setForceUse from AudioManager
     */
    public void setForceUseSpeaker(int usage, boolean on, IBinder cb, String callingPkg) {
        if (usage != AudioSystem.FOR_FM && usage != AudioSystem.FOR_MEDIA) {
            Log.e(TAG, "setForceUseSpeaker usage %d is invalid");
            return;
        }
        if (cb == null) {
            Log.e(TAG, "setForceUseSpeaker was called with no binder");
            return;
        }

        Log.d(TAG, callingPkg+ " setForceUseSpeaker(" + usage + ", " + on + ")");

        if (usage == AudioSystem.FOR_FM) {
            synchronized (mForceUseLock) {
                if (mFmForceUseDeathHandlers != null) {
                    mFmForceUseDeathHandlers.unlink();
                    mFmForceUseDeathHandlers = null;
                }
                if (on) {
                    mFmForceUseDeathHandlers = new ForceUseDeathHandler(cb, Binder.getCallingPid(), usage, on);
                }
            }
            //fix bug 835775.FM use setForceUse should keep synchronized not using sendMsg.
            AudioSystem.setForceUse(usage, on? AudioSystem.FORCE_SPEAKER: AudioSystem.FORCE_NONE);
            return;
        } else if (usage == AudioSystem.FOR_MEDIA) {
            synchronized (mForceUseLock) {
                if (mMediaForceUseDeathHandlers != null) {
                    mMediaForceUseDeathHandlers.unlink();
                    mMediaForceUseDeathHandlers = null;
                }
                if (on) {
                    mMediaForceUseDeathHandlers = new ForceUseDeathHandler(cb, Binder.getCallingPid(), usage, on);
                }
            }
        }
        ...
        //AudioSystem.setForceUse(usage, on? AudioSystem.FORCE_SPEAKER: AudioSystem.FORCE_NONE);
        sendMsg(mAudioHandler, MSG_SET_FORCE_USE, SENDMSG_QUEUE,
                usage, on? AudioSystem.FORCE_SPEAKER: AudioSystem.FORCE_NONE, null, 0);
    }

    private class FMAudioPathDeathHandler implements IBinder.DeathRecipient {
        IBinder mCb;
        int mPid;  // setter

        FMAudioPathDeathHandler(IBinder cb, int pid) {
            mCb = cb;
            mPid = pid;
            link();
        }
        ...
        public void binderDied() {
            Log.w(TAG, "setDeviceConnectionStateForFM client died, pid " + mPid);
            Log.w(TAG, "close DEVICE_OUT_FM_HEADSET,send fm_record=0 and handleFm=0 ");
            AudioSystem.setDeviceConnectionState(AudioSystem.DEVICE_OUT_FM_HEADSET,
                        AudioSystem.DEVICE_STATE_UNAVAILABLE, "", "");
            AudioSystem.setParameters("fm_record=0");
            AudioSystem.setParameters("handleFm=0");

            unlink();
            mFMAudioPathDeathHandler = null;
        }

        public void link() {
            try {
                mCb.linkToDeath(this, 0);
                Log.d(TAG, "FMAudioPathDeathHandler() linked to " + mCb);
            } catch (RemoteException exp) {
                Log.w(TAG, "FMAudioPathDeathHandler() can not link to " + mCb + " binder death");
                mCb = null;
            }
        }
        ...
        public void unlink() {
            if (mCb != null) {
                mCb.unlinkToDeath(this, 0);
                Log.d(TAG, "FMAudioPathDeathHandler unlink from " + mCb);
                mCb = null;
            }
        }
    };
    private FMAudioPathDeathHandler mFMAudioPathDeathHandler = null;

    public void setDeviceConnectionStateForFM(int device, int state,
                                                      String device_address, String device_name, IBinder cb) {
        if (cb == null) {
            Log.e(TAG, "setDeviceConnectionStateForFM was called with no binder");
            return;
        }

        Log.d(TAG, " setDeviceConnectionStateForFM(" + device + ", " + state + ")");

        AudioSystem.setDeviceConnectionState(device,state, "", "");
        if (mFMAudioPathDeathHandler != null) {
            mFMAudioPathDeathHandler.unlink();
            mFMAudioPathDeathHandler = null;
        }
        mFMAudioPathDeathHandler = new FMAudioPathDeathHandler(cb, Binder.getCallingPid());
    }
}
