...
public final class ColorDisplayService extends SystemService
    ...
    private SprdNightDisplayController mSprdController = null;

    public ColorDisplayService(Context context) {
        super(context);
        mHandler = new Handler(Looper.getMainLooper());
        if (SystemProperties.getBoolean("ro.sprd.nightdisplay.enhance", false)) {
            mSprdController = new SprdNightDisplayController();
        }
    }
    ...
    private void setUp() {
        ...
        // Force the initialization current ColorTemperatur value.
        if (mSprdController != null && mNightValue == 0) {
            mNightValue = mController.getColorTemperature();
        }
    ...
    public void onActivated(boolean activated) {
        if (mIsActivated == null || mIsActivated != activated) {
            ...
            setNightDisplayStatus(activated);
    ...
    private void applyTint(boolean immediate) {
        if (mSprdController != null) {
            mSprdController.setNightDispalyStatusAndValue(mIsActivated, mNightValue);
            return;
        }
    ...
    public void setNightDisplayStatus(boolean activate) {
        if (mSprdController == null) {
            return;
        }
        if(activate){
            if(!isSetupPhase) {
                int lastColorValue = System.getIntForUser(getContext().getContentResolver(),
                        SPRD_DISPLAY_COLOR_TEMPERATURE_MODE,1,mCurrentUser);
                System.putIntForUser(getContext().getContentResolver(),
                        SPRD_DISPLAY_COLOR_TEMPERATURE_MODE_BACKUP,lastColorValue,mCurrentUser);
            }
            mSprdController.setNightDispalyStatusAndValue(activate, mNightValue);
            System.putIntForUser(getContext().getContentResolver(),
                    SPRD_DISPLAY_COLOR_TEMPERATURE_MODE,0,mCurrentUser);
        } else {
            if(!isSetupPhase) {
                int backupVolorValue = System.getIntForUser(getContext().getContentResolver(),
                        SPRD_DISPLAY_COLOR_TEMPERATURE_MODE_BACKUP,1,mCurrentUser);
                System.putIntForUser(getContext().getContentResolver(),
                        SPRD_DISPLAY_COLOR_TEMPERATURE_MODE,backupVolorValue,mCurrentUser);
            }
