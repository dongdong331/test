...
final class DisplayPowerController implements AutomaticBrightnessController.Callbacks {
    ...
    // add for bug#909291
    private boolean mBrightnessTrackerStopped = false;
    // NOTE:  PQ Feature BEG-->
    private boolean PQ_ENABLED = SystemProperties.getBoolean("persist.sys.pq.enabled", false); //"ro.display.pq.enabled"
    private boolean SYNC_SLP = SystemProperties.getBoolean("persist.sys.pq.synslp", false);

    // the battery level that should change the screenAutoBrightnessSpline
    private int mPQLowBatteryLevel = 30;
    private boolean mPQBatteryLevelLow = false;
    private SprdSunLightProtector mSprdSunLightProtector;
    // <-- NOTE: PQ Feature END
    ...
    public DisplayPowerController(Context context,
        ...
        if (mUseSoftwareAutoBrightnessConfig) {
            ...
            // NOTE:  PQ Feature BEG-->
            if (PQ_ENABLED) {
                mSprdSunLightProtector = new SprdSunLightProtector(mContext, handler.getLooper(), sensorManager,
                        lightSensorRate, initialLightSensorRate);

                mPQLowBatteryLevel = resources.getInteger(
                        com.android.internal.R.integer.config_lowBatteryLevelForPQ);
    ...
    private void updatePowerState() {
        ...
        // add for bug#909291 --> start
        // check if stop/start mBrightnessTracker
        if (!mBrightnessTrackerStopped && !autoBrightnessEnabled) {
            if (DEBUG) {
                Slog.d(TAG, "Screen will be off, stop BrightnessTracker!");
            }
            if (mBrightnessTracker != null)
                mBrightnessTracker.screenOff(true);

            mBrightnessTrackerStopped = true;
        } else if (mBrightnessTrackerStopped && autoBrightnessEnabled) {
            if (DEBUG) {
                Slog.d(TAG, "Screen will be on, start BrightnessTracker!");
            }
            if (mBrightnessTracker != null)
                mBrightnessTracker.screenOff(false);

            mBrightnessTrackerStopped = false;
        }
        // add for bug#909291 <-- end
        ...
        // NOTE:  PQ Feature BEG-->
        if (PowerManager.BRIGHTNESS_OFF == brightness) {
            if (PQ_ENABLED && mSprdSunLightProtector != null) {
                mSprdSunLightProtector.screenOff(true);
            }
        } else {
            if (PQ_ENABLED && mSprdSunLightProtector != null) {
                mSprdSunLightProtector.screenOff(false);
            }
        }
        // <-- NOTE: PQ Feature END
        ...
        if (!mPendingScreenOff) {
            ...
            // add for bug#955182, for low memory version, mColorFadeEnabled will be false
    ...
    private boolean updateUserSetScreenBrightness() {
        ...
        //add for bug 956218 BEG
        if (mPendingScreenBrightnessSetting > 0 && (mCurrentScreenBrightnessSetting == mTemporaryScreenBrightness)){
            Slog.d(TAG,"updateUserSetScreenBrightness:current setting brightness no changed, return.");
            mCurrentScreenBrightnessSetting = mPendingScreenBrightnessSetting;
            mLastUserSetScreenBrightness = mPendingScreenBrightnessSetting;
            mPendingScreenBrightnessSetting = -1;
            return true;
        }
        //add for bug 956218 END
    ...
    private final SensorEventListener mProximitySensorListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (mProximitySensorEnabled) {
                ...
                Slog.d(TAG, "Proximity Sensor changed distance= " + distance
                        + " positive= " + positive);
    ...
    public void updateBatteryLevel(int level) {
        if (!PQ_ENABLED) return;

        boolean batteryLow = false;
        if (level < mPQLowBatteryLevel)
            batteryLow = true;
        else
            batteryLow = false;

        if (batteryLow != mPQBatteryLevelLow) {
            mPQBatteryLevelLow = batteryLow;
            sendUpdatePowerState();

            if (PQ_ENABLED && !SYNC_SLP && mSprdSunLightProtector != null) {
                mSprdSunLightProtector.setBatteryLevelLow(mPQBatteryLevelLow);
            }
        }
    }
    ...
    public void setSunLightProtectOn(boolean on) {
        if (!PQ_ENABLED) return;
        if (mSprdSunLightProtector != null) {
            mSprdSunLightProtector.setSunLightProtectOn(on, mPQBatteryLevelLow);
        }
    }
    ...
    public void setSunLightProtectTemporayDisabled(boolean disabled) {
        if (!PQ_ENABLED) return;
        if (mSprdSunLightProtector != null) {
            mSprdSunLightProtector.setSunLightProtectTemporayDisabled(disabled);
        }
    }
