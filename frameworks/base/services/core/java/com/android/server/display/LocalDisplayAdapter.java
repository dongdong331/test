...
import static com.android.server.display.DisplayManagerService.MSG_DISPLAY_CONFIG_CHANGE;
...
final class LocalDisplayAdapter extends DisplayAdapter {
    ...
    private final class LocalDisplayDevice extends DisplayDevice {
        ...
        private DisplayConfigObserver mDisplayConfigObserver;
        private int mDisplayConfigId;
        public static final String ACTION_CHANGE_DISPLAY_CONFIG = "sprd.action.change_display_config";
        public static final String SYSTEM_DISPLAY_SIZE_DEFAULT =
                "ro.sr.displaysize.defaultresolution";
        public static final String SYSTEM_DISPLAY_SIZE_LOW = "ro.sr.displaysize.lowresolution";
        private final boolean mIsSupportSR = SystemProperties.getBoolean("ro.sprd.superresolution",
                false);
        private int mDisplayDensity;
        private int DISPLAY_SIZE_1080_1920 = 0;
        private int DISPLAY_SIZE_720_1280 = 1;
        private int DISPLAY_SIZE_720_1440 = 2;
        private int DISPLAY_SIZE_540_960 = 3;
        private int DISPLAY_SIZE_ERROR = 100;
        private int mDisplaySizeLow = DISPLAY_SIZE_ERROR;
        private int mDisplaySizeDefault = DISPLAY_SIZE_ERROR;
        ...
        public LocalDisplayDevice(IBinder displayToken, int builtInDisplayId,
            ...
            if (mIsSupportSR){
                configDisplaySizeIndex();
            }
        }

        /* SPRD: For Super Resolution @{ */
        public void registerDisplayContentObserver() {
            // TODO: Needs to remove the *.debug.* property
            if (mIsSupportSR) {
                mDisplayConfigObserver = new DisplayConfigObserver();
            }
        }

        public void configDisplaySizeIndex(){
            SurfaceControl.PhysicalDisplayInfo[] configs = SurfaceControl.getDisplayConfigs(
                    getDisplayTokenLocked());
            for(int i = 0; i < configs.length; i++) {
                SurfaceControl.PhysicalDisplayInfo info = configs[i];
                int width = info.width;
                int height = info.height;
                if((width == 1080 && height == 1920) || (width == 1920 && height == 1080)){
                    setDisplaySizeIndex(DISPLAY_SIZE_1080_1920, i);
                } else if((width == 720 && height == 1280) || (width == 1280 && height == 720)){
                    setDisplaySizeIndex(DISPLAY_SIZE_720_1280, i);
                } else if((width == 720 && height == 1440) || (width == 1440 && height == 720)){
                    setDisplaySizeIndex(DISPLAY_SIZE_720_1440, i);
                } else if((width == 540 && height == 960) || (width == 960 && height == 540)){
                    setDisplaySizeIndex(DISPLAY_SIZE_540_960, i);
                }
            }
            Slog.d(TAG,"Now the index of Defalut is " + mDisplaySizeDefault
                    + " , the index of low resolution is " + mDisplaySizeLow);
        }

        private void setDisplaySizeIndex(int size, int index){
            int defaultValue = SystemProperties.getInt(SYSTEM_DISPLAY_SIZE_DEFAULT, DISPLAY_SIZE_ERROR);
            int lowValue = SystemProperties.getInt(SYSTEM_DISPLAY_SIZE_LOW, DISPLAY_SIZE_ERROR);
            if( defaultValue == size) {
                mDisplaySizeDefault = index;
            }
            if( lowValue == size) {
                mDisplaySizeLow = index;
            }
        }

        private final class DisplayConfigObserver extends ContentObserver {
            public DisplayConfigObserver() {
                super(new Handler());
                ContentResolver resolver = getContext().getContentResolver();
                resolver.registerContentObserver(Settings.System.getUriFor(ACTION_CHANGE_DISPLAY_CONFIG),
                        false, this, UserHandle.USER_ALL);
                if (setActiveConfigIfNeeded()) {
                    performWindowManagerSizeDensityChange();
                }
            }
            ...
            public void onChange(boolean selfChange, Uri uri) {
                synchronized (getSyncRoot()) {
                    Slog.d(TAG, "display config changed, " + Settings.System.getInt(getContext().getContentResolver(),
                                ACTION_CHANGE_DISPLAY_CONFIG, 0));
                    if (!setActiveConfigIfNeeded()) {
                        Slog.w(TAG, "Set active config failed");
                        return;
                    }
                    performWindowManagerSizeDensityChange();
                }
            ...
            private void performWindowManagerSizeDensityChange() {
                int id = mDisplayConfigId;
                SurfaceControl.PhysicalDisplayInfo phys = mDisplayInfos[id];
                // set size & density for default display
                float density = (id == mDisplaySizeDefault) ? phys.density :
                        (phys.width > 480) ? (float)phys.width/360f : (float)phys.width/320f;
                Message msg = getHandler().obtainMessage(MSG_DISPLAY_CONFIG_CHANGE,
                        phys.width, phys.height, (int)(density * 160 + 0.5f));
                getHandler().sendMessage(msg);
                mDisplayDensity = (int)(density * 160 + 0.5f);
                // sync to DisplayManagerService for WMS to get to update
                updateDeviceInfoLocked();
            }

            private boolean setActiveConfigIfNeeded() {
                synchronized (getSyncRoot()) {
                    int id = Settings.System.getInt(getContext().getContentResolver(),
                            ACTION_CHANGE_DISPLAY_CONFIG, 0);
                    int currentIndex = SurfaceControl.getActiveConfig(getDisplayTokenLocked());
                    mDisplayConfigId =  currentIndex;
                    if(id == 0) {
                        if(mDisplaySizeDefault != DISPLAY_SIZE_ERROR) {
                            mDisplayConfigId = mDisplaySizeDefault;
                        }
                    } else {
                        if(mDisplaySizeLow != DISPLAY_SIZE_ERROR) {
                            mDisplayConfigId = mDisplaySizeLow;
                        }
                    }
                    Slog.d(TAG, "Set active config from " + currentIndex + " to "
                            + mDisplayConfigId);
                    if(currentIndex == mDisplayConfigId) {
                        Slog.d(TAG," Do not set active config to " + mDisplayConfigId
                                + ", Because currentid is " + currentIndex);
                        return false;
                    } else {
                        SurfaceControl.setActiveConfig(getDisplayTokenLocked(), mDisplayConfigId);
                    }
                    return true;
                }
            }
        ...
        public DisplayDeviceInfo getDisplayDeviceInfoLocked() {
            if (mInfo == null) {
                ...
                if (mBuiltInDisplayId == SurfaceControl.BUILT_IN_DISPLAY_ID_MAIN) {
                    ...
                    /* UNISOC: modify for SR change */
                    if (mIsSupportSR && mDisplayConfigId == mDisplaySizeLow) {
                        mInfo.displayCutout = DisplayCutout.fromResources(res, mInfo.width,
                            mInfo.height, mDisplayDensity);
                    } else {
                        mInfo.displayCutout = DisplayCutout.fromResources(res, mInfo.width,
                            mInfo.height);
                    }
