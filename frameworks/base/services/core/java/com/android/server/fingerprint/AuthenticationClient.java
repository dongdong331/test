...
public abstract class AuthenticationClient extends ClientMonitor {
        ...
        if (receiver != null) {
            try {
                ...
                if (!authenticated) {
                    ...
                    /** SPRD: add for fp launch app {@ */
                    String mKeyguardPackage = ComponentName.unflattenFromString(getContext().getResources().getString(
                            com.android.internal.R.string.config_keyguardComponent)).getPackageName();
                    if(mKeyguardPackage.equals(getOwnerString())){
                        Slog.v(TAG, " onAuthenticationSucceeded by keyguard,may need send launchapp msg");
                        needSendLaunchAppMsg(new Fingerprint("" /* TODO */, groupId, fingerId, getHalDeviceId()));
                    }
                    /** @} */
                }
            } catch (RemoteException e) {
    ...
    /** SPRD: add for fp launch app {@ */
    private void needSendLaunchAppMsg(Fingerprint fp){
        ...
        boolean isAllowedForUser = false;
        if(lockPatternUtils !=null){
            isAllowedForUser = lockPatternUtils.isFingerprintAllowedForUser(getTargetUserId());
        }
        if(isAllowedForUser){
            Intent intent = new Intent("com.sprd.intent.action.keyguard_fp_authenticate_success");
            intent.setPackage("com.sprd.applock");
            Bundle args = new Bundle();
            if (fp == null) {
                Slog.e("LaunchAppReceiver", " onAuthenticationSucceeded, fpInfo is null");
                return;
            }
