...
public class JobSchedulerService extends com.android.server.SystemService
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (Intent.ACTION_PACKAGE_RESTARTED.equals(action) || PowerManager.ACTION_POWER_CONTROLLER_KILL_APP.equals(action)) {
                // possible force-stop
                if (pkgUid != -1) {
                    if (DEBUG) {
    ...
    void updateUidState(int uid, int procState) {
        synchronized (mLock) {
            ...
            if (mPowerControllerHelper != null)
                mPowerControllerHelper.updateJobAppStartedRequiredConstraintLocked(uid, procState);
        }
    }
    ...
    private JobStatus getRescheduleJobForFailureLocked(JobStatus failureToReschedule) {
        ...
        if (mPowerControllerHelper != null)
            mPowerControllerHelper.copyJobAppStartedConstranit(failureToReschedule, newJob);
    ...
    private JobStatus getRescheduleJobForPeriodic(JobStatus periodicToReschedule) {
        ...
        JobStatus newJob = new JobStatus(periodicToReschedule, getCurrentHeartbeat(),
        ...
        if (mPowerControllerHelper != null)
            mPowerControllerHelper.copyJobAppStartedConstranit(periodicToReschedule, newJob);
    ...
    private void stopNonReadyActiveJobsLocked() {
        if (DEBUG) Slog.d(TAG, "stopNonReadyActiveJobsLocked " );
        for (int i=0; i<mActiveServices.size(); i++) {
            JobServiceContext serviceContext = mActiveServices.get(i);
            final JobStatus running = serviceContext.getRunningJobLocked();
            if (running != null && !running.isReady()) {
                serviceContext.setJobParamsReschedule(running.mReschedule);
    ...
    final class LocalService implements JobSchedulerInternal {
        ...
        @Override
        public boolean setJobAppStartedConstraintRequired(int uid, int jobId, boolean required) {
            if (mPowerControllerHelper != null)
                return mPowerControllerHelper.setJobAppStartedConstraintRequired(uid, jobId, required);
            return false;
        }
    ...
    private final class PowerControllerHelper {
        ...
        // if orignal job has 'App Started', then also apply this constraint
        public boolean copyJobAppStartedConstranit(JobStatus oldJob, JobStatus newJob) {

            if (oldJob.hasAppStartedConstraint()) {
                newJob.setAppStartedConstraintRequired(true);
                boolean satisfied = oldJob.isAppStartedConstraintSatisfied();
                newJob.setAppStartedConstraintSatisfied(satisfied);
                if (DEBUG) {
                    Slog.d(TAG, "Set 'APP Started' Constraint from Orignal JOb: " + oldJob);
                }
            }
            return true;
        }
        ...
        public boolean setJobAppStartedConstraintRequired(int uid, int jobId, boolean required) {
            synchronized (mLock) {
                ...
                for (int i = jobs.size() - 1; i >= 0; i--) {
                    ...
                    if (job.getJobId() == jobId) {
                        if (DEBUG) {
                            Slog.d(TAG, "set App-Started Required:" + required + " for: " + job);
                        }
                        job.setAppStartedConstraintRequired(required);
                        job.setAppStartedConstraintSatisfied(false);
                        return true;
                    }
                }

                List<JobStatus> sysjobs = mJobs.getJobsByUid(Process.SYSTEM_UID);
                for (int i = sysjobs.size() - 1; i >= 0; i--) {
                    JobStatus sjob = sysjobs.get(i);
                    if (sjob.getJobId() == jobId && uid == sjob.getSourceUid() ) {
                        if (DEBUG) {
                            Slog.d(TAG, "set App-Started Required:" + required + " for: " + sjob);
                        }
                        sjob.setAppStartedConstraintRequired(required);
                        sjob.setAppStartedConstraintSatisfied(false);
                        return true;
                    }
                }
        ...
        // To updata the job that has 'App-Started' constraint.
        public void updateJobAppStartedRequiredConstraintLocked(int uid, int procState) {
            if (procState != ActivityManager.PROCESS_STATE_TOP) return;

            try {
                List<JobStatus> jobs = mJobs.getJobsByUid(Process.SYSTEM_UID);
                for (int i = jobs.size() - 1; i >= 0; i--) {
                    JobStatus job = jobs.get(i);
                    int owningUid = job.getJob().getExtras().getInt("owningUid");

                    if ((job.getSourceUid() == uid || owningUid == uid)
                        && job.hasAppStartedConstraint()) {

                        Slog.d(TAG, "setAppStartedConstraintSatisfied for " + job.getSourceUid() + " job: " + job);
                        job.setAppStartedConstraintSatisfied(true);
                    }
                }

                List<JobStatus> ownJobs = mJobs.getJobsByUid(uid);
                for (int i = ownJobs.size() - 1; i >= 0; i--) {
                    JobStatus ownjob = ownJobs.get(i);
                    int owningUid = ownjob.getJob().getExtras().getInt("owningUid");

                    if ((ownjob.getSourceUid() == uid || owningUid == uid)
                        && ownjob.hasAppStartedConstraint()) {

                        Slog.d(TAG, "setAppStartedConstraintSatisfied for" + ownjob.getSourceUid() + " job: " + ownjob);
                        ownjob.setAppStartedConstraintSatisfied(true);
                    }
                }
            }catch (Exception e) {}
