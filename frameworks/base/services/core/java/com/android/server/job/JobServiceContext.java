...
public final class JobServiceContext implements ServiceConnection {
    ...
    boolean executeRunnableJob(JobStatus job) {
        synchronized (mLock) {
            ...
            if (!binding) {
                ...
                // we fail to bind target service, so we wait
                // until the target service is started
                job.setAppStartedConstraintRequired(true);
    ...
    void setJobParamsReschedule(boolean reschedule) {
        mParams.setReschedule(reschedule);
    }
