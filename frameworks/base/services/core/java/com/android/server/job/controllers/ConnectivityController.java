...
public final class ConnectivityController extends StateController implements
    ...
    private void setDownloadJobsReschedule(boolean reschedule) {
        synchronized (mLock) {
            for (int i = 0; i < mTrackedJobs.size(); i++) {
                final JobStatus js = mTrackedJobs.valueAt(i);
                int index = js.batteryName.indexOf("providers.downloads");
                if (index != -1) {
                    js.mReschedule = reschedule;
                    if (DEBUG) Slog.d(TAG, "setJobsReschedule, jobId="+js.job.getId()+"  reschedule="+reschedule);
                }
            }
        }
    }
    ...
    private final NetworkCallback mNetworkCallback = new NetworkCallback() {
        ...
        public void onNetworkSuspended(Network network) {
            if (DEBUG) {
                Slog.v(TAG, "onNetworkSuspended: " + network);
            }
            updateTrackedJobs(-1, network);
        }
        ...
        public void onNetworkResumed(Network network) {
            if (DEBUG) {
                Slog.v(TAG, "onNetworkResumed: " + network);
            }
            updateTrackedJobs(-1, network);
        }
        ...
        public void onLost(Network network) {
            ...
            if (DownloadManager.isDownloadAlertEnabled()) {
                ...
                if (DEBUG) Slog.d(TAG, "Network lost, networkType="+networkType);
                if (!ConnectivityManager.isNetworkTypeMobile(networkType)) {
                    setDownloadJobsReschedule(false);
                }
            }
            updateTrackedJobs(-1, network);
        }
