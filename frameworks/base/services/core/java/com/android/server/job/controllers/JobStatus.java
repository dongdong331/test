...
public final class JobStatus {
    ...
    public boolean isReady() {
        ...
        final boolean isReschedule =  job.isRescheduleEnable() ? mReschedule : true;
        // NOTE: Bug #627645 low power Feature BEG-->
        final  boolean appNotIdle =  isAppNotIdleSatisfied();
        // <-- NOTE: Bug #627645 low power Feature END
        return (isConstraintsSatisfied() || deadlineSatisfied) && notDozing && notRestrictedInBg && isReschedule && appNotIdle;
    }
    ...
    // This definition must sync with definition of 'CONSTRAINT_XXX'
    static final int CONSTRAINT_APP_STARTED = 1<<10;
    static final int CONSTRAINT_APP_NOT_IDLE = 1<<27;

    private int extRequiredConstraints = 0;
    private int extSatisfiedConstraints = 0;

    public boolean hasAppStartedConstraint() {
        return (extRequiredConstraints&CONSTRAINT_APP_STARTED) != 0;
    }

    public void setAppStartedConstraintRequired(boolean required) {
        extRequiredConstraints = (extRequiredConstraints&~CONSTRAINT_APP_STARTED) | (required ? CONSTRAINT_APP_STARTED : 0);
    }

    public boolean setAppStartedConstraintSatisfied(boolean state) {
        boolean old = (extSatisfiedConstraints&CONSTRAINT_APP_STARTED) != 0;
        if (old == state) {
            return false;
        }
        extSatisfiedConstraints = (extSatisfiedConstraints&~CONSTRAINT_APP_STARTED) | (state ? CONSTRAINT_APP_STARTED : 0);
        return true;
    }

    public boolean isAppStartedConstraintSatisfied() {
        return (extSatisfiedConstraints&CONSTRAINT_APP_STARTED) != 0;
    }

    boolean setAppNotIdleConstraintStisfied(boolean state) {
        boolean old = (extSatisfiedConstraints&CONSTRAINT_APP_NOT_IDLE) != 0;
        if (old == state) {
            return false;
        }
        extSatisfiedConstraints = (extSatisfiedConstraints&~CONSTRAINT_APP_NOT_IDLE) | (state ? CONSTRAINT_APP_NOT_IDLE : 0);
        return true;
    }

    boolean hasAppNotIdleConstraint() {
        return (extRequiredConstraints&CONSTRAINT_APP_NOT_IDLE) != 0;
    }

    boolean isAppNotIdleConstraintSatisfied() {
        return (extSatisfiedConstraints&CONSTRAINT_APP_NOT_IDLE) != 0;
    }

    void setAppNotIdleConstraintRequired(boolean required) {
        extRequiredConstraints = (extRequiredConstraints&~CONSTRAINT_APP_NOT_IDLE) | (required ? CONSTRAINT_APP_NOT_IDLE : 0);
    }

    boolean isAppNotIdleSatisfied() {
        return (!hasAppNotIdleConstraint() ||
            (hasAppNotIdleConstraint() && isAppNotIdleConstraintSatisfied()));
    }
    // <-- NOTE: Bug #627645 low power Feature END
