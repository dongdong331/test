...
import com.android.server.SprdGestureLauncherService;
...
public class PhoneWindowManager extends AbsPhoneWindowManager implements WindowManagerPolicy {
    ...
    static /*final*/ boolean DEBUG_WAKEUP = true; //SPRD set to true
    static final boolean SHOW_SPLASH_SCREENS = true;
    boolean isPowerKeyUseWhenShutDown = true;
    ...
    // Sprd: Boost duration when rotate screen
    private static int mRotationBoostDuration = SystemProperties.getInt("persist.sys.power.rotatedur", 1000);
    ...
    boolean mScreenshotChordVolumeDownKeyConsumed;
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.Secure.getUriFor(
                    ...
                    UserHandle.USER_ALL);
            sprdObserve(resolver, this);  //SPRD: add pocket mode acquirement
            updateSettings();
        }
    ...
    class MyOrientationListener extends WindowOrientationListener {
        ...
        private class UpdateRunnable implements Runnable {
            ...
            public void run() {
                ...
                mPowerManagerInternal.powerHint(PowerHintVendorSprd.POWER_HINT_VENDOR_SCREEN_ROTATION, mRotationBoostDuration);
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        context.registerReceiver(mBootCompleteReceiver, filter);
        ...
        initNavStatus(); // SPRD: add for dynamic navigationbar
        registerSprdIntentFilter();
        updateQuickStepStatus();
    }

    private void scheduleBoostWhenTouch(){
        PerformanceManagerInternal pm = PerformanceManagerInternal.getDefault();
        if(pm != null) {
            pm.scheduleBoostWhenTouch();
        }
    }
    ...
    private void readConfigurationDependentBehaviors() {
        ...
        /* SPRD: long press home show googlesearch @{ */
    ...
    public void setInitialDisplaySize(Display display, int width, int height, int density) {
        ...
        if (!mNaviBooted) {    // SPRD: add for dynamic navigationbar
    ...
    public StartingSurface addSplashScreen(IBinder appToken, String packageName, int theme,
        ...
        try {
            ...
            Intent intent = ((ActivityManagerServiceEx)ActivityManager.getService()).getLaunchedActivityIntent(appToken);
            if (intent != null) {
    ...
    private Drawable getStartingWindowBackgroundDrawable(IBinder appToken, Intent intent) {
        ...
        android.app.TaskThumbnail thumb = null;
        if (intent != null) {
            try {
                thumb = pm.getTaskThumbnail(intent);
                if (thumb != null) {
    ...
    */
    ...
    class StartingView extends ImageView {
        ...
        PerformanceManagerInternal pm;

        public StartingView(Context context,IBinder binder) {
            super(context);
            token = binder;
            pm = PerformanceManagerInternal.getDefault();
        }
        ...
        public boolean dispatchTouchEvent(MotionEvent event) {
            ...
            try {
                pm.removePendingUpdateThumbTask();
            } catch (Exception e) {}
            return false;
        }
    ...
    /**
     * Notifies the screenshot service to stop.
     */
    private void notifyStopScreenshot() {
        Intent stopIntent = new Intent("action.stop.longscreenshot");
        mContext.sendBroadcastAsUser(stopIntent, UserHandle.CURRENT);
    }
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME || keyCode == KeyEvent.KEYCODE_MENU || keyCode == KeyEvent.KEYCODE_BACK) {
            if (Settings.System.getIntForUser(mContext.getContentResolver(), "longshot_switch", 0, UserHandle.USER_OWNER) == 1) {
                Log.d(TAG, "interceptKeyBeforeDispatching when in longscreenshot");
                notifyStopScreenshot();
                Settings.System.putIntForUser(mContext.getContentResolver(), "longshot_switch", 0, UserHandle.USER_CURRENT_OR_SELF);
                return -1;
            }
        }
    ...
    private boolean layoutNavigationBar(DisplayFrames displayFrames, int uiMode, Rect dcf,
        ...
        if (mNavigationBarPosition == NAV_BAR_BOTTOM) {
            ...
            final int top = cutoutSafeUnrestricted.bottom
                    - (mNaviHided ? 0 : getNavigationBarHeight(rotation, uiMode));
            ...
            if (mNaviHided) {  // SPRD: add for dynamic navigationbar
                mTmpNavigationFrame.offset(0, displayFrames.mUnrestricted.bottom - top);
            }
            ...
            final int left = cutoutSafeUnrestricted.right
                    - (mNaviHided ? 0 : getNavigationBarWidth(rotation, uiMode));
            mTmpNavigationFrame.set(left, 0, displayFrames.mUnrestricted.right, displayHeight);
            if (mNaviHided) {  // SPRD: add for dynamic navigationbar
                mTmpNavigationFrame.offset(displayFrames.mUnrestricted.right - left, 0);
            }
            ...
            if (mNaviHided) {  // SPRD: add for dynamic navigationbar
                mTmpNavigationFrame.offset(displayFrames.mUnrestricted.left - right, 0);
            }
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                ...
                if (down) {
                    ...
                    handleQuickCamera(event, down);//SPRD:add for Quick Capture
                }
                if (mUseTvRouting || mHandleVolumeKeysInWM) {
            ...
            case KeyEvent.KEYCODE_CAMERA: {//SPRD:add for Quick Capture
    ...
    BroadcastReceiver mShutDownReceiver = new BroadcastReceiver() {
        ...
        if (Intent.ACTION_SHUTDOWN.equals(intent.getAction())) {
                 isPowerKeyUseWhenShutDown = false;
                 }
                }
              };
    BroadcastReceiver mBootCompleteReceiver = new BroadcastReceiver() {
         ...
         if (Intent.ACTION_BOOT_COMPLETED.equals(intent.getAction())) {
                isPowerKeyUseWhenShutDown = true;
                }
               }
             };
