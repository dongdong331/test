...
public final class PowerManagerService extends SystemService
    ...
    private static final boolean DEBUG_WAKELOCK = (!"user".equals(SystemProperties.get("ro.build.type"))) || getDeBugWakeLockLog();
    /*SPRD : add power debug log end*/
    private static final boolean DEBUG_SPEW = false || DEBUG_WAKELOCK;
    ...
    // Sprd: Add property control
    private final boolean mTouchBoostEnabled = (1 == SystemProperties.getInt("persist.sys.power.touch", 1));
    private final int mTouchBoostDuration = SystemProperties.getInt("persist.sys.power.touchdur", 500);
    ...
    public void systemReady(IAppOpsService appOps) {
        synchronized (mLock) {
            ...
            //Note: Bug#692786 Porting Featrue-Keyboard Lights -->BEG
            PMSFactory.getInstance().initExtraPowerManagerService(mContext, getUiContext());
            //Note: Bug#692786 Porting Featrue-Keyboard Lights --END
    ...
    private void updateSettingsLocked() {
        ...
        // NOTE: Bug #627645 low power Feature BEG-->
        final boolean autoLowPowerModeConfigured = mPowerControllerHelper.isEnabled() ? false : Settings.Global.getInt(resolver,
    ...
    private void applyWakeLockFlagsOnAcquireLocked(WakeLock wakeLock, int uid) {
        if ((wakeLock.mFlags & PowerManager.ACQUIRE_CAUSES_WAKEUP) != 0
            ...
            /*SPRD : add power debug log start*/
            if (DEBUG_WAKELOCK) {
                Slog.d(TAG, "acquire the wakelock flags=0x" + Integer.toHexString(wakeLock.mFlags) +
                        " tag=" + wakeLock.mTag + " packagename=" + opPackageName + " with wakeup flag");
            }
            /*SPRD : add power debug log end*/
    ...
    private void notifyWakeLockChangingLocked(WakeLock wakeLock, int flags, String tag,
        ...
        if (mSystemReady && wakeLock.mNotifiedAcquired) {
            ...
            // NOTE: Bug #627645 low power Feature BEG-->
            // notify listeners
            mPowerControllerHelper.notifyWakeLockChangingLocked(wakeLock, flags, tag,
                        packageName, uid, pid, ws, historyTag);
            // <-- NOTE: Bug #627645 low power Feature END
        }
    }
    ...
    private boolean userActivityNoUpdateLocked(long eventTime, int event, int flags, int uid) {
        ...
        try {
            if (eventTime > mLastInteractivePowerHintTime) {
                /* < Sprd: Add for Power hint extention bgn */
                int sceneId = PowerHintVendorSprd.POWER_HINT_INTERACTION;
                switch (event) {
                    case PowerManager.USER_ACTIVITY_EVENT_TOUCH:
                        sceneId = PowerHintVendorSprd.POWER_HINT_VENDOR_INTERACTION_TOUCH;
                        break;
                    case PowerManager.USER_ACTIVITY_EVENT_BUTTON:
                        sceneId = PowerHintVendorSprd.POWER_HINT_VENDOR_INTERACTION_BUTTON;
                        break;
                    case PowerManagerEx.USER_ACTIVITY_EVENT_WAKEUP:
                        sceneId = PowerHintVendorSprd.POWER_HINT_VENDOR_INTERACTION_WAKEUP;
                        /* Sprd: Distinguish the wakeup event, do a special setting for PowerHint
                         * Set the event to USER_ACTIVITY_EVENT_OTHER to avoid an exception in BatteryStatsService */
                        event = PowerManager.USER_ACTIVITY_EVENT_OTHER;
                        break;
                }

                // if boost for button and touch event
                if (!mTouchBoostEnabled && (event == PowerManager.USER_ACTIVITY_EVENT_BUTTON || event == PowerManager.USER_ACTIVITY_EVENT_TOUCH)) {
                } else {
                    if (sceneId == PowerHintVendorSprd.POWER_HINT_INTERACTION) {
                        Slog.d(TAG, "userActivityNoUpdateLocked: Scene not defined, event:" + event);
                        //new Exception("Scene not defined, event:" + event).printStackTrace();
                    } else {
                        powerHintInternal(sceneId, mTouchBoostDuration);
                    }
                }
                /* Sprd: Add for Power hint extention end > */
        ...
        try {
            switch (mWakefulness) {
                case WAKEFULNESS_ASLEEP:
                    Slog.i(TAG, "Waking up from sleep due to"+opPackageName+" "+reason+" (uid=" + reasonUid + " reason=" + reason
                            + ")...");
                    break;
                case WAKEFULNESS_DREAMING:
                    Slog.i(TAG, "Waking up from dream due to"+opPackageName+" "+reason+" (uid=" + reasonUid + " reason=" + reason
                            + ")...");
                    break;
                case WAKEFULNESS_DOZING:
                    Slog.i(TAG, "Waking up from dozing due to"+opPackageName+" "+reason+" (uid=" + reasonUid + " reason=" + reason
                            + ")...");
                    break;
            }
            ...
            userActivityNoUpdateLocked(
                    eventTime, PowerManagerEx.USER_ACTIVITY_EVENT_WAKEUP, 0, reasonUid);
        } finally {
            Trace.traceEnd(Trace.TRACE_TAG_POWER);
        }
        return true;
    ...
    void setWakefulnessLocked(int wakefulness, int reason) {
        if (mWakefulness != wakefulness) {
            ...
            // NOTE: Bug #627645 low power Feature BEG-->
            mPowerControllerHelper.onWakefulnessChangeStarted(wakefulness, reason);
            // <-- NOTE: Bug #627645 low power Feature END
    ...
    private void logScreenOn() {
        ...
        if (latencyMs >= SCREEN_ON_LATENCY_WARNING_MS) {
            Slog.w(TAG, "Screen on took " + latencyMs+ " ms"
                + "(now:" + SystemClock.uptimeMillis() + " mLastWakeTime:" + mLastWakeTime + ")");
        }
    }
    ...
    private void finishWakefulnessChangeIfNeededLocked() {
        if (mWakefulnessChanging && mDisplayReady) {
            ...
            // NOTE: Bug #627645 low power Feature BEG-->
            mPowerControllerHelper.onWakefulnessChangeFinished();
            // <-- NOTE: Bug #627645 low power Feature END
    ...
    private void updateIsPoweredLocked(int dirty) {
        if ((dirty & DIRTY_BATTERY_STATE) != 0) {
            ...
            // NOTE:  PQ Feature BEG-->
            if (oldBatteryLevel != mBatteryLevel && mDisplayManagerInternal != null) {
                mDisplayManagerInternal.updateBatteryLevel(mBatteryLevel);
            }
            // <-- NOTE: PQ Feature END
    ...
    private boolean updateDisplayPowerStateLocked(int dirty) {
        ...
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY | DIRTY_WAKEFULNESS
            ...
            //Add for bug#915500 BEG
            //limit the screenBrightnessOverride's range
            if (screenBrightnessOverride >= 0) {
                screenBrightnessOverride = Math.max(Math.min(screenBrightnessOverride, mScreenBrightnessSettingMaximum),
                        mScreenBrightnessSettingMinimum);
            }
            //Add for bug#915500 END
    ...
    void updatePowerRequestFromBatterySaverPolicy(DisplayPowerRequest displayPowerRequest) {
        ...
        // NOTE: Bug #627645 low power Feature BEG-->
        if (mPowerControllerHelper.isEnabled()) {
            displayPowerRequest.lowPowerMode = mPowerControllerHelper.isLowPowerMode();
            displayPowerRequest.screenLowPowerBrightnessFactor = mPowerControllerHelper.getLowPowerBrightnessFactor();
        }
        // <-- NOTE: Bug #627645 low power Feature END
    }
    ...
    private final class WakeLock implements IBinder.DeathRecipient {
        ...
        // NOTE: Bug #xxxx low power Feature BEG-->
        // The time stamp (elapsed time) of starting acquire this wake lock
        public long mStartAcquireTimeStamp;
        // <-- NOTE: Bug #627645 low power Feature END
    ...
    private final class BinderService extends IPowerManager.Stub {
        ...
        public void rebootSafeMode(boolean confirm, boolean wait) {
            ...
            /*SPRD : add power debug log start*/
            Log.d(TAG,
                    "rebootSafeMode the device , UID : " + Binder.getCallingUid() + " , PID : "
                            + Binder.getCallingPid() + " , reason  : " + PowerManager.REBOOT_SAFE_MODE + " , confirm = " + confirm + " , wait = " + wait);
            /*SPRD : add power debug log end*/
            final long ident = Binder.clearCallingIdentity();
            try {
        ...
        public void shutdown(boolean confirm, String reason, boolean wait) {
            ...
            /*SPRD : add power debug log start*/
            Log.d(TAG,
                    "shutdown the device , UID : " + Binder.getCallingUid() + " , PID : "
                            + Binder.getCallingPid() + " , reason  : " + reason + " , confirm = " + confirm + " , wait = " + wait);
            /*SPRD : add power debug log end*/
            final long ident = Binder.clearCallingIdentity();
            try {
        ...
        /** Binder call {hide} */
        @Override // Binder call
        public void setSunLightProtectOn(boolean on) {
            final long ident = Binder.clearCallingIdentity();
            try {
                setSunLightProtectOnInternal(on);
            } finally {
                Binder.restoreCallingIdentity(ident);
            }
        }
        ...
        public void setSunLightProtectTemporayDisabled(boolean disabled) {
            final long ident = Binder.clearCallingIdentity();
            try {
                setSunLightProtectTemporayDisabledInternal(disabled);
            } finally {
                Binder.restoreCallingIdentity(ident);
            }
        }
    ...
    private final class LocalService extends PowerManagerInternal {
        ...
        @Override
        public void updateWakeLockDisabledThreshold(long threshold) {
            mPowerControllerHelper.updateWakeLockDisabledThreshold(threshold);
        }

        @Override
        public void checkWakeLock(WakeLockCallback callback, long durationThreshold) {
            mPowerControllerHelper.checkWakeLock(callback, durationThreshold);
        }

        @Override
        public void registerPowerControllerInternalCallback(PowerControllerInternalCallback callback) {
            mPowerControllerHelper.registerPowerControllerInternalCallback(callback);
        }

        @Override
        public void registerWakefulnessCallback(WakefulnessCallback callback) {
            mPowerControllerHelper.registerWakefulnessCallback(callback);
        }
        // <-- NOTE: Bug #627645 low power Feature END
    ...
    private final class PowerControllerHelper {
        ...
        // Set of app ids that we will always disable the wake locks for.
        private ArrayList<Integer>mWakeLockDisabledApplist = new ArrayList<>();

        // Threshold time to disable wakelock in mWakeLockDisablelist
        // default is 1min, but it will get the real value from PowerController
        private long mWakeLockDisabledThreshold = (1 * 60 * 1000L); // 1min


        private PowerManagerInternal.PowerControllerInternalCallback mPowerControllerInternalCallback;
        private PowerManagerInternal.WakefulnessCallback mWakefulnessCallback;

        private PowerController.LocalService mPowerControllerInternal = null;

        public boolean isEnabled() {
            return PowerManagerEx.isPowerControllerEnabled();
        }

        public boolean isLowPowerMode() {
            getPowerControllerInternal();
            int mode = mPowerControllerInternal.getPowerSaveMode();
            if ((PowerManagerEx.MODE_PERFORMANCE != mode) && (PowerManagerEx.MODE_SMART != mode)
                    && (PowerManagerEx.MODE_INVALID != mode) && PowerManagerEx.MODE_NONE != mode) {
                return true;
            }
            return false;
        }

        public float getLowPowerBrightnessFactor() {
            getPowerControllerInternal();
            int mode = mPowerControllerInternal.getPowerSaveMode();
            float factor = 0.3f;
            if (PowerManagerEx.MODE_POWERSAVING == mode)
                factor = 0.75f;
            return factor;
        }

        public void registerPowerControllerInternalCallback(
                PowerManagerInternal.PowerControllerInternalCallback callback) {
            synchronized (mLock) {
                mPowerControllerInternalCallback = callback;
            }
        }

        public void updateWakeLockDisabledThreshold(long threshold) {
            mWakeLockDisabledThreshold = threshold;
        }
        ...
        public void notifyWakeLockChangingLocked(WakeLock wakeLock, int flags, String tag,
                String packageName, int uid, int pid, WorkSource ws, String historyTag) {

            // if Audio update a wakelock notify listeners
            if ((wakeLock.mFlags & PowerManager.WAKE_LOCK_LEVEL_MASK) == PowerManager.PARTIAL_WAKE_LOCK
                && Process.AUDIOSERVER_UID == wakeLock.mOwnerUid) {
                if (mPowerControllerInternalCallback != null) {
                    mPowerControllerInternalCallback.onWakeLockChanging(wakeLock.mTag, wakeLock.mPackageName,
                        wakeLock.mOwnerUid, wakeLock.mOwnerPid, wakeLock.mWorkSource,
                        tag, packageName, uid, pid, ws);
                }
            }
        }
        ...
        public void registerWakefulnessCallback(
                PowerManagerInternal.WakefulnessCallback callback) {
            synchronized (mLock) {
                mWakefulnessCallback = callback;
            }
        }

        public void onWakefulnessChangeFinished() {
            if (DEBUG) Slog.d(TAG, "callback onWakefulnessChangeFinished: " + mWakefulnessCallback);
            if (PowerManagerInternal.isInteractive(mWakefulness)) {
                if (mWakefulnessCallback != null) {
                    mWakefulnessCallback.onWakefulnessChangeFinished(true);
                }
            } else {
                if (mWakefulnessCallback != null) {
                    mWakefulnessCallback.onWakefulnessChangeFinished(false);
                }
            }
        }

        public void onWakefulnessChangeStarted(final int wakefulness, int reason) {
            if (DEBUG) Slog.d(TAG, "callback onWakefulnessChangeStarted: " + mWakefulnessCallback);
            if (mWakefulnessCallback != null) {
                mWakefulnessCallback.onWakefulnessChangeStarted(wakefulness, reason);
            }
        }

        // Update the wakelock disabled state that is held by app
        public void updateUidProcWakeLockDisabledState(int uid, boolean disabled) {
            synchronized (mLock) {
        ...
        /*
         * Check if this wakelock shoud be disabled
         * It should be disabled for:
         * 1) application uid in mWakeLockDisablelist
         * 2) not in device idle whitelist
         * 3) acquired time has > WAKE_LOCK_DISABLE_THRESHOLD (default 1min)
         * 4) not changed the disable state that make by DeviceIdleMode
         */
        public boolean shouldWakeLockDisabledLocked(WakeLock wakeLock) {
            if ((wakeLock.mFlags & PowerManager.WAKE_LOCK_LEVEL_MASK) == PowerManager.PARTIAL_WAKE_LOCK) {
                boolean checkAudioWakeLock = false;
                boolean checkSyncJobWakeLock = false;

                int index = mWakeLockDisabledApplist.indexOf(wakeLock.mOwnerUid);
                if (index < 0) {
                    checkAudioWakeLock = shouldAudioWakeLockDisabledLocked(wakeLock);
                    checkSyncJobWakeLock = shouldSyncJobWakeLockDisabledLocked(wakeLock);

                    // if this wakeLock is not hold by audio server
                    // and its uid is not in disable list, then it should not be disabled
                    if (!checkAudioWakeLock && !checkSyncJobWakeLock) {
                        if (wakeLock.mDisabled) {
                            Slog.d(TAG, "wake lock:" + wakeLock.mTag + " from " + wakeLock.mPackageName
                                + "(" + wakeLock.mOwnerUid +") is disabled: false");
                        }
                        return false; // not in disable list
                    }
                }
                ...
                // for application uids that are not whitelisted.
                // or special handle for wakelock held by audioserver
                if ((appid >= Process.FIRST_APPLICATION_UID &&
                        Arrays.binarySearch(mDeviceIdleWhitelist, appid) < 0 &&
                        Arrays.binarySearch(mDeviceIdleTempWhitelist, appid) < 0)
                        /*handle for wakelock held by audioserver (Be carefull)*/
                     || (checkAudioWakeLock && wakeLock.mOwnerUid == Process.AUDIOSERVER_UID)
                     || checkSyncJobWakeLock) {
                    long now = SystemClock.elapsedRealtime();

                    // if screen is off, that is mHalInteractiveModeEnabled is false
                    if (!mHalInteractiveModeEnabled && wakeLock.mStartAcquireTimeStamp > 0
                        &&  (now - wakeLock.mStartAcquireTimeStamp) >= mWakeLockDisabledThreshold) {
                        disabled = true;
                    } else {
                        if (mPowerControllerInternalCallback != null)
                            mPowerControllerInternalCallback.onConstraintAppAcquireWakeLock(now, wakeLock.mStartAcquireTimeStamp);
                    }
                }

                // If not in Device Idle mode
                // or this wakeLock is not disabled by Device Idle Mode
                // For shouldWakeLockDisabledLocked is only called when wake is not disable by Device Idle Mode
                // So here ignore the DeviceIdleMode checking
                if (wakeLock.mDisabled != disabled) {
                    Slog.d(TAG, "wake lock:" + wakeLock.mTag + " from " + wakeLock.mPackageName
                        + "(" + wakeLock.mOwnerUid +") is disabled:" + disabled);
                }
                return disabled;
            }
            return false;
        ...
        /**
         * Check if a wakelock which holding duration >= durationThreshold
         * if wakelock holding duration >= durationThreshold, then call the callback function
         *
         * @param callback: the callback function that will be called when holding duration >= durationThreshold
         * @param durationThreshold: the threshold time in ms
         */
        public void checkWakeLock(PowerManagerInternal.WakeLockCallback callback, long durationThreshold) {
            ...
            synchronized (mLock) {
                ...
                for (int i = 0; i < numWakeLocks; i++) {
                    ...
                    if ((wakeLock.mFlags & PowerManager.WAKE_LOCK_LEVEL_MASK)
                        ...
                        Slog.d(TAG, "checkWakeLock wake lock:" + wakeLock.mTag + " from " + wakeLock.mPackageName
                            + "(" + wakeLock.mOwnerUid +") duration:" + (nowElapsed - wakeLock.mStartAcquireTimeStamp));

                        if (!wakeLock.mDisabled && wakeLock.mStartAcquireTimeStamp > 0 &&
                            (nowElapsed - wakeLock.mStartAcquireTimeStamp) >= durationThreshold) {
                                long duration = nowElapsed - wakeLock.mStartAcquireTimeStamp;
                                if (wakeLock.mOwnerUid == Process.AUDIOSERVER_UID) {

                                    // for wakelock hold by Audio, check if it hold the wakelock for other apps.
                                    // if so, use the exactly app uid
                                    if (wakeLock.mWorkSource != null) {
                                        int num = wakeLock.mWorkSource.size();
                                        int count = 0;
                                        for (; count<num; count++) {
                                            int suid = wakeLock.mWorkSource.get(count);
                                            callback.onWakeLock(wakeLock.mPackageName, wakeLock.mTag, suid, duration);
                                        }
                                    } else {
                                        callback.onWakeLock(wakeLock.mPackageName, wakeLock.mTag, wakeLock.mOwnerUid, duration);
                                    }
        ...
        // check the wake lock held by audio server
        // generally, audio server held wake lock for other apps
        // if all this apps are disabled, then we can disable this wake lock also
        private boolean shouldAudioWakeLockDisabledLocked(WakeLock wakeLock) {
            boolean disabled = false;
            ...
            if (wakeLock.mOwnerUid == Process.AUDIOSERVER_UID) {
                if (wakeLock.mWorkSource != null) {
                    ...
                    int count = 0;
                    for (; count<num; count++) {
                        int uid = wakeLock.mWorkSource.get(count);
                        //Slog.d(TAG, "Audio wake lock:" + wakeLock.mTag + " from " + wakeLock.mPackageName
                        //    + "(" + wakeLock.mOwnerUid +") mWorkSource uid:" + uid);
                        if (mWakeLockDisabledApplist.indexOf(uid) < 0) break;
                    }
                    if (count > 0 && count == num) {
                        //Slog.d(TAG, "Audio wake lock:" + wakeLock.mTag + " from " + wakeLock.mPackageName
                        //    + "(" + wakeLock.mOwnerUid +") can be disabled for all its apps that it stands for.");
                        disabled = true;
                    }
                }
            }
        ...
        private boolean shouldSyncJobWakeLockDisabledLocked(WakeLock wakeLock) {
            boolean disabled = false;

            if (wakeLock.mOwnerUid == Process.SYSTEM_UID
                && "android".equals(wakeLock.mPackageName)
                && wakeLock.mTag != null && wakeLock.mTag.contains("*job*")) {
                if (wakeLock.mWorkSource != null) {
                    int num = wakeLock.mWorkSource.size();
                    int count = 0;
                    for (; count<num; count++) {
                        int uid = wakeLock.mWorkSource.get(count);
                        //Slog.d(TAG, "SyncJob wake lock:" + wakeLock.mTag + " from " + wakeLock.mPackageName
                        //    + "(" + wakeLock.mOwnerUid +") mWorkSource uid:" + uid);
                        if (mWakeLockDisabledApplist.indexOf(uid) < 0) break;
                    }
                    if (count > 0 && count == num) {
        ...
        private void getPowerControllerInternal() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            synchronized (mLock) {
                for (int i = 0; i < mWakeLocks.size(); i++) {
                    ...
                    Slog.d(TAG, "onReceive SCREEN_OFF remain wakelock:" + wakelock.mLock + " flag:"
                            + wakelock.mFlags + " tag:" + wakelock.mTag + " pkg:" + wakelock.mPackageName);
                }
            }
        }
    }
    ...
    // NOTE:  PQ Feature BEG-->
    private void setSunLightProtectOnInternal(boolean on) {
        Slog.d(TAG, "setSunLightProtectOnInternal :" + on + " mSystemReady " + mSystemReady);

        if (mSystemReady && mDisplayManagerInternal != null) {
            mDisplayManagerInternal.setSunLightProtectOn(on);
        }
    }

    private void setSunLightProtectTemporayDisabledInternal(boolean disabled) {
        Slog.d(TAG, "setSunLightProtectOnInternal :" + disabled + " mSystemReady " + mSystemReady);

        if (mSystemReady && mDisplayManagerInternal != null) {
            mDisplayManagerInternal.setSunLightProtectTemporayDisabled(disabled);
        }
    }
    // <-- NOTE: PQ Feature END
}
