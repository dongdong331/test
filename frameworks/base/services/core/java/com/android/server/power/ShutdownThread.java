...
public final class ShutdownThread extends Thread {
    ...
    private static ShutdownAnimation shutdownAnim = new ShutdownAnimation(); // SPRD:add for bootanim
    ...
    public static void rebootAnimation(Context context){
        try {
            Log.i(TAG, "play the bootanimation for alarm-boot");
            SystemProperties.set("ctl.start", "bootanim");
        } catch (Exception e){
            Log.e(TAG, "bootanimation command exe error!");
        }
        ...
        Log.i(TAG, "disable the AirPlane_Mode!");
    }
    /* @} */
    ...
    private static ProgressDialog showShutdownDialog(Context context) {
      ...
      if(sIsPowerOffAlarm){
        Log.i(TAG, "isPowerOffAlarm = " +sIsPowerOffAlarm + ". needn't anim");
      } else if (pd != null){
        pd.show();
      }
        return pd;
