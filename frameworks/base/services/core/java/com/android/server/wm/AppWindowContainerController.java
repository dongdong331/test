...
public class AppWindowContainerController
    ...
    private /*final*/ IApplicationToken mToken = null;
    ...
    private final Runnable mAddStartingWindow = new Runnable() {
        ...
        public void run() {
            ...
            if (startingData == null) {
                ...
                synchronized (mWindowMap) {
                    container.startingAdding = false;
    ...
    public void removeContainer(int displayId) {
        ...
        synchronized(mWindowMap) {
            ...
            if (mContainer != null && mContainer.startingData != null) {
                if (DEBUG_STARTING_WINDOW) Slog.v(TAG_WM, "Force Remove starting " + mContainer
        ...
        if (surface != null) {
            final StartingSurface s = surface;
            mService.mAnimationHandler.post(new Runnable() {
                @Override
                public void run() {
                    try {
                        if (DEBUG_STARTING_WINDOW) {
                            Slog.i(TAG_WM, "Container remove startingWindow in anim thread");
                        }
                        s.remove();
                    } catch (Exception e) {
