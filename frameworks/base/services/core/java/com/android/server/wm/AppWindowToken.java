...
class AppWindowToken extends WindowToken implements WindowManagerService.AppFreezeListener {
    ...
    // UNISOC: fix staring window issue
    boolean startingShouldRemoved;
    boolean startingAdding;
    ...
    // for Go devices
    private boolean mLowRamDevice = ActivityManager.isLowRamDeviceStatic();
    /**
    ...
    void onFirstWindowDrawn(WindowState win, WindowStateAnimator winAnimator) {
        ...
        } else if (startingAdding) {
            startingShouldRemoved = true;
            if (DEBUG_STARTING_WINDOW) {
                Slog.v(TAG, "should abort the Starting window, the starting window is in adding" + this);
            }
        }
        ...
        if (!hiddenRequested) {
            if (!mFreezingScreen) {
                ...
                if (mService.mAppsFreezingScreen == 1) {
                    ...
                    mService.mH.sendEmptyMessageDelayed(H.APP_FREEZE_TIMEOUT, mLowRamDevice ? 8000 : 2000);
                }
            }
            final int count = mChildren.size();
            for (int i = 0; i < count; i++) {
    ...
    void dump(PrintWriter pw, String prefix, boolean dumpAll) {
        ...
        pw.print(prefix); pw.print("task="); pw.println(getTask()!=null ? getTask().getName():null);
