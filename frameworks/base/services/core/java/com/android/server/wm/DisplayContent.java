...
class DisplayContent extends WindowContainer<DisplayContent.DisplayChildWindowContainer> {
    ...
    /* SPRD: for SR */
    int mSuperResDisplayDensity = 0;
    boolean mSuperResDisplayDensityChanged = false;
    ...
    private boolean mTraversalImeWindowInAboveContainer;
    ...
    boolean updateRotationUnchecked(boolean forceUpdate) {
        ...
        if (SystemProperties.getBoolean("service.bootanim.shutdown", false)) {
            Slog.v(TAG_WM, "No rotation during shutdown animation.");
            return false;
        }
    ...
    private boolean traversalImeWindowInAboveContainer(WindowContainer child) {
        if (child == mImeWindowsContainers && mService.mInputMethodTarget != null
                && hasSplitScreenPrimaryStack()) {
            return true;
        }
    ...
    boolean forAllWindows(ToBooleanFunction<WindowState> callback, boolean traverseTopToBottom) {
        ...
        mTraversalImeWindowInAboveContainer = false;
        if (traverseTopToBottom) {
            for (int i = mChildren.size() - 1; i >= 0; --i) {
                final DisplayChildWindowContainer child = mChildren.get(i);
                if (skipTraverseChild(child)) {
                    continue;
                } else if (traversalImeWindowInAboveContainer(child)){
                    mTraversalImeWindowInAboveContainer = true;
                    continue;
                }
    ...
    void initializeDisplayBaseInfo() {
        ...
        /* SPRD: for SR */
        mSuperResDisplayDensity = mService.computeInitialDisplayDensity(this, mInitialDisplayDensity);
    }
    ...
    void removeImmediately() {
        ...
        // UNISOC: Make sure we stop freeze right before display is gone
        if (mService.mDisplayFrozen && mService.mFrozenDisplayId == mDisplayId) {
            Slog.d(TAG, "Display is still frozen, stop freezing");
            try {
                mService.mForceStopFreeze = true;
    ...
    private void setInputMethodTarget(WindowState target, boolean targetWaitingAnim) {
        ...
        mService.mInputMethodTargetWaitingAnim = targetWaitingAnim;
        // UNISOC: When inputmethodtarget changed, request to relayout once more
        //         to ensure that we passed the right target to PWM
    ...
    private final class AboveAppWindowContainers extends NonAppWindowContainers {
        ...
        boolean forAllWindows(ToBooleanFunction<WindowState> callback,
                boolean traverseTopToBottom) {
            boolean needAssignIme = mImeWindowsContainers != null
                    && mImeWindowsContainers.getSurfaceControl() != null;
            int inputDialogLayer = mService.mPolicy.getWindowLayerFromTypeLw(
                    TYPE_INPUT_METHOD_DIALOG, true);
            if (traverseTopToBottom) {
                for (int i = mChildren.size() - 1; i >= 0; --i) {
                    if (mTraversalImeWindowInAboveContainer && needAssignIme) {
                        final WindowToken wt = mChildren.get(i);
                        int layer = mService.mPolicy.getWindowLayerFromTypeLw(
                                wt.windowType, wt.mOwnerCanManageAppTokens);
                        if (layer > inputDialogLayer) {
                            if (mChildren.get(i).forAllWindows(callback, traverseTopToBottom)) {
                                return true;
                            }
                        } else {
                            //traversel the ime here
                            mImeWindowsContainers.forAllWindows(callback, traverseTopToBottom);
                            needAssignIme = false;
                            if (mChildren.get(i).forAllWindows(callback, traverseTopToBottom)) {
                                return true;
                            }
