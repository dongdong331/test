...
class RootWindowContainer extends WindowContainer<DisplayContent> {
    ...
    public boolean getWindowVisibleBounds(String windowName, Rect bounds) {
        ...
        forAllWindows((w) -> {
            ...
            if (windowName != null) {
                if ((w.mAttrs.getTitle().toString().contains(windowName) ||
                    windowName.equals(w.mAttrs.packageName)) && w.isVisibleLw()) {
                    w.getVisibleBounds(bounds);
                    isVisible[0] = true;
                    return;
                }
            }
    ...
    public int getWindowVisibleBounds(String windowName, Rect[] bounds, int size) {
        final int[] findSize = new int[1];
        if (bounds == null || size <= 0) return 0;
        forAllWindows((w) -> {
            if (findSize[0] >= size) {
                return;
            }
            if (windowName != null) {
                if ((w.mAttrs.getTitle().toString().contains(windowName) ||
                    windowName.equals(w.mAttrs.packageName)) && w.isVisibleLw()) {
                    w.getVisibleBounds(bounds[findSize[0]]);
                    findSize[0]++;
                }
            }
        }, true /* traverseTopToBottom */);
