...
import static android.view.WindowManager.LayoutParams.PRIVATE_FLAG_SNAPSHOT_NOT_RUN;
...
class TaskSnapshotSurface implements StartingSurface {
    ...
    private static final String DEBUG_TASKSNAPSHOT = "debug.tasksnap";
    ...
    static TaskSnapshotSurface create(WindowManagerService service, AppWindowToken token,
        ...
        if (SystemProperties.getBoolean(DEBUG_TASKSNAPSHOT, false)) {
            Slog.d(TAG, "disable TaskSnapshot by debug, token = " + token);
            return null;  //SPRD: debug disable TaskSnapshot
        }
        ...
        synchronized (service.mWindowMap) {
            ...
            if (snapshot.getOrientation() != mainWindow.getConfiguration().orientation
                    || snapshot.getOrientation() != topFullscreenWindow.getConfiguration().orientation) {
                Slog.w(TAG, "orientation mismatch, return");
                return null;  //SPRD: just don't add this snapshot rather than remove it later
            }
            ...
            if((windowPrivateFlags & PRIVATE_FLAG_SNAPSHOT_NOT_RUN) != 0) {
                Slog.w(TAG, "Do not run snapshot , return");
                return null;
            }
