...
#include "com_android_server_AlarmNtp.cpp"
#include <memory>
 ...
 */
static const size_t ANDROID_ALARM_TYPE_COUNT = 7;
...
int AlarmImpl::setTime(struct timeval *tv)
{
    ...
    if (old_time.tv_sec && firstConnectNetwork) {
        ...
        ALOGD("AlarmImplTimerFd::setTime, drm_ntp, _delta: %lld, delta_alarm: %lld, tv->tv_sec: %ld, old_time.tv_sec: %ld.", (long long)_delta, (long long)delta_alarm, tv->tv_sec, old_time.tv_sec);
    }
    pthread_mutex_unlock(&_mutex);
...
static jint android_server_AlarmManagerService_set(JNIEnv*, jobject, jlong nativeData, jint type, jlong seconds, jlong nanoseconds)
{
    ...
    ALOGD("set alarm to kernel: %lld.%09lld, type=%d \n", static_cast<long long>(seconds), static_cast<long long>(nanoseconds), type);
