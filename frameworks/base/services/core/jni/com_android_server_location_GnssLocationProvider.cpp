...
#include <vendor/sprd/hardware/gnss/2.0/IGnssExt.h>
#include <vendor/sprd/hardware/gnss/2.0/IAGnssRilExt.h>
...
using IGnss_V2_0 = vendor::sprd::hardware::gnss::V2_0::IGnssExt;
using vendor::sprd::hardware::gnss::V2_0::IAGnssRilExt;
using vendor::sprd::hardware::gnss::V2_0::IGnssUnisocExt;
...
sp<IAGnssRilExt> agnssRilIface_V2_0 = nullptr;
sp<IGnssUnisocExt> gnssUnisocExtIface = nullptr;
...
static void android_location_GnssLocationProvider_class_init_native(JNIEnv* env, jclass clazz) {
    gnssHal = IGnss_V1_0::getService();
    if (gnssHal != nullptr) {
            ALOGI("Loaded GNSS HAL 1.0 service");
            gnssHal_V1_1 =  IGnss_V1_1::castFrom(gnssHal);
            if (gnssHal_V1_1 == nullptr) {
			 ALOGD("gnssHal_V1_1 Null");	
            } else {
                ALOGI("Loaded GNSS HAL 1.1 service");
            }	
            gnssHal_V2_0 =  IGnss_V2_0::castFrom(gnssHal_V1_1);
            if (gnssHal_V2_0 == nullptr) {
			 ALOGD("gnssHal_V2_0 Null");				
            } else {
                ALOGI("Loaded GNSS HAL Vendor 2.0 service");
            }
    } else {
            ALOGI("Couldn't load GNSS HAL service");
    }
}
#endif
...
static void android_location_GnssLocationProvider_init_once(JNIEnv* env, jclass clazz) {
    ...
    if (gnssHal != nullptr) {
      ...
      hardware::Return<bool> linked = gnssHal->linkToDeath(
        ...
        if (gnssHal_V2_0 != nullptr) {
	        ...
	        auto gnssRil = gnssHal_V2_0->getExtensionAGnssRilExt();
	        if (!gnssRil.isOk()) {
	            ALOGD("Unable to get a handle to AGnssRil 2.0");
			} else {
			 agnssRilIface_V2_0 = gnssRil;
			 agnssRilIface = agnssRilIface_V2_0;
			 ALOGD("set agnssRilIface_V2_0 to agnssRilIface");			 
			}			
        } else {
			auto agnssRilIface_V1_0 = gnssHal->getExtensionAGnssRil();
			if (!agnssRilIface_V1_0.isOk()) {
			 ALOGD("Unable to get a handle to AGnssRil 1.0");
			} else {
			 agnssRilIface = agnssRilIface_V1_0;
			 ALOGD("set agnssRilIface_V1_0 to agnssRilIface");				 
			}		
        }
	...
	if (gnssHal_V1_1 != nullptr)
	{
		...
		{
			ALOGD("get gnssMeasurement_V1_1!!!");
	...
	else
	{
		...
		{
			ALOGD("get gnssMeasurement_V1_0!!!");
	...
	if (!gnssDebug.isOk()) {
        ...
        auto gnssUnisocExt = gnssHal_V2_0->getExtensionUnisocExt();
        if (!gnssUnisocExt.isOk()) {
            ALOGD("Unable to get a handle to UnisocExt");
		} else {
			gnssUnisocExtIface = gnssUnisocExt;
		}
    } else {
...
static jboolean android_location_GnssLocationProvider_init(JNIEnv* env, jobject obj) {
    ...
    if (gnssHal_V1_1 != nullptr) {
        ALOGI("Call GNSS HAL setCallback_1_1\n");
        result = gnssHal_V1_1->setCallback_1_1(gnssCbIface);
    } else {
        ALOGI("Call GNSS HAL setCallback1.0\n");    
...
static void android_location_GnssLocationProvider_agps_set_reference_location_cellid_ex(
        JNIEnv* /* env */, jobject /* obj */, jint type, jint mcc, jint mnc, jint lac, jint cid, jint tac, jint pcid) {
    IAGnssRil::AGnssRefLocation location;

    ALOGD("call AGPS RIL interface in agps_set_reference_location_cellid support LTE");
    if (agnssRilIface == nullptr) {
        ALOGE("No AGPS RIL interface in agps_set_reference_location_cellid");
        return;
    }
...
static void android_location_GnssLocationProvider_agps_ni_message(JNIEnv* env, jobject /* obj */,
        jbyteArray data, jint length) {
	ALOGD("invoke AGPS interface in agps_ni_message entry");
	if (agnssRilIface_V2_0 == nullptr)
	{
		ALOGE("No AGPS RIL interface 2.0 in agps_ni_message");
		return;
	}
	else if (agnssRilIface == nullptr) 
	{
		ALOGE("No AGPS RIL interface in agps_ni_message");
		return;
	}
	...
	std::string msgString = std::string((const char*)bytes, length);
	if (agnssRilIface_V2_0 != nullptr)
	{
		agnssRilIface_V2_0->agpsNiMessage(msgString, (uint32_t)length);
	}
	else
	{
		ALOGE("No AGPS RIL interface agpsNiMessage");
	}
	env->ReleasePrimitiveArrayCritical(data, bytes, JNI_ABORT);

	ALOGD("invoke AGPS interface in agps_ni_message OUT");
	return;
...
static void android_location_GnssLocationProvider_set_network_handle(JNIEnv*,
                                                                   jobject,
                                                                   jlong networkhandle) {
	if (gnssUnisocExtIface == nullptr) {
		ALOGE("No Unisoc Ext interface");
		return;
	}
	auto result = gnssUnisocExtIface->setNetWorkHandle(networkhandle);
	if (!result.isOk() || !result) {
		ALOGE("set network handle failed");
	}
	ALOGD("invoke Ext interface in set_network_handle end");
}
...
static const JNINativeMethod sMethods[] = {
    ...
    {"native_agps_ni_message",
                            "([BI)V",
                            reinterpret_cast<void *>(android_location_GnssLocationProvider_agps_ni_message)},
    {"native_agps_set_id",
    ...
    {"native_agps_set_ref_location_cellid_ex",
            "(IIIIIII)V",
            reinterpret_cast<void *>(
                                    android_location_GnssLocationProvider_agps_set_reference_location_cellid_ex)},
    ...
    {"native_agps_data_set_network_handle",
            "(J)V",
            reinterpret_cast<void *>(android_location_GnssLocationProvider_set_network_handle)},
};
