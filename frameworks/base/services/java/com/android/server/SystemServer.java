...
import com.android.server.display.SunLightService;
...
import com.android.server.policy.SprdPhoneWindowManager;
import com.android.server.power.sprdpower.PowerController;
import com.android.server.power.sprdpower.AbsPowerGuruService;
import com.android.server.power.sprdpower.PowerGuruServiceFactory;
...
import com.android.server.boardScore.BoardScoreService;
import com.android.server.performance.PerformanceManagerService;
import com.android.server.ifaa.IFAAService;
...
public final class SystemServer {
    ...
    private void startBootstrapServices() {
        ...
        Installer installer = mSystemServiceManager.startService(InstallerEx.class);
        traceEnd();
        ...
        // NOTE:  PQ Feature BEG-->
        if (SystemProperties.getBoolean("persist.sys.pq.enabled", false)) {
            try {
                mSystemServiceManager.startService(SunLightService.class);
            } catch (Exception e) {
                Slog.i(TAG, "SunLightService Service Exception:" + e);
            }
        }
        // <-- NOTE: PQ Feature END
	...
	PowerController powerctl = null;
        ...
        try {
            ...
            //SPRD: Bug 692795 add for heartbeat powerguru --> BEG
            try {
                if (AbsPowerGuru.isEnabled()) {
                    Slog.i(TAG, "PowerGuru Manager");
                    powerguru = PowerGuruServiceFactory.getInstance().createPowerGuruService(context);
                    ServiceManager.addService(Context.POWERGURU_SERVICE, powerguru);
                }
            } catch (Throwable e) {
                Slog.e(TAG, "Failure PowerGuruService", e);
            }
            //SPRD: Bug 692795 add for heartbeat powerguru --> END
            ...
            wm = WindowManagerService.main(context, inputManager,
                    ...
                    !mFirstBoot, mOnlyCore, new SprdPhoneWindowManager());
            ...
            //add for performance
            try {
                mPerformanceManagerService = new PerformanceManagerService(context);
                ServiceManager.addService("performance_fw", mPerformanceManagerService);
                mActivityManagerService.setPerformanceManager(mPerformanceManagerService);
            } catch (Exception e) {
                Slog.e(TAG, "Failure PerformanceService", e);
            }
            //add for performance
        ...
        if (mFactoryTestMode != FactoryTest.FACTORY_TEST_LOW_LEVEL) {
            ...
            if (GestureLauncherService.isGestureLauncherEnabled(context.getResources())) {
                ...
                mSystemServiceManager.startService(SprdGestureLauncherService.class);
                traceEnd();
            }
            ...
            if (mPackageManager.hasSystemFeature(PackageManager.FEATURE_FACEID)) {
                traceBeginAndSlog("FaceService");
                try {
                    mSystemServiceManager.startService(FaceService.class);
                } catch (Throwable e) {
                    reportWtf("starting FaceService", e);
                }
                traceEnd();
            }

            if (SystemProperties.getBoolean("ro.ifaa.support", false)) {
                traceBeginAndSlog("IFAAService");
                mSystemServiceManager.startService(IFAAService.class);
                traceEnd();
            }
        ...
        final PowerController powerctlF = powerctl;
        ...
        mActivityManagerService.systemReady(() -> {
            ...
            traceEnd();
            //SPRD: Bug#881948 Add for LTE/SoftAp coexisting BEG-->
            if (WifiFeaturesUtils.FeatureProperty.SUPPORT_SPRD_SOFTAP_COEXIST_LTE) {
                //Create WcnManager
                try {
                    Slog.i(TAG, "WcnManager Service");
                    final WcnManagerService   wcnManager = WcnManagerService.create(context);
                } catch (Throwable e) {
                    reportWtf("starting WcnManagerService Service", e);
                }
            }
            //<-- Add for LTE/SoftAp coexisting END
	   ...
	   try {
		...
		reportWtf("starting PowerController", e);
            ...
            if (SystemProperties.get("persist.support.securetest").equals("1")) {
                if (securityF != null) {
                    traceBeginAndSlog("MakeSecurityServiceReady");
                    try {
        ...
        if ("1".equals(SystemProperties.get("persist.vendor.bsservice.enable"))) {
            try {
                Slog.i(TAG, "start BoardScore Service");
