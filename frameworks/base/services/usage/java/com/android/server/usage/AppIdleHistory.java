...
public class AppIdleHistory {
    ...
    // History for all users and all packageso
    ...
    private AppUsageHistory getPackageHistory(ArrayMap<String, AppUsageHistory> userHistory,
        ...
        if (appUsageHistory == null && create) {
            ...
            // UNISOC: bug702053, cancel event record while monkey test.
            recordPkghistoryIntoUserhistoryIfNeeded(userHistory,
                    packageName, appUsageHistory);
        }
        return appUsageHistory;
    }
    ...
    private void recordPkghistoryIntoUserhistoryIfNeeded(
            ArrayMap<String, AppUsageHistory> userHistory,
            String packageName, AppUsageHistory packageHistory) {
        ...
        if (!am.isUserAMonkeyNoCheck()) {
            ...
            if (UsageStatsService.DEBUG) {
                Slog.i(TAG, "->readAppIdleTimesLocked packageName:" + packageName
                        + ", monkey test do not record the package history into"
                        + " userHistory!");
            }
        }
    }
