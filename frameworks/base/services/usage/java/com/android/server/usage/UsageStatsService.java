...
public class UsageStatsService extends SystemService implements
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                default:
                    // NOTE: Bug #627645 low power Feature BEG-->
                    if (mPowerControllerHelper != null
                        && mPowerControllerHelper.handleMessage(msg)) {
                        return;
                    }
                    // <-- NOTE: Bug #627645 low power Feature END
    ...
    private final class LocalService extends UsageStatsManagerInternal {
       ...
       // NOTE: Bug #627645 low power Feature BEG-->
        ...
        @Override
        public void setAppStandbyBucket(String packageName,
                int bucket, int userId, String caller) {
            if (mPowerControllerHelper != null)
                mPowerControllerHelper.setAppStandbyBucket(packageName, bucket, userId, caller);
        }

        @Override
        public void forceAppIdleEnabled(boolean enabled) {
            if (mPowerControllerHelper != null)
                mPowerControllerHelper.forceAppIdleEnabled(enabled);
        }
        // <-- NOTE: Bug #627645 low power Feature END
    }
    ...
    private final class PowerControllerHelper {
        static final int MSG_INFORM_APP_STATE = MSG_UID_STATE_CHANGED + 1;
        ...
        public void setAppStandbyBucket(String packageName,
            ...
            if (bucket < UsageStatsManager.STANDBY_BUCKET_ACTIVE
                    || bucket > UsageStatsManager.STANDBY_BUCKET_NEVER) {
                Slog.d(TAG, "Cannot set the standby bucket to " + bucket);
                return;
            }
            try {
                ...
                final boolean shellCaller = callingUid == 0 || callingUid == Process.SHELL_UID;
                final boolean systemCaller = UserHandle.isCore(callingUid);
                final int reason = (systemCaller && !"ssense".equals(caller))
                        ? UsageStatsManager.REASON_MAIN_FORCED
                        : UsageStatsManager.REASON_MAIN_PREDICTED;
        ...
        public void forceAppIdleEnabled(boolean enabled) {
            mAppStandby.setAppIdleEnabled(enabled);
        }
    }
    //
