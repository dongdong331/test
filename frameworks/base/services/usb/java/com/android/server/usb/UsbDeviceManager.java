...
public class UsbDeviceManager implements ActivityManagerInternal.ScreenObserver {
    ...
    /* SPRD add sprd usb feature @{*/
    private static final boolean DEBUG = true;
    private static boolean SUPPORT_UMS = false;
    private static boolean mIsUmsShared = false;
    private static final int MSG_SWITCH_FOR_USB_ACTIVE_CHANGED = 101;
    private static final int MSG_SIM_CHECKING = 102;
    private final static String USB_ACTIVATED = "usb_actived";
    private static final int SIM_CHECKING_TIMEOUT = 5000;
    private static boolean mIsSimChecking = true;
    private static boolean mIsSimStateRegister = false;
    /* @} */
    ...
    BroadcastReceiver umsStateChangeReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            final String action = intent.getAction();
            if (DEBUG) Slog.d(TAG, "action = " + action);
            if (Intent.ACTION_MEDIA_SHARED.equals(action)) {
                mIsUmsShared = true;
            } else if (!Environment.MEDIA_SHARED.equals(EnvironmentEx.getExternalStoragePathState())) {
                    mIsUmsShared = false;
            }
            mHandler.updateUsbNotification(false);
        }
    };

    private void registerUmsReceiver() {
        if (SUPPORT_UMS) {
            ...
            mContext.registerReceiver(umsStateChangeReceiver, umsStateFilter);
        }
    }
    /* @} */
    ...
    abstract static class UsbHandler extends Handler {
        ...
        /* SPRD:Add for bug692660 androido porting:new UI for usb @{ */
        ...
        protected String mCurrentFunctionsStr;
        /* @} */
        ...
        /*SPRD: add sprd usb feature @{ */
        private void updateDisconnectStateAndNotification() {
            if (mNotificationManager != null && mUsbNotificationId != 0) {
                mNotificationManager.cancelAsUser(null, mUsbNotificationId,
                        UserHandle.ALL);
                Slog.d(TAG, "clear usb notification");
        ...
        public void updateState(String state) {
            ...
            sendMessageDelayed(msg, (connected == 0 && !mCurrentFunctionsStr.equals(UsbManager.USB_FUNCTION_NONE)) ? UPDATE_DELAY_MORE : 0);
        }
        ...
        protected void updateMidiFunction() {
        ...
        protected void updateUsbNotification(boolean force) {
            ...
            if (mAudioAccessoryConnected && !mAudioAccessorySupported) {
                ...
                if (SUPPORT_UMS && mIsUmsShared) {
                    titleRes = com.android.internal.R.string.usb_storage_stop_title;
                    id = SystemMessage.NOTE_STORAGE_DISK;
                } else if (mCurrentFunctions == UsbManager.FUNCTION_CDROM) {
                    titleRes = com.android.internal.R.string.usb_cdrom_notification_title;
                    id = SystemMessage.NOTE_USB_CDROM;
                } else if (mCurrentFunctions == UsbManager.FUNCTION_MTP) {
                ...
                if (id != 0) {
                    ...
                    if (titleRes
                        ...
                        Intent intent = Intent.makeRestartActivityTask(
                            new ComponentName("com.android.settings", orignal_support?
                                        "com.android.settings.Settings$UsbDetailsActivity" :
                                        "com.android.settings.Settings$SprdUsbSettingsActivity"));
        ...
        protected void updateAdbNotification(boolean force) {
            ...
            if (mAdbEnabled && mConnected) {
                ...
                if (force && mAdbNotificationShown) {
                    ...
                    Slog.d(TAG, "clear adb notification");
                }
        ...
        protected boolean isUsbShouldActived() {
        ...
        private class UsbActiveSwitchSettingsObserver extends ContentObserver {
            ...
            public void onChange(boolean selfChange) {
                ...
                sendMessage(MSG_SWITCH_FOR_USB_ACTIVE_CHANGED, enable);
            }
        }
        /* @} */
    }
    ...
    private static final class UsbHandlerLegacy extends UsbHandler {
        ...
        private boolean trySetEnabledFunctions(long usbFunctions, boolean forceRestart) {
            ...
            /*SPRD: add usb sim activate feature @{ */
            if (usbFunctions == UsbManager.FUNCTION_NONE && isUsbShouldActived() && !mIsSimChecking) {
                Slog.d(TAG, "usb sim activate on, and no sim inserted, set usb function to none");
            ...
            if ((!functions.equals(oemFunctions)
                ...
                /*SPRD: add for bug 978820 to release midi node before setprop none @{ */
                if(containsFunction(mCurrentFunctionsStr, UsbManager.USB_FUNCTION_MIDI)) {
                    updateMidiFunction();
                }
                /* @} */
