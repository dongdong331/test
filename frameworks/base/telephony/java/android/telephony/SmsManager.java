...
public final class SmsManager {
    ...
    public static final String SMS_ENCODING_CHANGE_ENABLE =
            CarrierConfigManager.KEY_SMS_ENCODING_CHANGE_ENABLE_BOOL;

    public static final String SMS_USE_SIMPLE_CHARACTERS_ONLY =
            CarrierConfigManager.KEY_SMS_USE_SIMPLE_CHARACTERS_ONLY_BOOL;
   ...
   /***************************************************************
     * <Fdn Contacts to be checked>
     * @param destinationAddress
     * @param scAddress
     * @param text
     * @param sentIntent
     * @param deliveryIntent
     * @param persistMessageForCarrierApp
     * @throw
     * @return void
     * @hide
     ****************************************************************/
    private void checkSendToTaget(String destinationAddress, String scAddress) {
        Context context = ActivityThread.currentApplication()
                .getApplicationContext();
        boolean needStop = false;
        try {
            Log.e(TAG, "checkSendToTaget");
            //SmsManagerEx mSmsManagerEx = SmsManagerEx.getDefault();
            //add for 612674 --begin
            int subID =getSubscriptionId();
            Log.d(TAG, "subID =" + subID);
            SmsManagerEx mSmsManagerEx = SmsManagerEx.getSubActivity(subID);
            //add for 612674 --end
            needStop = mSmsManagerEx.checkFdnContact(destinationAddress);
            Log.e(TAG, "SmsManger------1A--needStop---->" + needStop);
        } catch (Exception e) {
            // TODO: handle exception
        }

        if (needStop) {
            Log.e(TAG, "SmsManger------1B--needStop---->" + needStop);
            // AddCMSCToTarget(scAddress);
            throw new IllegalArgumentException("Invalid FDN destinationAddress");
        }
    }

    // sprd: 596495 fdn feature
    /**
     * <when ril return error no : fdn check error, we had to add smsc number to >
     * @param number
     * @throw
     * @hide
     */
    public static void AddCMSCToTarget(String number,int subid) {
        Context context = ActivityThread.currentApplication()
                .getApplicationContext();
        //Add for bug 758344 begin
        UserHandle uh = android.os.Process.myUserHandle();
        try {
            UserInfo userInfo = ActivityManager.getService().getCurrentUser();
            if (userInfo != null) {
                uh = userInfo.getUserHandle();
            }
        } catch (RemoteException re) {
            Log.e(TAG, "RemoteException when getCurrentUser");
        }//Add for bug 758344 end
        try {
            Log.i(TAG, "SmsManger------AddCMSCToTarget---->", new Exception());
            Log.i(TAG, "SmsManger------AddCMSCToTarget---number---->"+number);
            Log.e(TAG, "SmsManger------AddCMSCToTarget---SUB ="+subid);
            SubscriptionManager subscriptionManager = SubscriptionManager
                    .from(context);
            Intent it = new Intent();
            //Log.i(TAG, "SmsManger------AddCMSCToTarget---DefaultSmsSUB ="+subscriptionManager.getDefaultSmsSubscriptionId());
            it.setComponent(new ComponentName("com.spread.cachefdn",
                    "com.spread.cachefdn.StopSmscActivity"));
            it.putExtra(
            ...
            it.putExtra("smcsNumber", number);
            context.startActivityAsUser(it, uh);//Modify for bug 758344
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     * @hide
     */
    private boolean CanSendEmptyMessgeOrNot(String text) {
        Context context = ActivityThread.currentApplication()
                .getApplicationContext();
        /*String tempt = Settings.Global.getString(context.getContentResolver(),
                "ro.message.EmptyMessage");
        int flag = -1;
        if (TextUtils.isEmpty(tempt)) {
            flag = 1;
        } else {
            flag = Integer.valueOf(tempt);
        }
        return (flag > 0);*/
        return (Settings.Global.getInt(context.getContentResolver(),"ro.message.EmptyMessage", 1)  <= 0);
    }
    //add for 616304 --begin
    private boolean checkFdnEnable(){
        TelephonyManagerEx mTelephonyManagerEx =TelephonyManagerEx.from(ActivityThread.currentApplication().getApplicationContext());
        try {
            return mTelephonyManagerEx.getIccFdnEnabled(getSubscriptionId());
            } catch (Exception e) {
            e.printStackTrace();
        }
        return false;
    ...
    private void sendTextMessageInternal(String destinationAddress, String scAddress,
        ...
        if (TextUtils.isEmpty(text) &&  CanSendEmptyMessgeOrNot(text)) {
            throw new IllegalArgumentException("Invalid message body");
        }
        // add for bug 616304 --begin
        // sprd: 596495 fdn feature start
        boolean checkEnable =checkFdnEnable();
        Log.e(TAG,"checkEnable ="+checkEnable);
        if (checkEnable){
            checkSendToTaget(destinationAddress,scAddress);
        }
        // sprd: 596495 fdn feature end
        // add for bug 616304 --end
        ...
        try {
            ...
            // ignore it
            ex.printStackTrace();
            Log.i(TAG, "RemoteException ex:     "+ ex.getMessage());
        }
    }
   ...
   /**
    ...
    public int getSubscriptionId() {
        ...
        isSmsSimPickActivityNeeded = false;//add for bug597384,sms dosn't need to toast a sim dialog anyway for android N.
    ...
    public static Bundle getMmsConfig(BaseBundle config) {
        ...
        /*add for bug 843395 start*/
        filtered.putBoolean(SMS_ENCODING_CHANGE_ENABLE,
                config.getBoolean(SMS_ENCODING_CHANGE_ENABLE));
        Log.d(TAG, "getMmsConfig smsEncodingChangeable =" + config.getBoolean(SMS_ENCODING_CHANGE_ENABLE));
        filtered.putBoolean(SMS_USE_SIMPLE_CHARACTERS_ONLY,
                config.getBoolean(SMS_USE_SIMPLE_CHARACTERS_ONLY));
        Log.d(TAG, "getMmsConfig smsUsesSimpleCharactersOnly =" + config.getBoolean(SMS_USE_SIMPLE_CHARACTERS_ONLY));
        /*add for bug 843395 end*/
