...
public class ImsManagerEx {
    ...
    /**
     * Check Dual VOLTE state
     */
    public static boolean isDualVoLTEActive(){
        String mSimConfig = SystemProperties.get("persist.vendor.radio.volte.mode");
        if(mSimConfig != null && mSimConfig.equalsIgnoreCase("DualVoLTEActive")){
            return true;
        } else {
            return false;
        }
    ...
    /**
     * SPRD: Add for DSDA
     * For L+L modem, dual VoLte registered does not mean ready for dual active call.
     * @return true, if it is ready for dual active call.
     */
    public static boolean isReadyForDualActiveCall() {
        boolean ready = isDualVoLTEActive() && isDualVoLTERegistered();
        if(isDualLteModem()) {
            ready = ready && SystemProperties.get("ril.dsda.status", "0").contains("1");
        }
        return ready;
    }
    ...
    public static boolean separateSettingForWFCandVoLTE(Context context) {
        Log.d(TAG, "separateSettingForWFCandVoLTE =  " + getBooleanCarrierConfig(
                CarrierConfigManagerEx.KEY_SEPARATE_SETTING_FOR_WFC_VOLTE, context));
        return getBooleanCarrierConfig(
                CarrierConfigManagerEx.KEY_SEPARATE_SETTING_FOR_WFC_VOLTE, context);
    }
    /* @} */
