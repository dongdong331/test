...
//    private static Context mContext = null;
    ...
    /*SPRD modified for Bug 843395 end*/
    /**
    ...
    public static byte[] stringToGsm7BitPackedWithHeader(String data, byte[] header,
        ...
        byte[] ret = stringToGsm7BitPacked(data, headerSeptets, true/*"1".equals(mEncodeType)*/, languageTable,
                languageShiftTable);
    ...
    public static byte[] stringToGsm7BitPacked(String data, int startingSeptetOffset,
        ...
        Rlog.i("SmsMessage" + TAG, "throwException:" + throwException + "; septetCount="+ septetCount);
        if (septetCount == -1) {
    ...
    public static String gsm7BitPackedToString(byte[] pdu, int offset,
        ...
        Rlog.d("SmsMessage-" + TAG, "gsm7BitPackedToString, offset:" + offset + ", lengthSeptets:" +
                lengthSeptets + ", numPaddingBits:" + numPaddingBits
                + ", languageTable:" + languageTable + ", shiftTable:" + shiftTable);
        try {
    ...
    public static boolean isGsmSeptets(char c) {
        ...
        Rlog.d("SmsMessage", TAG  + "-isGsmSeptets, sEnabledLockingShiftTables.length:" + sEnabledLockingShiftTables.length + ", sEnabledSingleShiftTables.length:" + sCharsToShiftTables.length);
        if (sEnabledLockingShiftTables.length > 0) {
        ...
        if (sEnabledSingleShiftTables.length > 0) {
            for (int i : sEnabledSingleShiftTables) {
                if (sCharsToGsmTables[i].get(c, -1) != -1) {
                    return true;
                }
            }
    ...
    countGsmSeptets(CharSequence s, boolean use7bitOnly) {
        ...
        if (sEnabledSingleShiftTables.length + sEnabledLockingShiftTables.length == 0) {
            ...
            int septets = GsmAlphabetEx.countGsmSeptetsUsingTables(s, use7bitOnly, 0, 0);
            if (septets == -1) {
                return null;
            }
    ...
    public static byte[] isAsciiStringToGsm8BitUnpackedField(String s)
        ...
        return GsmAlphabetEx.stringToGsm8BitPacked(s);
    ...
    public static byte[] stringToGsmAlphaSS(String s)
        ...
        switch (alphaType) {
            case 0:// ***
                ret = GsmAlphabetEx.isAsciiStringToGsm8BitUnpackedField(s.toString());
                break;
            case 80:// 0x80+***
                ret = GsmAlphabetEx.isAsciiStringToGsm8BitUnpackedField(s.toString());
                ret[0] = (byte) 0x80;
            ...
            case 82:// 0x82+length+base_high++base_low+***
                ...
                for (i = 0; i < count; i++) {
                    if (s.codePointAt(i) >= 0x80) {
                        ...
                        tempUcs2 = GsmAlphabetEx.charToGsm((char) tempUcs2);
                        /* @} */
                    }
    ...
    private static byte convertAsciiCharToGsm8BitUnpacked(char c) {
        SparseIntArray charToLanguageTable = GsmAlphabetEx.getCharsToGsmTables()[0];
        SparseIntArray charToShiftTable = GsmAlphabetEx.getCharsToShiftTables()[0];
        ...
        if (v == -1) {
            ...
            if (v == -1) {
                ...
                return GsmAlphabetEx.GSM_EXTENDED_ESCAPE;
            }
        }
    ...
    /* SPRD:add for Bug 474538  @{ */
    public static String getGsm8BitString(byte[] data, int offset, int length,
            String characterset) {
        String gsm8Bit = null;
        try {
    ...
    public static byte[] stringToGsmAlphaSSForDialer(String s)
            throws IccPBForEncodeException{
        try{
            return stringToGsmAlphaSS(s);
        }catch(EncodeException e){
            throw new IccPBForEncodeException();
        }
    }
}
