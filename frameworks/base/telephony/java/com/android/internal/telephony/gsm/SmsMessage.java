...
public class SmsMessage extends SmsMessageBase {
    ...
    public static SubmitPdu getSubmitPdu(String scAddress,
        ...
        Rlog.i(LOG_TAG, "getSubmitPdu encoding " + encoding);
        boolean use7bitOnly = false;
        //for bug778189 begin
        /*if (encoding == ENCODING_7BIT)//{//delete by Bug 742056
            use7bitOnly = true;
//        if (encoding == ENCODING_UNKNOWN) {
            // Find the best encoding to use
        TextEncodingDetails ted = calculateLength(message, use7bitOnly);
        encoding = ted.codeUnitSize;
        languageTable = ted.languageTable;
        languageShiftTable = ted.languageShiftTable;
        */
        if (encoding == ENCODING_UNKNOWN || encoding == ENCODING_7BIT){
            if (encoding == ENCODING_7BIT){
                ...
                languageTable = ted.languageTable;
                languageShiftTable = ted.languageShiftTable;
            }else if (encoding == ENCODING_UNKNOWN) {
                // Find the best encoding to use
                TextEncodingDetails ted = calculateLength(message, false);
                encoding = ted.codeUnitSize;
                languageTable = ted.languageTable;
                languageShiftTable = ted.languageShiftTable;
                Rlog.i(LOG_TAG, "getSubmitPdu: encoding change to: " + encoding +" ;languageShiftTable:" +"languageShiftTable");
            }
           if (encoding == ENCODING_7BIT &&
        ...
        try {
            if (encoding == ENCODING_7BIT) {
                ...
                userData = GsmAlphabetEx.stringToGsm7BitPackedWithHeader(message, header,
                        languageTable, languageShiftTable);
            } else { //assume UCS-2
                try {
    ...
    public static TextEncodingDetails calculateLength(CharSequence msgBody,
       ...
       /*SPRD modified for Bug 843395 start*/
       /* int encoding = SmsConstants.ENCODING_UNKNOWN;
        String encodeType = GsmAlphabetEx.getEncodeType();
        if (!"".equals(encodeType)) {
            encoding = Integer.parseInt(encodeType);
        }*/
        GsmAlphabetEx.setUse7bitOnly(use7bitOnly);
        Rlog.d(LOG_TAG, "calculateLength use7bitOnly = " + use7bitOnly);
        if (use7bitOnly && r.getBoolean(com.android.internal.R.bool.config_sms_force_7bit_encoding)) {
            newMsgBody  = Sms7BitEncodingTranslator.translate(msgBody);
        }
        if (TextUtils.isEmpty(newMsgBody)) {
            newMsgBody = msgBody;
        }
       // Rlog.d(LOG_TAG, "calculateLength encodeType = " + encodeType);
      /*  if(encoding == SmsConstants.ENCODING_16BIT)
            return SmsMessageBase.calcUnicodeEncodingDetails(newMsgBody);*/       /*SPRD modified for Bug 843395 end*/
        Rlog.d(LOG_TAG, "calculateLength use7bitOnly = " + use7bitOnly);
        TextEncodingDetails ted = GsmAlphabet.countGsmSeptets(newMsgBody, use7bitOnly);
        Rlog.d(LOG_TAG, "calculateLength ted = " + ted);
        if (ted == null) {
    ...
    private void parseUserData(PduParser p, boolean hasUserDataHeader) {
        ...
        case ENCODING_8BIT:
            ...
            if (r.getBoolean(com.android.internal.
                    R.bool.config_sms_decode_gsm_8bit_data)) {
               if (mUserDataHeader != null && mUserDataHeader.portAddrs != null) { /*modified for Bug 808476 start*/
                   mMessageBody = null;
                    Rlog.d(LOG_TAG, "SMS message ENCODING_8BIT mMessageBody = null.");
               } else {
                   mMessageBody = p.getUserDataGSM8bit(count);
                    Rlog.d(LOG_TAG, "SMS message ENCODING_8BIT mMessageBody != null." );
                }/*modified for Bug 808476 end*/
            } else {
                Rlog.d(LOG_TAG, "SMS message ENCODING_8BIT mMessageBody = null.");
                mMessageBody = null;
            }
    ...
    /* SPRD add for Bug 489211 begin */
    protected static int validity;

    public static void getSmsValidity(Context context, int subId) {
        int tvalidity = -1;
        String validityStr = null;
        if (null != context) {
            validityStr = Settings.Global.getString(context.getContentResolver(), "persist.radio.smstime" + subId);
        }
        Rlog.i(LOG_TAG, "getSmsValidity()--->>      validity = " + validityStr + ", subId = "
                + subId);
        try {
            tvalidity = Integer.parseInt(validityStr);
        } catch (NumberFormatException exception) {
            Rlog.e(LOG_TAG, "getMmsValidity  Exception : " + exception);
        }
        validity = tvalidity;
    }
    /* SPRD add for Bug 489211 end */

    public static byte[] getReceivedPdu(String destinationAddress,
                                        String message, Time time, int encoding) {
        Rlog.d(LOG_TAG, "getReceivedPdu");
        if (message == null || destinationAddress == null) {
            return null;
        }
        byte mtiByte = (byte) 0x24;
        ByteArrayOutputStream bo = getReceivedPduHead(destinationAddress,
                mtiByte);
        byte[] userData;
        if (encoding == ENCODING_UNKNOWN) {
        ...
        try {
            if (encoding == ENCODING_7BIT) {
                ...
                userData = GsmAlphabetEx.stringToGsm7BitPackedWithHeader(message,
                        null);
            } else {
                // assume UCS-2
                try {
                    userData = com.android.internal.telephony.gsm.SmsMessage.encodeUCS2(message, null);
                } catch (UnsupportedEncodingException uex) {
        ...
        byte[] bTime = getGsmSctsTime(time);
        bo.write(bTime, 0, bTime.length);
        bo.write(userData, 0, userData.length);
        return bo.toByteArray();
    }

    public static ByteArrayOutputStream getReceivedPduHead(String destinationAddress, byte mtiByte) {
        ByteArrayOutputStream bo = new ByteArrayOutputStream(MAX_USER_DATA_BYTES + 40);
        bo.write(mtiByte);
        byte[] oaBytes;
        oaBytes = PhoneNumberUtils.networkPortionToCalledPartyBCD(destinationAddress);
        /*
        * SPRD:
        *   Bug286659
        * @orig:
        ...
        */
        if (oaBytes != null) {
            ...
            Rlog.d(LOG_TAG, "Address is invalid format,write empty number");
            bo.write(0);
        ...
        * @}
        */
        bo.write(0);//write PID
        return bo;
    }

    public static byte[] getGsmSctsTime(Time time) {
        byte[] scts = new byte[7];
        int year = time.year > 2000 ? time.year - 2000 : time.year - 1900;
        scts[0] = intoBcdEncodeByte(year);
        scts[1] = intoBcdEncodeByte(time.month + 1);
        scts[2] = intoBcdEncodeByte(time.monthDay);
        scts[3] = intoBcdEncodeByte(time.hour);
        scts[4] = intoBcdEncodeByte(time.minute);
        scts[5] = intoBcdEncodeByte(time.second);
        scts[6] = intoBcdEncodeByte(0);
        return scts;
    }

    //chanage for encode 4 bit bcd
    public static byte intoBcdEncodeByte(int i) {
        byte b = (byte) (((i % 10) << 4) & 0xf0);
        byte c = (byte) ((i / 10) & 0x0f);
        byte data = (byte) (b | c);
    ...
    //SPRD: add by TS
    public static byte[] getReceivedPdu(String destinationAddress, String message, Time time) {
        return getReceivedPdu(destinationAddress, message, time, ENCODING_UNKNOWN);
    }
}
