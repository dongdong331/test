...
interface IWifiManager
{
    ...
    /**
    * NOTE: Bug #692685 Add For SoftAp advance Feature
    */

    //To block a station with a mac string. Then this station will can not connected to our softap
    boolean softApBlockStation(String mac, String packageName);

    //To ublock the statition
    boolean softApUnblockStation(String mac, String packageName);

    //get the current connected station, return a string with format:
    //XX:XX:XX:XX:XX:XX XX:XX:XX:XX:XX:XX ...
    //each mac string separate with a blank
    String softApGetConnectedStations(String packageName);

    //get the current blocked station, return a string with format:
    //XX:XX:XX:XX:XX:XX XX:XX:XX:XX:XX:XX ...
    //each mac string separate with a blank
    String softApGetBlockedStations(String packageName);

    List<String> softApGetConnectedStationsDetail(String packageName);
    List<String> softApGetBlockedStationsDetail(String packageName);

    boolean softApAddClientToWhiteList(String mac, String name, String packageName);
    boolean softApDelClientFromWhiteList(String mac, String name, String packageName);
    boolean softApSetClientWhiteListEnabled(boolean enabled, String packageName);
    List<String> softApGetClientWhiteList(String packageName);
    boolean softApIsWhiteListEnabled(String packageName);
    //NOTE: Bug #505201 Add for softap support wps connect mode and hidden ssid Feature BEG-->
    boolean softApWpsCheckPin(String wpsPin, String packageName);
    //<-- Add for softap support wps connect mode and hidden ssid Feature END
    //SPRD: Add for Wifi Network Event Observer -->
    boolean registerWifiNetworkEventObserver(IWifiNetworkEventObserver observer);
    boolean unregisterWifiNetworkEventObserver(IWifiNetworkEventObserver observer);
    int getCurrentRssi();
    boolean isNetworkValidForVoWifi();
    boolean setCheckingInternetForVoWifi(boolean enabled);
    void notifyAndCloseWifi();
