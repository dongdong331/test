...
#ifndef NO_USE_PROJECT_SEC /*vendor define NO_USE_PROJECT_SEC all the time for vndk rules*/
#ifdef USE_PROJECT_SEC /*system define USE_PROJECT_SEC when open ccsa*/
//    ALOGI("SPRD Security IPCThreadState transact start");
    if (handle == 0) {
        if (code == IServiceManager::ADD_SERVICE_TRANSACTION) {
            data.setDataPosition(0);
            data.readInt32();//int32_t policy = data.readInt32();
            //ALOGI("SPRD Security IPCThreadState policy: %d", policy);
            String16 service = data.readString16();
            //ALOGI("SPRD Security IPCThreadState service: %s", String8(service).string());
            String16 name = data.readString16();
            //ALOGI("SPRD Security IPCThreadState name: %s", String8(name).string());
            sp<IBinder> b = data.readStrongBinder();
//            ALOGI("SPRD Security IPCThreadState b: 0x%x", b.get());
            data.setDataPosition(0);
            //ALOGI("SPRD Security IPCThreadState transact initKeyService name1:%s",String8(name).string());
            initKeyService(name , b);
            //ALOGI("SPRD Security IPCThreadState transact initKeyService name2:%s",String8(name).string());
        }
    }
#endif /*USE_PROJECT_SEC*/
#endif /*NO_USE_PROJECT_SEC*/
