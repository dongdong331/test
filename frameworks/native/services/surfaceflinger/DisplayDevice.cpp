...
void DisplayDevice::onSwapBuffersCompleted() {
    mDisplaySurface->onFrameCommitted();
#ifdef SPRD_SR_DEFER_FREE_FB
    if(mType == DisplayDevice::DISPLAY_PRIMARY) {
        if(mNewBufferPostCnt>-1) {
            mNewBufferPostCnt++;
        }
        if(mNewBufferPostCnt>10) {
            ALOGI("SPRD_SR hwc2 free fb buffers.");
            mDisplaySurfaceSR.clear();
            mNativeWindowSR.clear();
            mNewBufferPostCnt=-1;
        }
    }
#endif
...
void DisplayDevice::setDisplaySize(const int newWidth, const int newHeight,
        const sp<DisplaySurface>& displaySurface, const sp<ANativeWindow> nativeWindow,
        std::unique_ptr<RE::Surface> renderSurface) {
    static int count_num = 0;
    ALOGD("SPRD_SR count_num = %d", count_num++);
    if(mType == DisplayDevice::DISPLAY_PRIMARY) {

#ifdef SPRD_SR_DEFER_FREE_FB
        mNewBufferPostCnt=0;//set 0 to enable counting
        mDisplaySurfaceSR.clear();
        mNativeWindowSR.clear();
        mDisplaySurfaceSR=mDisplaySurface;
        mNativeWindowSR=mNativeWindow;
        mSurfaceSR = std::move(mSurface);
        ...
        ALOGD("SPRD SR: newWidth, newHeight: %d, %d", newWidth, newHeight);
        ...
        if (property_get("ro.sf.swrotation", property, NULL) > 0) {
            //displayOrientation
...
void DisplayDevice::setProjection(int orientation,
	...
	if (mEnabledSR) {
	    if(mType == DisplayDevice::DISPLAY_PRIMARY
	        &&(frame.getWidth()*frame.getHeight() != mDisplayWidth*mDisplayHeight)) {
	        if(frame.getWidth()>frame.getHeight()) {
	            frame = Rect(max(mDisplayWidth,mDisplayHeight), min(mDisplayWidth,mDisplayHeight));
	        } else {
	            frame = Rect(min(mDisplayWidth,mDisplayHeight), max(mDisplayWidth,mDisplayHeight));
	        }
	    }
	}
    ...
    mFrame = frame;
	if (mEnabledSR) {
    ALOGI_IF((mType == DisplayDevice::DISPLAY_PRIMARY),"SPRD_SR setProjection(),display_size(%4dx%4d), v:[%d,%d,%d,%d], f:[%d,%d,%d,%d], s:[%d,%d,%d,%d],"
            "transform:[[%0.3f,%0.3f,%0.3f][%0.3f,%0.3f,%0.3f][%0.3f,%0.3f,%0.3f]]\n",
            mDisplayWidth, mDisplayHeight,
            mViewport.left, mViewport.top, mViewport.right, mViewport.bottom,
            mFrame.left, mFrame.top, mFrame.right, mFrame.bottom,
            mScissor.left, mScissor.top, mScissor.right, mScissor.bottom,
            mGlobalTransform[0][0], mGlobalTransform[1][0], mGlobalTransform[2][0],
            mGlobalTransform[0][1], mGlobalTransform[1][1], mGlobalTransform[2][1],
            mGlobalTransform[0][2], mGlobalTransform[1][2], mGlobalTransform[2][2]);
	}
}
