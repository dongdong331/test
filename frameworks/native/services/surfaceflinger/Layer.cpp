...
FloatRect Layer::computeCrop(const sp<const DisplayDevice>& hw) const {
    ...
    crop.right -= insetR;
    crop.bottom -= insetB;
	// ALOGI("SPRD_SR Layer::computeCrop return crop(%f,%f,%f,%f)", crop.left,crop.top,crop.right,crop.bottom);
    return crop;
}
...
half Layer::getAlpha() const {
    const auto& p = mDrawingParent.promote();
    half parentAlpha = 0.0_hf;
    if ((mFlinger->isReverseColorsMode()) &&
            (!strncmp(mName.string(),"com.android.settings",strlen("com.android.settings")) ||
            !strncmp(mName.string(),"Splash Screen",strlen("Splash Screen")))) {
        parentAlpha = 1.0_hf;
    } else {
        parentAlpha = (p != nullptr) ? p->getAlpha() : 1.0_hf;
    }
