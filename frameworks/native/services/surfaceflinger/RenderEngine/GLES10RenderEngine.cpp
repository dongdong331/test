...
namespace impl {
// ---------------------------------------------------------------------------
GLES10RenderEngine::GLES10RenderEngine(uint32_t featureFlags)
      : GLES11RenderEngine(featureFlags) {
}
...
void GLES10RenderEngine::setupLayerBlending(
#ifdef USE_HWC2
    bool premultipliedAlpha, bool opaque, bool disableTexture, const half4& color) {
#else
    ...
    // through.
#ifdef USE_HWC2
    if (CC_UNLIKELY(color.a < 1.0f)) {
        if (premultipliedAlpha) {
            glColor4f(color.a, color.a, color.a, color.a);
        } else {
