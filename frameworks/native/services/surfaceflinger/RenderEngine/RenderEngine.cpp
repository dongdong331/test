...
std::unique_ptr<RenderEngine> RenderEngine::create(int hwcFormat, uint32_t featureFlags) {
    ...
    case GLES_VERSION_1_0:
        ALOGI("GLES10RenderEngine");
         engine = std::make_unique<GLES10RenderEngine>(featureFlags);
        break;
    case GLES_VERSION_1_1:
        ALOGI("GLES11RenderEngine");
         engine = std::make_unique<GLES11RenderEngine>(featureFlags);
        break;
...
void RenderEngine::primeCache() const {
    if(mGlesVersion >= GLES_VERSION_2_0)
