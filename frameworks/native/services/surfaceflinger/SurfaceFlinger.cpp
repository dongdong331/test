    ...
    if (device != nullptr) {
		if (mEnabledSR) {
			if(device->isPrimary()) {
	            ...
	            int configscnt = configs.size();
	            int hwActiveConfig = device->getActiveConfig();

	            //const auto& hwConfigs = mFlinger->getHwComposer().getConfigs(DisplayDevice::DISPLAY_PRIMARY);
	            if(0<=hwActiveConfig && hwActiveConfig<configscnt) {
	                int configw = configs[hwActiveConfig].w;
	                int configh = configs[hwActiveConfig].h;
	                int hww = device->getWidth();
	                int hwh = device->getHeight();
	                if(configw*configh == hww*hwh) {
	                    ALOGI("SPRD_SR PRIMARYDISPLAY getActiveConfig hwActiveConfig:%d validate, (hww:%d hwh:%d)",
	                        hwActiveConfig,hww,hwh);
	                    return hwActiveConfig;
	                } else {
	                    ALOGI("SPRD_SR PRIMARYDISPLAY getActiveConfig hwActiveConfig:%d invalidate,"
	                        "(hww:%d hwh:%d)(configw:%d configh:%d)",
	                        hwActiveConfig,hww,hwh,configw,configh);
	                    return BAD_VALUE;
	                }
	            }
	            return BAD_VALUE;
...
void SurfaceFlinger::setActiveConfigInternal(const sp<DisplayDevice>& hw, int mode) {
	...
	if (mEnabledSR) {
	    if(hw->isPrimary()) {
	        if(mPendingConfig > -1) {
	            ALOGW("SPRD_SR setActiveConfigInternal too frequently! mPendingConfig:%d, mode=%d.",
	                mPendingConfig, mode);
	        }
	        mPendingConfig = mode;
	    }
...
void SurfaceFlinger::setupRenderSurface(
	const sp<DisplayDevice> &disp,	const DisplayDeviceState& state,
    ...
    disp->setDisplaySize(state.width, state.height, dispSurface, nativeWindow, std::move(renderSurface));
    if (maxFrameBufferAcquiredBuffers >= 3) {
        ALOGD("SPRD_SR preallocateBuffers for AnativeWindowSurface");
            ...
            if (j < 0) {
                ...
                if (disp != nullptr) {
                    ...
                    if ((state.orientation != draw[i].orientation) ||
                        ...
                        if (mEnabledSR) {
                            if(mPendingConfig > -1 && disp->isPrimary())
                            {
                                Vector<DisplayInfo> configs;
                                getDisplayConfigs(mBuiltinDisplays[DisplayDevice::DISPLAY_PRIMARY], &configs);
                                if(configs[mPendingConfig].w < 240 || configs[mPendingConfig].h < 240) {
                                    ALOGE("SPRD_SR configs[%d] w%d h%d is illegal",
                                          mPendingConfig, configs[mPendingConfig].w, configs[mPendingConfig].h);
                                    return;//BAD_VALUE
                                }
                                //getHwComposer().setActiveConfig(disp->getDisplayType(), mPendingConfig);
                                ALOGI("SPRD_SR processDisplayChangesLocked configs[%d] w%d h%d",
                                      mPendingConfig, configs[mPendingConfig].w, configs[mPendingConfig].h);
                                DisplayDeviceState& curstate(mCurrentState.displays.editValueAt(j));
                                curstate.width = configs[mPendingConfig].w;
                                curstate.height = configs[mPendingConfig].h;
                                if (curstate.width != draw[i].width || curstate.height != draw[i].height) {
                                    ALOGI("SPRD_SR processDisplayChangesLocked[%d] set display:%zd size(%d,%d) type(%d)",
                                          __LINE__,j,curstate.width,curstate.height, curstate.type);
                                    int32_t hwcId = -1;
                                    ...
                                    setupRenderSurface(disp, curstate, dispSurface, bqProducer);
                                }
                                mPendingConfig = -1;
                            }
                        }
        ...
        const sp<Layer>& parent)
{
    // add this layer to the current state list
    {
        ...
        if (mNumLayers >= 4000) {
            mCurrentState.traverseInZOrder([&](Layer* layer) {
                ALOGE("Too many open layers,dump layer names:[%zu/%zu],%s",mNumLayers,mGraphicBufferProducerList.size(),layer->getName().string());
            });
        }
...
status_t SurfaceFlinger::captureLayers(const sp<IBinder>& layerHandleBinder,
    ...
    int32_t reqWidth = ceilf(crop.width() * frameScale);
    int32_t reqHeight = ceilf(crop.height() * frameScale);
