...
public class ScanResultMatchInfo {
    ...
    public static final int NETWORK_TYPE_PSK_SHA256 = 8;
    public static final int NETWORK_TYPE_EAP_SHA256 = 9;
    ...
    public static ScanResultMatchInfo fromWifiConfiguration(WifiConfiguration config) {
        ...
        if (WifiConfigurationUtil.isConfigForWapiPskNetwork(config)) {
            info.networkType = NETWORK_TYPE_WAPI_PSK;
        } else if (WifiConfigurationUtil.isConfigForWapiCertNetwork(config)) {
            info.networkType = NETWORK_TYPE_WAPI_CERT;
        } else if (WifiConfigurationUtil.isConfigForPskSha256Network(config)) {
            info.networkType = NETWORK_TYPE_PSK_SHA256;
        } else if (WifiConfigurationUtil.isConfigForEapSha256Network(config)) {
            info.networkType = NETWORK_TYPE_EAP_SHA256;
    ...
    public static ScanResultMatchInfo fromScanResult(ScanResult scanResult) {
        ...
        if (ScanResultUtil.isScanResultForWapiPskNetwork(scanResult)) {
            ...
            info.networkType = NETWORK_TYPE_WAPI_CERT;
        } else if (ScanResultUtil.isScanResultForPskSha256Network(scanResult)) {
            info.networkType = NETWORK_TYPE_PSK_SHA256;
        } else if (ScanResultUtil.isScanResultForEapSha256Network(scanResult)) {
            info.networkType = NETWORK_TYPE_EAP_SHA256;
