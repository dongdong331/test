...
public class WifiConfigurationUtil {
    ...
    private static final int GB2312_FLAG = 0x1;
    private static final int GB2312_FLAG_STRING_LENGTH = 2;
    @VisibleForTesting
    ...
    /**
     * Helper method to check if the provided |config| corresponds to a PSK SHA256 network or not.
     */
    public static boolean isConfigForPskSha256Network(WifiConfiguration config) {
        return config.allowedKeyManagement.get(WifiConfiguration.KeyMgmt.PSK_SHA256);
    }

    /**
     * Helper method to check if the provided |config| corresponds to a EAP SHA256 network or not.
     */
    public static boolean isConfigForEapSha256Network(WifiConfiguration config) {
        return (config.allowedKeyManagement.get(WifiConfiguration.KeyMgmt.EAP_SHA256));
    }
    ...
    private static boolean validateSsid(String ssid, boolean isAdd) {
        ...
        if (ssid.startsWith("\"")) {
            // UTF-8 SSID string
            int awardLengthForGB2312 = 0;
            int length = ssid.length();
            if (length > GB2312_FLAG_STRING_LENGTH &&
                ssid.charAt(length - GB2312_FLAG_STRING_LENGTH) == GB2312_FLAG) {
                awardLengthForGB2312 += GB2312_FLAG_STRING_LENGTH;
                Log.d(TAG, "Award for GB2312 SSID");
            }
            ...
            if (ssidBytes.length > (SSID_UTF_8_MAX_LEN + awardLengthForGB2312)) {
