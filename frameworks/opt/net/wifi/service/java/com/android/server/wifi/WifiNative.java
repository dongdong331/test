...
public class WifiNative {
    ...
    public boolean softApStartWpsPinKeypad(String ifaceName, String pin) {
        if (TextUtils.isEmpty(pin)) return false;
        return doHostapdBooleanCommand(ifaceName, "WPS_PIN any " + pin);
    }

    public boolean softApCancelWps(String ifaceName) {
      return doHostapdBooleanCommand(ifaceName, "WPS_CANCEL");
    }

    public boolean softApCheckWpsPin(String ifaceName, String pin) {
       if (TextUtils.isEmpty(pin)) return false;
     String res = doHostapdStringCommand(ifaceName, "WPS_CHECK_PIN " + pin);
    ...
    //<-- Add for SoftAp Advance Feature END
    public void softApGetStations(String ifaceName) {
        doHostapdStringCommand(ifaceName, "GETSTATIONS");
    }
    //<--Add for Softap Feature END

    public boolean softApSetCountry(String ifaceName, String country) {
        if (country == null) {
            Log.e(TAG, "softApSetCountry country == null ");
            return false;
        }
        doHostapdStringCommand(ifaceName, "SET_HOSTAPD_COUNTRY_CODE " + country);
        return true;
    }

    public String softApGetSupportChannel(String ifaceName) {
        String channels = doHostapdStringCommand(ifaceName, "GET_HOSTAPD_SUPPORT_CHANNEL ");
        Log.i(TAG, "softApGetSupportChannel channels = " + channels);
        return channels;
    }
}
