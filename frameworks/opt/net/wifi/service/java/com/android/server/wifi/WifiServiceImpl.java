...
import com.android.server.WcnManagerService;
...
public class WifiServiceImpl extends IWifiManager.Stub {
    ...
    public void checkAndStartWifi() {
        ...
        mContext.registerReceiver(
                new BroadcastReceiver() {
                    private boolean sim1Present = false;
                    private boolean sim2Present = false;
                    @Override
                    public void onReceive(Context context, Intent intent) {
                        String state = intent.getStringExtra(IccCardConstants.INTENT_KEY_ICC_STATE);
                        int slotId = intent.getIntExtra(PhoneConstants.SLOT_KEY, 0);
                        Log.d(TAG, "resetting networks because SIM changed, slotId = " + slotId);
                        if (IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(state)) {
                            ...
                            mWifiStateMachine.resetSimAuthNetworks(sim1Present | sim2Present);
    ...
    public synchronized boolean setWifiEnabled(String packageName, boolean enable)
        ...
        if (!WifiFeaturesUtils.FeatureProperty.SUPPORT_SPRD_WCN) return false;
    ...
    private boolean is5GConfiguration (WifiConfiguration wifiConfig) {
        if (WifiFeaturesUtils.FeatureProperty.SUPPORT_SPRD_SOFTAP_5G_FEATURE) {
            if (wifiConfig != null) {
                if (wifiConfig.apChannel == HOSTAPD_SUPPORT_CHANNEL_INIT
                    || wifiConfig.apChannel > HOSTAPD_SUPPORT_2G_CHANNEL_MAX) {
                    return true;
                }
            } else {
                WifiConfiguration mStoredwifiConfig = getWifiApConfiguration();
                if (mStoredwifiConfig.apChannel == HOSTAPD_SUPPORT_CHANNEL_INIT
                    || mStoredwifiConfig.apChannel > HOSTAPD_SUPPORT_2G_CHANNEL_MAX) {
                    return true;
                }
            }
        }
    ...
    public boolean startSoftAp(WifiConfiguration wifiConfig) {
        ...
        // if lte & softap coexist, and softap will start with 2.4G channel, change the apChannel to the ideaChannel if exist.
        if (WifiFeaturesUtils.FeatureProperty.SUPPORT_SPRD_SOFTAP_COEXIST_LTE) {
            if (wifiConfig == null) wifiConfig = getWifiApConfiguration();
            if (wifiConfig.apChannel > HOSTAPD_SUPPORT_CHANNEL_INIT
                    && wifiConfig.apChannel <= HOSTAPD_SUPPORT_2G_CHANNEL_MAX) {
                try{
                    int ideaChannel = WcnManagerService.create(mContext).calculateWifiApIdeaChannel();
                    if (ideaChannel != -1) wifiConfig.apChannel = ideaChannel;
                } catch(Exception e) {
                    Slog.e(TAG, "Trying to get ideaChannel, but a exception: " + e);
                }
            }
        }
    ...
    public boolean registerWifiNetworkEventObserver(IWifiNetworkEventObserver observer) {
        ...
        return mWifiStateMachine.registerWifiNetworkEventObserver(observer);
    }
    ...
    public boolean unregisterWifiNetworkEventObserver(IWifiNetworkEventObserver observer) {
        ...
        return mWifiStateMachine.unregisterWifiNetworkEventObserver(observer);
    }
    ...
    public int getCurrentRssi() {
        ...
        if (mWifiStateMachineChannel != null) {
            return mWifiStateMachine.syncGetCurrentRssi(mWifiStateMachineChannel);
        } else {
            Log.e(TAG, "mWifiStateMachineChannel is not initialized");
    ...
    public boolean isNetworkValidForVoWifi() {
        ...
        return mWifiStateMachine.isNetworkValidForVoWifi();
    }
    ...
    public boolean setCheckingInternetForVoWifi(boolean enabled) {
        ...
        return mWifiStateMachine.setCheckingInternetForVoWifi(enabled);
    }
...
public boolean softApWpsCheckPin(String wpsPin, String packageName) {
    ...
    return mWifiStateMachine.syncSoftApWpsCheckPin(mWifiStateMachineChannel, wpsPin);
}
//<-- Add for softap support wps connect mode and hidden ssid Feature END
    ...
    public boolean softApBlockStation(String mac, String packageName) {
        ...
        return mWifiStateMachine.syncSoftApBlockStation(mWifiStateMachineChannel, mac);
    }
 ...
 //To ublock the statition
    public boolean softApUnblockStation(String mac, String packageName) {
        ...
        return mWifiStateMachine.syncSoftApUnblockStation(mWifiStateMachineChannel, mac);
    }

 //get the current connected station, return a string with format:
 //XX:XX:XX:XX:XX:XX XX:XX:XX:XX:XX:XX ...
 //each mac string separate with a blank
    public String softApGetConnectedStations(String packageName) {
        if (enforceChangePermission(packageName) != MODE_ALLOWED) {
            return null;
        }
        return mWifiStateMachine.syncSoftApGetConnectedStations(mWifiStateMachineChannel);
    }

 //get the current blocked station, return a string with format:
 //XX:XX:XX:XX:XX:XX XX:XX:XX:XX:XX:XX ...
 //each mac string separate with a blank
    public String softApGetBlockedStations(String packageName) {
        if (enforceChangePermission(packageName) != MODE_ALLOWED) {
            return null;
        }
        return mWifiStateMachine.syncSoftApGetBlockedStations(mWifiStateMachineChannel);
    }
  ...
  */
    public List<String> softApGetConnectedStationsDetail(String packageName) {
        ...
        return mWifiStateMachine.syncSoftApGetConnectedStationsDetail(mWifiStateMachineChannel);
    }
  ...
  */
    public List<String> softApGetBlockedStationsDetail(String packageName) {
        ...
        return mWifiStateMachine.syncSoftApGetBlockedStationsDetail(mWifiStateMachineChannel);
    }
  ...
  */
    public boolean softApAddClientToWhiteList(String mac, String name, String packageName) {
        ...
        return mWifiStateMachine.syncSoftApAddClientToWhiteList(mWifiStateMachineChannel, mac, name);
    }
  ...
  */
    public boolean softApDelClientFromWhiteList(String mac, String name, String packageName) {
        ...
        return mWifiStateMachine.syncSoftApDelClientFromWhiteList(mWifiStateMachineChannel, mac, name);
    }
  ...
  */
    public boolean softApSetClientWhiteListEnabled(boolean enabled, String packageName) {
        ...
        return mWifiStateMachine.syncSoftApSetClientWhiteListEnabled(mWifiStateMachineChannel, enabled);
    }
  ...
  */
    public List<String> softApGetClientWhiteList(String packageName) {
        ...
        return mWifiStateMachine.syncSoftApGetClientWhiteList();
    }
  ...
  */
    public boolean softApIsWhiteListEnabled(String packageName) {
        ...
        return mWifiStateMachine.syncSoftApIsWhiteListEnabled();
    }
}
