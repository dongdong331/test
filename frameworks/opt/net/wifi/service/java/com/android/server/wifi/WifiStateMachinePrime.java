...
public class WifiStateMachinePrime {
    ...
    private static final int MOBILE_DATA_TYPE = 0;
    private static final int WIFI_DATA_TYPE = 1;
    private static final int MOBILE_DATA_ENABLE = 1;
    private static int SOFTAP_CHANNEL_ORIGINAL_STATE = 0;
    ...
    private boolean mIsSupportCucc = false;
    /**
    ...
    WifiStateMachinePrime(WifiInjector wifiInjector,
        ...
        mIsSupportCucc = mWifiFeaturesUtils.isSupportCUCC();
        if (mIsSupportCucc) {
            Log.d(TAG, "CUCC operator");
            registerPreferNetworkObserver();
        }
    }
        ...
        public void onStateChanged(int state, int reason) {
            if (state == WifiManager.WIFI_AP_STATE_DISABLED) {
                Global.putInt(mContext.getContentResolver(),
                        Global.SOFTAP_CHANNEL_UPDATE_FLAG, SOFTAP_CHANNEL_ORIGINAL_STATE);
    ...
    private void startSoftAp(SoftApModeConfiguration softapConfig) {
        ...
        ActiveModeManager manager = mWifiInjector.makeSoftApManager(mWifiInjector, callback, softapConfig);
    ...
    private void registerPreferNetworkObserver() {
        final FrameworkFacade facade = mWifiInjector.getFrameworkFacade();
        final WifiStateMachine wifiStateMachine = mWifiInjector.getWifiStateMachine();

        facade.registerContentObserver(mContext, Global.getUriFor(Global.NETWORK_PREFERENCE), false,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                        int networktype = facade.getIntegerSetting(mContext,
                                Global.NETWORK_PREFERENCE, WIFI_DATA_TYPE);
                        Log.d(TAG, "CUCC, networktype is changed to" + networktype);
                        if(networktype == MOBILE_DATA_TYPE) {
                            enterScanOnlyMode();
                        } else {
                            enterClientMode();
                        }
        ...
        facade.registerContentObserver(mContext, Global.getUriFor(Global.MOBILE_DATA), false,
                new ContentObserver(mHandler) {
                    @Override
                    public void onChange(boolean selfChange) {
                        int mobileDataStatus = facade.getIntegerSetting(mContext,
                        ...
                        int networktype = facade.getIntegerSetting(mContext,
                                Global.NETWORK_PREFERENCE, WIFI_DATA_TYPE);
                        Log.d(TAG, "CUCC, Mobile status changed, Mobile data status is "
                                + mobileDataStatus + "networktype is " + networktype);

                        if(networktype == MOBILE_DATA_TYPE) {
                            if(mobileDataStatus == MOBILE_DATA_ENABLE){
                                enterScanOnlyMode();
                            } else {
                                enterClientMode();
                            }
