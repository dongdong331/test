...
public class DefaultPhoneNotifier implements PhoneNotifier {
    ...
    public void notifyServiceState(Phone sender) {
        ...
        if (ss == null) {
            Rlog.d(LOG_TAG,"ss is null");
            ss = new ServiceState();
            ss.setStateOutOfService();
        }
        try {
            if (mRegistry != null) {
                /* For SubsidyLock feature: slc demand, when broadcast full service, the default subId must be active@{ */
                if (Resources.getSystem().getBoolean(com.android.internal.R.bool.config_subsidyLock) && (ss.getVoiceRegState() == ServiceState.STATE_IN_SERVICE || 
                        ((ss.getDataRegState() == ServiceState.STATE_IN_SERVICE)&&(ss.getRilDataRadioTechnology() != ServiceState.RIL_RADIO_TECHNOLOGY_IWLAN)))) {
                    Rlog.d(LOG_TAG, "in service, should check the default data subId");
                    int dds = SubscriptionManager.getDefaultDataSubscriptionId();
                    if (SubscriptionController.getInstance().isActiveSubId(dds)) {
                        mRegistry.notifyServiceStateForPhoneId(phoneId, subId, ss);
                    } else {
                        Rlog.d(LOG_TAG, "dds " + dds + " is not active, not to notify full service ");
                    }
                } /* @} */else {
                    mRegistry.notifyServiceStateForPhoneId(phoneId, subId, ss);
                }
