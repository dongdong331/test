...
public abstract class InboundSmsHandler extends StateMachine {
    ...
    private void handleNewSms(AsyncResult ar) {
	    ...
	    if (ar.userObj !=null){
	       String auth = (String)ar.userObj;
               logd("handleNewSms: auth = " + auth);
	    }
        }catch(Exception ex){
	    loge("handleNewSms ar.userobj  hehe  convert error", ex);
        }
    ...
    private void handleInjectSms(AsyncResult ar) {
	    ...
	    if (ar.userObj !=null){
	       String auth = (String)ar.userObj;
               logd("handleInjectSms: auth = " + auth);
	    }
        }catch(Exception ex){
	    loge("handleInjectSms ar.userobj  hehe  convert error", ex);
        }
    ...
    private boolean processMessagePart(InboundSmsTracker tracker) {
        ...
        if (!mUserManager.isUserUnlocked()&&!block) {
