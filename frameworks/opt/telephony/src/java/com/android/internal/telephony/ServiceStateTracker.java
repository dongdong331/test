...
public class ServiceStateTracker extends Handler {
    ...
    protected void handlePollStateResult(int what, AsyncResult ar) {
        ...
        if (ar.exception != null) {
            ...
            if (err == CommandException.Error.RADIO_NOT_AVAILABLE) {
                ...
                /* UNISOC: [bug713284] Make sure service state can be set as POWER_OFF without delay. @{ */
        ...
        switch (what) {
            ...
            case EVENT_POLL_STATE_OPERATOR: {
                if (mPhone.isPhoneTypeGsm()) {
                    ...
                    // UNISOC Add for Bug814455. Update operator name for Dual VoLTE DSDA:
            ...
            if (configLoader != null) {
                try {
                    ...
                    if (alwaysOnHomeNetwork(b)) {
                        ...
                        roaming = false;
                    /* Unisoc: bug691317 Free Carrier Ignore domestic roaming. @{ */
                    } else if (isNonRoamingInDomesticNetwork(b, mNewSS.getOperatorNumeric())) {
    ...
    /* Unisoc: bug691317 Free Carrier Ignore domestic roaming for specific country. @{ */
    protected final boolean isNonRoamingInDomesticNetwork(BaseBundle b, String network) {
        return isNonDomesticRoamingEnabled(b) && inSameCountry(network);
    }

    protected final boolean isNonDomesticRoamingEnabled(BaseBundle b) {
        boolean retVal = b.getBoolean(CarrierConfigManagerEx.KEY_SUPPORT_NON_DOMESTIC_ROAMING);
        if (DBG) {
    ...
    protected void updateSpnDisplay() {
        ...
        if (mPhone.isPhoneTypeGsm()) {
            ...
            //UNISOC:Translation service provider name
            String simNumeric = (iccRecords != null) ? iccRecords.getOperatorNumeric() : "";
            ...
            if(plmn!= null && plmn.equals(Resources.getSystem().
    ...
    private void pollStateDone() {
        ...
        if (hasChanged) {
            ...
            //UNISOC:Translation service provider name
            String networkOperatorName = TeleUtils.translateOperatorName(mSS.getOperatorNumeric(),mSS.getOperatorAlpha());
            tm.setNetworkOperatorNameForPhone(mPhone.getPhoneId(), /*mSS.getOperatorAlpha()*/networkOperatorName);
    ...
    protected int getCombinedRegState() {
        //UNISOC:Modify for Bug928841,When registered on VoWiFi if ServiceState is no service or emergency,don't show operator name.
        ServiceState serviceState = mSS;
        Phone imsPhone = mPhone.getImsPhone();
        if (imsPhone != null && imsPhone.isWifiCallingEnabled()) {
            log("getCombinedRegState: WifiCallingEnabled,save service state as ims service state");
            serviceState = imsPhone.getServiceState();
        }
