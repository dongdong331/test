...
public class SubscriptionController extends ISub.Stub {
    ...
    static final boolean DBG_CACHE = true;
    ...
    // UNISOC: add cookie for slotIdx2SubId, see bug #565648.
    private static Map<Integer, Integer> sSlotIdxToSubIdCookie =
            new ConcurrentHashMap<Integer, Integer>();
    ...
    private boolean isSubInfoReady() {
        // UNISOC: query temp slotIdxToSubId cookie, see bug #681014.
        return sSlotIndexToSubId.size() > 0 || sSlotIdxToSubIdCookie.size() > 0;
    }
    ...
    public void refreshCachedActiveSubscriptionInfoList() {
        ...
        synchronized (mCacheActiveSubInfoList) {
            ...
            if (activeSubscriptionInfoList != null) {
                //UNISOC: Sort subInfolists from small to large according to slotId
    ...
    public int addSubInfoRecord(String iccId, int slotIndex) {
        ...
        try {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    do {
                        ...
                        if (currentSubId == null
                            ...
                            /* UNISOC: Add for Bug588429 mcc/mnc performance optimization @{ */
                            int defaultVoiceSubId = getDefaultVoiceSubId();
                            boolean isSingleSIM = getActiveSubInfoCount(mContext.getOpPackageName()) == 1;
                            boolean isDefaultId = subId == defaultVoiceSubId &&
                                    subId != SubscriptionManager.MAX_SUBSCRIPTION_ID_VALUE;
                            if (DBG) {
                                logdl("defaultVoiceSubId=" + defaultVoiceSubId
                                        + "isSingleSIM=" + isSingleSIM
                                        + "isDefaultId=" + isDefaultId);
                            }
                            if (!SubscriptionManager.isValidSubscriptionId(defaultSubId)
                                    || subIdCountMax == 1 || isDefaultId || isSingleSIM) {
            ...
            * So trigger broadcast operation as siminfo is available. @{ */
            ...
            // UNISOC: clear slotIdx2SubId cookie, see bug #565648.
            clearSlotIdToSubIdCookie(slotIndex);
        } finally {
            Binder.restoreCallingIdentity(identity);
        }
        return 0;
    ...
    /* UNISOC:clear slotIdx2SubId cookie, see bug #565648. @{ */
    protected void clearSlotIdToSubIdCookie(int slotIndex) {
        if (sSlotIndexToSubId.size() == mTelephonyManager.getPhoneCount()) {
            sSlotIdxToSubIdCookie.clear();
        } else {
            if (sSlotIdxToSubIdCookie.containsKey(slotIndex)) {
                sSlotIdxToSubIdCookie.remove(slotIndex);
            }
            int otherSlotId = slotIndex == 0 ? 1 : 0;
            int simState = getSimStateForSlotIndex(otherSlotId);
    ...
    private int setCarrierText(String text, int subId) {
        ...
        try {
            ...
            /* UNISOC:Bug708681,catch SQLiteDiskIOException and SQLiteFullException @{ */
        } catch (SQLiteDiskIOException e) {
            loge("SQLiteFullException:  " + e.toString());
        } catch (SQLiteFullException e) {
            loge("SQLiteDiskIOException:  " + e.toString());
        } finally {
            Binder.restoreCallingIdentity(identity);
            return result;
            /* @} */
    ...
    public int getSlotIndex(int subId) {
        ...
        if (size == 0)
        {
            /* UNISOC: query slotIdx2SubId cookie, see bug #565648. @{ */
            int cookieSize = sSlotIdxToSubIdCookie.size();
            if (cookieSize > 0) {
                for (Entry<Integer, Integer> entry: sSlotIdxToSubIdCookie.entrySet()) {
                    int sim = entry.getKey();
                    int sub = entry.getValue();

                    if (subId == sub) {
                        if (DBG) logdl("[getSlotIndex]- found subId=" + subId + " phoneId=" + sim + " from temporary COOKIE");
                        return sim;
                    }
                }
            }
        ...
        /* UNISOC: query slotIdx2SubId cookie, see bug #640724. @{ */
        int cookieSize = sSlotIdxToSubIdCookie.size();
        if (cookieSize > 0) {
            for (Entry<Integer, Integer> entry : sSlotIdxToSubIdCookie.entrySet()) {
                int sim = entry.getKey();
                ...
                if (subId == sub) {
                    if (DBG) logdl("[getSlotIndex]- found subId="
                            + subId + " getSlotIndex=" + sim + " from temporary COOKIE");
                    return sim;
                }
            }
        }
    ...
    public int[] getSubId(int slotIndex) {
        ...
        int size = sSlotIndexToSubId.size();
        if (size == 0) {
            /* UNISOC: query slotIdx2SubId cookie, see bug #813043. @{ */
            int cookieSize = sSlotIdxToSubIdCookie.size();
            if (cookieSize > 0) {
                int tempArraySize = 1;
                for (Entry<Integer, Integer> entry: sSlotIdxToSubIdCookie.entrySet()) {
                    ...
                    int sub = entry.getValue();
                    if (slotIndex == sim) {
                        int[] tempSubIdArr = new int[tempArraySize];
                        tempSubIdArr[0] = sub;
                        if (DBG) logdl("[getSubId]- found slotIndex="
                                + slotIndex + " subId=" + sub + " from temporary COOKIE");
                        return tempSubIdArr;
                    }
                }
            }
    ...
    public int getPhoneId(int subId) {
        ...
        if (size == 0) {
            ...
            if (cookieSize > 0) {
                for (Entry<Integer, Integer> entry: sSlotIdxToSubIdCookie.entrySet()) {
                    ...
                    if (subId == sub) {
                        if (DBG) logdl("[getPhoneId]- found subId=" + subId + " phoneId=" + sim + " from temporary COOKIE");
                        return sim;
                    }
                }
            }
            /* @} */
            // UNISOC:[708974] It's inappropriate to return DEFAULT_PHONE_INDEX when the giving subId is not active.
        ...
        if (cookieSize > 0) {
            for (Entry<Integer, Integer> entry : sSlotIdxToSubIdCookie.entrySet()) {
                ...
                if (subId == sub) {
                    if (DBG) logdl("[getPhoneId]- found subId="
                            + subId + " phoneId=" + sim + " from temporary COOKIE");
                    return sim;
                }
            }
        }
    ...
    public int clearSubInfo() {
        ...
        try {
            ...
            /* UNISOC: refresh slotIdx2SubId cookie, see bug #565648. @{ */
            sSlotIdxToSubIdCookie.clear();
            sSlotIdxToSubIdCookie.putAll(sSlotIndexToSubId);
            /* @} */
