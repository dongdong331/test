...
public class SubscriptionInfoUpdater extends Handler {
    ...
    //UNISOC: Send broadcast of dual card loaded,even one of the card was pin locked
    ...
    private void handleSimLoaded(int slotId) {
        ...
        if (isAllIccIdQueryDone()) {
            ...
            for (int subId : subIds) {
                ...
                /* UNISOC: Send broadcast of dual card loaded, even one of the card was pin locked */
                if (!mSubscriptionManager.isValidPhoneId(slotId)
                        || !IccCardConstants.INTENT_VALUE_ICC_LOADED.equals(mSimState[slotId])) {
                    continue;
                }
                /* @} */
    ...
    synchronized private void updateSubscriptionInfoByIccId() {
        ...
        for (int i = 0; i < PROJECT_SIM_NUM; i++) {
            if (mInsertSimState[i] == SIM_NOT_INSERT) {
                // UNISOC: clear slotIdx2SubId cookie, see bug #902776.
                SubscriptionController.getInstance().clearSlotIdToSubIdCookie(i);
        ...
        //SPRD: [bug918124] No need to set default data subId here. Instead, let PrimaryCardController handle it.
