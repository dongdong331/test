            ...
            synchronized (mLock) {
                ...
                for (int phoneId = 0; phoneId < mPhoneSubId.length; phoneId++) {
                    ...
                    if (oldSubId != newSubId) {
                        ...
                        if(Resources.getSystem().getBoolean(com.android.internal.R.bool.config_subsidyLock) && SubscriptionManager.from(mContext).isActiveSubId(mDefaultDataSubId)) {
                            log("update serviceState for Default Data SubId Changed");
                            for (int i = 0; i < mPhoneSubId.length; i++){
                                ((GsmCdmaPhone) PhoneFactory.getPhone(i)).notifyServiceStateChanged();
                            }
                        }
                        /* @} */
                    }
