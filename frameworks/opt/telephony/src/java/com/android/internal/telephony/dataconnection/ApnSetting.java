 ...
 */
public class ApnSetting extends AbsApnSetting {
    ...
    public static final int Apn_Types_XCAP = 0x400;
    ...
    public static boolean mvnoMatches(IccRecords r, String mvnoType, String mvnoMatchData) {
        if (mvnoType.equalsIgnoreCase("spn")) {
            if (r instanceof SIMRecords) {
                /* UNISOC: Bug 907188 Get SPN value from sim card @{
                if ((r.getServiceProviderName() != null) &&
                        r.getServiceProviderName().equalsIgnoreCase(mvnoMatchData)) {
                    return true;
                }
                */
                String spn = ((SIMRecords) r).getOriginalServiceProviderName();
                Rlog.d(LOG_TAG, "mvnoMatches() oldInferfaceSpn = " + " spn = "
        ...
        //to fix
        /* UNISOC: [bug607736] Add for mvno match type PNN. @{ */
        else if (mvnoType.equalsIgnoreCase("pnn")) {
            if (r instanceof SIMRecords) {
                String pnn = ((SIMRecords) r).getPnnHomeName();
    ...
    private static int getApnBitmask(String apn) {
        switch (apn) {
            ...
            case PhoneConstants.APN_TYPE_XCAP: return Apn_Types_XCAP;
