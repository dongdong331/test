...
public class DataConnection extends StateMachine {
    ...
    private void checkSetMtu(ApnSetting apn, LinkProperties lp) {
        ...
        if (carrierConfig != null) {
            config = carrierConfig.getConfigForPhoneId(mPhone.getPhoneId());
            if (config != null){
                mtuForIpv6 = config.getInt(CarrierConfigManagerEx.KEY_MTU_CONFIG_FOR_IPV6, 0);
            }
        }
        if (mtuForIpv6 != PhoneConstants.UNSET_MTU) {
            log ("MTU set by CarrierConfig to mtuForIpv6: "+ mtuForIpv6);
        ...
        if (config != null){
            mtu = config.getInt(CarrierConfigManagerEx.KEY_MTU_CONFIG, 1500);
        }

        if (mtu != PhoneConstants.UNSET_MTU) {
            if (DBG) log("MTU set by CarrierConfig to mtu: " + mtu);
            lp.setMtu(mtu);
        }

/*        mtu = mPhone.getContext().getResources().getInteger(
    ...
    private SetupResult onSetupConnectionCompleted(@DataServiceCallback.ResultCode int resultCode,
        ...
        if (cp.mTag != mTag) {
            ...
            //UNISOC:If data connection support xcap, send xcapip command to modem
            if (cp.mApnContext.getApnSetting().canHandleType(PhoneConstants.APN_TYPE_XCAP)) {
                String ipv4Address = "0.0.0.0";
                String ipv6Address = "FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF";
                String xcapIpCommand;
                if (response.getAddresses().size() > 0) {
                    for (LinkAddress la : response.getAddresses()) {
                        if (la.isIPv4()) {
                            ipv4Address = la.getAddress().getHostAddress();
                            log("IPv4 address is " + ipv4Address);
                        } else if (la.isIPv6()) {
                            ipv6Address = la.getAddress().getHostAddress();
                            log("IPv6 address is " + ipv6Address);
                        }
                    }
                    xcapIpCommand = "AT+XCAPIP=" + String.valueOf(mCid) + ",\"" + ipv4Address + "," + "[" + ipv6Address + "]\"";
                    log("XCAPIP AT command is: " + xcapIpCommand);
                    RadioInteractor radioInteractor = new RadioInteractor(mPhone.getContext());
                    radioInteractor.sendAtCmd(xcapIpCommand, null, mPhone.getPhoneId());
                }
            }
        }
        ...
        if (sst.getCurrentDataConnectionState() != ServiceState.STATE_IN_SERVICE) {
            ...
            if (otherPhone != null && otherPhone.getState() != PhoneConstants.State.IDLE ) {
                ...
                if (sst.isConcurrentVoiceAndDataAllowed() == false) {
                    /* UNISOC: bug559552 data icon shown under GSM when make emergency call @{ */
