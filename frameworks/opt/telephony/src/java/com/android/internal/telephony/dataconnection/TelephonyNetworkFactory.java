...
public class TelephonyNetworkFactory extends NetworkFactory {
    ...
    private void applyRequests(HashMap<NetworkRequest, LocalLog> requestMap, boolean action,
            String logStr) {
        boolean isDefaultRequest = false;
        for (NetworkRequest networkRequest : requestMap.keySet()) {
            LocalLog localLog = requestMap.get(networkRequest);
            localLog.log(logStr);
            //UNISOC: FL1000060339
            isDefaultRequest = networkRequest.networkCapabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET);
            if (action == REQUEST && (isDefaultRequest || !mDcTracker.getPsRejectProp())) {
    ...
    private void onNeedNetworkFor(Message msg) {
        ...
        //UNISOC: FL1000060339
        isDefaultRequest = networkRequest.networkCapabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET);
        if (mIsActive && isApplicable && (isDefaultRequest || !mDcTracker.getPsRejectProp())) {
