...
public class UiccCardApplication {
    ...
    public void update (IccCardApplicationStatus as, Context c, CommandsInterface ci) {
        synchronized (mLock) {
            ...
            /*@{*/
            if (mPersoSubState != oldPersoSubState && (mPersoSubState.isSimlockPersoSubState())) {
                notifyNetworkLockedRegistrantsIfNeeded(null);
            }
            /*@}*/
...
//                mPersoSubState == PersoSubState.PERSOSUBSTATE_SIM_NETWORK) {
        if (mAppState == AppState.APPSTATE_SUBSCRIPTION_PERSO && (mPersoSubState.isSimlockPersoSubState())) {
            if (r == null) {
