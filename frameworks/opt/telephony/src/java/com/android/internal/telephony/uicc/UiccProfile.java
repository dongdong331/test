...
public class UiccProfile extends IccCard {
    ...
    private void handleCarrierNameOverride() {
        ...
        String updateSimOperatorName = TeleUtils.translateOperatorName(mTelephonyManager.getSimOperatorNumericForPhone(mPhoneId), simOperatorName);
        if (!updateSimOperatorName.equals(simOperatorName)) {
    ...
    public void updateExternalState() {
        ...
        if (pin1State == PinState.PINSTATE_ENABLED_PERM_BLOCKED) {
            ...
            if (appState == IccCardApplicationStatus.AppState.APPSTATE_PIN) {
                ...
                if(persoSubState.isSimlockPersoSubState()){
            ...
            if (mExternalState == IccCardConstants.State.LOADED) {
                ...
                if (mIccRecords != null) {
                    ...
                    if (!TextUtils.isEmpty(operator)) {
                        ...
                        //UNISOC:Translation service provider name
                        String simOperatorName = mTelephonyManager.getSimOperatorNameForPhone(mPhoneId);
                        String updateSimOperatorName = TeleUtils.translateOperatorName(operator, simOperatorName);
                        if (!updateSimOperatorName.equals(simOperatorName)) {
                            mTelephonyManager.setSimOperatorNameForPhone(mPhoneId, updateSimOperatorName);
                        }
                    } else {
    ...
    public void update(Context c, CommandsInterface ci, IccCardStatus ics) {
        synchronized (mLock) {
            ...
            //if stk app is removed,no need to create catService
    ...
    /**
     * Returns true if stk app is installed;
     * if stk app is removed,no need to create catService
     */
