...
public class PduComposer {
    ...
    public byte[] make() {
        ...
        switch (type) {
            case PduHeaders.MESSAGE_TYPE_SEND_REQ:
                if (makeSendReqPdu(false) != PDU_COMPOSE_SUCCESS) {
                    return null;
                }
                break;
            ...
            case PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF:
                int res = makeSendReqPdu(true);
                if ( res!= PDU_COMPOSE_SUCCESS) {
                    Log.e("MmsBackup", "makeSendReqPdu(true) res is "+res);
                    return null;
                }
                break;
            default:
     ...
     * Make Send.req.
     */
    private int makeSendReqPdu(boolean forBackup) {
        if (mMessage == null) {
            mMessage = new ByteArrayOutputStream();
            mPosition = 0;
        }
        ...
        //  Message body
        return makeMessageBody(forBackup);
    }

    /**
     * Make message body.
     */
    private int makeMessageBody(boolean forBackup) {
