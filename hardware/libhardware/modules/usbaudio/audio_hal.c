...
{
    ALOGI("adev_open_output_stream() handle:0x%X, devicesSpec:0x%X, flags:0x%X, addr:%s",
    ...
    profile_read_out_device_info(&out->adev->out_profile);
    ...
    device_unlock(out->adev);
    ALOGI("%s %d sample_rate:%d :%d",__func__,__LINE__,config->sample_rate,out->adev->device_sample_rate);
    /* Format */
...
{
    ALOGI("adev_open_input_stream() rate:%" PRIu32 ", chanMask:0x%" PRIX32 ", fmt:%" PRIu8,
    ...
    ALOGI("%s %d sample_rate:%d :%d",__func__,__LINE__,config->sample_rate,in->adev->device_sample_rate);
