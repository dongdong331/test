...
static const struct sprd_cpufreqhw_info sprd_cpufreqhw_info_1h10 = {
	.type = SPRD_CPUFREQHW_SHARKL3,
	.def_freq0 = 2,
	.def_freq1 = 3,
	.def_freq2 = 3,
};

static const struct sprd_cpufreqhw_info sprd_cpufreqhw_info_3h10 = {
	.type = SPRD_CPUFREQHW_SHARKL3_3H10,
	.def_freq0 = 2,
	.def_freq1 = 3,
	.def_freq2 = 3,
};
...
struct sprd_cpufreqhw {
	...
	const struct sprd_cpufreqhw_info *info;
...
static const struct of_device_id sprd_cpufreqhw_of_match[] = {
	{
	 ...
	 .data = (void *)&sprd_cpufreqhw_info_1h10,
	 },
	{
	 .compatible = "sprd,sharkl3-cpufreqhw-3h10",
	 .data = (void *)&sprd_cpufreqhw_info_3h10,
	 },
};
				      ...
				      const char *buf, size_t n)
{
	...
	if (en == 0 &&
	!hwdvfs_trigger_lit(cpufreqhw->info->def_freq0,
			true, true) &&
	!hwdvfs_trigger_big_scu(cpufreqhw->info->def_freq1,
			true, true)) {
...
static int sprd_cpufreqhw_init_param(struct device_node *np)
{
	...
	cfg[0] = cpufreqhw->info->def_freq0 ? 0x06 : 0x02;
	_dvfs_wr_(VAL2REG(0x00, BIT_DVFS_CTRL_FCFG_PD_SW_CHNL00) |
		  ...
		  BIT_DVFS_CTRL_FSEL_SW_CHNL00(cfg[0]),
		  REG_DVFS_CTRL_CFG_CHNL00);

	cfg[0] = cpufreqhw->info->def_freq1 ? 0x07 : 0x02;
	_dvfs_wr_(VAL2REG(0x00, BIT_DVFS_CTRL_FCFG_PD_SW_CHNL01) |
		  ...
		  BIT_DVFS_CTRL_FSEL_SW_CHNL01(cfg[0]),
		  REG_DVFS_CTRL_CFG_CHNL01);

	cfg[0] = cpufreqhw->info->def_freq2 ? 0x05 : 0x02;
	_dvfs_wr_(VAL2REG(0x00, BIT_DVFS_CTRL_FCFG_PD_SW_CHNL02) |
		  ...
		  BIT_DVFS_CTRL_FSEL_SW_CHNL02(cfg[0]),
...
unsigned int sprd_cpufreqhw_get(int cluster)
{
	...
	cpufreqhw->triggered[cluster]) {
		...
		if (cluster == CPUFREQHW_CHNL00)
			regval = cpufreqhw->info->def_freq0;
		else if (cluster == CPUFREQHW_CHNL01)
			regval = cpufreqhw->info->def_freq1;
		else if (cluster == CPUFREQHW_CHNL02)
			regval = cpufreqhw->info->def_freq2;
	}
...
static int sprd_cpufreqhw_probe(struct platform_device *pdev)
{
	...
	cpufreqhw->info = info;
	pr_info("default freq %u %u %u\n",
		info->def_freq0, info->def_freq1, info->def_freq2);
