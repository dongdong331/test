...
static int isp_k_common_block(struct isp_io_param *param, uint32_t com_idx)
{
	int ret = 0;
	unsigned int val = 0;
	...
	ISP_REG_MWR(com_idx, ISP_COMMON_SPACE_SEL,
			3, common_info.fetch_color_space_sel);

	ISP_REG_MWR(com_idx, ISP_COMMON_SPACE_SEL, 3 << 2,
		...
		struct isp_k_block *isp_k_param, uint32_t com_idx)
{
	...
	case ISP_PRO_COMMON_BLOCK:
		ret = isp_k_common_block(param, com_idx);
		break;
