...
static void sprdbat_adapt_plugout_recheck_monitor(void)
{
	if((sprdbat_data->bat_info.bat_current < 0)&&
		sprdchg_read_vchg_vol() < sprdbat_read_vbat_vol()){
		SPRDBAT_DEBUG("usb adapt plug but vchg is high\n");
		sprdbat_data->stop_charge();
	}
	return;
}
...
static void sprdbat_charge_works(struct work_struct *work)
{
	...
	sprdbat_adapt_plugout_recheck_monitor();
	mutex_unlock(&sprdbat_data->lock);
