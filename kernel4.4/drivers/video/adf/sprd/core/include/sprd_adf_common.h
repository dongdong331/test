...
/**
 * sprd adf platform id
 *
 * @SPRD_PLATFORM_SPRD: sprd
 * @SPRD_N_PLATFORM: the max number platform that sprd can support
 */
enum {
	SPRD_PLATFORM_SPRD = 0xFF,
	SPRD_N_PLATFORM,
};
 ...
 * @get_modes: get supported modelist(drm mode)
 * @set_mode: set new display mode(drm mode)
 */
struct sprd_adf_interface_ops {
	...
	int (*get_modes)(struct sprd_adf_interface *intf,
			struct drm_mode_modeinfo *mode, size_t *n_modes);
	int (*set_mode)(struct sprd_adf_interface *intf,
			struct drm_mode_modeinfo *mode);
	int (*get_mode)(struct sprd_adf_interface *intf, u32 *index);
};
...
struct sprd_adf_device_ops {
	...
	struct sprd_restruct_config *
		(*restruct_post_config)(struct adf_post *post);

	void (*free_restruct_config)(struct sprd_adf_interface *intf,
