...
public final class Daemons {
    ...
    /*start fixed bug656139 finalize timeout*/
    private static final long MAX_FINALIZE_NANOS2 = 30L * NANOS_PER_SECOND;
    /*end fixed bug656139 finalize timeout*/
        ...
        private Object waitForFinalization() {
            ...
            if (getNeedToWork() && FinalizerDaemon.INSTANCE.progressCounter.get() == startCount) {
                ...
                if (getNeedToWork()
                    ...
                    /*start fixed bug656139 finalize timeout*/
                    if(finalizing!=null){
                       //for bug656139, if 10s timeout, finalizer not finished, do another 30s wait
                       System.logE("Daemons waitForFinalization for 10seconds, try another 30seconds");
                       sleepFor(MAX_FINALIZE_NANOS2);
                       //for bug: 824091  after aleep another MAX_FINALIZE_NANOS2 seconds
                       //we should check if the current finalizer is still the old by startCount
                       //if has been a new one, should return null.
                       //if still the old, return the old object.
                       //finalizing = FinalizerDaemon.INSTANCE.finalizingObject;
                       if(!getNeedToWork() || FinalizerDaemon.INSTANCE.progressCounter.get() != startCount){
                            System.logE("Daemons waitForFinalization for 10seconds, after 30seconds finalizing and the old object finished");
                            return null;
                       }
                       System.logE("Daemons waitForFinalization for 10seconds, after 30seconds finalizing="+finalizing);
                    }
                    /*end fixed bug656139 finalize timeout*/
                    return finalizing;
                }
