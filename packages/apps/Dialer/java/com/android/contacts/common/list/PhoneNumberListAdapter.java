...
public class PhoneNumberListAdapter extends ContactEntryListAdapter {
  ...
  private static final Uri REGULAR_MATCH_URI =
          Uri.parse(CallLog.Calls.CONTENT_URI + "/regularmatch");
  private static final Uri CALLABLE_MATCH_URI =
          Uri.parse(CallLog.Calls.CONTENT_URI + "/callabledialmatch");
  /* @} */
  ...
  public void configureLoader(CursorLoader loader, long directoryId) {
    ...
    if (isExtendedDirectory(directoryId)) {
      ...
      if (isSearchMode()) {
        ...
        if (isRemoteDirectoryQuery) {
          ...
          *baseUri = CallableCompat.getContentFilterUri();*/
          baseUri = mIsDialMatchMode ? CALLABLE_MATCH_URI : CallableCompat.getContentFilterUri();
        } else {
         /* SPRD: Matching callLog when search in dialpad feature @{
          * @orig
          * baseUri = PhoneCompat.getContentFilterUri();*/
          baseUri = mIsDialMatchMode ? REGULAR_MATCH_URI : PhoneCompat.getContentFilterUri();
        }
      ...
      loader.setUri(builder.build());
      /* SPRD: Matching callLog when search in dialpad feature @{ */
      Log.d(TAG, "[Projection] getContactNameDisplayOrder() = "
              + getContactNameDisplayOrder() + " mIsDialMatchMode =" + mIsDialMatchMode);
      /* @} */
      ...
      if (getContactNameDisplayOrder() == ContactsPreferences.DISPLAY_ORDER_PRIMARY) {
        ...
        * @orig
        * loader.setProjection(PhoneQuery.PROJECTION_PRIMARY); */
        loader.setProjection(mIsDialMatchMode ? PhoneQuery.PROJECTION_DIALMATCH
  ...
  public static class PhoneQuery {
    ...
    public static final String[] PROJECTION_DIALMATCH = new String[] {
            ...
            Contacts.DISPLAY_ACCOUNT_TYPE, // 10
            Contacts.DISPLAY_ACCOUNT_NAME, // 11
            "item_type", //12
            Calls.DATE, //13
            Calls.TYPE, //14
