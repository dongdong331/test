...
public class CallLogAdapter extends GroupingListAdapter
  ...
  public CallLogAdapter(
    ...
    /* SPRD: new feature-display FDN contact name in calllog @{ */
    if (activity.getResources().getBoolean(R.bool.is_telcel_version)) {
        ...
        for (int i = 0; i < phoneCount; i++) {
          ...
          if (subscriptionInfo != null) {
            ...
            if (TelephonyManagerEx.from(activity).getIccFdnEnabled(subId)) {
              DialerTelcelHelper.getInstance(activity).queryFdnList(
                      subId, activity);
            }
          }
        }
  ...
  private boolean loadData(CallLogListItemViewHolder views, long rowId, PhoneCallDetails details) {
    ...
    if (PhoneNumberHelper.canPlaceCallsTo(details.number, details.numberPresentation)
      ...
      /* SPRD: add for bug 963817 @{ */
      int position = RecyclerView.NO_POSITION;
      try {
        position = views.getAdapterPosition();
      } catch (IndexOutOfBoundsException exp) {
        Log.d("CallLogAdapter.loadData", "getAdapterPosition is wrong, just return false.");
        exp.printStackTrace();
        return false;
      } catch (NullPointerException exp) {
        Log.d("CallLogAdapter.loadData", "getAdapterPosition is wrong(null pointer), just return false.");
        exp.printStackTrace();
        return false;
      }
      /* @} */
          ...
          for (int i = 0; i < phoneCount; i++) {
            ...
            if (subscriptionInfo != null) {
              ...
              if (TelephonyManagerEx.from(activity).getIccFdnEnabled(subId)) {
                ...
                details.namePrimary = DialerTelcelHelper.getInstance(activity).queryFdnCache(
                        details.number, subId, details.namePrimary);
                /**@}*/
              }
            }
          }
    ...
    /* @} */
    /**UNISOC:modify the bug for  961509 @{*/
    LogUtil.d(
        "CallLogAdapter.loadData",
