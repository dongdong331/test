...
public final class CallLogListItemViewHolder extends RecyclerView.ViewHolder
  ...
  private void updatePrimaryActionButton(boolean isExpanded) {
    ...
    switch (callbackAction) {
      case CallbackAction.IMS_VIDEO:
        /* UNISOC: modify for bug911245 */
        primaryActionButtonView.setTag(callbackAction);
        primaryActionButtonView.setContentDescription(
            TextUtils.expandTemplate(
    ...
    if (callType == Calls.VOICEMAIL_TYPE) {
      ...
      detailsButtonView.setTag(
          IntentProvider.getCallDetailIntentProvider(
              callDetailsEntries, buildContact(), canReportCallerId, canSupportAssistedDialing(),isVoicemail));
      /** @} */
    }
  ...
  public void onClick(View view) {
    ...
    /* @} */
    if (view.getId() == R.id.primary_action_button || view.getId() == R.id.call_action) {
      /* SPRD: add for bug900020 @{ */
      if (isAirplaneModeOn
              && !PhoneNumberUtils.isEmergencyNumber(buildContact().getNumber())
              && !isImsRegistered) {
    ...
    /* UNISOC: modify for bug911245 @{ */
    if (view.getId() == primaryActionButtonView.getId() && view.getTag() instanceof Integer) {
      if ((int) view.getTag() == CallbackAction.IMS_VIDEO) {
        if (DialerUtils.getRegisteredImsSlotForDualLteModem() == DialerUtils.SLOT_ID_ONE_TWO) {
          ...
          primaryActionButtonView.setTag(
                  IntentProvider.getReturnVideoCallIntentProvider(number,
                          DialerUtils.getPhoneAccountHandleBySlotId(context,
    ...
    /* UNISOC: modify for bug916537 @{ */
    if(videoCallButtonView != null
            && videoCallButtonView.getVisibility() == View.VISIBLE
            && view.getId() == videoCallButtonView.getId()) {
            if (DialerUtils.getRegisteredImsSlotForDualLteModem() == DialerUtils.SLOT_ID_ONE_TWO) {
