...
public class FastDialManager {
    ...
    private FastDialManager(Context context) {
        ...
        mPerSavedFDNumbers = appContext.getSharedPreferences(
                    SHARED_PREFERENCES_NAME, Context.MODE_PRIVATE);
        mPerSavedFDNumbers.registerOnSharedPreferenceChangeListener(
                    mSharedPreferenceChangeListener);
        // UNISOC: add for feature customized fast dial numer 892169
        initEmergencyNumber();
    ...
    public String getCallNumber(int key) {
        if (mPerSavedFDNumbers == null) {
            return "";
        }
        return mPerSavedFDNumbers.getString("fast_dial_" + key, "");
    }

    /* UNISOC: add for feature customized fast dial numer 892169@{ */
    private void initEmergencyNumber() {
        if (mContext.getResources().getBoolean(R.bool.config_fastdial_number_customized)) {
            Log.d(TAG, "initEmergencyNumber");
            saveFastDialNumber(5, "112", null, null, null, null, null);
            saveFastDialNumber(9, "112", null, null, null, null, null);
        }
    }
    /* @} */
}
