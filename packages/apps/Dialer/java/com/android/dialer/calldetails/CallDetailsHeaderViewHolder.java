...
public class CallDetailsHeaderViewHolder extends RecyclerView.ViewHolder
  ...
  void updateContactInfo(DialerContact contact, @CallbackAction int callbackAction) {
    ...
    /* SPRD: Add for Bug 973172 it shows emergency number in CallDetail display @{ */
    boolean shouldShowEmergency = context.getResources()
            .getBoolean(R.bool.allow_emergency_numbers_in_call_log);
    String nameText;
    if (PhoneNumberUtils.isEmergencyNumber(contact.getNameOrNumber().toString()) && shouldShowEmergency) {
      nameText = context.getResources().getString(R.string.emergency_call);
    } else {
      nameText = contact.getNameOrNumber();
    }
  ...
  public void onClick(View view) {
    if (view == callbackButton) {
      ...
      // SPRD: 895147 905862 modify the videoIcon to dialerIcon
      ...
      TelephonyManager telephonyManager = (TelephonyManager)
              view.getContext().getSystemService(Context.TELEPHONY_SERVICE);
      boolean isImsRegistered = telephonyManager.isImsRegistered();
      if (isAirplaneModeOn
              && !PhoneNumberUtils.isEmergencyNumber(contact.getNumber())
              && !isImsRegistered) {
        Toast.makeText(view.getContext(), R.string.dialog_make_call_airplane_mode_message,
                Toast.LENGTH_LONG).show();
      } else {
          PreCall.start(
                  view.getContext(), new CallIntentBuilder(contact.getNumber(),CallInitiationType.Type.DIALPAD));
      }
      /* @} */
    } else {
