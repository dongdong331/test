...
public class PostCall {
  ...
  private static final String KEY_POST_CALL_CALL_SUBID = "post_call_call_subId";
  ...
  private static void promptUserToSendMessage(Activity activity, View rootView) {
    ...
    int subId = getPhoneSubId(activity);
    OnClickListener onClickListener =
        v -> {
          Logger.get(activity)
              .logImpression(DialerImpression.Type.POST_CALL_PROMPT_USER_TO_SEND_MESSAGE_CLICKED);
          activity.startActivity(PostCallActivity.newIntent(activity, number, isRcsPostCall, subId));
        };
  ...
  private static void promptUserToViewSentMessage(Activity activity, View rootView) {
    ...
    OnClickListener onClickListener =
        v -> {
          ...
          /** SPRD: add for bug896094
           *  bug 900457 After the call is disconnected, explain the reason for the call -
           *  click to view after sending the message, prompting "This device has no operation
  ...
  public static void onCallDisconnected(Context context, String number, long callConnectedMillis,
    ...
    StorageComponent.get(context)
        ...
        .putInt(KEY_POST_CALL_CALL_SUBID, subId)
        .apply();
  }
  ...
  /*UNISOC: add for bug916945 @{ */
  @Nullable
  private static int getPhoneSubId(Context context) {
    return StorageComponent.get(context)
            .unencryptedSharedPrefs()
            .getInt(KEY_POST_CALL_CALL_SUBID, SubscriptionManager.INVALID_SUBSCRIPTION_ID);
  }
  /* @} */
