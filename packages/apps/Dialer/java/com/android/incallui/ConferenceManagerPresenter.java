...
public class ConferenceManagerPresenter extends Presenter<ConferenceManagerUi>
  ...
  private void update(CallList callList) {
    ...
    for (String callerId : currentCall.getChildCallIds()) {
          /**
         * SPRD: Modify for Feature: FL1000060352 conference member state @{
         * calls.add(callList.getCallById(callerId));
         */
        if (InCallUICmccHelper.getInstance(context).isOnlyDislpayActiveOrHoldCall()) {
            if (callList.getCallById(callerId).getRealState() == DialerCall.State.ACTIVE
                    || callList.getCallById(callerId).getRealState() == DialerCall.State.ONHOLD) {
                calls.add(callList.getCallById(callerId));
            }
        } else {
            calls.add(callList.getCallById(callerId));
