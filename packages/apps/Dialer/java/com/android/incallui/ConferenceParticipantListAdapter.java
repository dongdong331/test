...
public class ConferenceParticipantListAdapter extends BaseAdapter {
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    // SPRD: Modify for Feature : FL1000060352 display state of conference member
  ...
  private void setCallerInfoForRow(
    ...
    /* SPRD: Modify for Feature : FL1000060352 display state of conference member@{ */
    if (InCallUICmccHelper.getInstance(view.getContext()).shouldShowParticipantState()) {
        DialerCall call = CallList.getInstance().getCallById((String) view.getTag());
        statusTextView.setVisibility(View.VISIBLE);
        statusTextView.setText(getCallStateLabelFromState(view.getContext(), call.getState()));
    } else if (callState == DialerCall.State.ONHOLD) {
