...
import android.content.SharedPreferences;//UNISOC:add for bug903996
...
public class VideoCallPresenter
 ...
 // private int previewSurfaceState = PreviewSurfaceState.NONE;  //modify for bug906119
  /**
  ...
  private DialerCall activeCall;//SPRD:add fot feature FL1000060387
  /* UNISOC: add for bug 903996 @{ */
  private SharedPreferences mPrefs;
  private final String BACKGROUND_CLOSE_CAMERA_NAME = "background_close_camera_name";
  private final String KEY_BACKGROUND_CLOSE_CAMERA = "key_background_close_camera";
  /* @} */
  ...
  public void onCameraPermissionGranted() {
    ...
    if(primaryCall != null) { //add for bug968801
      showVideoUi(
  ...
  public void onStateChange(
    ...
    if (newState == InCallPresenter.InCallState.INCOMING) {
      ...
      if (context != null && context.checkSelfPermission(READ_PHONE_STATE) == PackageManager.PERMISSION_GRANTED
        ...
        LogUtil.i("VideoCallPresenter.onStateChange", "this is conference call");
        if(!InCallPresenter.getInstance().getLocalVideoSurfaceTexture().getSmallSurface()){
          InCallPresenter.getInstance().getLocalVideoSurfaceTexture().setSmallSurface(true);
          InCallPresenter.getInstance().getRemoteVideoSurfaceTexture().setSmallSurface(false);
          LogUtil.i("VideoCallPresenter.onStateChange", "change the local surface as small");
        }
  ...
  public void onFullscreenModeChanged(boolean isFullscreenMode) {
    ...
    if (primaryCall != null) {
      updateFullscreenAndGreenScreenMode(
          primaryCall.getState(), primaryCall.getVideoTech().getSessionModificationState(), primaryCall.getVideoState());//add for bug858314,913336
    } else {
  ...
  private void checkForOrientationAllowedChange(@Nullable DialerCall call) {
    InCallPresenter.getInstance()
        .setInCallAllowsOrientationChange((isVideoCall(call) || isVideoUpgrade(call)) && (call != null && !call.isRingToneOnAudioCall()));//add for bug908219
  }
  ...
  private void updateFullscreenAndGreenScreenMode(
    ...
    if (videoCallScreen != null) {
      ...
      boolean notShowFullscreenAndGreenScreen = isInLockTaskMode() &&
              (callState == State.DISCONNECTED || callState == State.INVALID);
      ...
      boolean shouldShowGreenScreen =
          callState == State.DIALING
              ...
              || isVideoUpgrade(sessionModificationState);
      if(!notShowFullscreenAndGreenScreen) {//add for bug907768
  ...
  /*SPRD:add for bug858314, 913336 @{*/
  private void updateFullscreenAndGreenScreenMode(
    int callState, @SessionModificationState int sessionModificationState, int videoState) {
      if (videoCallScreen != null) {
        boolean shouldShowFullscreen = InCallPresenter.getInstance().isFullscreen();
        boolean shouldShowGreenScreen =
                callState == State.DIALING
                        || callState == State.CONNECTING
                        || callState == State.INCOMING
                        || (isVideoUpgrade(sessionModificationState) && VideoProfile.isAudioOnly(videoState));
          videoCallScreen.updateFullscreenAndGreenScreenMode(
                  shouldShowFullscreen, shouldShowGreenScreen);
      }
  }
  ...
  private void adjustVideoMode(DialerCall call) {
    ...
    if (videoCall != null) {
      ...
      LogUtil.i("VideoCallPresenter.adjustVideoMode"," getPreviewSurfaceState():"+InCallPresenter.getInstance().getPreviewSurfaceState());
      Assert.checkState(
      ...
      enableCamera(
          ...
          && InCallPresenter.getInstance().getPreviewSurfaceState() != PreviewSurfaceState.CAPABILITIES_RECEIVED);//add for bug906119
    }
  ...
  private void enableCamera(DialerCall call, boolean isCameraRequired) {
    LogUtil.i("VideoCallPresenter.enableCamera", " call:"+call+" enabling:"+isCameraRequired);
    if (call == null) {
    ...
    //SPRD:add fot feature FL1000060387
    if (context != null && InCallUiUtils.isSupportRingTone(context) && activeCall != null) {
    ...
    if (!hasCameraPermission) {
      ...
      //modify for bug906119
     // previewSurfaceState = PreviewSurfaceState.NONE;
      InCallPresenter.getInstance().setPreviewSurfaceState(PreviewSurfaceState.NONE);
      ...
      InCallPresenter.getInstance().setPreviewSurfaceState(PreviewSurfaceState.CAMERA_SET);
    } else {
      //modify for bug906119
      //previewSurfaceState = PreviewSurfaceState.NONE;
      InCallPresenter.getInstance().setPreviewSurfaceState(PreviewSurfaceState.NONE);
  ...
  private void showVideoUi(
    ...
    LogUtil.i(
        ...
        showOutgoingVideo,
        isRemotelyHeld);
    if(InCallUiUtils.isSupportRingTone(context) && activeCall != null){
  ...
  public void onCameraDimensionsChange(DialerCall call, int width, int height) {
    LogUtil.i(
        ...
        height);
  /*  if (videoCallScreen == null) {
      LogUtil.i("VideoCallPresenter.onCameraDimensionsChange", "ui is null");
      return;
    }*/  //modify for bug906119

    if (!call.equals(primaryCall)) {
      LogUtil.i("VideoCallPresenter.onCameraDimensionsChange", "not the primary call");
      return;
    }

    //modify for bug906119
    //previewSurfaceState = PreviewSurfaceState.CAPABILITIES_RECEIVED;
    InCallPresenter.getInstance().setPreviewSurfaceState(PreviewSurfaceState.CAPABILITIES_RECEIVED);
    ...
    if (surface != null) {
     ...
     // previewSurfaceState = PreviewSurfaceState.SURFACE_SET;
      InCallPresenter.getInstance().setPreviewSurfaceState(PreviewSurfaceState.SURFACE_SET);
  ...
  private void changePreviewDimensions(int width, int height) {
    if (videoCallScreen == null) {
      LogUtil.i("VideoCallPresenter.changePreviewDimensions", "videoCallScreen is null.");
      return;
    }
  ...
  private class LocalDelegate implements VideoSurfaceDelegate {
    ...
    public void onSurfaceCreated(VideoSurfaceTexture videoCallSurface) {
      if (videoCallScreen == null) {
        LogUtil.i("VideoCallPresenter.LocalDelegate.onSurfaceCreated", "no UI");
        return;
      }
      if (videoCall == null) {
        LogUtil.i("VideoCallPresenter.LocalDelegate.onSurfaceCreated", "no video call");
        return;
      }
      LogUtil.i("VideoCallPresenter.LocalDelegate.onSurfaceCreated", "previewSurfaceState :" + InCallPresenter.getInstance().getPreviewSurfaceState());
      ...
      if (InCallPresenter.getInstance().getPreviewSurfaceState() == PreviewSurfaceState.CAPABILITIES_RECEIVED) {
        //modify for bug906119
        //previewSurfaceState = PreviewSurfaceState.SURFACE_SET;
        InCallPresenter.getInstance().setPreviewSurfaceState(PreviewSurfaceState.SURFACE_SET);
        videoCall.setPreviewSurface(videoCallSurface.getSavedSurface());
      } else if (InCallPresenter.getInstance().getPreviewSurfaceState() == PreviewSurfaceState.NONE && isCameraRequired()) {
  ...
  public static class PreviewSurfaceState {
  ...
  private boolean isAllowChangeSurface(DialerCall call) {
    ...
    if(isVideoCall(call) && call.getState() == DialerCall.State.ACTIVE) {
      return true;
    }
    return false;
  }

  // SPRD: add for bug907768
