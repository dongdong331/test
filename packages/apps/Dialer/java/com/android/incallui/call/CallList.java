...
public class CallList implements DialerCallDelegate {
        ...
        public void handleMessage(Message msg) {
          switch (msg.what) {
            case EVENT_DISCONNECTED_TIMEOUT:
              LogUtil.i("CallList.handleMessage", "EVENT_DISCONNECTED_TIMEOUT ", msg.obj);
  ...
  private void onIncoming(DialerCall call) {
    ...
    PerformanceManagerInternal.getDefault().scheduleBoostRRForApp(true);  //add for bug961162
    if (updateCallInMap(call)) {
    ...
    LogUtil.i("CallList.onIncoming","end");
    PerformanceManagerInternal.getDefault().scheduleBoostRRForApp(false);  //add for bug961162
    Trace.endSection();
  }
  ...
  public boolean hasValidGroupCall() {
    for (DialerCall call : callById.values()) {
      if (call.getChildCallIds().size() > 0 && call.getState() != DialerCall.State.INVALID
              & call.getState() != DialerCall.State.IDLE
              && call.getState() != DialerCall.State.DISCONNECTED) {
        return true;
      }
    }
    return false;
  ...
  private boolean isforgroundCall(DialerCall call) {
      final int state = call.getState();
      return (DialerCall.State.ACTIVE == state
              || DialerCall.State.CONFERENCED == state
              || DialerCall.State.DIALING == state
              || DialerCall.State.REDIALING == state);
  }
   ...
   * SPRD: add for DSDA
   */
  public boolean callOnSamePhone (DialerCall primaryCall, Context context) {
    int subId = -1;
    if (primaryCall.getAccountHandle() != null) {
      subId = InCallUiUtils.getSubIdForPhoneAccountHandle(context, primaryCall.getAccountHandle());
    }
    for (DialerCall call :callById.values()) {
      if (call.getAccountHandle() != null) {
        if (InCallUiUtils.getSubIdForPhoneAccountHandle(context, call.getAccountHandle()) != subId) {
          return false;
        }
      }
    }
   ...
   */
  public void markAsUserPrimaryCall (DialerCall userPrimaryCall) {
    userPrimaryCall.markAsUserPrimary(true);
    for (DialerCall call : callById.values()) {
      if (call.isUserPrimary() && call != userPrimaryCall) {
        call.markAsUserPrimary(false);
      }
    }
    notifyGenericListeners();
  }
  ...
  /**
   * SPRD: add for DSDA
   * @return The user primary call.
   */
  public DialerCall getUserPrimaryCall() {
    for (DialerCall call : callById.values()) {
      if (call.isUserPrimary()) {
        return call;
      }
    }
    return null;
  ...
  /**
   * SPRD: add for DSDA
   * @return Another ringing call.
   */
  public DialerCall getOtherRingingCall(DialerCall ignore) {
    for (DialerCall call : callById.values()) {
      if (call.getState() == DialerCall.State.INCOMING && call != ignore) {
        return call;
      }
    }
    return null;
  ...
  /**
   * UNISOC: add for bug915636
   */
  public boolean isConferenceParticipant() {
    for (DialerCall call : callById.values()) {
      if (call!= null && call.isConferenceCall() && call.getChildCallIds() != null && call.getChildCallIds().size() < 1) {
        return true;
      }
    }
    return false;
