...
public class DialerCall implements VideoTechListener, StateChangedListener, CapabilitiesListener {
  ...
  //SPRD : add for feature FL1000060387
  ...
  private String mUserDialedString; // SPRD: added for bug 957870(812244), this variable will hold the actual number entered by user.
  private boolean mIsVideoCallCallbackRegistered = false;//UNISOC: add for bug973472
  ...
  private boolean mIsUserPrimary = false;
  ...
  public DialerCall(
    ...
    if(!isIncoming()){
      isDialingAudioCall = !VideoProfile.isVideo(getVideoState());//SPRD : add for feature FL1000060387
    }

    // add for bug 957870(812244)
    mUserDialedString = getNumber();
  }
  ...
  private void update() {
    ...
    // UNISOC: add for bug941256
    int oldCause = getDisconnectCause().getCode();
    // Clear any cache here that could potentially change on update.
    videoTech = null;
    // We want to potentially register a video call callback here.
    updateFromTelecomCall();
    // UNISOC: add for bug941256
    if ((oldState != getState() || oldCause != getDisconnectCause().getCode())
            && getState() == DialerCall.State.DISCONNECTED) {
      for (DialerCallListener listener : listeners) {
  ...
  private void updateFromTelecomCall() {
    ...
    if (!Objects.equals(handle, newHandle)
            || (PhoneNumberUtilsEx.isEmergencyNumber(newHandle == null ? "" : newHandle.getSchemeSpecificPart())
                    && !isEmergencyCall())) {
      handle = newHandle;
      LogUtil.i("DialerCall.updateFromTelecomCall", "updateEmergencyCallState");
  ...
  public void setState(int state) {
    ...
    /* SPRD: add for feature FL0108020006, use cpu time instead of system time @{ */
    if (this.state == State.ACTIVE || this.state == State.ONHOLD || this.state == State.CONFERENCED) {
  ...
  public boolean isRingToneOnAudioCall() {
    ...
    if (isDialingAudioCall && VideoProfile.isVideo(getVideoState())) {
      if (getState() == State.ACTIVE) {
        return false;
      } else if (getState() == State.CONNECTING || getState() == State.DIALING) {
        return true;
      }
    }
    return false;
  ...
  //  if (videoTech == null) {  //modify for bug917108
  ...
  /**
   * SPRD: add for Feature : FL1000060352 display state of conference member
   */
  ...
  /**
   * SPRD: add for DSDA
   * when user clicked secondary caller info, we mark the call as user primary call
   * and show it as primay call in CallCard and CallButton
   */
  public void markAsUserPrimary (boolean isUserPrimary) {
    LogUtil.i("DialerCall.markAsPrimary", "isUserPrimary " + isUserPrimary);
    mIsUserPrimary = isUserPrimary;
  }

  /**
   * SPRD: add for DSDA
   * @return if the call is user primary call.
   */
  public boolean isUserPrimary() {
    return mIsUserPrimary;
  }

  /* SPRD: add for bug 957870(812244) @{ */
  public String getUserDialedString() {
    LogUtil.d("DialerCall", "phone number: " + mUserDialedString);
    if(mUserDialedString != null && mUserDialedString.length() == 0){ //add for bug969697,978333
      mUserDialedString = getNumber();
      LogUtil.d("DialerCall", "phone number reload: " + mUserDialedString);
    }
    return mUserDialedString;
  }
  /* @} */
