...
public class CreateDirectoryFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        mEditText.setFilters(new InputFilter[] {new LengthFilterToast(getActivity(), Shared.MAX_TEXT_LENGTH)});
    ...
    private boolean isValidDocumentName(String docName) {
        if(docName.isEmpty() || docName.trim().isEmpty()) {
            Toast.makeText(getActivity(), R.string.create_error, Toast.LENGTH_LONG).show();
            return false;
        }

        if (!Shared.IsValidFilename(docName)) {
            Toast.makeText(getActivity(), R.string.illegal_chars_of_foldername,Toast.LENGTH_LONG).show();
            return false;
        }
        return true;
    }
