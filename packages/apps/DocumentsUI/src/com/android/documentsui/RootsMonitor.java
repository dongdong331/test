...
final class RootsMonitor<T extends Activity & CommonAddons> {
    ...
    RootsMonitor(
        ...
        mReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                final String rootChangeUri = intent.getStringExtra(Shared.EXTRA_ROOT);
    ...
    private static class HandleRootsChangedTask<T extends Activity & CommonAddons>
        ...
        private final String mRootChangeUri;
        ...
        protected RootInfo run(RootInfo... roots) {
            ...
            for (final RootInfo root : cachedRoots) {
                if (root.getUri().equals(mCurrentRoot.getUri()) || mState.stack.getRoot() == null) {
                    // We don't need to change the current root as the current root was not removed.
                    // Or currently there is no select any root
                    return null;
                }
            }
        ...
        protected void finish(RootInfo defaultRoot) {
            if (defaultRoot == null) {
                RootInfo currentRoot = mState.stack.getRoot();
                if(currentRoot != null && !currentRoot.isRecents() && !currentRoot.isMtp()) {
                    Uri currentRootUri = currentRoot.getUri();
                    if ((currentRootUri != null) && (mRootChangeUri != null) &&
                            (currentRootUri.toString().startsWith(mRootChangeUri))) {
