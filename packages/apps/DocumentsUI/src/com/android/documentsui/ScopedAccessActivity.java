...
public class ScopedAccessActivity extends Activity {
    ...
    private static boolean grantDirectly = false;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (grantDirectly) {
            ...
            final File volumeRoot = volume.getPathFile();
            try {
                file = isRoot ? volume.getPathFile() : new File(volumeRoot, directoryName).getCanonicalFile();
            } catch (IOException e) {
                Log.e(TAG, "Could not get canonical file for volume " + volume.getPath()
                        + " and directory " + directoryName);
                setResult(RESULT_CANCELED);
                finish();
                return;
            }
            final Intent grantIntent = createGrantedUriPermissionsIntent(this,
                    getExternalStorageClient(), file);
            final Uri grantedUri = grantIntent.getData();
            Log.d(TAG, "grantedUri="+grantedUri);
            setResult(RESULT_OK, grantIntent);
            finish();
            return;
        } else {
