...
public class IconHelper {
    ...
    public IconHelper(Context context, int mode) {
        mContext = context;
        mFeatures = Features.create(context);
        setViewMode(mode);
    ...
    public void load(
        ...
        if (doc.grantUri != null) {
            uri = Uri.parse(doc.grantUri);
        } else {
            uri = doc.derivedUri;
        }
        load(uri, doc.mimeType, doc.displayName, doc.flags, doc.icon, doc.lastModified,
     ...
     * @return
     */
    public void load(Uri uri, String mimeType, String displayName, int docFlags, int docIcon, long docLastModified,
        ...
        final boolean allowThumbnail = (mMode == MODE_GRID)
                ...
                || (mimeType != null && mimeType.equals(MimeTypes.DRM_TYPE));

        String originalMimeType = mimeType;
        try {
            mDrmPath = DocumentsUIPlugInDrm.getInstance().getDrmPath(mContext,uri);
            mIsDrm = DocumentsUIPlugInDrm.getInstance().getIsDrm(mContext,mDrmPath);
            if (mIsDrm) {
                originalMimeType = DocumentsUIPlugInDrm.getInstance().getDocMimeType(mContext, mDrmPath, mimeType);
            }
        } catch (Exception e) {
            Log.d(TAG, "Failed to get drm info, maybe drm is not supported in this resolver");
        }

        Log.d(TAG,"Load supportsThumbnail "+ supportsThumbnail + ",allowThumbnail " + allowThumbnail + ",mThumbnailsEnabled  "+ mThumbnailsEnabled);
        final boolean showThumbnail = supportsThumbnail && allowThumbnail && mThumbnailsEnabled;
        if (showThumbnail || mIsDrm) {
            loadedThumbnail =
        ...
        try {
            if (mIsDrm) {
                Bitmap result = DocumentsUIPlugInDrm.getInstance().getIconBitmap(mContext, MimeTypes.DRM_TYPE, mDrmPath);
                assert(result != null);
                mimeIcon = new BitmapDrawable(mContext.getResources(), result);
            }else {
                mimeIcon = getDocumentIcon(mContext, docAuthority, DocumentsContract.getDocumentId(uri), originalMimeType, docIcon);
            }
        }catch (Exception e) {
            Log.d(TAG, "Failed to get drm info, maybe drm is not supported in this resolver");
        }
    ...
    private boolean loadThumbnail(Uri uri, String docAuthority, long docLastModified,
        ...
        try {
            ...
            if (!result.isExactHit() || stale) {
                ...
                task.setVideoThumbnailEnable(mFeatures.isVideoThumbnailEnabled());
