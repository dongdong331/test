...
final class ListDocumentHolder extends DocumentHolder {
    ...
    public void bind(Cursor cursor, String modelId) {
        ...
        if (mDoc.isDirectory()) {
            ...
            String originalMimeType = mDoc.mimeType;
            try {
                String path = DocumentsUIPlugInDrm.getInstance().getDrmPath(mContext, mDoc.getUri(cursor));
                originalMimeType = DocumentsUIPlugInDrm.getInstance().getDocMimeType(mContext, path, mDoc.mimeType);
            } catch (Exception e) {
                Log.d(TAG, "Failed to get drm info, maybe drm is not supported in this resolver");
            }
            mType.setText(mFileTypeLookup.lookup(originalMimeType));
        }
