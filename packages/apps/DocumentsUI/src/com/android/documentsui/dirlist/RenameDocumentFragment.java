...
public class RenameDocumentFragment extends DialogFragment {
    ...
    private boolean isValidDocumentName(String docName) {
        if(docName.isEmpty() || docName.trim().isEmpty() || (!mDocument.isDirectory() && docName.lastIndexOf('.') == 0)) {
            Toast.makeText(getActivity(), R.string.rename_error, Toast.LENGTH_LONG).show();
            return false;
        }

        if (!Shared.IsValidFilename(docName)) {
            int stringId = mDocument.isDirectory() ? R.string.illegal_chars_of_foldername : R.string.illegal_chars_of_filename;
    ...
    private void renameDocuments(String newDisplayName) {
        ...
        String newDisplayNameTrimed = newDisplayName.trim();
        if (newDisplayNameTrimed.equals(mDocument.displayName)) {
            mDialog.dismiss();
        } else if (!isValidDocumentName(newDisplayNameTrimed)) {
            Log.w(TAG, "Failed to rename file - invalid name:" + newDisplayNameTrimed);
        } else if (activity.getInjector().getModel().hasFileWithName(newDisplayNameTrimed)){
            mRenameInputWrapper.setError(getContext().getString(R.string.name_conflict));
            selectFileName(mEditText);
            Metrics.logRenameFileError(getContext());
        } else {
            new RenameDocumentsTask(activity, newDisplayNameTrimed).execute(mDocument);
        }
