...
public class SelectionMetadata extends SelectionObserver
    ...
    private int mManagedDownload = 0;
    private int mDRMfileCount = 0;
    ...
    public void onItemStateChanged(String modelId, boolean selected) {
        ...
        if((displayname != null) && (MIMETYPE_DRM_MESSAGE.equals(mimeType) || displayname.endsWith(".dcf")
            || displayname.endsWith(".dr") || displayname.endsWith(".drc"))) {
            mDRMfileCount += delta;
        }
        ...
        if (MimeTypes.isApkType(mimeType) && Providers.AUTHORITY_DOWNLOADS.equals(authority)) {
            mManagedDownload += delta;
        }
    }
    ...
    public boolean canOpenWith() {
        return size() == 1 && mDirectoryCount == 0 && mInArchiveCount == 0
                && mPartialCount == 0 && mManagedDownload == 0 && mDRMfileCount ==0;
    }

    @Override
    public boolean canCopyTo() {
        return size() > 0 && mDRMfileCount == 0;
    }
}
