...
public class ActionHandler<T extends Activity & Addons> extends AbstractActionHandler<T> {
    ...
    /*Add for AlertDialog*/
    private DocumentInfo mDrmDocInfo;
    ...
    public boolean openDocument(DocumentInfo doc, @ViewType int type, @ViewType int fallback) {
        Log.d(TAG, "openDocument, authority: " + doc.authority + ", mimeType: " + doc.mimeType);
        ...
        if (docs.size() == 1) {
            ...
            */
            try {
                ...
                if (DEBUG) Log.d(TAG, "doc fail to setDocMimeType ");
            }
            /*@}*/
            ...
            for (DocumentInfo doc : docs) {
                ...
                */
                try {
                    ...
                    if (DEBUG) Log.d(TAG, "docs fail to setDocMimeType ");
                }
                /*@}*/
    ...
    private void onDocumentPicked(DocumentInfo doc, @ViewType int type, @ViewType int fallback) {
        ...
        if (MimeTypes.isApkType(doc.mimeType) &&(!doc.isInArchive())) {
            viewDocument(doc);
            return;
        }
    ...
    private boolean DRMDocument(DocumentInfo doc) {
        try {
            if (DocumentsUIPlugInDrm.getInstance().sendDRMFileIntent(mActivity, doc.derivedUri,
                new Handler(mActivity.getApplicationContext().getMainLooper()))) {
                Log.d(TAG, "openDocument, open Drm file!");
                ThumbnailCache cache = DocumentsApplication.getThumbnailCache(mActivity);
                if(doc.grantUri != null){
                    cache.removeUri(Uri.parse(doc.grantUri));
                }else{
                    cache.removeUri(doc.derivedUri);
                }
                mDrmDocInfo = doc;
                return true;
            }
    ...
    private boolean isManagedDownload(DocumentInfo doc) {
        ...
        if (mActivity.getCurrentRoot().isDownloads()) {
            ...
            return MimeTypes.isApkType(doc.mimeType)
                    || doc.isPartial()
                    || doc.mimeType.equals("application/vnd.oma.drm.convert");
        }
    ...
    private Intent buildViewIntent(DocumentInfo doc) {
        ...
        // and other roots.
        int flags = Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_GRANT_READ_URI_PERMISSION;
    ...
    public void showInspector(DocumentInfo doc) {
        ...
        intent.setData(doc.derivedUri);
        final boolean isInDevMode = Settings.Global.getInt(mActivity.getContentResolver(),
                Settings.Global.DEVELOPMENT_SETTINGS_ENABLED, 0) != 0;
        // permit the display of debug info about the file.
        intent.putExtra(
                Shared.EXTRA_SHOW_DEBUG,
                mFeatures.isDebugSupportEnabled() &&
                        ((Build.IS_DEBUGGABLE && isInDevMode) || DebugFlags.getDocumentDetailsEnabled()));
    ...
    public void showDRMDialog() {
        DRMDocument(mDrmDocInfo);
    }

    public void onSaveInstanceState(Bundle bundle, boolean IsSave) {
        if (IsSave) {
            bundle.putParcelable(Shared.EXTRA_DOC, mDrmDocInfo);
        }
        else {
            mDrmDocInfo = bundle.getParcelable(Shared.EXTRA_DOC);
        }
    }
}
