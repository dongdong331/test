...
public class FilesActivity extends BaseActivity implements ActionHandler.Addons {
    ...
    private static final String ALERTDIALOG = "alertDialog";
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (icicle != null) {
            mShowAlertDialog = icicle.getBoolean(ALERTDIALOG, false);
            mInjector.actions.onSaveInstanceState(icicle, false);

            if (mShowAlertDialog) {
                mInjector.actions.showDRMDialog();
            }
         }
    ...
    protected void onSaveInstanceState(Bundle bundle) {
        ...
        bundle.putBoolean(ALERTDIALOG, mShowAlertDialog);
        mInjector.actions.onSaveInstanceState(bundle, true);
    }

    @Override
    public void enableDRMDialog(boolean isShow) {
        mShowAlertDialog = isShow;
    }
    ...
    private void updateOptionMenuSelectAll(Menu menu) {
        boolean enabled = false;
        MenuItem selectAll = menu.findItem(R.id.option_menu_select_all);

        if(mInjector.getModel().getItemCount() == 0) {
            enabled = false;
        } else {
            enabled = true;
        }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        updateOptionMenuSelectAll(menu);
    ...
    @Override
    public void updateSelectAllMenu(Menu menu) {
        MenuItem menuSelectAll = menu.findItem(R.id.action_menu_select_all);
        MenuItem menuUnSelectAll = menu.findItem(R.id.menu_unselect_all);

        int selectedCount = mInjector.selectionMgr.getSelection().size();
        int itemCount = mInjector.getModel().getItemCount();
        //Log.d(TAG, "updateSelectAllMenu selectedCount="+selectedCount+"  itemCount="+itemCount);

        if (itemCount > 0) {
            boolean selectAll = selectedCount == itemCount;
            menuSelectAll.setVisible(!selectAll);
            menuSelectAll.setEnabled(!selectAll);
            menuUnSelectAll.setVisible(selectAll);
            menuUnSelectAll.setEnabled(selectAll);
        } else {
            menuSelectAll.setVisible(false);
            menuSelectAll.setEnabled(false);
            menuUnSelectAll.setVisible(false);
            menuUnSelectAll.setEnabled(false);
        }
    }
}
