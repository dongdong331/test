...
public class SearchViewManager implements
    ...
    public void openSearch() {
        if(mMenuItem != null && mMenuItem.isVisible()){
            mMenuItem.expandActionView();
            mSearchView.setIconified(false);
        }
    }
    ...
    public boolean cancelSearch() {
        if(mSearchView == null) {
            mCurrentSearch = null;
            Log.d(TAG,"mSearchView is null, maybe UI not initialized.");
            return true;
        }
    ...
    public boolean onQueryTextChange(String newText) {
        if(newText.length() >= Shared.MAX_TEXT_LENGTH){
            Toast.makeText(mSearchView.getContext(),R.string.length_limited , Toast.LENGTH_SHORT).show();
            return false;
        }
        cancelQueuedSearch();
        synchronized (mSearchLock) {
