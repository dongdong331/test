...
class CopyJob extends ResolvedResourcesJob {
    ...
    private void updateRemainingTimeEstimate() {
        ...
        if (mSpeed == 0) {
            ...
            mSpeed = ((9 * mSpeed) + sampleSpeed) / 10;
        }

        if (mSampleTime > 0 && mSpeed > 0) {
            mRemainingTime = ((mBytesRequired - bytesCopied) * 1000) / mSpeed;
        } else {
            Log.d(TAG, "mRemainingTime = 0 , mSampleTime = " + mSampleTime + ", mSpeed = " + mSpeed);
            mRemainingTime = 0;
        }
    ...
    void start() {
        ...
        for (int i = 0; i < mResolvedDocs.size() && !isCanceled(); ++i) {
            ...
            try {
                ...
                if (mDstInfo.equals(srcInfo) || isDescendentOf(srcInfo, mDstInfo) || (isDrmFile(srcInfo) && !(this instanceof MoveJob))) {
    ...
    void byteCopyDocument(DocumentInfo src, DocumentInfo dest) throws ResourceException {
        ...
        if (src.isVirtual()) {
            ...
            dstMimeType = isDrmFile(src) ?
                    DocumentsUIPlugInDrm.MIMETYPE_DRM_MESSAGE : src.mimeType;
            dstDisplayName = src.displayName;
        }
    ...
    private void copyFileHelper(DocumentInfo src, DocumentInfo dest, DocumentInfo destParent,
        ...
        if(isDrmFile(src) && !(this instanceof MoveJob)) {
            try {
    ...
    boolean isDescendentOf(DocumentInfo doc, DocumentInfo parent)
        ...
        } else if(doc.isDirectory() && getDocPath(parent) != null && getDocPath(doc) != null
                   && parent.derivedUri.getAuthority().equals(doc.derivedUri.getAuthority())){
            return getDocPath(parent).startsWith(getDocPath(doc));
        }
        return false;
    }
