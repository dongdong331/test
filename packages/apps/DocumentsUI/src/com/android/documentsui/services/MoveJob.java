...
final class MoveJob extends CopyJob {
    ...
    void processDocument(DocumentInfo src, DocumentInfo srcParent, DocumentInfo dest)
        ...
        String srcPath = getDocPath(src);
        String destPath = getDocPath(dest);
        if((srcPath != null) && ( destPath != null ) 
             && srcPath.substring(0,srcPath.lastIndexOf('/')).equals(destPath)){
            Log.d(TAG , "not move file to same directory: " + srcPath);
            return;
        }
