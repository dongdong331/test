...
public class LauncherAppTransitionManagerImpl extends LauncherAppTransitionManager
    ...
    public static final int RECENTS_LAUNCH_DURATION = FeatureOption.SPRD_ANIM_LAUNCHTASK ? 136 : 336;
    public static final int RECENTS_QUICKSCRUB_LAUNCH_DURATION = 300;
    private static final int LAUNCHER_RESUME_START_DELAY = 100;
    private static final int CLOSING_TRANSITION_DURATION_MS = FeatureOption.SPRD_ANIM_APP_EXIT ? 200 : 250;
    ...
    private boolean composeRecentsLaunchAnimator(View v,
        ...
        if (launcherClosing) {
            if (!FeatureOption.SPRD_ANIM_LAUNCHTASK) {
    ...
    private AnimatorSet getIconAnimator(View v, Rect windowTargetBounds) {
        ...
        if (Float.isNaN(scale)) {
            if (LogUtils.DEBUG) {
                LogUtils.d(TAG, "windowTargetBounds.width() = " + windowTargetBounds.width()
                        + "windowTargetBounds.height() = " + windowTargetBounds.height()
                        + "rect.width() = " + rect.width() + "rect.height() = " + rect.height());
            }
            scale = Math.max(windowTargetBounds.width(), windowTargetBounds.height());
        }
        ...
        return new LauncherAnimationRunner(mHandler, false /* startAtFrontOfQueue */) {
            ...
            public void onCreateAnimation(RemoteAnimationTargetCompat[] targetCompats,
                ...
                if (anim == null) {
                    ...
                    // is initialized.
                    if (!FeatureOption.SPRD_ANIM_APP_EXIT) {
    ...
    private Animator getClosingWindowAnimators(RemoteAnimationTargetCompat[] targets) {
        ...
        closingAnimator.addUpdateListener(new MultiValueUpdateListener() {
            ...
            FloatProp mScale = FeatureOption.SPRD_ANIM_APP_EXIT ? new FloatProp(1f, 0.5f, 0, duration, ACCEL)
                                                                : new FloatProp(1f, 1f, 0, duration, DEACCEL_1_7);
            FloatProp mAlpha = FeatureOption.SPRD_ANIM_APP_EXIT ? new FloatProp(1f, 0f, 0, duration, LINEAR)
                                                                : new FloatProp(1f, 0f, 25, 125, LINEAR);
