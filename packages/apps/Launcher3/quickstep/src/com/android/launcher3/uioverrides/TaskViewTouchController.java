...
public abstract class TaskViewTouchController<T extends BaseDraggingActivity>
    ...
    private final View mRecentsViewContainer;
    private int mViewIndex = -1;

    public TaskViewTouchController(T activity) {
        mActivity = activity;
        mRecentsView = activity.getOverviewPanel();
        if (FeatureOption.SPRD_TASK_LOCK_SUPPORT && activity instanceof Launcher) {
            mRecentsViewContainer = ((Launcher)activity).getOverviewPanelContainer();
        } else if (FeatureOption.SPRD_TASK_LOCK_SUPPORT && activity instanceof RecentsActivity) {
            mRecentsViewContainer = ((RecentsActivity) activity).getOverviewPanelContainer();
        }  else {
            ...
            if (mCurrentAnimation != null) {
                ...
                for (int i = 0; i < mRecentsView.getChildCount(); i++) {
                    ...
                    if (mRecentsView.isTaskViewVisible(view) && mActivity.getDragLayer()
                        ...
                        if (FeatureOption.SPRD_TASK_LOCK_SUPPORT) {
                            directionsToDetectScroll = SwipeDetector.DIRECTION_BOTH;
                        } else if (!OverviewInteractionState.getInstance(mActivity)
    ...
    private void reInitAnimationController(boolean goingUp) {
        ...
        if (goingUp) {
            ...
            if(FeatureOption.SPRD_TASK_LOCK_SUPPORT && mTaskBeingDragged.isLocked()) {
                mPendingAnimation = mRecentsView.createTaskLockAnimation(mTaskBeingDragged, mEndDisplacement, maxDuration,false);
            } else {
            ...
            if(FeatureOption.SPRD_TASK_LOCK_SUPPORT) {
                mPendingAnimation = mRecentsView.createTaskLockAnimation(mTaskBeingDragged, mEndDisplacement, maxDuration,true);
            } else {
    ...
    private void bringToFront() {
        if (null == mRecentsViewContainer || mViewIndex >= 0) {
            return;
        }
        mViewIndex = ((ViewGroup) mRecentsViewContainer.getParent()).indexOfChild(mRecentsViewContainer);
        LogUtils.d(TAG,"bringToFront mViewIndex" + mViewIndex);
        mRecentsViewContainer.bringToFront();
    }
    private void moveToBack() {
        if (null == mRecentsViewContainer || -1 == mViewIndex) {
            return;
        }
        LogUtils.d(TAG,"moveToBack mViewIndex:" + mViewIndex);

        ViewGroup viewGroup = ((ViewGroup) mRecentsViewContainer.getParent());
        int count = viewGroup.getChildCount();
        for(int i = mViewIndex; i < count; i++) {
            View view = viewGroup.getChildAt(mViewIndex);
            if (view != mRecentsViewContainer) {
                if (LogUtils.DEBUG) LogUtils.d(TAG,"moveToBack i:" + i);
                viewGroup.bringChildToFront(viewGroup.getChildAt(mViewIndex));
            }
        }
        mViewIndex = -1;
    }
