...
import com.sprd.quickstepext.locktask.TaskLockStatus;
...
public abstract class RecentsView<T extends BaseActivity> extends PagedView implements Insettable {
    ...
    private static final int DISMISS_TASK_DURATION = 100;//300;
    ...
    private void applyLoadPlan(RecentsTaskLoadPlan loadPlan) {
        ...
        for (int i = requiredChildCount - 1; i >= 0; i--) {
            ...
            if (LogUtils.DEBUG_ALL) {
                LogUtils.d(TAG, "applyLoadPlan task: " + task.getTopComponent() + ", " + task.key);
            }
    ...
    private void removeTask(Task task, int index, PendingAnimation.OnEndListener onEndListener,
                            boolean shouldLog) {
        if (task != null) {
            LogUtils.d(TAG,"removeTask task:" + task.getTopComponent() + ", " + task.key);
            ...
            if (FeatureOption.SPRD_TASK_LOCK_SUPPORT) {
                TaskLockStatus.removeLockState(getContext(), task);
            }
        }
    }
    ...
    public PendingAnimation createAllTasksDismissAnimation(long duration) {
        ...
        mPendingAnimation.addEndListener((onEndListener) -> {
            if (onEndListener.isSuccess) {
                int i = 0;
                long mem = 0;
                if (FeatureOption.SPRD_SHOW_CLEAR_MEM_SUPPORT) {
                    mem =TaskMeminfo.getMemoryInfo(getContext()).availMem;
                }
                while (getChildCount() - i != 0) {
                    TaskView taskView = getPageAt(getChildCount() - i - 1);
                    Task task = taskView.getTask();
                    if (FeatureOption.SPRD_TASK_LOCK_SUPPORT && taskView.isLocked()) {
                        i++;
                        continue;
                    }
                ...
                onAllTasksRemoved();
                if (FeatureOption.SPRD_SHOW_CLEAR_MEM_SUPPORT) {
                    TaskMeminfo.showReleaseMemorySize(getContext(), i < count, mem);
                }
                if (FeatureOption.SPRD_TASK_LOCK_SUPPORT) {
                    mNeedResetTaskVisuals = (i > 0);
                }
            }
            mPendingAnimation = null;
        });
    ...
    private void addLockTaskAnimations(View taskView, AnimatorSet anim, float endDisplacement, long duration) {
        addAnim(ObjectAnimator.ofFloat(taskView, TRANSLATION_Y, endDisplacement),
                duration, LINEAR, anim);
    }

    public PendingAnimation createTaskLockAnimation(TaskView taskView ,
                                                    float endDisplacement, long duration,boolean needRemoveTask) {
        if (FeatureFlags.IS_DOGFOOD_BUILD && mPendingAnimation != null) {
            throw new IllegalStateException("Another pending animation is still running");
        }
        ...
        boolean needsCurveUpdates = false;
        addLockTaskAnimations(taskView, anim, endDisplacement, duration);
        ...
        // Add a tiny bit of translation Z, so that it draws on top of other views
        taskView.setTranslationZ(-1f);

        mPendingAnimation = pendingAnimation;
        mPendingAnimation.addEndListener((onEndListener) -> {
            if (onEndListener.isSuccess && needRemoveTask) {
                taskView.changeLockState();
            }
            resetTaskVisuals();
    ...
    protected int getMeminfoBottomPadding(DeviceProfile dp) {
        return 0;
    }

    public int getMeminfoBottomPadding() {
        return getMeminfoBottomPadding(mActivity.getDeviceProfile());
    }
