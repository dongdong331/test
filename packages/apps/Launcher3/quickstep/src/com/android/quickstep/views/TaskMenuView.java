...
public class TaskMenuView extends AbstractFloatingView {
    ...
    private void addMenuOptions(TaskView taskView) {
        Drawable icon;
        if (null !=  taskView.getTask().icon) {
            icon = taskView.getTask().icon.getConstantState().newDrawable();
        } else {
            icon = getResources().getSystem()
    ...
    private void orientAroundTaskView(TaskView taskView) {
        ...
        if (Utilities.isRtl(getResources())) {
            int x = mActivity.getRootView().getWidth() - 1
                    - (sTempRect.right - (sTempRect.width() - getMeasuredWidth()) / 2) - insets.right;
            setX(-x);
        } else {
