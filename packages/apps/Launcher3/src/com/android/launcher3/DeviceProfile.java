...
public class DeviceProfile {
    ...
    public int iconDrawablePaddingOffsetPx;
    ...
    public DeviceProfile(Context context, InvariantDeviceProfile inv,
        ...
        iconDrawablePaddingOffsetPx =
                res.getDimensionPixelSize(R.dimen.icon_drawable_padding_offset);
        iconLabelMaxLines = res.getInteger(R.integer.icon_label_max_lines);
    ...
    private void adjustToHideWorkspaceLabels() {
        ...
        allAppsCellHeightPx = allAppsIconSizePx + allAppsIconDrawablePaddingPx
                + Utilities.calculateTextHeight(allAppsIconTextSizePx) * iconLabelMaxLines
                + topBottomPadding * 2;
    }
    ...
    private void updateAvailableDimensions(DisplayMetrics dm, Resources res) {
        ...
        float rowHeight = getCellSize().y;
        if (cellHeightPx > rowHeight) {
            float scale = rowHeight / cellHeightPx;
            ...
            int maxHeight = (availableHeightPx - workspacePadding.y);
            int maxWidth = (availableWidthPx - workspacePadding.x);
            if (usedHeight > maxHeight || usedWidth > maxWidth) {
                float scale = Math.min(maxHeight / usedHeight, maxWidth / usedWidth);
                updateIconSize(scale, res, dm);
            }
        }
    ...
    private void updateIconSize(float scale, Resources res, DisplayMetrics dm) {
        ...
        cellHeightPx = iconSizePx + iconDrawablePaddingPx
                + Utilities.calculateTextHeight(iconTextSizePx) * iconLabelMaxLines;
        int cellYPadding = (getCellSize().y - cellHeightPx) / 2;
        if (cellYPadding > 0 && iconDrawablePaddingPx > cellYPadding && !isVerticalLayout
                && !isMultiWindowMode) {
    ...
    public void updateFolderIconSize(){
        boolean isNative = FolderIconUtils.getInstance().isNativeFolderIcon();
        folderIconSizePx = isNative ? IconNormalizer.getNormalizedCircleSize(iconSizePx): iconSizePx;
        folderIconOffsetYPx = (iconSizePx - folderIconSizePx) / 2;
    }

    public void updateAvailableFolderCellDimensions(DisplayMetrics dm, Resources res) {
        int folderBottomPanelSize = res.getDimensionPixelSize(R.dimen.folder_label_padding_top)
                + res.getDimensionPixelSize(R.dimen.folder_label_padding_bottom)
                + Utilities.calculateTextHeight(res.getDimension(R.dimen.folder_label_text_size)) * iconLabelMaxLines;
        ...
        if (FolderIconUtils.getInstance().isGridFolderIcon()) {
            int folderTopPanelSize = res.getDimensionPixelSize(R.dimen.grid_folder_footer_height)
                    + res.getDimensionPixelSize(R.dimen.grid_folder_name_margin_bottom);
            maxHeight = availableHeightPx - totalWorkspacePadding.y - folderMargin
                    - folderBottomPanelSize - folderTopPanelSize;
        } else {
            maxHeight = availableHeightPx - totalWorkspacePadding.y - folderMargin - folderBottomPanelSize;
        }
    ...
    private void updateFolderCellSize(float scale, DisplayMetrics dm, Resources res) {
        ...
        int textHeight = Utilities.calculateTextHeight(folderChildTextSizePx) * iconLabelMaxLines;
