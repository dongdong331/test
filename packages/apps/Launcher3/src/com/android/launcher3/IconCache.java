...
public class IconCache {
    ...
    protected CacheEntry cacheLocked(
        ...
        if (FeatureOption.SPRD_FAST_UPDATE_LABEL && entry != null) {
            LauncherActivityInfo info = infoProvider.get();
            if (info != null && LauncherModel.sIsLanguageChanging) {
    ...
    public synchronized void clear() {
        Preconditions.assertWorkerThread();
        for (ComponentKey key: mCache.keySet()){
            LauncherAppState.getInstance(mContext).getWidgetCache()
                    .removePackage(key.componentName.getPackageName(), key.user);
        }
        mIconDb.clear();
    }
    ...
    private static final class IconDB extends SQLiteCacheHelper {
        ...
        public IconDB(Context context, int iconPixelSize) {
            super(context, LauncherFiles.getAppIconsDb(),
