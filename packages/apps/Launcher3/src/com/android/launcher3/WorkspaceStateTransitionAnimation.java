    ...
    private void setWorkspaceProperty(LauncherState state, PropertySetter propertySetter,
        ...
        if (playAtomicComponent) {
            ...
            propertySetter.setViewAlpha(mLauncher.getHotseat().getLayout(), hotseatIconsAlpha,
                    fadeInterpolator);
            boolean isGridFolderOpened = FolderIconUtils.getInstance().isGridFolderIcon() &&
                    Folder.getOpen(mLauncher) != null;
            if (MultiModeController.isSingleLayerMode()) {
                //CASE: page indicator show only these case
                hotseatIconsAlpha = (!isGridFolderOpened
                        && (state == NORMAL || state == SPRING_LOADED)) ? 1 : 0;
            } else {
                hotseatIconsAlpha = isGridFolderOpened ? 0 : hotseatIconsAlpha;
            }
