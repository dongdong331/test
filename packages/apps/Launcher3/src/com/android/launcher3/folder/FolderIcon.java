...
public class FolderIcon extends FrameLayout implements FolderListener,
    ...
    public static FolderIcon fromXml(int resId, Launcher launcher, ViewGroup group,
        ...
        lp.topMargin = grid.iconSizePx + grid.iconDrawablePaddingPx - grid.iconDrawablePaddingOffsetPx;
        ...
        bindFolder(launcher, icon);
        icon.setAccessibilityDelegate(launcher.getAccessibilityDelegate());

        folderInfo.addListener(icon);

        if (FeatureOption.SPRD_FOLDER_ICON_MODE_SUPPORT) {
            FolderIconUtils.getInstance().fillIconLayoutAndBackground(icon);
            FolderIconUtils.getInstance().addListener(icon);
        }
    ...
    public static void bindFolder(Launcher launcher, FolderIcon icon){
        Folder folder = Folder.fromXml(launcher);
        folder.setDragController(launcher.getDragController());
        folder.setFolderIcon(icon);
        folder.bind(icon.mInfo);
    ...
    private void onDrop(final ShortcutInfo item, DragView animateView, Rect finalRect,
        ...
        int maxNumItems = getLayoutRule().maxNumItems();
        ...
        if (animateView != null) {
            ...
            int numItemsInPreview = Math.min(maxNumItems, index + 1);
            boolean itemAdded = false;
            if (itemReturnedOnFailedDrop || index >= maxNumItems) {
            ...
            float finalAlpha = index < maxNumItems ? 0.5f : 0f;
    ...
    public void setLayoutRule(ClippedFolderIconLayoutRule rule) {
        mPreviewLayoutRule = rule;
    }
    ...
    private float getLocalCenterForIndex(int index, int curNumItems, int[] center) {
        mTmpParams = mPreviewItemManager.computePreviewItemDrawingParams(
                Math.min(getLayoutRule().maxNumItems(), index), curNumItems, mTmpParams);
    ...
    private boolean shouldDrawUnreadEvent() {
        boolean shouldDrawUnreadEvent = false;
        Object obj = getTag();
        if (obj instanceof ItemInfo) {
            ItemInfo info = (ItemInfo) obj;
            if (info.unreadNum > 0 || LauncherAppState.isShowNotificationCount(mLauncher)) {
                shouldDrawUnreadEvent = true;
            }
        }
        return shouldDrawUnreadEvent;
    }
    ...
    private int getBadgeUnreadCount() {
        ...
        for (ShortcutInfo si : shortcutInfos) {
            BadgeInfo badgeInfo = mLauncher.getBadgeInfoForItem(si);
            if (si.unreadNum > 0) {
                unreadNum += si.unreadNum;
            } else if (badgeInfo != null && LauncherAppState.isShowNotificationCount(mLauncher)) {
                unreadNum += badgeInfo.getNotificationCount();
            }
        }
        return unreadNum;
    }
    ...
    public void drawBadge(Canvas canvas) {
        ...
        boolean shouldDrawNotification = (mBadgeInfo != null && mBadgeInfo.hasBadge()) || mBadgeScale > 0;

        if(shouldDrawUnreadEvent()) {
            UnreadLoaderUtils.drawUnreadEventIfNeed(canvas, this, getBadgeUnreadCount());
        } else if(shouldDrawNotification) {
    ...
    public List<BubbleTextView> getPreviewItemsOnPage(int page) {
        ...
        for (int rank = 0; rank < numItems; ++rank) {
            ...
            if (itemsToDisplay.size() == getLayoutRule().maxNumItems()) {
                break;
            }
        }
    ...
    public void onAdd(ShortcutInfo item, int rank) {
        ...
        if (FeatureOption.SPRD_BADGE_SUPPORT && UnreadLoaderUtils.isUnreadItemType(item.itemType)) {
            UnreadLoaderUtils.updateFolderUnreadNum((FolderInfo) this.getTag(), item.getTargetComponent(), item.unreadNum, item.user);
        }
    ...
    public void onRemove(ShortcutInfo item) {
        ...
        if (FeatureOption.SPRD_BADGE_SUPPORT && UnreadLoaderUtils.isUnreadItemType(item.itemType)) {
            UnreadLoaderUtils.updateFolderUnreadNum((FolderInfo) this.getTag(), item.intent.getComponent(), item.unreadNum, item.user);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (getTag() != null) {
            if (((ItemInfo)getTag()).container == LauncherSettings.Favorites.CONTAINER_HOTSEAT &&
                    HotseatAutoArrangeHelper.IS_SUPPORT) {
                boolean isInIconArea = getIconRect().contains((int)event.getX() + getScrollX(),
                        (int)event.getY() + getScrollY());
                if (!isInIconArea && event.getAction() == MotionEvent.ACTION_DOWN) {
                    return false;
                }
            }
        }
    ...
    public void removeListeners() {
        ...
        if (FeatureOption.SPRD_FOLDER_ICON_MODE_SUPPORT) {
            FolderIconUtils.getInstance( ).removeListener(this);
        }
    }
    ...
    public void onFolderIconModelChanged() {
        mBackground = null;
        mPreviewLayoutRule = null;
        mFolder = null;
        mPreviewVerifier = null;

        FolderIconUtils.getInstance().updateFolderIconIfModelChanged(mLauncher,this);
    }
