...
public class FolderIconPreviewVerifier {
    ...
    public void setFolderInfo(FolderInfo info) {
        ...
        mDisplayingUpperLeftQuadrant = numItemsInFolder > FolderIconUtils.getInstance().getMaxNumItems();
    }
    ...
    public boolean isItemInPreview(int page, int rank) {
        ...
        if (FolderIconUtils.getInstance().isNativeFolderIcon()
                && (page > 0 || mDisplayingUpperLeftQuadrant)) {
            int col = rank % mGridCountX;
            int row = rank / mGridCountX;
            return col < 2 && row < 2;
        }
        return rank < FolderIconUtils.getInstance().getMaxNumItems();
    }
}
