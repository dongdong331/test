...
public class PreviewItemManager {
    ...
    public void computePreviewDrawingParamsIfNeeded() {
        if (mReferenceDrawable != null) {
            mIcon.mBackground.setup(mIcon.mLauncher, mIcon, mIcon.getMeasuredWidth(), mIcon.getPaddingTop());
            mIcon.mPreviewLayoutRule.init(mIcon.mBackground.previewSize, mReferenceDrawable.getIntrinsicWidth(),
                    Utilities.isRtl(mIcon.getResources()));
    ...
    public void hidePreviewItem(int index, boolean hidden) {
        ...
        index = index + Math.max(mFirstPageParams.size() - mIcon.getLayoutRule().maxNumItems(), 0);
    ...
    void buildParamsForPage(int page, ArrayList<PreviewItemDrawingParams> params, boolean animate) {
        ...
        int numItemsInFirstPagePreview = page == 0 ? items.size() : mIcon.getLayoutRule().maxNumItems();
