...
public class LauncherIcons implements AutoCloseable {
    ...
    public BitmapInfo createBadgedIconBitmap(Drawable icon, UserHandle user, int iconAppTargetSdk,
        ...
        if (icon instanceof LayerDrawable) {
            icon = drawableToFixedSizeBitmapDrawable(icon);
        }
    ...
    private FixedSizeBitmapDrawable drawableToFixedSizeBitmapDrawable(Drawable drawable)
    {
        ...
        mCanvas.setBitmap(null);
        return new FixedSizeBitmapDrawable(bitmap);
    }
    /**
...
//        }
        ...
        width = (int) Math.ceil(textureWidth * scale);
        height = (int) Math.ceil(textureHeight * scale);
        ...
        if (Utilities.ATLEAST_OREO && icon instanceof AdaptiveIconDrawable) {
            ...
            int size = Math.min(Math.max(width, height), textureWidth - 2 * offset);
