...
public class GridSizeMigrationTask {
    ...
    public static void markForMigration(
        ...
        Utilities.getPrefs(context).edit()
                .putString(getWorkspaceMigrationPrefKey(context), UtilitiesExt.getPointString(gridX, gridY))
                .putInt(getHotseatMigrationPrefKey(context), hotseatSize)
                .apply();
    }
    ...
    public static boolean migrateGridIfNeeded(Context context) {
        ...
        String gridSizeString = UtilitiesExt.getPointString(idp.numColumns, idp.numRows);
        String workspace_migration_key = getWorkspaceMigrationPrefKey(context);
        String hotseat_migration_key = getHotseatMigrationPrefKey(context);

        if (gridSizeString.equals(prefs.getString(workspace_migration_key, "")) &&
                idp.numHotseatIcons == prefs.getInt(hotseat_migration_key, idp.numHotseatIcons)) {
        ...
        try {
            ...
            // Hotseat
            int srcHotseatCount = prefs.getInt(hotseat_migration_key, idp.numHotseatIcons);
            ...
            Point sourceSize = UtilitiesExt.parsePoint(prefs.getString(workspace_migration_key, gridSizeString));
            ...
            prefs.edit()
                    .putString(workspace_migration_key, gridSizeString)
                    .putInt(hotseat_migration_key, idp.numHotseatIcons)
                    .apply();
        }
    }
    ...
    private static String getWorkspaceMigrationPrefKey(Context context) {
        String prefWorkspaceKey = KEY_MIGRATION_SRC_WORKSPACE_SIZE;
        if (MultiModeController.isSupportDynamicChange()) {
            String mode = MultiModeController.getHomeScreenStylePrefValue(context);
            prefWorkspaceKey = mode + "_" + prefWorkspaceKey;
        }
        return prefWorkspaceKey;
    }

    private static String getHotseatMigrationPrefKey(Context context) {
        String prefHotSeatKey = KEY_MIGRATION_SRC_HOTSEAT_COUNT;
        if (MultiModeController.isSupportDynamicChange()) {
            String mode = MultiModeController.getHomeScreenStylePrefValue(context);
            prefHotSeatKey = mode + "_" + prefHotSeatKey;
        }
        return prefHotSeatKey;
    }
}
