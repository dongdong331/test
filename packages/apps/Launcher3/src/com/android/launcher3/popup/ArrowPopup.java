...
public abstract class ArrowPopup extends AbstractFloatingView {
    ...
    protected void reorderAndShow(int viewsToFlip) {
        ...
        if (mIsLeftAligned) {
            mArrow.post(() -> mArrow.setX(getX() + arrowCenterOffset - halfArrowWidth));
        } else {
            mArrow.post(() -> mArrow.setX(getX() + getMeasuredWidth() - arrowCenterOffset - halfArrowWidth));
        }
    ...
    protected void orientAboutObject() {
        ...
        int x = leftAlignedX;
        boolean canBeLeftAligned = leftAlignedX + width
        ...
        // Fix the error which the result x is not in valid Rect when RTL prefer to right or LTR prefer to left.
        ...
        int alignedEndOffset = iconWidth / 2 - shortcutDragHandleWidth / 2 - shortcutPaddingEnd;
        if ( x < (dragLayer.getLeft() + insets.left) && !mIsLeftAligned && canBeLeftAligned && mIsRtl) {
            mIsLeftAligned = true;
            x = leftAlignedX + alignedEndOffset;
        } else if ( x > (dragLayer.getRight() - width - insets.right) && mIsLeftAligned && canBeRightAligned && !mIsRtl) {
            mIsLeftAligned = false;
            x = rightAlignedX - alignedEndOffset;
        }
        ...
        if (y + height > dragLayer.getBottom() - insets.bottom) {
            ...
            int rightSide = leftAlignedX + iconWidth;
            int leftSide = rightAlignedX - iconWidth;
            if (!mIsRtl) {
                if (rightSide + width < dragLayer.getRight() - insets.right) {
                    x = rightSide;
                    mIsLeftAligned = true;
                } else {
                    x = leftSide;
                ...
                if (leftSide > dragLayer.getLeft() + insets.left) {
                    x = leftSide;
                    mIsLeftAligned = false;
                } else {
                    x = rightSide;
        ...
        if (x - insets.left < 0
                || x + width > dragLayer.getWidth() - insets.right) {
            // If we are still off screen, center horizontally too.
            mGravity |= Gravity.CENTER_HORIZONTAL;
            x = dragLayer.getWidth() / 2 - getMeasuredWidth() / 2;
        }
        final float tempX = x;
        post(() -> {
            setX(tempX);
