...
public class PopupContainerWithArrow extends ArrowPopup implements DragSource,
    ...
    public static PopupContainerWithArrow showForIcon(BubbleTextView icon) {
        ...
        if (pkg == null || !UtilitiesExt.isAppInstalled(icon.getContext(), pkg, itemInfo.user)) {
            return null;
        }
    ...
    public void closePopupWindowIfNeeded(PackageUserKey postedPackageUserKey) {
        if (mOriginalIcon == null || !(mOriginalIcon.getTag() instanceof ItemInfo)) {
            return;
        }
        ItemInfo itemInfo = (ItemInfo) mOriginalIcon.getTag();
        PackageUserKey originalPackageUser = PackageUserKey.fromItemInfo(itemInfo);
        if (postedPackageUserKey != null && originalPackageUser != null
                &&originalPackageUser.equals(postedPackageUserKey)) {
            closeComplete();
        }
    }
}
