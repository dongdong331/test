...
public class LooperIdleLock implements MessageQueue.IdleHandler, Runnable {
    ...
    public boolean awaitLocked(long ms) {
        if (mLock instanceof LoaderTask) {
            LoaderTask task = (LoaderTask) mLock;
            if (task != null && task.isWaitForUnLock()) {
                if (LogUtils.DEBUG_LOADER) {
                    LogUtils.d("LoaderTask", "Stop the task when wait for unlock");
                }
                task.stopLocked();
                return false;
            }
