...
public class AppActionButtonPreferenceController extends BasePreferenceController
    ...
    // UNISOC: Bug894966 cannot disabled app list.
    private String[] mCannotDisabledApps;
    private String[] mCannotForceStopApps;
    ...
    public AppActionButtonPreferenceController(Context context, AppInfoDashboardFragment parent,
        ...
        // UNISOC: Bug894966 cannot disabled app list.
        mCannotDisabledApps = context.getResources().getStringArray(R.array.cannot_disabled_app);
        mCannotForceStopApps = context.getResources().getStringArray(R.array.cannot_force_stop_app);
    }
    ...
    boolean handleDisableable(AppEntry appEntry, PackageInfo packageInfo) {
        ...
        // UNISOC: Bug894966 cannot disabled app list.
        if (mCannotDisabledApps != null) {
            for (String pkg : mCannotDisabledApps) {
                if (pkg != null && pkg.equals(appEntry.info.packageName)) {
                    disableable = false;
                    break;
                }
            }
    ...
    void checkForceStop(AppEntry appEntry, PackageInfo packageInfo) {
        ...
        if (mCannotForceStopApps != null) {
            Log.w(TAG, "User can't force stop whitelist packages");
            for (String pkg : mCannotForceStopApps) {
                if (pkg != null && pkg.equals(packageInfo.packageName)) {
                    updateForceStopButton(false);
                    break;
                }
            }
