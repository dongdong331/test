...
abstract class BluetoothNameDialogFragment extends InstrumentedDialogFragment
    ...
    public void onResume() {
        ...
        if (mOkButton == null) {
            ...
            mOkButton.setEnabled(mDeviceNameEdited && ((deviceName != null) ? !(deviceName.toString().trim().isEmpty()) : false));    // Ok button enabled after user edits
        }
        if (mAlertDialog != null && mAlertDialog.isShowing() && mDeviceNameView != null) {
            mDeviceNameView.requestFocus();
        }
    }
