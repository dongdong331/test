...
public class AdvancedConnectedDeviceController extends BasePreferenceController {
    ...
    public static int getConnectedDevicesSummaryResourceId(Context context) {
        ...
        if (Utils.WCN_DISABLED) {
            return getConnectedDevicesSummaryResourceIdForNoWCN(context, nfcPreferenceController,
    ...
    @VisibleForTesting
    static int getConnectedDevicesSummaryResourceIdForNoWCN(Context context, NfcPreferenceController
            nfcPreferenceController, boolean isDrivingModeAvailable) {
        final int resId;
        boolean isEnableWifiDisplay = context.getResources().getBoolean(com.android.internal.R.bool.config_enableWifiDisplay);
        if (nfcPreferenceController.isAvailable()) {
            if (Utils.WCN_DISABLED) {
                resId = R.string.connected_devices_dashboard_no_wcn_summary;
            } else {
                if (isEnableWifiDisplay) {
                    resId = R.string.connected_devices_dashboard_summary;
                } else {
                    resId = R.string.connected_devices_dashboard_no_cast_summary;
                }
            }
        } else {
            if (Utils.WCN_DISABLED) {
                resId = R.string.connected_devices_dashboard_no_wcn_no_nfc_summary;
            } else {
                if (isEnableWifiDisplay) {
                    resId = R.string.connected_devices_dashboard_no_nfc_summary;
                } else {
                    resId = R.string.connected_devices_dashboard_no_nfc_no_cast_summary;
                }
            }
        }
        return resId;
