...
import com.android.settings.fuelgauge.SprdAppStandbyOptimizerPreferenceController;
import com.android.settings.fuelgauge.SprdBatterySaverManageController;
import com.android.settings.fuelgauge.SprdLockScreenBatterySaverPreferenceController;
import com.android.settings.fuelgauge.SprdPowerIntensiveAppsPreferenceController;
...
public class PowerUsageSummary extends PowerUsageBase implements OnLongClickListener,
    ...
    /*UNISOC: 894618 remove battery saver in guest mode. @{*/
    private static final String KEY_BATTERY_SAVER_MANAGE = "battery_saver_manage";
    private static final String KEY_LOW_RESOLUTION_POWER_SAVING = "low_resolution_power_saving";
    ...
    private SwitchPreference mLowResolutionPowerSaverPref;
    ...
    public void onCreate(Bundle icicle) {
        ...
        mLowResolutionPowerSaverPref = (SwitchPreference) findPreference(KEY_LOW_RESOLUTION_POWER_SAVING);
        /*UNISOC : 894618 remove battery saver in guest mode. @{*/
        if (isSupportBMFeature && UserHandle.myUserId() != UserHandle.USER_OWNER) {
            removePreference(KEY_BATTERY_SAVER_MANAGE);
            removePreference(KEY_APP_BATTERY_SAVER_SETTING);
            removePreference(KEY_APP_AUTO_RUN);
        }
        /*@}*/
    ...
    /*Bug958788: Cancelling the split screen , lowResolutionPowerSaver should be enabled.@{*/
    @Override
    public void onMultiWindowModeChanged(boolean isInMultiWindowMode) {
        super.onMultiWindowModeChanged(isInMultiWindowMode);
        if (null != mLowResolutionPowerSaverPref) {
            mLowResolutionPowerSaverPref.setEnabled(!isInMultiWindowMode);
        }
    }
    /*@}*/
    ...
    protected List<AbstractPreferenceController> createPreferenceControllers(Context context) {
        ...
        //UNISOC: 894618 remove battery saver in guest mode.
        if (isSupportBMFeature && UserHandle.myUserId() == UserHandle.USER_OWNER) {
            controllers.add(new SprdAppStandbyOptimizerPreferenceController(context));
            controllers.add(new SprdLockScreenBatterySaverPreferenceController(context));
            controllers.add(new SprdPowerIntensiveAppsPreferenceController(context));
            controllers.add(new SprdBatterySaverManageController(context));
        }
        /*@}*/
        return controllers;
    }
            ...
            new BaseSearchIndexProvider() {
                ...
                public List<SearchIndexableResource> getXmlResourcesToIndex(
                    ...
                    if (1 == SystemProperties.getInt("persist.sys.pwctl.enable", 1)) {
                        sir.xmlResId = R.xml.sprd_power_usage_summary;
                    } else {
                ...
                public List<String> getNonIndexableKeys(Context context) {
                    ...
                    /*Bug 900576  remove battery saver in guest mode@{*/
                    if (1 == SystemProperties.getInt("persist.sys.pwctl.enable", 1) &&
                            UserHandle.myUserId() != UserHandle.USER_OWNER) {
                        niks.add(KEY_BATTERY_SAVER_MANAGE);
                        niks.add(KEY_APP_BATTERY_SAVER_SETTING);
                        niks.add(KEY_APP_BATTERY_SAVER);
                        niks.add(KEY_LOCK_SCREEN_BATTERY_SAVE);
                        niks.add(KEY_APP_AUTO_RUN);
                        niks.add(KEY_POWER_INTENSIVE_APPS);
                    }
                    /*@}*/
                    //Bug 897942 : low reolution power saving should not be searched if it is not supported or in guest mode.
                    if (UserHandle.myUserId() != UserHandle.USER_OWNER ||
                            !LowResolutionPowerSaving.getInstance(context).isSupport()) {
                        niks.add(KEY_LOW_RESOLUTION_POWER_SAVING);
                    }
