...
public class WifiSettings extends RestrictedSettingsFragment
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        //UNISOC: added for bug 896975, hide setting entrance in Ultra Saving Mode
        if (isUltraSavingMode()) {
    ...
    private void addPreferences() {
        ...
        mWifiFeaturesUtils = WifiFeaturesUtils.getInstance(getActivity());
        mAddonStub = SprdWifiSettingsAddon.getInstance(getActivity());
        supportCMCC = mAddonStub.isSupportCmcc();
        if (supportCMCC) {
            addPreferencesFromResource(R.xml.cmcc_wifi_settings);
        } else {
        ...
        if (SettingsActivity.getInstance() != null
                && SettingsActivity.getInstance().isSubsidyLocked()) {
            mAdditionalSettingsPreferenceCategory.removeAll();
            mAdditionalSettingsPreferenceCategory.setVisible(false);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mWifiTracker = WifiTrackerFactory.create(
                getActivity().getApplicationContext(), this, getLifecycle(), true, true);
    ...
    private WifiEnabler createWifiEnabler() {
        ...
        return new WifiEnabler(activity.getApplicationContext(), new SwitchBarController(activity.getSwitchBar()),
    ...
    public void onResume() {
        ...
        if (mWifiEnabler != null) {
            mWifiEnabler.resume(activity.getApplicationContext());
        }
    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo info) {
            ...
            if (preference instanceof LongPressAccessPointPreference) {
                ...
                Log.d(TAG,"AUTO_JOIN_VODAFONE_WIFI_SSID= "+WifiFeaturesUtils.AUTO_JOIN_VODAFONE_WIFI_SSID);
                if (WifiFeaturesUtils.SUPPORT_OPERATOR_VODAFONE_UK.equals(mWifiFeaturesUtils.getOperatorSupport())
                        && WifiFeaturesUtils.AUTO_JOIN_VODAFONE_WIFI_SSID.equals(mSelectedAccessPoint.getSsid().toString())) {
    ...
    public void onWifiStateChanged(int state) {
        ...
        switch (wifiState) {
            ...
            case WifiManager.WIFI_STATE_ENABLING:
                if (supportCMCC) mAccessPointsPreferenceCategory.setVisible(false);
    ...
    private void updateAccessPointPreferences() {
        ...
        if (supportCMCC) removeConnectedAccessPointPreference();
        mAccessPointsPreferenceCategory.removeAll();

        int index =
                configureConnectedAccessPointPreferenceCategory(accessPoints) ? 1 : 0;
        int numAccessPoints = accessPoints.size();
        if (supportCMCC && numAccessPoints > 0) mAccessPointsPreferenceCategory.setVisible(true);
        boolean isConnectedPreferenceCategoryVisible = index > 0;
        if (supportCMCC && isConnectedPreferenceCategoryVisible) mConnectedAccessPointPreferenceCategory.setVisible(true);
    ...
    private void setAdditionalSettingsSummaries() {
        if (SettingsActivity.getInstance() != null
            ...
            mConfigureWifiSettingsPreference.setVisible(false);
            mSavedNetworksPreference.setVisible(false);
            Log.d(TAG, "setAdditionalSettingsSummaries--- SubsidyLocked");
            return;
        }
    ...
    private void setOffMessage() {
        ...
        if (SettingsActivity.getInstance() != null
                && SettingsActivity.getInstance().isSubsidyLocked()) {
            mStatusMessagePreference.setText(title, null, null);
        } else {
    ...
    /* package */ void submit(WifiConfigController configController) {
        ...
        // Add for bug939604 Autoconnect to the ap when modify the conf
    ...
    /*
     * Added for bug 907292, do not need to config the search Menu in UlralSavingMode
     */
    @Override
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
       if (isUltraSavingMode()) return;
       super.onCreateOptionsMenu(menu, inflater);
    }

    /*
     * judge if it's in Ultra SavingMode
     */
    private boolean isUltraSavingMode(){
        mPowerManagerEx = IPowerManagerEx.Stub.asInterface(ServiceManager.getService("power_ex"));
        if (null != mPowerManagerEx) {
            return (getPowerSaveModeInternal() == PowerManagerEx.MODE_ULTRASAVING);
        }
        return false;
    }
     ...
     * get Power SaveMode
     */
    private int getPowerSaveModeInternal(){
        try {
