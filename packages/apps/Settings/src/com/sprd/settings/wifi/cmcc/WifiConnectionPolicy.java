...
public class WifiConnectionPolicy extends BroadcastReceiver {
    ...
    private static WifiFeaturesUtils mWifiFeaturesUtils;
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(!mWifiFeaturesUtils.isSupportCMCC()) {
            logd("Do nothing when not cmcc version");
            return;
        }
        ...
        if(WifiManager.WIFI_STATE_CHANGED_ACTION.equals(action)) {
            ...
            if (manulConnect && SupplicantState.isConnecting(state)) {
                ...
                || WifiFeaturesUtils.WIFI_SCAN_RESULTS_AVAILABLE_ACTION.equals(action)) {
        ...
        } else if (WifiFeaturesUtils.WIFI_CONNECTED_AP_ABSENT_ACTION.equals(action)) {
            mWlanToWlanDialogCancleFlag = false;
        ...
        else if (WifiFeaturesUtils.WIFI_DISABLED_WHEN_CONNECTED_ACTION.equals(action)) {
            if (!handledRememberedFlag(context, DIALOG_TYPE_WLAN_TO_MOBILE)) {
                mLastWifiInfo = (WifiInfo) intent.getParcelableExtra(WifiManager.EXTRA_WIFI_INFO);
                popUpWlanToMobileDialog(context);
            }
        }
        else if (WifiFeaturesUtils.WIFI_SCAN_RESULT_BSS_REMOVED_ACTION.equals(action)) {
        ...
        else if (action.equals(WifiFeaturesUtils.ALARM_FOR_CONNECT_WIFI_ACTION)) {
            setConnectWifiAlarm(context);
        } else if (action.equals(WifiFeaturesUtils.ALARM_FOR_DISCONNECT_WIFI_ACTION)) {
    ...
    public static void setDialogShowing(boolean flag) {
        mDialogIsShowing = flag;
        logd("setDialogShowing " + flag + ", mDialogIsShowing=" +mDialogIsShowing);
    }

    public static void setMobileToWlanPolicy(Context context, int value) {
        Settings.Global.putInt(context.getContentResolver(), WifiFeaturesUtils.WIFI_MOBILE_TO_WLAN_POLICY, value);
    ...
    public static int getMobileToWlanPolicy(Context context) {
        int value =Settings.Global.getInt(context.getContentResolver(),
               WifiFeaturesUtils.WIFI_MOBILE_TO_WLAN_POLICY, MOBILE_TO_WLAN_AUTO);
        return value;
    }
    ...
    private static void enableAutoConnect() {
        if (mDialogIsShowing == false) {
            ...
            mWifiFeaturesUtils.setAutoConnect(true);
        }
    }
    ...
    private static void disableAutoConnect(boolean forceDisconnect) {
        ...
        //}
        mWifiFeaturesUtils.setAutoConnect(false);
    }
    ...
    private static List<AccessPoint> getMatchedAccessPoints(Context context) {
        ...
        logd("getMatchedAccessPoints matchedAPs.size = " + matchedAPs.size() + ", scannedAPs.size = " + scannedAPs.size() + ", configuredAPs.size = " + configuredAPs.size());
        return matchedAPs;
    }
