...
public class WallpaperCropActivity extends Activity implements Handler.Callback {
    ...
    protected void onPause() {
        ...
        super.onPause();
        if (Utilities.DEBUG) {
            Log.d(LOGTAG, "onPause took " + (SystemClock.uptimeMillis() - start) + "ms");
        }
    }
    ...
    protected void onStop() {
        ...
        super.onStop();
        if (Utilities.DEBUG) {
            Log.d(LOGTAG, "onStop took " + (SystemClock.uptimeMillis() - start) + "ms");
        }
    }
    ...
    public boolean handleMessage(Message msg) {
        if (msg.what == MSG_LOAD_IMAGE) {
            ...
            if (req.src == null) {
                Drawable defaultWallpaper = WallpaperManager.getInstance(this).getBuiltInDrawable();
        ...
        if (success) {
            ...
            if (req.scaleAndOffsetProvider != null) {
                ...
                Point wallpaperSize = WallpaperUtils.getDefaultWallpaperSize(
                        getResources(), getWindowManager(), src.getImageWidth());
    ...
    public void cropImageAndSetWallpaper(Resources res, int resId, boolean shouldFadeOutOnFinish) {
        ...
        Point outSize = WallpaperUtils.getDefaultWallpaperSize(getResources(),
                getWindowManager(), drawable.getIntrinsicWidth());
        RectF crop = Utils.getMaxCropRect(
                inSize.x, inSize.y, outSize.x, outSize.y, false);
        // Passing 0, 0 will cause launcher to revert to using the
        // default wallpaper size
        CropAndFinishHandler onEndCrop = new CropAndFinishHandler(outSize,
    ...
    public void setBoundsAndFinish(Point bounds, boolean overrideTransition) {
        ...
        if (Utilities.DEBUG) {
            Log.d(LOGTAG, "setBoundsAndFinish: start to finish the activity.");
        }
        finish();
