...
public abstract class InputStreamProvider {
     ...
     */
    public abstract InputStream newStreamNotNull() throws Exception;
    ...
    public Point getImageBounds() {
        ...
        if (is != null) {
            ...
            if (Utilities.DEBUG) {
                Log.d(TAG, "getImageBounds: BitmapFactory.decodeStream took "
    ...
    public Bitmap readCroppedBitmap(RectF cropBounds, int outWidth, int outHeight, int rotation) {
        if (outWidth <= 0 || outHeight <= 0) {
            Log.w(TAG, "output rect has bad values for image");
            return null;
        }
        ...
        try {
            ...
            if (Utilities.DEBUG) {
                Log.d(TAG, "readCroppedBitmap: BitmapRegionDecoder.newInstance took "
                        + (SystemClock.uptimeMillis() - now) + "ms");
            }
        } catch (Exception e) {
        ...
        if (decoder != null) {
            ...
            if (Utilities.DEBUG) {
                Log.d(TAG, "readCroppedBitmap: BitmapRegionDecoder.decodeRegion took "
        ...
        if (outWidth > 0 && outHeight > 0 || rotation > 0) {
            ...
            if (tmp != null) {
                ...
                if (Utilities.DEBUG) {
                    Log.d(TAG, "readCroppedBitmap: Canvas.drawBitmap took "
                            + (SystemClock.uptimeMillis() - start) + "ms");
                }
                crop = tmp;
            }
    ...
    public static InputStreamProvider fromUri(final Context context, final Uri uri) {
        return new InputStreamProvider() {
            @Override
            public InputStream newStreamNotNull() throws Exception {
                long now = SystemClock.uptimeMillis();
                InputStream stream = context.getContentResolver().openInputStream(uri);
                if (Utilities.DEBUG) {
                    Log.d(TAG, "fromUri:newStreamNotNull took "
                            + (SystemClock.uptimeMillis() - now) + "ms");
                }
