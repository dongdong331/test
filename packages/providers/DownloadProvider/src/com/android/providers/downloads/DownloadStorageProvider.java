...
public class DownloadStorageProvider extends FileSystemProvider {
    private static final String TAG = "DownloadManager:DownloadStorageProvider";
    ...
    private static final String[] DEFAULT_DOCUMENT_PROJECTION = new String[] {
            ...
            Document.COLUMN_SIZE, Document.COLUMN_GRANT_URI,
    };
    ...
    private void includeDownloadFromCursor(MatrixCursor result, Cursor cursor,
        ...
        if (DEBUG) Log.d(TAG, "includeDownloadFromCursor  docId=" + docId);
        final String displayName = cursor.getString(
                cursor.getColumnIndexOrThrow(DownloadManager.COLUMN_TITLE));
        final String  extensionMimeType = getTypeForName(displayName);
        int fileType = MediaFile.getFileTypeForMimeType(extensionMimeType);
        ...
        } else if (extensionMimeType != null && !mimeType.equals(extensionMimeType)
        ...
        switch (status) {
            case DownloadManager.STATUS_SUCCESSFUL:
                ...
                if (isSupportThumbnail(mimeType)) {
                    extraFlags |= Document.FLAG_SUPPORTS_THUMBNAIL;
                }
                break;
            case DownloadManager.STATUS_PAUSED:
            case DownloadManager.STATUS_WAITING_FOR_CONFIRM_NETWORK:
            case DownloadManager.STATUS_CONFIRMED:
        ...
        String grantUriName = null;
        if (Helpers.isWritePermissionNeeded(localFilePath)) {
            grantUriName = Helpers.generateGrantUri(localFilePath).toString();
        }

        final RowBuilder row = result.newRow();
        row.add(Document.COLUMN_DOCUMENT_ID, docId);
        row.add(Document.COLUMN_GRANT_URI, grantUriName);
