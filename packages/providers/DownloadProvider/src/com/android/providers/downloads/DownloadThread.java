...
import static android.provider.Downloads.Impl.STATUS_PENDING;
import static android.provider.Downloads.Impl.STATUS_WAITING_FOR_CONFIRM_NETWORK;
import static android.provider.Downloads.Impl.STATUS_CONFIRMED;
...
public class DownloadThread extends Thread {
    ...
    public class DownloadInfoDelta {
        ...
        private ContentValues buildContentValues() {
            ...
            logDebug("buildContentValues, mStatus="+mStatus+"  mControl="+mControl);
            logDebug("buildContentValues, mToNetConfirm="+mToNetConfirm);
            if(mToNetConfirm && !Downloads.Impl.isStatusCompleted(mStatus)){
                logDebug("buildContentValues, confirming so not to write status into database");
            } else {
        ...
        public void writeToDatabaseOrThrow() throws StopRequestException {
            ...
            if (count == 0) {
                ...
                if ( control== CONTROL_PAUSED || control == Constants.TAG_PAUSED_BY_OWNER) {
    ...
    public void run() {
        ...
        try {
            ...
            if (isStatusRetryable(mInfoDelta.mStatus)) {
                ...
                if (mIsDownloadAlertEnabled) {
                    ...
                    logDebug("StopRequestException, old networkType="+mNetworkType);
                    logDebug("StopRequestException, new networkInfo="+info);
            ...
            if(mInfoDelta.mStatus == STATUS_PAUSED_BY_APP){
                mInfoDelta.mControl = Constants.TAG_PAUSED_BY_OWNER;
                ...
                if (mInfo.mStatus == STATUS_PAUSED_BY_APP) {
                    ...
                    mInfoDelta.mControl = Constants.TAG_PAUSED_BY_OWNER;
                    logDebug("StopRequestException, STATUS_PAUSED_BY_APP!");
                }
            }
             ...
             *@{
             */
            DownloadsDRMUtils.getInstance(mContext).notifyDownloadCompleted(this, mContext, mInfoDelta);
            /*@}*/
        ...
        if (isFinalStatusRetryable(mInfoDelta.mStatus)) {
            logDebug("DownloadThread run finish and retry, mToNetConfirm="+mToNetConfirm);
            if(!mToNetConfirm){
                logDebug("DownloadThread run finish and retry, needsReschedule set true");
                needsReschedule = true;
            }
        } else {
            if (Downloads.Impl.isStatusCompleted(mInfoDelta.mStatus)) {
                if (mInfo.shouldScanFile(mInfoDelta.mStatus) && !DownloadDrmHelper.isDrmConvertNeeded(mInfoDelta.mMimeType)) {
    ...
    private void transferData(HttpURLConnection conn) throws StopRequestException {
        ...
        logDebug("transferData  HttpURLConnection");
        ...
        try {
            ...
            try {
                final Uri fileUri;
                if (Helpers.isWritePermissionNeeded(mInfoDelta.mFileName)) {
                    fileUri = Helpers.generateGrantUri(mInfoDelta.mFileName);
                } else {
                    fileUri = mInfo.getAllDownloadsUri();
                }
            ...
            if (mInfoDelta.mFileName != null) {
                if (Helpers.isWritePermissionNeeded(mInfoDelta.mFileName)) {
                    try {
                        ...
                        logWarning("finalizeDestination deleteDocument FileNotFoundException");
                    }
                } else {
    ...
    private boolean isFinalStatusRetryable(int status) {
        boolean result;
        logDebug("isFinalStatusRetryable status="+status);
        switch (status) {
        ...
        logDebug("isFinalStatusRetryable : "+result);
        return result;
    }
    ...
    private NetworkCallback mNetworkCallback = new NetworkCallback() {
        ...
        public void onLost(Network network) {
            ...
            logDebug("Network lost, networkType="+networkType);
            if (!ConnectivityManager.isNetworkTypeMobile(networkType)) {
                 logDebug("Network lost, mToNetConfirm set true");
                 mToNetConfirm = true;
            }
        }
    };
