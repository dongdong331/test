...
//                   "END;");
    ...
    private boolean upgradeDatabaseToVersion68(SQLiteDatabase db) {
        try {
            String szSQL ="DROP VIEW  IF EXISTS SMS_MMS_VIEW;";

            db.execSQL(szSQL);
            Log.e(TAG, "[upgradeDatabaseToVersion68] drop old view "+szSQL);
            CreateViewMMssmsView(db) ;
			CheckTable(db);
            return true;
        } catch (SQLiteException e) {
            Log.e(TAG, "[upgradeDatabaseToVersion68] Exception create view  "
                    + "display_originating_addr; " + e);
            return false;
        }
    }
