...
public class MmsBackupService extends Service implements Defines {
    ...
    private int deduplicate(IAppBackupRestoreObserver observer, final int categoryCode) {
        BackupLog.log(TAG, "=========1==onDeduplicate, categoryCode = " + categoryCode);
        final long origId = Binder.clearCallingIdentity();
        //final IAppBackupRestoreObserver dObserver = observer;
        try {
            // SQLiteDatabase db =
            // MmsSmsDatabaseHelper.getInstance(MmsBackupService.this)
            // .getWritableDatabase();
            SQLiteDatabase db = MmsSmsProviderAdapter
                    .get(MmsBackupService.this).getSQLiteDatabase();
            if (categoryCode == SHORT_MESSAGE_CODE) {
                BackupLog.log(TAG,
                        "=========2=====onDeduplicate, categoryCode = "
                                + categoryCode);
                uniqueSms(db);
                observer.onResult(FLAG_DUPLICATION_SUCCESS);
                BackupLog.log(TAG,
                        "==========222=====onDeduplicate, FLAG_DUPLICATION_SUCCESS = "
                                + FLAG_DUPLICATION_SUCCESS);
            } else if (categoryCode == MEDIA_MESSAGE_CODE) {
                BackupLog.log(TAG,
                        "=========3====onDeduplicate, categoryCode = "
                                + categoryCode);
                uniqueMms(db);
                observer.onResult(FLAG_DUPLICATION_SUCCESS);
                BackupLog.log(TAG,
                        "=========3333====onDeduplicate, FLAG_DUPLICATION_SUCCESS: "
                                + FLAG_DUPLICATION_SUCCESS);
            }
            ...
            try {
                BackupLog.log(TAG,
                        "=========4====onDeduplicate, categoryCode = "
                                + categoryCode);
                observer.onResult(FLAG_FAIL);
                BackupLog.log(TAG,
                        "=========44444====onDeduplicate, FLAG_FAIL = "
                                + FLAG_FAIL);
    ...
    private void uniqueMms(SQLiteDatabase db) {
        ...
        Cursor cursor = db
                .rawQuery("select _data from part where mid in (" +
                        ...
                        "select _id from pdu group by date, thread_id))",
                        null);
        try {
    ...
    private List<Account> getAccounts() {
        ...
        if (!isEmptyCollection(activeSubIds)) {
            ...
            for (int i = 0; i < activeSubIds.size(); i++) {
                ...
                /*@}*/
                String name = MmsBackupService.this.getString(R.string.sim_x,phoneId+1);
                Account account = new Account(String.valueOf(i), name, null, true, true);
                BackupLog.log(TAG, "name:" + name + ", subId:" + subId + ", phoneId:" + phoneId);
            ...
            String name = MmsBackupService.this.getString(R.string.sim_all);
            ...
            try {
                switch (nMsg) {
                    case CMD_REPORT_RESUALT:
                        BackupLog.log(TAG, "=========OnNotify====nValue: "+nValue);
