...
public abstract class MmsRequest {
    ...
    public void execute(Context context, MmsNetworkManager networkManager,boolean isfirstTry) {
        ...
        boolean isVowifiConnected=PhoneUtils.isOperatorSupport(context,mSubId) && networkManager.isVowifiSmsEnable(mSubId);
        Log.d("andy"," MmsRequest execute isVowifiConnected = " + isVowifiConnected);
            ...
            for (int i = 0; i < RETRY_TIMES; i++) {
                try {
                    ...
                    try {
                        ...
                        response = doHttp(context, networkManager, apn,firstTry);
                    ...
                    if (e.getMessage().contains(
                            "Invalid FDN destinationAddress")) {
                        result = SmsManager.MMS_ERROR_FDN_CHECKED_CONTACRS;
                    }
                    // sprd 596495 end
                }catch (Exception e) {
     ...
     */
    protected abstract byte[] doHttp(Context context, MmsNetworkManager netMgr, ApnSettings apn,boolean firstTry)
