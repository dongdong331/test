...
public final class CallLogManager extends CallsManagerListenerBase {
    ...
    public void onCallStateChanged(Call call, int oldState, int newState) {
        /* SPRD: Log MT conference calllog.
        add for bug 903561    @{ */
        boolean shouldLogConference = false;
        if ((call.getConnectionCapabilities() & Connection.PROPERTY_IS_MT_CONFERENCE_CALL) > 0
                && (call.getConnectionCapabilities() & Connection.CAPABILITY_CONFERENCE_HAS_NO_CHILDREN) > 0
                && !((call.getConnectionCapabilities() & Connection.CAPABILITY_MANAGE_CONFERENCE) > 0)) {
            shouldLogConference = true;
            Log.d(TAG, "onCallStateChanged, log mt conference call log.");
        }
        /* @}  */
        ...
        if (isNewlyDisconnected &&
                (oldState != CallState.SELECT_PHONE_ACCOUNT &&
                        (!call.isConference()|| shouldLogConference)  &&
                        !isCallCanceled) &&
                !call.isExternalCall() &&
                (!call.isSelfManaged() ||
    ...
    void logCall(Call call, int type, boolean showNotificationForMissedCall) {
        ...
        // SPRD Feature Porting: Show Rejected calls notifier feature.
    ...
    void logCall(Call call, int callLogType,
        ...
        /* SPRD: Bug 698812 & 920819 add For VOLTE and VoWiFi CallLog icon. @{ */
        int extraFeature = 0;
    ...
    private void logCall(
        ...
        boolean mtnokToLogEmergencyNumber = false;
        boolean telcelokToLogEmergencyNumber = false;
       /* CarrierConfigManager configManager = (CarrierConfigManager) mContext.getSystemService(
        ...
        // SPRD: Modify for bug 900121
        okToLogEmergencyNumber = mContext.getResources()
                .getBoolean(R.bool.allow_emergency_numbers_in_call_log);

        //UNISOC:add for bug971976
        mtnokToLogEmergencyNumber=mContext.getResources()
                .getBoolean(R.bool.mtn_allow_emergency_numbers_in_call_log);
        //UNISOC:add for bug972709
        telcelokToLogEmergencyNumber=mContext.getResources()
                .getBoolean(R.bool.telcel_allow_emergency_numbers_in_call_log);
        // Don't log emergency numbers if the device doesn't allow it.
        //UNISOC:modify for bug971976,bug972709
        final boolean isOkToLogThisCall = (!isEmergency || okToLogEmergencyNumber ||(mtnokToLogEmergencyNumber && number.equals("112")) ||(telcelokToLogEmergencyNumber && (number.equals("112") ||number.equals("911") ||number.equals("060")))) && !isPotentialMMICode(number);
    ...
    private boolean shouldSaveHdInfo(Call call, PhoneAccountHandle accountHandle) {
        ...
        // UNISOC:add for 1068889
        if(mContext != null && mContext.getResources().getBoolean(R.bool.config_is_show_ims_icon_in_calllog) 
            && call.getExtras() != null
    ...
    private String getLogNumber(Call call) {
        ...
        if (!PhoneNumberUtils.isUriNumber(handleString)) {
            //SPRD Add for Bug1008958. 
            handleString = PhoneNumberUtils.extractNetworkPortionAlt(
                PhoneNumberUtils.stripSeparators(handleString));
        }
        return handleString;
    }
