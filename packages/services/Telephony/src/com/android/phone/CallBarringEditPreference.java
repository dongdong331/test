...
public class CallBarringEditPreference extends EditPinPreference {
    ...
    void init(TimeConsumingPreferenceListener listener, boolean skipReading, Phone phone) {
        ...
        if (!skipReading) {
            ...
            mPhone.getCallBarring(mFacility, "", mHandler.obtainMessage(
                    MyHandler.MESSAGE_GET_CALL_BARRING), getCallBarringServiceClass(mPhone));//UNISOC: Modify for Bug 1244239
    ...
    protected void onDialogClosed(boolean positiveResult) {
        ...
        if (mButtonClicked != DialogInterface.BUTTON_NEGATIVE) {
            ...
            /* UNISOC : Modify for bug 905924
            mPhone.setCallBarring(mFacility, !mIsActivated, password,
                    mHandler.obtainMessage(MyHandler.MESSAGE_SET_CALL_BARRING), 0);*/
            ((GsmCdmaPhoneEx) mPhone).setFacilityLock(mFacility, !mIsActivated, password,
                    getCallBarringServiceClass(mPhone), mHandler.obtainMessage(MyHandler.MESSAGE_SET_CALL_BARRING));//UNISOC: Modify for Bug 1244239
            if (mTcpListener != null) {
    ...
    private void setShowPassword() {
        ...
        // UNISOC : modify for bug 905924, 1057639 & 1145363
    ...
    private static class MyHandler extends Handler {
        ...
        private void handleSetCallBarringResponse(Message msg) {
            ...
            if (pref.mTcpListener != null) {
                pref.mTcpListener.setSuppServiceFlag(GsmCdmaPhoneEx.SS_FLAG_DES_END);
            }
            ...
            pref.mPhone.getCallBarring(
                    ...
                    obtainMessage(MESSAGE_GET_CALL_BARRING, 0, MESSAGE_SET_CALL_BARRING,
                            ar.exception),
                    getCallBarringServiceClass(pref.mPhone)); //UNISOC: Modify for Bug 1244239
        }
    }

    /* UNISOC: add for Bug 1244239 @{ */
    private static int getCallBarringServiceClass(Phone phone) {
        int serviceClass = 0;
        int phoneId = phone.getPhoneId();
        CarrierConfigManager carrierMgr = (CarrierConfigManager) phone.getContext()
                .getSystemService(Context.CARRIER_CONFIG_SERVICE);

        if (carrierMgr != null && carrierMgr.getConfigForPhoneId(phoneId) != null) {
            serviceClass = carrierMgr.getConfigForPhoneId(phoneId).getInt(
                    CarrierConfigManagerEx.KEY_CALL_BARRING_SERVICE_CLASS_INT, 0);
        }
        Log.d(LOG_TAG, "getCallBarringServiceClass, serviceClass = " + serviceClass);
