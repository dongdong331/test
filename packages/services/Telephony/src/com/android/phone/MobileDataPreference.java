...
public class MobileDataPreference extends DialogPreference {
    ...
    public void initialize(int subId) {
        ...
        /* UNISOC:Bug 791516 mobile data switch status is not same with status bar @{ */
        boolean enabled = mTelephonyManager.getDataEnabled(mSubId);
        int dds = SubscriptionManager.getDefaultDataSubscriptionId();
        //fix bug 926588
        int dataPhoneId = SubscriptionManager.getPhoneId(dds);
        boolean isDataSimEnable = (dataPhoneId == SubscriptionManager.INVALID_PHONE_INDEX) ? false
                : mTelephonyManagerEx.isSimEnabled(dataPhoneId);
        Log.d(TAG, "initialize(" + enabled + "," + mSubId + "," + dds +")" + ", isDataSimEnable:"
                + isDataSimEnable + ", phoneid=" +dataPhoneId);
        if ((dds != mSubId) && enabled && isDataSimEnable) {
            setChecked(false);
    ...
    private void disableDataForOtherSubscriptions(int subId) {
        ...
        if (subInfoList != null) {
            for (SubscriptionInfo subInfo : subInfoList) {
                //UNISOC: Modify for the bug900047
