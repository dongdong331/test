...
public class NetworkQueryService extends Service {
    ...
    static final int QUERY_READY = -1;
    static final int QUERY_IS_RUNNING = -2;
    ...
    // UNISOC: FEATURE_MANUAL_QUERY_NETWORK
    private static final String PACKAGE_NAME = "com.android.phone.NetworkQueryServiceEx";
    ...
    static final int SCAN_TYPE = NetworkScanRequest.SCAN_TYPE_ONE_SHOT;
     ...
     */
    private INetworkQueryService.Stub mBinder = new INetworkQueryService.Stub() {
                ...
                try {
                    switch (msg.what) {
                        case EVENT_NETWORK_SCAN_VIA_PHONE_COMPLETED:
                            ...
                            if (DBG) log("EVENT_NETWORK_SCAN_VIA_PHONE_COMPLETED: " + phoneId);
                            if (ar != null) {
                             ...
                             * @{
                             */
                            cb.onCompleteForPhone(phoneId);
                            /* @} */
                            break;

                        case EVENT_NETWORK_SCAN_RESULTS:
                            cb.onResults((List<CellInfo>) (((AsyncResult) msg.obj).result));
                            break;

                        case EVENT_NETWORK_SCAN_COMPLETED:
                            if (DBG) log("EVENT_NETWORK_SCAN_COMPLETED: " + phoneId);
                            /* UNISOC: Bug 791054
                             * @orig
                            cb.onComplete();
                             * @{
    ...
    private List<CellInfo> getCellInfoList(List<OperatorInfo> operatorInfoList) {
        ...
        for (OperatorInfo oi: operatorInfoList) {
            ...
            // UNISOC supports radio access technology @{
            if(operatorNumeric != null && operatorNumeric.matches("^[0-9]{5,6}\\s{1}[027]{1}$")) {
                String[] tokens = operatorNumeric.split(" ");
                mcc = tokens[0].substring(0, 3);
                mnc = tokens[0].substring(3);
                if(tokens[1].equals("0")) {
                    ...
                    CellIdentityWcdma cig = new CellIdentityWcdma(
                            Integer.MAX_VALUE /* lac */,
                            Integer.MAX_VALUE /* cid */,
                    ...
                    CellInfoWcdma ci = new CellInfoWcdma();
                    ci.setCellIdentity(cig);
                    cellInfoList.add(ci);
                } else if(tokens[1].equals("7")) {
                    CellIdentityLte cig = new CellIdentityLte(
                    ...
                    CellInfoLte ci = new CellInfoLte();
                    ci.setCellIdentity(cig);
                    cellInfoList.add(ci);
                }
                continue;
    ...
    void notifyScanResult(AsyncResult ar, int error, int phoneId, int id) {
        switch (id) {
            case EVENT_NETWORK_SCAN_VIA_PHONE_COMPLETED:
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_NETWORK_SCAN_VIA_PHONE_COMPLETED, error, phoneId, ar));
                break;
            case EVENT_NETWORK_SCAN_RESULTS:
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_NETWORK_SCAN_RESULTS, error, phoneId, ar));
                break;
            case EVENT_NETWORK_SCAN_COMPLETED:
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_NETWORK_SCAN_COMPLETED, error, phoneId, ar));
                break;
            case EVENT_NETWORK_SCAN_ERROR:
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_NETWORK_SCAN_ERROR, error, phoneId, ar));
                break;
        }
    }
    ...
    private void createBinder() {
        Class clazz = null;
        try {
            clazz = Class.forName(PACKAGE_NAME);
            if (clazz != null) {
                Constructor ctor = clazz.getDeclaredConstructor(NetworkQueryService.class);
                if (ctor != null) {
                    mBinder = (INetworkQueryService.Stub) ctor.newInstance(this);
                    Log.d(LOG_TAG, "Created extension binder=" + mBinder);
                }
            }
        } catch (Throwable t) {
            Log.d(LOG_TAG, "The NetworkQueryService extension is not found", t);
        }
    }
    /* @} */
}