...
public class PhoneSearchIndexablesProvider extends SearchIndexablesProvider {
    ...
    public Cursor queryRawData(String[] projection) {
        ...
        String apnTitle = context.getString(R.string.apn_settings);
        cursor.newRow()
                .add(RawData.COLUMN_RANK, 1)
                .add(RawData.COLUMN_TITLE, apnTitle)
                .add(RawData.COLUMN_SCREEN_TITLE, apnTitle)
                .add(RawData.COLUMN_KEY, "button_apn_key")
                .add(
                        RawData.COLUMN_INTENT_ACTION,
                        Intent.ACTION_MAIN)
                .add(
    ...
    public Cursor queryNonIndexableKeys(String[] projection) {
        ...
        try {
            ...
            if (!mUserManager.isAdminUser()) {
                // UNISOC: bug 900592 & bug 905694 & bug 907103
                final String[] values = new String[]{"preferred_network_mode_key",
                        ...
                        "data_usage_summary", "wifi_calling_key", "video_calling_key" ,"dualVoLTE_switch"
                        ,"vt_resolution_set_key", "preferred_data_roaming_key",
                        "data_always_online"};
                for (String nik : values) {
                    cursor.addRow(createNonIndexableRow(nik));
                }
            } else {
                if (isApnSettingsHidden()) {
                    Log.e(TAG,"hide apn");
                    cursor.addRow(createNonIndexableRow("button_apn_key" /* key */));
                }
                ...
                /* UNISOC: bug 882560 @{ */
                if (getContext().getResources().getBoolean(R.bool.config_hide_data_usage)) {
                    cursor.addRow(createNonIndexableRow("data_usage_summary"));
                }
                /* @} */
                /**
                 * UNISOC: add for Bug 793219, modify for Bug 1174972
                 * @{*/
                if (isVtResolutionHidden()) {
                    cursor.addRow(createNonIndexableRow("vt_resolution_set_key"));
                }
                /** @} */
            }
            ...
            /* UNISOC: add for Bug 904893 @{ */
            cursor.addRow(createNonIndexableRow("dualVoLTE_switch"));
            /* @} */
            return cursor;
        } finally {
    ...
    @VisibleForTesting boolean isApnSettingsHidden() {
        return !MobileNetworkSettings.showApnSettings(getContext());
    }

    //Unisoc: add for bug 1174972
    @VisibleForTesting boolean isVtResolutionHidden() {
        return !MobileNetworkSettings.showVtResolutionSettings(getContext());
    }
