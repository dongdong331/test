...
import com.unisoc.phone.callsettings.UssdCustomizedUtils;
...
public class PhoneUtils {
    ...
    private static final int EVENT_SEND_DISMISS_DIALOG_COMPLETE = 300;
    private static AlertDialog sMmiPreviousAlertDialog = null; //UNISOC: modify for 1023960, set null defaultly
    /* @} */
     ...
     * the message.
     * UNISOC: modify for ussd customized feature 886786
     *
     * @param context context to get strings.
    ...
    public static AlertDialog displayMMIComplete(final Phone phone, Context context, final MmiCode mmiCode,
        ...
        if ((app.getPUKEntryActivity() != null) && (state == MmiCode.State.COMPLETE)) {
            ...
            pd.setTitle(title);
            if (UssdCustomizedUtils.getInstance(context).getTextView(context, text) != null) {
                TextView tv = UssdCustomizedUtils.getInstance(context).getTextView(context, text);
                tv.setText(text);
                UssdCustomizedUtils.getInstance(context).setSpan(context, tv);
                pd.setView(tv);
            } else {
                pd.setMessage(text);
            }
    ...
    public static void handleMMIDialogDismiss(final Phone phone, Context context,
        ...
        * */
        ...
        mConnectionHandler.removeMessages(EVENT_SEND_DISMISS_DIALOG_COMPLETE);
        mConnectionHandler.sendMessageDelayed(
                mConnectionHandler.obtainMessage(EVENT_SEND_DISMISS_DIALOG_COMPLETE),
                dlgDuration * 1000);
    }
