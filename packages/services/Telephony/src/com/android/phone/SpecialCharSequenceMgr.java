        ...
        if ((input.startsWith("**04") || input.startsWith("**05"))
            ...
            if (userManager.isSystemUser()) {
                ...
                if (input.startsWith("**04") & phoneCount > 1) {
                    ...
                    Phone phone = PhoneGlobals.getPhone();
                    /* UNISOC: Modify for bug 911208 @{ */
                    if (phoneCount < 2) {
                        List<SubscriptionInfo> subInfo = subscriptionManager
                                .getActiveSubscriptionInfoList();
                        if (subInfo != null && subInfo.size() > 0) {
                            int phoneId = subInfo.get(0).getSimSlotIndex();
                ...
                AlertDialog dialog = new AlertDialog.Builder(context)
                        .setMessage(R.string.outgoing_call_not_allowed_user_restriction)
