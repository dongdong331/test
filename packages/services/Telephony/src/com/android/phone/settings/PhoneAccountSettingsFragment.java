...
public class PhoneAccountSettingsFragment extends PreferenceFragment
    ...
    /* UNISOC: modify by BUG 748230 & 827856 @{ */
    private List<PhoneAccount> mLastAccounts = new ArrayList<>();
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (shouldShowConnectionServiceList(allNonSimAccounts)) {
            ...
            /** Orig
             * UNISOC: Add for bug1112622 display no available phone accounts only
    ...
    public void onResume() {
        ...
        if (shouldShowConnectionServiceList(allNonSimAccounts)) {
            List<PhoneAccountHandle> enabledAccounts =
                    getCallingAccounts(true /* includeSims */, false /* includeDisabled */);
            /* UNISOC: add by bug589612 & 827856 @{ */
            if (mNoAccounts != null) {
            ...
            /* @} */
            if (enabledAccounts.size() != mLastAccounts.size()) {
                initAccountList(enabledAccounts);
                /* UNISOC: add for bug841863 @{ */
                if (enabledAccounts.size() > 1) {
    ...
    private void removeAccount(List<PhoneAccount> accounts) {
        for (PhoneAccount account : mLastAccounts) {
            if (!isPhoneAccountRemoved(account, accounts)) {
                if (mAccountList != null) {
                    Log.d(LOG_TAG, "Remove phoneaccount : " + account.getLabel()
                            + "; id: " + account.getAccountHandle().getId());
                    mAccountList.removePreference(mAccountList.findPreference(account
                            .getAccountHandle().getId()));
                }
            }
        }
    }

    private boolean isPhoneAccountContains (PhoneAccount phoneAccount) {
        for (PhoneAccount account : mLastAccounts) {
            if (phoneAccount.getAccountHandle().getId()
                    .equals(account.getAccountHandle().getId())) {
                return true;
            }
        }
        return false;
    ...
    private boolean isPhoneAccountRemoved(PhoneAccount phoneAccount, List<PhoneAccount> accounts) {
    ...
    private void updateDefaultOutgoingAccountsModel() {
        /* UNISOC: modify for bug731278 @{ */
        String nullSelectionString;
        if (ImsManagerEx.isDualVoLTEActive()) {
            nullSelectionString = getString(R.string.sim_calls_not_set_up);
        } else {
            nullSelectionString = getString(R.string.phone_accounts_ask_every_time);
        }
        mDefaultOutgoingAccount.setModel(
                mTelecomManager,
                getCallingAccounts(true /* includeSims */, false /* includeDisabled */),
                mTelecomManager.getUserSelectedOutgoingPhoneAccount(), nullSelectionString);
        /* @} */
    }
    ...
    private void initAccountList(List<PhoneAccountHandle> enabledAccounts) {
        ...
        for (PhoneAccount account : accounts) {
            ...
            // UNISOC: modify by BUG 748230
            accountPreference.setKey(account.getAccountHandle().getId());
            // Add an icon.
            ...
            /* UNISOC: add for bug708850 @{ */
            if (!isPrimaryUser()
                    && enabledAccounts.size() == 1
                    && mSubscriptionManager.getActiveSubscriptionInfoCount() == 0) {
                accountPreference
                        .setOnPreferenceClickListener(new OnPreferenceClickListener() {
                            public boolean onPreferenceClick(Preference preference) {
                                Toast.makeText(getActivity(),
                                        R.string.call_settings_admin_user_only,
            ...
            /* UNISOC: modify by BUG 748230 & 841863 @{ */
            if (!isPhoneAccountContains(account)) {
                Log.d(LOG_TAG, "Add phoneaccount : " + account.getLabel()
                        + "; id: " + account.getAccountHandle().getId());
                mAccountList.addPreference(accountPreference);
            } else {
                Preference existPre
                      = mAccountList.findPreference(account.getAccountHandle().getId());
                if (existPre != null) {
                    existPre.setOrder(order - 1);
                }
            }
            /* @} */
        }

        if (mLastAccounts.size() > accounts.size()) {
            removeAccount(accounts);
        }

        if (mLastAccounts.size() != accounts.size()) {
            mLastAccounts = accounts;
        }
    }
